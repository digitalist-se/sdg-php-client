        ...
        if ($rc->hasMethod($setter)) {
            ...
            if ($rmeth->isPublic() || $this->bIgnoreVisibility) {
                ...
                if (\count($rparams) > 0) {
                    ...
                    if ($pclass !== null) {
                        return array(\true, $rmeth, '\\' . $pclass->getName(), $isNullable);
                    }
                }
                ...
                if (!isset($annotations['param'][0])) {
                    ...
                    if (\PHP_MAJOR_VERSION >= 7) {
                        ...
                        if (\is_string($ptype)) {
                            return array(\true, $rmeth, $ptype, $isNullable);
                        }
                        if (\PHP_VERSION_ID >= 70100 && $ptype instanceof \ReflectionNamedType) {
                            return array(\true, $rmeth, $ptype->getName(), $ptype->allowsNull());
                        }
 ...
 * @link     http://cweiske.de/
 */
\class_alias('SdgScoped\\JsonMapper', 'JsonMapper', \false);
