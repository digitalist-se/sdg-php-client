...
namespace SdgScoped\Phan\Plugin;

use AssertionError;
use ast\Node;
use Closure;
use SdgScoped\Phan\AST\Parser;
...
use SdgScoped\Phan\LanguageServer\CompletionRequest;
use SdgScoped\Phan\LanguageServer\CompletionResolver;
use SdgScoped\Phan\LanguageServer\DefinitionResolver;
use SdgScoped\Phan\LanguageServer\GoToDefinitionRequest;
use SdgScoped\Phan\Library\RAII;
use SdgScoped\Phan\Plugin\Internal\ArrayReturnTypeOverridePlugin;
use SdgScoped\Phan\Plugin\Internal\BaselineLoadingPlugin;
use SdgScoped\Phan\Plugin\Internal\BaselineSavingPlugin;
use SdgScoped\Phan\Plugin\Internal\BuiltinSuppressionPlugin;
use SdgScoped\Phan\Plugin\Internal\CallableParamPlugin;
use SdgScoped\Phan\Plugin\Internal\ClosureReturnTypeOverridePlugin;
use SdgScoped\Phan\Plugin\Internal\CompactPlugin;
use SdgScoped\Phan\Plugin\Internal\DependentReturnTypeOverridePlugin;
use SdgScoped\Phan\Plugin\Internal\ExtendedDependentReturnTypeOverridePlugin;
use SdgScoped\Phan\Plugin\Internal\IssueFixingPlugin\IssueFixer;
use SdgScoped\Phan\Plugin\Internal\MiscParamPlugin;
use SdgScoped\Phan\Plugin\Internal\NodeSelectionPlugin;
use SdgScoped\Phan\Plugin\Internal\NodeSelectionVisitor;
use SdgScoped\Phan\Plugin\Internal\RedundantConditionCallPlugin;
use SdgScoped\Phan\Plugin\Internal\RequireExistsPlugin;
use SdgScoped\Phan\Plugin\Internal\StringFunctionPlugin;
use SdgScoped\Phan\Plugin\Internal\ThrowAnalyzerPlugin;
use SdgScoped\Phan\Plugin\Internal\VariableTrackerPlugin;
use SdgScoped\Phan\PluginV3;
use SdgScoped\Phan\PluginV3\AfterAnalyzeFileCapability;
use SdgScoped\Phan\PluginV3\AnalyzeClassCapability;
use SdgScoped\Phan\PluginV3\AnalyzeFunctionCallCapability;
use SdgScoped\Phan\PluginV3\AnalyzeFunctionCapability;
use SdgScoped\Phan\PluginV3\AnalyzeLiteralStatementCapability;
use SdgScoped\Phan\PluginV3\AnalyzeMethodCapability;
use SdgScoped\Phan\PluginV3\AnalyzePropertyCapability;
use SdgScoped\Phan\PluginV3\AutomaticFixCapability;
use SdgScoped\Phan\PluginV3\BeforeAnalyzeCapability;
use SdgScoped\Phan\PluginV3\BeforeAnalyzeFileCapability;
use SdgScoped\Phan\PluginV3\BeforeAnalyzePhaseCapability;
use SdgScoped\Phan\PluginV3\BeforeLoopBodyAnalysisCapability;
use SdgScoped\Phan\PluginV3\BeforeLoopBodyAnalysisVisitor;
use SdgScoped\Phan\PluginV3\FinalizeProcessCapability;
use SdgScoped\Phan\PluginV3\HandleLazyLoadInternalFunctionCapability;
use SdgScoped\Phan\PluginV3\MergeVariableInfoCapability;
use SdgScoped\Phan\PluginV3\PluginAwarePostAnalysisVisitor;
use SdgScoped\Phan\PluginV3\PluginAwarePreAnalysisVisitor;
use SdgScoped\Phan\PluginV3\PostAnalyzeNodeCapability;
use SdgScoped\Phan\PluginV3\PreAnalyzeNodeCapability;
use SdgScoped\Phan\PluginV3\ReturnTypeOverrideCapability;
use SdgScoped\Phan\PluginV3\SubscribeEmitIssueCapability;
use SdgScoped\Phan\PluginV3\SuppressionCapability;
use SdgScoped\Phan\PluginV3\UnloadablePluginException;
use SdgScoped\Phan\Suggestion;
use Throwable;
use SdgScoped\UnusedSuppressionPlugin;
 ...
 */
final class ConfigPluginSet extends PluginV3 implements AfterAnalyzeFileCapability, AnalyzeClassCapability, AnalyzeFunctionCapability, AnalyzeFunctionCallCapability, AnalyzeLiteralStatementCapability, AnalyzeMethodCapability, AnalyzePropertyCapability, BeforeAnalyzeCapability, BeforeAnalyzePhaseCapability, BeforeAnalyzeFileCapability, FinalizeProcessCapability, ReturnTypeOverrideCapability, SubscribeEmitIssueCapability, SuppressionCapability
{
     ...
     */
    public function preAnalyzeNode(CodeBase $code_base, Context $context, Node $node) : void
    {
        $plugin_callback = $this->pre_analyze_node_plugin_set[$node->kind] ?? null;
        if ($plugin_callback !== null) {
            $plugin_callback($code_base, $context, $node);
        }
    }
    /**
     * @param CodeBase $code_base
     ...
     */
    public function postAnalyzeNode(CodeBase $code_base, Context $context, Node $node, array $parent_node_list = []) : void
    {
        $plugin_callback = $this->post_analyze_node_plugin_set[$node->kind] ?? null;
        if (\is_object($plugin_callback)) {
            $plugin_callback($code_base, $context, $node, $parent_node_list);
        }
    }
    /**
     * @param CodeBase $code_base
     ...
     * The php-ast Node being analyzed.
     */
    public function analyzeLoopBeforeBody(CodeBase $code_base, Context $context, Node $node) : void
    {
     ...
     * @override
     */
    public function beforeAnalyzeFile(CodeBase $code_base, Context $context, string $file_contents, Node $node) : void
    {
        foreach ($this->before_analyze_file_plugin_set as $plugin) {
            $plugin->beforeAnalyzeFile($code_base, $context, $file_contents, $node);
        }
    }
    /**
    ...
    {
        $this->did_analyze_phase_start = \true;
    ...
    public function afterAnalyzeFile(CodeBase $code_base, Context $context, string $file_contents, Node $node) : void
    {
        foreach ($this->after_analyze_file_plugin_set as $plugin) {
            $plugin->afterAnalyzeFile($code_base, $context, $file_contents, $node);
        }
    }
    /**
     * @param CodeBase $code_base
     ...
     */
    public function shouldSuppressIssue(CodeBase $code_base, Context $context, string $issue_type, int $lineno, array $parameters, ?Suggestion $suggestion) : bool
    {
        foreach ($this->suppression_plugin_set as $plugin) {
            if ($plugin->shouldSuppressIssue($code_base, $context, $issue_type, $lineno, $parameters, $suggestion)) {
     ...
     */
    public function getIssueSuppressionList(CodeBase $code_base, string $file_path) : array
    {
        $result = [];
        foreach ($this->suppression_plugin_set as $plugin) {
            $result += $plugin->getIssueSuppressionList($code_base, $file_path);
        }
        return $result;
    }
    /**
    ...
    public function analyzeStringLiteralStatement(CodeBase $code_base, Context $context, string $statement) : bool
    {
        ...
        foreach ($this->analyze_literal_statement_plugin_set as $plugin) {
            $consumed = $plugin->analyzeStringLiteralStatement($code_base, $context, $statement) || $consumed;
        }
        return $consumed;
    }
    /**
    ...
    public function analyzeProperty(CodeBase $code_base, Property $property) : void
    {
        foreach ($this->analyze_property_plugin_set as $plugin) {
            try {
                $plugin->analyzeProperty($code_base, $property);
     ...
     */
    public function addTemporaryAnalysisPlugin(CodeBase $code_base, ?\SdgScoped\Phan\Daemon\Request $request) : ?RAII
    {
        if (!$request) {
            return null;
        }
    ...
    private function ensurePluginsExist() : void
    {
        ...
        $load_plugin = static function (string $plugin_file_name) use(&$loaded_plugin_files) : ?PluginV3 {
            ...
            $loaded_plugin_files[$plugin_file_name] = \true;
            try {
                ...
                $message = \sprintf("Failed to initialize plugin %s, exiting: %s: %s at %s:%d\nStack Trace:\n%s", $plugin_file_name, get_class($e), $e->getMessage(), $e->getFile(), $e->getLine(), $e->getTraceAsString());
                \error_log($message);
                exit(EXIT_FAILURE);
            }
        ...
        $plugin_set = \array_values(\array_filter(\array_map($load_plugin, Config::getValue('plugins'))));
        ...
        if (Config::getValue('enable_internal_return_type_plugins')) {
            $internal_return_type_plugins = [new ArrayReturnTypeOverridePlugin(), new CallableParamPlugin(), new CompactPlugin(), new ClosureReturnTypeOverridePlugin(), new DependentReturnTypeOverridePlugin(), new StringFunctionPlugin(), new MiscParamPlugin()];
        ...
        if (Config::getValue('dead_code_detection') && \count(self::filterByClass($plugin_set, \SdgScoped\UnreachableCodePlugin::class)) === 0) {
    ...
    private static function addClosuresForPreAnalyzeNodeCapability(ClosuresForKind $closures_for_kind, PreAnalyzeNodeCapability $plugin) : void
    {
        ...
        if (\count($handled_node_kinds) === 0) {
            // @phan-suppress-next-line PhanPluginRemoveDebugCall
            \fprintf(STDERR, "Plugin %s has a preAnalyzeNode visitor %s (subclass of %s) which doesn't override any known visit<Suffix>() methods, but expected at least one method to be overridden\n", \get_class($plugin), $plugin_analysis_class, PluginAwarePreAnalysisVisitor::class);
        }
     ...
     */
    private static function addClosuresForPostAnalyzeNodeCapability(ClosuresForKind $closures_for_kind, PostAnalyzeNodeCapability $plugin) : void
    {
        ...
        if (!\is_subclass_of($plugin_analysis_class, PluginAwarePostAnalysisVisitor::class)) {
            throw new \TypeError(\sprintf("Result of %s::getAnalyzeNodeVisitorClassName must be the name of a subclass of '%s', but '%s' is not", \get_class($plugin), PluginAwarePostAnalysisVisitor::class, $plugin_analysis_class));
        }
        ...
        if (\count($handled_node_kinds) === 0) {
            // @phan-suppress-next-line PhanPluginRemoveDebugCall
            \fprintf(STDERR, "Plugin %s has an analyzeNode visitor %s (subclass of %s) which doesn't override any known visit<Suffix>() methods, but expected at least one method to be overridden\n", \get_class($plugin), $plugin_analysis_class, PluginAwarePostAnalysisVisitor::class);
        }
        $closures_for_kind->recordForKinds($handled_node_kinds, $closure);
    }
    /**
     ...
     */
    private static function addClosuresForBeforeLoopBodyAnalysisCapability(ClosuresForKind $closures_for_kind, BeforeLoopBodyAnalysisCapability $plugin) : void
    {
        ...
        if (!\is_subclass_of($plugin_analysis_class, BeforeLoopBodyAnalysisVisitor::class)) {
            throw new \TypeError(\sprintf("Result of %s::getBeforeLoopBodyAnalysisVisitorClassName must be the name of a subclass of '%s', but '%s' is not", \get_class($plugin), BeforeLoopBodyAnalysisVisitor::class, $plugin_analysis_class));
        }
        ...
        if (\count($handled_node_kinds) === 0) {
            // @phan-suppress-next-line PhanPluginRemoveDebugCall
            \fprintf(STDERR, "Plugin %s has an analyzeNode visitor %s (subclass of %s) which doesn't override any known visit<Suffix>() methods, but expected at least one method to be overridden\n", \get_class($plugin), $plugin_analysis_class, BeforeLoopBodyAnalysisVisitor::class);
        } else {
            ...
            if ($additional_kinds) {
                throw new AssertionError(\sprintf("The following node kinds cannot be used in %s: %s", $plugin_analysis_class, \implode(', ', \array_map([Parser::class, 'getKindName'], $additional_kinds))));
            }
        }
        $closures_for_kind->recordForKinds($handled_node_kinds, $closure);
    }
