 ...
 */
final class BaselineLoadingPlugin extends PluginV3 implements SubscribeEmitIssueCapability
{
    /**
    ...
    {
        return $this->shouldSuppressIssueTypeInFile($issue_instance->getIssue()->getType(), $issue_instance->getFile());
    }
    /**
    ...
    {
        $suppressed_by_file = \in_array($issue_type, $this->file_suppressions[$file] ?? [], \true);
        ...
        if (\in_array($issue_type, $this->directory_suppressions[''] ?? [], \true)) {
        ...
        foreach ($parts as $part) {
            ...
            $dirPath .= $part;
            if (\in_array($issue_type, $this->directory_suppressions[$dirPath] ?? [], \true)) {
                return \true;
            }
            $dirPath .= '/';
        }
