...
namespace SdgScoped\Phan\Plugin\Internal\IssueFixingPlugin;
...
use SdgScoped\Phan\AST\TolerantASTConverter\NodeUtils;
...
class IssueFixer
{
    private static function isMatchingNamespaceUseDeclaration(string $file_contents, NamespaceUseDeclaration $declaration, IssueInstance $issue_instance) : bool
    {
    ...
    private static function maybeRemoveNamespaceUseDeclaration(string $file_contents, NamespaceUseDeclaration $declaration, IssueInstance $issue_instance) : ?FileEdit
    {
    ...
    private static function createClosures() : array
    {
         ...
         * @return ?FileEditSet
         */
        $handle_unreferenced_use = static function (CodeBase $unused_code_base, FileCacheEntry $file_contents, IssueInstance $issue_instance) : ?FileEditSet {
            // 1-based line
        ...
        return \array_merge(self::$fixer_closures, [Issue::UnreferencedUseNormal => $handle_unreferenced_use, Issue::UnreferencedUseConstant => $handle_unreferenced_use, Issue::UnreferencedUseFunction => $handle_unreferenced_use]);
    }
    /**
    ...
    public static function computeFixersForInstances(array $instances) : array
    {
        ...
        foreach ($instances as $instance) {
            ...
            if ($closure) {
                 ...
                 * @return ?FileEditSet
                 */
                $fixers_for_files[$instance->getFile()][] = static function (CodeBase $code_base, FileCacheEntry $file_contents) use($closure, $instance) : ?FileEditSet {
     ...
     */
    public static function computeNewContentForFixers(CodeBase $code_base, string $file, string $raw_contents, array $fixers) : ?string
    {
     ...
     */
    private static function attemptFixForIssues(CodeBase $code_base, string $file, array $fixers) : void
    {
        try {
    ...
    public static function computeNewContents(string $file, string $contents, array $all_edits) : ?string
    {
        \usort($all_edits, static function (FileEdit $a, FileEdit $b) : int {
            return ($a->replace_start <=> $b->replace_start ?: $a->replace_end <=> $b->replace_end) ?: \strcmp($a->new_text, $b->new_text);
        });
