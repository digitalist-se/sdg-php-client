 ...
 */
final class CompactPlugin extends PluginV3 implements AnalyzeFunctionCallCapability
{
    /**
    ...
    private static function getAnalyzeFunctionCallClosuresStatic() : array
    {
         ...
         */
        $compact_callback = static function (CodeBase $code_base, Context $context, Func $unused_func, array $args, ?Node $node) : void {
            $check_variable_usage = static function (string $variable_name, $arg = null) use($code_base, $context, $node) : void {
                ...
                if (!$context->getScope()->hasVariableWithName($variable_name)) {
                    Issue::maybeEmitWithParameters($code_base, $context, Variable::chooseIssueForUndeclaredVariable($context, $variable_name), $arg->lineno ?? $context->getLineNumberStart(), [$variable_name], IssueFixSuggester::suggestVariableTypoFix($code_base, $context, $variable_name));
                }
            };
            foreach ($args as $arg) {
                if (\is_string($arg)) {
        ...
        return ['compact' => $compact_callback];
    }
}
