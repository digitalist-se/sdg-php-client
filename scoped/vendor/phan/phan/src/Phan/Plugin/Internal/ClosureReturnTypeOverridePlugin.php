 ...
 * TODO: Refactor this.
 */
final class ClosureReturnTypeOverridePlugin extends PluginV3 implements AnalyzeFunctionCallCapability, ReturnTypeOverrideCapability
{
    /**
    ...
    private static function getReturnTypeOverridesStatic() : array
    {
         ...
         */
        $call_user_func_callback = static function (CodeBase $code_base, Context $context, Func $unused_function, array $args) : UnionType {
         ...
         * @param list<Node|int|string|float> $args
         */
        $call_user_func_array_callback = static function (CodeBase $code_base, Context $context, Func $unused_function, array $args) : UnionType {
            $element_types = UnionType::empty();
            if (\count($args) < 2) {
                return $element_types;
            }
         ...
         * @param list<Node|int|string|float> $args
         */
        $from_callable_callback = static function (CodeBase $code_base, Context $context, Method $unused_method, array $args) : UnionType {
            if (\count($args) !== 1) {
         ...
         */
        $from_closure_callback = static function (CodeBase $code_base, Context $context, Method $unused_method, array $args) : UnionType {
            if (\count($args) < 2 || \count($args) > 3) {
                return NullType::instance(\false)->asRealUnionType();
            }
            $types = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $args[0], \true);
    ...
    private static function getAnalyzeFunctionCallClosuresStatic() : array
    {
         ...
         * @param list<Node|int|string|float> $args
         */
        $call_user_func_callback = static function (CodeBase $code_base, Context $context, Func $unused_function, array $args, ?Node $_) : void {
            if (\count($args) < 1) {
                return;
            }
            $function_like_list = UnionTypeVisitor::functionLikeListFromNodeAndContext($code_base, $context, $args[0], \true);
         ...
         * @param list<Node|int|string|float> $args
         */
        $call_user_func_array_callback = static function (CodeBase $code_base, Context $context, Func $unused_function, array $args, ?Node $_) : void {
            if (\count($args) < 2) {
                return;
            }
            // Currently, only analyze calls of the form call_user_func_array(callable expression, [$arg1, $arg2...])
        ...
        foreach ($arg_array_node->children as $child) {
            ...
            if ($child->flags & \ast\flags\ARRAY_ELEM_REF) {
                ...
                if ($el instanceof Node && $el->kind === \ast\AST_VAR) {
                    $cnode = new ContextNode($code_base, $context, $el);
                    try {
