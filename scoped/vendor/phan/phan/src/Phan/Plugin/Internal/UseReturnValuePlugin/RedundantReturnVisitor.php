...
use SdgScoped\Phan\AST\ASTHasher;
...
class RedundantReturnVisitor
{
    ...
    private function analyzeNode(Node $stmts) : array
    {
        ...
        if (!\in_array($kind, [\ast\AST_STMT_LIST, \ast\AST_IF, \ast\AST_SWITCH_LIST], \true)) {
        ...
        if ($exit_status !== BlockExitStatusChecker::STATUS_RETURN) {
            ...
            $possible_return_nodes[] = new Node(\ast\AST_RETURN, 0, ['expr' => new Node(\ast\AST_CONST, 0, ['name' => new Node(\ast\AST_NAME, \ast\flags\NAME_NOT_FQ, ['name' => 'null'], $line)], $line)], $line);
        }
    ...
    private function checkMultipleReturns(array $possible_return_nodes) : void
    {
        ...
        throw new IssueException(Issue::fromType(Issue::UnusedReturnBranchWithoutSideEffects)($this->context->getFile(), $last_return->lineno, [ASTReverter::toShortString($last_expr), \reset($remaining_returns)->lineno ?? 0]));
    }
}
