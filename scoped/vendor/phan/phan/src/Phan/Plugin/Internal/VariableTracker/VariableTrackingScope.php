...
class VariableTrackingScope
{
    ...
    public function recordDefinition(string $variable_name, Node $node) : void
    {
        ...
        $this->defs[$variable_name] = [spl_object_id($node) => \true];
        ...
        $this->defs_shadowing_set[$variable_name] = \true;
    }
    /**
    ...
    public function recordDefinitionById(string $variable_name, int $node_id) : void
    {
        // Create a new definition for variable_name.
        // Replace the definitions for $variable_name.
        $this->defs[$variable_name] = [$node_id => \true];
    }
    /**
    ...
    public function recordUsage(string $variable_name, Node $node) : void
    {
        ...
        if (!isset($this->defs_shadowing_set[$variable_name]) && ($this->defs[$variable_name][$node_id] ?? \false) !== \true) {
            $this->uses[$variable_name][$node_id] = \true;
        }
    }
    /**
     ...
     * @return static
     */
    public function mergeInnerLoopScope(VariableTrackingLoopScope $inner_loop_scope, VariableGraph $graph) : VariableTrackingScope
    {
        $result = clone $this;
    ...
    protected function flattenScopeToMergedLoopResult(VariableTrackingLoopScope $inner_loop_scope, VariableTrackingBranchScope $alternate_scope, VariableGraph $graph) : void
    {
    ...
    protected function flattenUsesFromScopeToMergedLoopResult(VariableTrackingLoopScope $inner_loop_scope, VariableTrackingBranchScope $alternate_scope, VariableGraph $graph) : void
    {
        // Need to flatten these to the same level
        // The LoopScope might have been cloned, so just keep going until the closest loop scope.
        // TODO: Look at this, see if this way of merging definitions and usages will miss any false positives
    ...
    private static function addScopeToMergedLoopResult(VariableTrackingScope $result, VariableTrackingBranchScope $scope, VariableGraph $graph) : void
    {
     ...
     * @return static
     */
    public function mergeWithSingleBranchScope(VariableTrackingBranchScope $scope) : VariableTrackingScope
    {
        $result = clone $this;
     ...
     */
    public function mergeBranchScopeList(array $branch_scopes, bool $merge_parent_scope, array $inner_exiting_scope_list) : VariableTrackingScope
    {
