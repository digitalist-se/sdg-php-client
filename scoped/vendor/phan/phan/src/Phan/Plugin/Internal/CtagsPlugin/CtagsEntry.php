...
class CtagsEntry
{
    ...
    public function __toString()
    {
        ...
        $common = \sprintf("%s\t%s\t%s\t%s\tline:%d", $this->name, $ctags_path, self::escapeFragment($this->fragment), $this->kind, $this->context->getLineNumberStart());
    ...
    {
        $escaped = \str_replace(['\\', '/', "\x00"], ['\\\\', '\\/', '\\0'], $fragment);
        return '/^' . $escaped . '$/;"';
    }
    /**
    ...
    public function getOrderKey() : string
    {
        ...
        return $is_analyzed_order . "\x00" . $type_order;
    }
}
