 ...
 */
final class ExtendedDependentReturnTypeOverridePlugin extends PluginV3 implements ReturnTypeOverrideCapability
{
    /**
     ...
     */
    public static function wrapNArgumentFunction(string $function_name, int $min_args, ?int $max_args = null) : Closure
    {
        $max_args = $max_args ?? $min_args;
        /**
         * @param list<Node|string|int|float> $args
         */
        return static function (CodeBase $code_base, Context $context, Func $function, array $args) use($function_name, $min_args, $max_args) : UnionType {
            ...
            try {
                $result = \SdgScoped\with_disabled_phan_error_handler(
                    /** @return mixed */
                    static function () use($function_name, $values) {
                        return @$function_name(...$values);
                    }
                );
            } catch (Throwable $e) {
                Issue::maybeEmit($code_base, $context, Issue::TypeErrorInInternalCall, $args[0]->lineno ?? $context->getLineNumberStart(), $function_name, $e->getMessage());
                return $function->getUnionType();
            }
    ...
    private static function getReturnTypeOverridesStatic(CodeBase $code_base) : array
    {
        ...
        $wrap = static function (string $function, int $min, ?int $max = null) use($basic_return_type_overrides) : ?Closure {
             ...
             * @param list<Node|string|int|float> $args
             */
            return static function (CodeBase $code_base, Context $context, Func $function_decl, array $args) use($cb, $cb_fallback) : UnionType {
