 ...
 * @internal
 */
final class MethodSearcherPlugin extends PluginV3 implements BeforeAnalyzeCapability
{
        ...
        foreach ($union_type->getTypeSet() as $type) {
            if ($type->isObjectWithKnownFQSEN()) {
                ...
                foreach ($replacement_element_types->getTypeSet() as $element_type) {
                    $replacement_type = GenericArrayType::fromElementType($element_type, $type->isNullable(), $type->getKeyType());
     ...
     */
    public static function getReplacementTypesForFullyQualifiedClassName(CodeBase $code_base, Type $type) : array
    {
    ...
    public function beforeAnalyze(CodeBase $code_base) : void
    {
        ...
        foreach ($results as $i => [$unused_score, $fqsen, $function]) {
            ...
            \printf("    (%s)%s\n", \implode(', ', $function->getParameterList()), $return_type->isEmpty() ? '' : ' : ' . $return_type);
            if ($i >= $limit) {
                break;
            }
        }
     ...
     */
    public function functionMatchesSignature(CodeBase $code_base, FunctionInterface $function) : float
    {
        ...
        if ($function instanceof Method && \in_array(\strtolower($function->getName()), ['__construct', '__clone'], \true)) {
