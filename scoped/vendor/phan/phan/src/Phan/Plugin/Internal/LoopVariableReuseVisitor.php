...
class LoopVariableReuseVisitor extends PluginAwarePostAnalysisVisitor
{
    ...
    {
        $directions = RedundantConditionLoopCheck::extractComparisonDirections($node->children['cond']) + RedundantConditionLoopCheck::extractIncrementDirections($this->code_base, $this->context, $node->children['loop']);
        if (!$directions) {
            return [];
        }
    ...
    private function warnCommonOuterVariables(array $variables, array $common_outer_variables) : void
    {
        foreach ($common_outer_variables as $variable_name => $node) {
            ...
            $this->emitPluginIssue($this->code_base, (clone $this->context)->withLineNumberStart($inner_node->lineno), 'PhanPluginLoopVariableReuse', 'Variable ${VARIABLE} used in loop was also used in an outer loop on line {LINE}', [$variable_name, $node->lineno]);
        }
    }
    /**
            ...
            case \ast\AST_VAR:
                if ($node->kind === \ast\AST_VAR) {
                    ...
                    if (is_string($var_name)) {
                        if (in_array($var_name, ['this', '_'], \true) || Variable::isHardcodedVariableInScopeWithName($var_name, $this->context->isInGlobalScope())) {
                            return [];
                        }
