...
namespace SdgScoped\Phan\Plugin\Internal\UseReturnValuePlugin;
...
use SdgScoped\Phan\Plugin\Internal\UseReturnValuePlugin;
...
class UseReturnValueVisitor extends PluginAwarePostAnalysisVisitor
{
        ...
        while (\true) {
            switch ($parent->kind) {
                ...
                case \ast\AST_BINARY_OP:
                    if (!\in_array($parent->flags, [\ast\flags\BINARY_COALESCE, \ast\flags\BINARY_BOOL_OR, \ast\flags\BINARY_BOOL_AND], \true) || $node !== $parent->children['right']) {
                        return [null, \true];
                    }
                    break;
                default:
    ...
    public function visitCall(Node $node) : void
    {
        ...
        try {
            $function_list_generator = (new ContextNode($this->code_base, $this->context, $expression))->getFunctionFromNode();
    ...
    public function visitStaticCall(Node $node) : void
    {
        ...
        try {
            $method = (new ContextNode($this->code_base, $this->context, $node))->getMethod($method_name, \true, \true);
        } catch (Exception $_) {
            return;
        }
        $this->checkUseReturnValueGenerator($method, $node);
    ...
    private function checkUseReturnValueGenerator(FunctionInterface $function, Node $node) : void
    {
        if ($function->hasYield() || $function->getUnionType()->isExclusivelyGenerators()) {
            $this->emitPluginIssue($this->code_base, (clone $this->context)->withLineNumberStart($node->lineno), UseReturnValuePlugin::UseReturnValueGenerator, 'Expected to use the return value of the function/method {FUNCTION} returning a generator of type {TYPE}', [$function->getRepresentationForIssue(\true), $function->getUnionType()]);
        }
    }
    ...
    private function shouldNotWarnForDynamicCall($node_name) : bool
    {
        if ($node_name instanceof Node) {
            foreach ((new ContextNode($this->code_base, $this->context, $node_name))->getFunctionFromNode() as $function) {
        ...
        return (UseReturnValuePlugin::HARDCODED_FQSENS[$fqsen_key] ?? null) !== \true;
    }
    /**
    ...
    private function quickWarn(FunctionInterface $method, string $fqsen, Node $node) : bool
    {
        ...
        if ($method->isPHPInternal()) {
            $this->emitPluginIssue($this->code_base, (clone $this->context)->withLineNumberStart($node->lineno), UseReturnValuePlugin::UseReturnValueInternalKnown, 'Expected to use the return value of the internal function/method {FUNCTION}', [$fqsen]);
            return \true;
        }
        ...
        $this->emitPluginIssue($this->code_base, (clone $this->context)->withLineNumberStart($node->lineno), UseReturnValuePlugin::UseReturnValueKnown, 'Expected to use the return value of the user-defined function/method {FUNCTION} defined at {FILE}:{LINE}', [$method->getRepresentationForIssue(), $method->getContext()->getFile(), $method->getContext()->getLineNumberStart()]);
        return \true;
    }
}
