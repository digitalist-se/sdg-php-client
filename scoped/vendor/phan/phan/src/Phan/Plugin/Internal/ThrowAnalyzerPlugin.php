...
class ThrowAnalyzerPlugin extends PluginV3 implements PostAnalyzeNodeCapability, AnalyzeMethodCapability
{
    ...
    public function analyzeMethod(CodeBase $code_base, Method $method) : void
    {
        ...
        Issue::maybeEmit($code_base, $method->getContext(), Issue::ThrowCommentInToString, $method->getContext()->getLineNumberStart(), $method->getRepresentationForIssue(), $throws_union_type);
    }
}
/**
...
class ThrowVisitor extends PluginAwarePostAnalysisVisitor
{
    ...
    public function visitThrow(Node $node) : void
    {
        ...
        $union_type = $this->withoutCaughtUnionTypes($union_type, \true);
        ...
        if (Config::get_closest_target_php_version_id() < 70400) {
            if ($analyzed_function instanceof Method && \strcasecmp('__toString', $analyzed_function->getName()) === 0) {
                $this->emitIssue(Issue::ThrowStatementInToString, $node->lineno, $analyzed_function->getRepresentationForIssue(), (string) $union_type);
            }
        }
     ...
     */
    protected function warnAboutPossiblyThrownType(Node $node, FunctionInterface $analyzed_function, UnionType $union_type, FunctionInterface $call = null) : void
    {
        if ($union_type->isEmpty()) {
            return;
        }
        if (!$union_type->canCastToDeclaredType($this->code_base, $this->context, UnionType::fromFullyQualifiedRealString('\\Throwable'))) {
            $this->emitIssue(Issue::TypeInvalidThrowStatementNonThrowable, $node->lineno, $analyzed_function->getRepresentationForIssue(), ASTReverter::toShortString($node->children['expr']), (string) $union_type, '\\Throwable');
        }
        foreach ($union_type->getTypeSet() as $type) {
            ...
            if ($throws_union_type->isEmpty()) {
                if ($call !== null) {
                    $this->emitIssue(Issue::ThrowTypeAbsentForCall, $node->lineno, $analyzed_function->getRepresentationForIssue(), (string) $union_type, $call->getRepresentationForIssue());
                } else {
                    $this->emitIssue(Issue::ThrowTypeAbsent, $node->lineno, $analyzed_function->getRepresentationForIssue(), ASTReverter::toShortString($node->children['expr']), (string) $union_type);
                }
                continue;
            }
            if (!$expanded_type->canCastToUnionType($throws_union_type)) {
                if ($call !== null) {
                    $this->emitIssue(Issue::ThrowTypeMismatchForCall, $node->lineno, $analyzed_function->getRepresentationForIssue(), (string) $union_type, $call->getRepresentationForIssue(), $throws_union_type);
                } else {
                    $this->emitIssue(Issue::ThrowTypeMismatch, $node->lineno, $analyzed_function->getRepresentationForIssue(), ASTReverter::toShortString($node->children['expr']), (string) $union_type, $throws_union_type);
                }
            }
        }
    }
     ...
     * @param Node $node a node of kind ast\AST_THROW
     */
    protected function warnAboutPossiblyThrownTypeIgnoringFunctionThrowsComment(Node $node, UnionType $union_type, ?FunctionInterface $analyzed_function) : void
    {
        if ($union_type->isEmpty()) {
            return;
        }
        ...
        if (!$union_type->canCastToDeclaredType($this->code_base, $this->context, $throwable)) {
            $this->emitIssue(Issue::TypeInvalidThrowStatementNonThrowable, $node->lineno, $analyzed_function ? $analyzed_function->getRepresentationForIssue() : '(top level statement)', ASTReverter::toShortString($node->children['expr']), (string) $union_type, '\\Throwable');
        }
    }
...
class ThrowRecursiveVisitor extends ThrowVisitor
{
    ...
    public function visitCall(Node $node) : void
    {
        ...
        $analyzed_function = $context->getFunctionLikeInScope($code_base);
        try {
            $function_list_generator = (new ContextNode($code_base, $context, $node->children['expr']))->getFunctionFromNode();
            foreach ($function_list_generator as $invoked_function) {
                // Check the types that can be thrown by this call.
                $this->warnAboutPossiblyThrownType($node, $analyzed_function, $this->withoutCaughtUnionTypes($invoked_function->getThrowsUnionType(), \false));
            }
    ...
    public function visitMethodCall(Node $node) : void
    {
        ...
        try {
            $invoked_method = (new ContextNode($code_base, $context, $node))->getMethod($method_name, \false, \true);
        ...
        $analyzed_function = $context->getFunctionLikeInScope($code_base);
        // Check the types that can be thrown by this call.
        $this->warnAboutPossiblyThrownType($node, $analyzed_function, $this->withoutCaughtUnionTypes($invoked_method->getThrowsUnionType(), \false), $invoked_method);
    }
    /**
     * @override
     */
    public function visitStaticCall(Node $node) : void
    {
        ...
        try {
            ...
            $invoked_method = (new ContextNode($code_base, $context, $node))->getMethod($method_name, \true, \true);
        } catch (\Exception $_) {
