 ...
 */
final class BuiltinSuppressionPlugin extends PluginV3 implements SuppressionCapability
{
    /**
    ...
    public function shouldSuppressIssue(CodeBase $code_base, Context $context, string $issue_type, int $lineno, array $parameters, ?Suggestion $suggestion) : bool
    {
        ...
        if (isset($suppressions_for_issue_type[0])) {
            $this->used_file_based_suppressions[Config::projectPath($context->getFile())][$issue_type] = \true;
            return \true;
        }
        return \false;
    }
     ...
     */
    public function getRawIssueSuppressionList(CodeBase $code_base, string $file_path) : array
    {
        ...
        $file_contents = FileCache::getOrReadEntry($absolute_file_path)->getContents();
        // This is the recommended way to fetch the file contents
        ...
        $this->current_line_suppressions[$absolute_file_path] = ['contents' => $file_contents, 'suppressions' => $suppress_issue_list];
     ...
     * The line number of 0 represents suppressing issues in the entire file.
     */
    private static function computeIssueSuppressionList(string $file_contents) : array
    {
        ...
        $suppression_list = [];
        foreach (self::yieldSuppressionComments($file_contents) as [$comment_text, $comment_start_line, $comment_start_offset, $comment_name, $kind_list_text]) {
            ...
            foreach ($kind_list as $issue_kind) {
                ...
                // How many lines until that comment?
     ...
     */
    private static function yieldSuppressionComments(string $file_contents) : Generator
    {
     ...
     */
    private static function yieldSuppressionCommentsPhpToken(string $file_contents) : Generator
    {
        ...
        foreach (PhpToken::tokenize($file_contents) as $token) {
            ...
            yield from self::yieldSuppressionCommentsFromTokenContents($comment_text, $token->line);
        }
    }
    /**
     * @return Generator<array{0:string,1:int,2:int,3:string,4:string}>
     ...
     */
    private static function yieldSuppressionCommentsOld(string $file_contents) : Generator
    {
        foreach (\token_get_all($file_contents) as $token) {
            ...
            yield from self::yieldSuppressionCommentsFromTokenContents($comment_text, $token[2]);
        }
    }
    /**
     ...
     */
    private static function yieldSuppressionCommentsFromTokenContents(string $comment_text, int $comment_start_line) : array
    {
        // TODO: Emit UnextractableAnnotation if the string begins with phan-suppress or phan-file-suppress but nothing matched
        $match_count = \preg_match_all(self::SUPPRESS_ISSUE_REGEX, $comment_text, $matches, \PREG_OFFSET_CAPTURE);
        $result = [];
        // Support multiple suppressions within a comment. (E.g. for suppressing multiple warnings about a doc comment)
        for ($i = 0; $i < $match_count; $i++) {
            $comment_start_offset = $matches[0][$i][1];
            // byte offset
            $comment_name = $matches[1][$i][0];
            $kind_list_text = $matches[3][$i][0];
            // byte offset
