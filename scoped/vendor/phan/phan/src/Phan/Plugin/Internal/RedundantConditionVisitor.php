...
use SdgScoped\Phan\AST\InferValue;
...
class RedundantConditionVisitor extends PluginAwarePostAnalysisVisitor
{
    ...
    public function visitEmpty(Node $node) : void
    {
        ...
        if (!$real_type->containsTruthy()) {
            RedundantCondition::emitInstance($var_node, $this->code_base, $this->context, Issue::RedundantCondition, [ASTReverter::toShortString($var_node), $type->getRealUnionType(), 'empty'], static function (UnionType $type) : bool {
                return !$type->containsTruthy();
            });
        } elseif (!$real_type->containsFalsey()) {
            RedundantCondition::emitInstance($var_node, $this->code_base, $this->context, Issue::ImpossibleCondition, [ASTReverter::toShortString($var_node), $type->getRealUnionType(), 'empty'], static function (UnionType $type) : bool {
     ...
     */
    public function checkImpossibleMatchArm($cond_node, UnionType $cond_type, Node $arm_node) : void
    {
        ...
        foreach ($arm_node->children['cond']->children ?? [] as $arm_expr) {
            ...
            if (!$cond_type->hasAnyTypeOverlap($code_base, $arm_expr_type)) {
                $this->emitIssueForBinaryOp($arm_node, $cond_type, $arm_expr_type, Issue::ImpossibleTypeComparison, static function (UnionType $new_left_type, UnionType $new_right_type) use($code_base) : bool {
    ...
    */
    public function visitBinaryOp(Node $node) : void
    {
        switch ($node->flags) {
            ...
            case \ast\flags\BINARY_IS_NOT_IDENTICAL:
                $this->checkImpossibleComparison($node, \true);
                break;
    ...
    private const EQUALITY_CHECKS = [\ast\flags\BINARY_IS_EQUAL, \ast\flags\BINARY_IS_IDENTICAL, \ast\flags\BINARY_IS_NOT_EQUAL, \ast\flags\BINARY_IS_NOT_IDENTICAL];
    private function checkImpossibleComparison(Node $node, bool $strict) : void
    {
        ...
        if (!$left->hasAnyTypeOverlap($code_base, $right) && ($strict || (\in_array($node->flags, self::EQUALITY_CHECKS, \true) ? !$left->hasAnyWeakTypeOverlap($right) : !$left->asNonLiteralType()->hasAnyWeakTypeOverlap($right->asNonLiteralType())))) {
            $this->emitIssueForBinaryOp($node, $left, $right, $strict ? Issue::ImpossibleTypeComparison : Issue::SuspiciousWeakTypeComparison, static function (UnionType $new_left_type, UnionType $new_right_type) use($strict, $code_base) : bool {
     ...
     */
    private function checkUselessScalarComparison(Node $node, UnionType $left, UnionType $right, $left_node, $right_node, int $flags) : bool
    {
        // Give up if any of the sides aren't constant
        $left_values = $left->asScalarValues(\true);
        if (!$left_values) {
            return \false;
        }
        $right_values = $right->asScalarValues(\true);
        if (!$right_values) {
            return \false;
        }
        $issue_args = [
        ...
        if ($check_as_if_in_loop_scope) {
            ...
            if ($left_type_fetcher || $right_type_fetcher) {
                ...
                $context->deferCheckToOutermostLoop(static function (Context $context_after_loop) use($code_base, $node, $left_type_fetcher, $right_type_fetcher, $left, $right, $issue_args, $context) : void {
                    ...
                    Issue::maybeEmit($code_base, $context, RedundantCondition::chooseSpecificImpossibleOrRedundantIssueKind($node, $context, Issue::SuspiciousValueComparison), $node->lineno, ...$issue_args);
                });
                return \true;
            }
        }
        ...
        Issue::maybeEmit($code_base, $context, RedundantCondition::chooseSpecificImpossibleOrRedundantIssueKind($node, $issue_context, Issue::SuspiciousValueComparison), $node->lineno, ...$issue_args);
        return \true;
    }
    /**
     ...
     * @suppress PhanAccessMethodInternal
     */
    public function emitIssueForBinaryOp(Node $node, UnionType $left, UnionType $right, string $issue_name, Closure $is_still_issue, $left_node = null, $right_node = null) : void
    {
        $left_node = $left_node ?? $node->children['left'];
        $right_node = $right_node ?? $node->children['right'];
        $issue_args = [ASTReverter::toShortString($left_node), $left, ASTReverter::toShortString($right_node), $right];
        $code_base = $this->code_base;
        $context = $this->context;
        ...
        if ($this->context->isInLoop()) {
            ...
            if ($left_type_fetcher || $right_type_fetcher) {
                ...
                $context->deferCheckToOutermostLoop(static function (Context $context_after_loop) use($code_base, $node, $left_type_fetcher, $right_type_fetcher, $left, $right, $is_still_issue, $issue_name, $issue_args, $context) : void {
                    ...
                    Issue::maybeEmit($code_base, $context, RedundantCondition::chooseSpecificImpossibleOrRedundantIssueKind($node, $context, $issue_name), $node->lineno, ...$issue_args);
                });
                return;
            }
        }
    ...
    */
    ...
    public function visitIsset(Node $node) : void
    {
        ...
        if (!$real_type->containsNullableOrUndefined()) {
            ...
            RedundantCondition::emitInstance($var_node, $this->code_base, (clone $this->context)->withLineNumberStart($node->lineno), Issue::RedundantCondition, [ASTReverter::toShortString($var_node), $real_type, 'isset'], static function (UnionType $type) : bool {
                return !$type->containsNullableOrUndefined();
            });
        } elseif ($real_type->isNull()) {
            RedundantCondition::emitInstance($var_node, $this->code_base, (clone $this->context)->withLineNumberStart($node->lineno), Issue::ImpossibleCondition, [ASTReverter::toShortString($var_node), $real_type, 'isset'], static function (UnionType $type) : bool {
                return $type->isNull();
            });
        }
    }
    ...
    public function visitFor(Node $node) : void
    {
        ...
        foreach ($increment_directions as $key => $is_increasing) {
            ...
            $this->emitIssue(Issue::SuspiciousLoopDirection, $cond_node->lineno, $is_increasing ? 'increase' : 'decrease', ASTReverter::toShortString($loop_node), ASTReverter::toShortString($cond_node));
        }
    }
    /**
     * @override
    ...
    public function visitInstanceof(Node $node) : void
    {
        ...
        if ($real_type_unresolved->isEqualTo($class_type->asRealUnionType()) || $real_type->isExclusivelySubclassesOf($code_base, $class_type)) {
            RedundantCondition::emitInstance($expr_node, $code_base, (clone $this->context)->withLineNumberStart($node->lineno), Issue::RedundantCondition, [ASTReverter::toShortString($expr_node), $real_type_unresolved, $class_type], static function (UnionType $type) use($code_base, $class_type) : bool {
                return $type->isExclusivelySubclassesOf($code_base, $class_type);
            });
        } elseif (!$real_type->canPossiblyCastToClass($code_base, $class_type)) {
            RedundantCondition::emitInstance($expr_node, $code_base, (clone $this->context)->withLineNumberStart($node->lineno), Issue::ImpossibleCondition, [ASTReverter::toShortString($expr_node), $real_type, $class_type], static function (UnionType $type) use($code_base, $class_type) : bool {
    ...
    private function getClassTypeFromNode(Node $class_node) : Type
    {
        if ($class_node->kind === \ast\AST_NAME) {
            $class_union_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $class_node, \false);
    ...
    {
        $expr_node = $node->children['expr'];
        $this->emitIssue($this->chooseIssue($expr_node, Issue::RedundantCondition), $expr_node->lineno ?? $node->lineno, ASTReverter::toShortString($expr_node), $real_expr_type, $expected_type);
    }
    /**
     * @override
     */
