...
use SdgScoped\Phan\Plugin\Internal\VariableTracker\VariableGraph;
use SdgScoped\Phan\Plugin\Internal\VariableTracker\VariableTrackerVisitor;
use SdgScoped\Phan\Plugin\Internal\VariableTracker\VariableTrackingScope;
 ...
 */
final class VariableTrackerPlugin extends PluginV3 implements PostAnalyzeNodeCapability
{
    /**
...
final class VariableTrackerElementVisitor extends PluginAwarePostAnalysisVisitor
{
    ...
    private function checkSideEffectFreeLoopNodes(VariableGraph $variable_graph, VariableTrackerVisitor $variable_tracker_visitor) : void
    {
        ...
        foreach ($loop_nodes as $loop_node) {
            if (!self::definitionInsideNodeHasUseOutsideNode($combined_def_uses, $loop_node)) {
                $this->emitIssue(self::SIDE_EFFECT_FREE_LOOP_ISSUE[$loop_node->kind] ?? Issue::SideEffectFreeForBody, $loop_node->lineno);
            }
        }
    }
    private function checkPossiblyInfiniteLoopNodes(VariableGraph $variable_graph, VariableTrackerVisitor $variable_tracker_visitor) : void
    {
        ...
        foreach ($loop_nodes as $loop_node) {
            ...
            if ($cond instanceof Node) {
                ...
                // @phan-suppress-current-line PhanSuspiciousTruthyString
                continue;
            }
            $this->emitIssue(Issue::PossiblyInfiniteLoop, $loop_node->lineno, ASTReverter::toShortString($cond));
        }
    }
    private const SIDE_EFFECT_FREE_LOOP_ISSUE = [\ast\AST_FOR => Issue::SideEffectFreeForBody, \ast\AST_FOREACH => Issue::SideEffectFreeForeachBody, \ast\AST_WHILE => Issue::SideEffectFreeWhileBody, \ast\AST_DO_WHILE => Issue::SideEffectFreeDoWhileBody];
    /**
    ...
    private const PARAM_MODIFIER_FLAG_SET = \ast\flags\PARAM_MODIFIER_PUBLIC | \ast\flags\PARAM_MODIFIER_PROTECTED | \ast\flags\PARAM_MODIFIER_PRIVATE;
    /**
     * @return array<int, string> maps unique definition ids to issue types
     */
    private static function addParametersAndUseVariablesToGraph(Node $node, VariableGraph $graph, VariableTrackingScope $scope) : array
    {
        $result = [];
     ...
     */
    private function warnAboutVariableGraph(Node $method_node, VariableGraph $graph, array $issue_overrides_for_definition_ids) : void
    {
     ...
     * @param list<string> $issue_overrides_for_definition_ids maps definition ids to issue types
     */
    private function checkSingleDefinitionReferenceOrGlobalOrStatic(VariableGraph $graph, string $variable_name, array $issue_overrides_for_definition_ids) : void
    {
        ...
        if ($type_bitmask === VariableGraph::IS_REFERENCE) {
            Issue::maybeEmitWithParameters($this->code_base, $this->context, Issue::UnusedVariableReference, $line, [$variable_name]);
        } elseif ($type_bitmask === VariableGraph::IS_STATIC) {
            Issue::maybeEmitWithParameters($this->code_base, $this->context, Issue::UnusedVariableStatic, $line, [$variable_name]);
        } elseif ($type_bitmask === VariableGraph::IS_GLOBAL) {
            if (\is_int($definition_id) && $graph->isGlobal($definition_id)) {
                Issue::maybeEmitWithParameters($this->code_base, $this->context, Issue::UnusedVariableGlobal, $line, [$variable_name]);
            }
        }
    }
    /**
    ...
    private function warnAboutCouldBeConstant(VariableGraph $graph, string $variable_name, int $definition_id, $value_node) : void
    {
        ...
        $line = $graph->def_lines[$variable_name][$definition_id] ?? 1;
        Issue::maybeEmitWithParameters($this->code_base, $this->context, $issue_type, $line, [$variable_name]);
    }
