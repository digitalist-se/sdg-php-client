 ...
 */
class DependencyGraphPlugin extends PluginV3 implements AnalyzeClassCapability, PostAnalyzeNodeCapability, FinalizeProcessCapability
{
    /** @var list<Clazz> */
    private $elements = [];
    /** @var string[] */
    ...
    public function processGraph(array $cached_graph = null) : void
    {
        ...
        if ($cmd === 'graph') {
            ...
            echo \json_encode(['cgraph' => $this->cgraph, 'fgraph' => $this->fgraph, 'ctype' => $this->ctype, 'file_to_class' => $this->file_to_class, 'class_to_file' => $this->class_to_file]);
        } elseif ($cmd === 'json' && $this->depth !== 0) {
...
class DependencyGraphVisitor extends PluginAwarePostAnalysisVisitor
{
    ...
    public function visitClassConst(Node $node) : void
    {
        ...
        if (\in_array($called_class_name, ['self', 'parent', 'class', 'static'], \true)) {
            return;
        }
