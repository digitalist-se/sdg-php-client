 ...
 */
final class StringFunctionPlugin extends PluginV3 implements AnalyzeFunctionCallCapability
{
    /**
    ...
    private static function isSimpleExpression($arg) : bool
    {
        ...
        if ($arg instanceof Node) {
            $kind = $arg->kind;
            if (\in_array($kind, [\ast\AST_CONST, \ast\AST_CLASS_CONST, \ast\AST_CLASS_NAME], \true)) {
                return \true;
            }
    ...
    private static function getAnalyzeFunctionCallClosuresStatic() : array
    {
        $make_order_warner = static function (int $expected_const_pos, int $expected_variable_pos) : Closure {
             ...
             */
            return static function (CodeBase $code_base, Context $context, FunctionInterface $function, array $args, ?Node $_) use($expected_const_pos, $expected_variable_pos, $expected_arg_count) : void {
                ...
                if (!self::isSimpleExpression($args[$expected_const_pos])) {
                    if (self::isSimpleExpression($args[$expected_variable_pos])) {
                        Issue::maybeEmit($code_base, $context, Issue::ParamSuspiciousOrder, $context->getLineNumberStart(), $expected_const_pos + 1, $function->getFQSEN(), $expected_variable_pos + 1);
                    }
                }
            };
        };
        ...
        return [
            ...
            'mb_strrchr' => $var_1_const_2,
            // what about mb_strchr?
