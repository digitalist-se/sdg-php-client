 ...
 */
final class DependentReturnTypeOverridePlugin extends PluginV3 implements ReturnTypeOverrideCapability
{
    /**
    ...
    public static function getReturnTypeOverridesStatic(CodeBase $code_base) : array
    {
        ...
        $nullable_string_union_type = StringType::instance(\true)->asPHPDocUnionType();
        ...
        $make_dependent_type_method = static function (int $expected_bool_pos, UnionType $type_if_true, UnionType $type_if_false, UnionType $type_if_unknown) : Closure {
             ...
             */
            return static function (CodeBase $code_base, Context $context, Func $function, array $args) use($type_if_true, $type_if_unknown, $type_if_false, $expected_bool_pos) : UnionType {
         ...
         * @param list<Node|int|float|string> $args
         */
        $bcdiv_callback = static function (CodeBase $code_base, Context $context, Func $function, array $args) use($nullable_string_union_type, $string_union_type) : UnionType {
        ...
        $make_arg_existence_dependent_type_method = static function (int $arg_pos, string $type_if_exists_string, string $type_if_missing_string) : Closure {
             ...
             * @param list<Node|int|float|string> $args
             */
            return static function (CodeBase $unused_code_base, Context $unused_context, Func $unused_function, array $args) use($arg_pos, $type_if_exists, $type_if_missing) : UnionType {
         ...
         * @param list<Node|int|float|string> $args
         */
        $json_decode_return_type_handler = static function (CodeBase $code_base, Context $context, Func $function, array $args) use($json_decode_array_types, $json_decode_object_types, $json_decode_array_or_object_types) : UnionType {
         ...
         * @param list<Node|int|float|string> $args
         */
        $third_argument_string_or_array_handler = static function (CodeBase $code_base, Context $context, Func $unused_function, array $args) use($string_union_type, $str_replace_types, $str_array_type) : UnionType {
            //  mixed json_decode ( string $json [, bool $assoc = FALSE [, int $depth = 512 [, int $options = 0 ]]] )
            //  $options can include JSON_OBJECT_AS_ARRAY in a bitmask
            // TODO: reject `...` operator? (Low priority)
            if (count($args) < 3) {
         ...
         * @param list<Node|int|float|string> $args
         */
        $getenv_handler = static function (CodeBase $unused_code_base, Context $unused_context, Func $unused_function, array $args) use($string_or_false) : UnionType {
         ...
         * @param list<Node|int|float|string> $args
         */
        $substr_handler = static function (CodeBase $unused_code_base, Context $unused_context, Func $unused_function, array $args) use($string_or_false, $string_union_type_with_false_in_real, $string_union_type_real) : UnionType {
            if (Config::get_closest_target_php_version_id() >= 80000) {
         ...
         * @param list<Node|int|float|string> $args
         */
        $count_handler = static function (CodeBase $code_base, Context $context, Func $unused_function, array $args) use($real_int_type) : UnionType {
        ...
        $parse_url_handler = $make_arg_existence_dependent_type_method(1, 'string|int|null|false', 'array{scheme?:string,host?:string,port?:int,user?:string,pass?:string,path?:string,query?:string,fragment?:string}|false');
        /**
         * @param list<Node|int|float|string> $args
         */
        $dirname_handler = static function (CodeBase $code_base, Context $context, Func $unused_function, array $args) use($string_union_type_with_null_in_real) : UnionType {
            if (count($args) !== 1) {
                if (count($args) !== 2) {
                    // Cut down on false positive warnings about substr($str, 0, $len) possibly being false
         ...
         * @param list<Node|int|float|string> $args
         */
        $explode_handler = static function (CodeBase $code_base, Context $context, Func $unused_function, array $args) : UnionType {
            ...
            if (count($args) > 2) {
                ...
                if (\is_object($limit) || $limit < 0) {
                    ...
                    return UnionType::fromFullyQualifiedPHPDocAndRealString('list<string>', $is_php8 ? 'list<string>' : '?list<string>');
                }
            }
            return UnionType::fromFullyQualifiedPHPDocAndRealString('non-empty-list<string>', $is_php8 ? 'non-empty-list<string>' : '?non-empty-list<string>');
        };
        /**
         * @param list<Node|int|float|string> $args
         */
        $one_or_two_string_handler = static function (CodeBase $code_base, Context $context, Func $function, array $args) : UnionType {
            if (Config::get_closest_target_php_version_id() >= 80000) {
    ...
    private static function makeStringFunctionHandler(callable $callable) : Closure
    {
         ...
         * @param list<Node|int|float|string> $args
         */
        return static function (CodeBase $code_base, Context $context, Func $unused_function, array $args) use($string_union_type, $callable) : UnionType {
            if (count($args) !== 1) {
                // Cut down on false positive warnings about substr($str, 0, $len) possibly being false
                return $string_union_type;
            }
