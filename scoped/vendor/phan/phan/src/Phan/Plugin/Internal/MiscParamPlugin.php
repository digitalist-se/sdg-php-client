...
use SdgScoped\Phan\PluginV3\AnalyzeFunctionCallCapability;
 ...
 */
final class MiscParamPlugin extends PluginV3 implements AnalyzeFunctionCallCapability
{
    /**
    ...
    private static function shouldWarnAboutImpossibleArrayKeyExists(CodeBase $code_base, Context $context, array $args, ?UnionType $key_type = null, ?UnionType $array_type = null) : bool
    {
        ...
        '@phan-var ?int|?string $key_value';
        // inferred from $key_can_be_int||$key_can_be_string
    ...
    private static function issueKindForInArrayCheck(CodeBase $code_base, Context $context, array $args) : string
    {
        ...
        $placeholder = new Node(\ast\AST_ARRAY, 0, [new Node(\ast\AST_ARRAY_ELEM, 0, ['key' => null, 'value' => $args[0]], 0), new Node(\ast\AST_ARRAY_ELEM, 0, ['key' => null, 'value' => $args[1]], 0)], 0);
    ...
    private static function emitIssueForInArray(CodeBase $code_base, Context $context, array $args, ?Node $node) : void
    {
        ...
        $issue_args = [ASTReverter::toShortString($needle_node), $needle, 'elements of ' . ASTReverter::toShortString($haystack_node), $haystack_string !== '' ? $haystack_string : '(no types in empty array)'];
        if ($context->isInLoop()) {
            ...
            if ($needle_type_fetcher || $haystack_type_fetcher) {
                ...
                $context->deferCheckToOutermostLoop(static function (Context $context_after_loop) use($code_base, $context, $args, $issue_args, $node, $haystack, $needle, $needle_type_fetcher, $haystack_type_fetcher) : void {
                    ...
                    Issue::maybeEmit($code_base, $context, self::issueKindForInArrayCheck($code_base, $context, $args), $node->lineno ?? $context->getLineNumberStart(), ...$issue_args);
                });
                return;
            }
        }
    ...
    private static function emitIssueForArrayKeyExists(CodeBase $code_base, Context $context, array $args, ?Node $node) : void
    {
        ...
        $issue_args = [ASTReverter::toShortString($key_node), $key_type, 'keys of ' . ASTReverter::toShortString($array_node), $array_string !== '' ? $array_string : '(no types in empty array)'];
        if ($context->isInLoop()) {
            ...
            if ($key_type_fetcher || $array_type_fetcher) {
                ...
                $context->deferCheckToOutermostLoop(static function (Context $context_after_loop) use($code_base, $context, $args, $issue_args, $node, $key_type, $array_type, $key_type_fetcher, $array_type_fetcher) : void {
                    ...
                    Issue::maybeEmit($code_base, $context, self::issueKindForArrayKeyExistsCheck($context, $args), $node->lineno ?? $context->getLineNumberStart(), ...$issue_args);
                });
                return;
            }
        }
    ...
    private static function getAnalyzeFunctionCallClosuresStatic() : array
    {
         ...
         */
        $min_max_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $function, array $args, ?Node $_) : void {
            if (\count($args) !== 1) {
                return;
            }
            ...
            self::analyzeNodeUnionTypeCast($args[0], $context, $code_base, ArrayType::instance(\false)->asPHPDocUnionType(), static function (UnionType $node_type) use($context, $function) : IssueInstance {
                // "arg#1(values) is %s but {$function->getFQSEN()}() takes array when passed only one arg"
                return Issue::fromType(Issue::ParamSpecial2)($context->getFile(), $context->getLineNumberStart(), [1, 'values', (string) $node_type, $function->getRepresentationForIssue(), 'array']);
            });
        };
        /**
         * @param list<Node|int|float|string> $args
         */
        $array_udiff_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $function, array $args, ?Node $_) : void {
            $argcount = \count($args);
            if ($argcount < 3) {
                return;
            }
            self::analyzeNodeUnionTypeCast($args[$argcount - 1], $context, $code_base, CallableType::instance(\false)->asPHPDocUnionType(), static function (UnionType $unused_node_type) use($context, $function) : IssueInstance {
                // "The last argument to {$function->getFQSEN()} must be a callable"
                return Issue::fromType(Issue::ParamSpecial3)($context->getFile(), $context->getLineNumberStart(), [$function->getRepresentationForIssue(), 'callable']);
            });
            for ($i = 0; $i < $argcount - 1; $i++) {
                self::analyzeNodeUnionTypeCast($args[$i], $context, $code_base, ArrayType::instance(\false)->asPHPDocUnionType(), static function (UnionType $node_type) use($context, $function, $i) : IssueInstance {
                    // "arg#".($i+1)." is %s but {$function->getFQSEN()}() takes array"
                    return Issue::fromType(Issue::ParamTypeMismatch)($context->getFile(), $context->getLineNumberStart(), [$i + 1, (string) $node_type, $function->getRepresentationForIssue(), 'array']);
                });
            }
        };
        /**
         ...
         */
        $join_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $function, array $args, ?Node $_) use($stop_exception) : void {
            $argcount = \count($args);
            ...
            if ($argcount === 1) {
                ...
                self::analyzeNodeUnionTypeCastStringArrayLike($args[0], $context, $code_base, static function (UnionType $node_type) use($context, $function) : IssueInstance {
                    // "arg#1(pieces) is %s but {$function->getFQSEN()}() takes array when passed only 1 arg"
                    return Issue::fromType(Issue::ParamSpecial2)($context->getFile(), $context->getLineNumberStart(), [1, 'pieces', $node_type->asNonLiteralType(), $function->getRepresentationForIssue(), 'string[]']);
                });
                throw $stop_exception;
            } elseif ($argcount === 2) {
                $arg1_type = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $args[0]);
                $arg2_type = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $args[1]);
                // TODO: better array checks
                if ($arg1_type->isExclusivelyArray()) {
                    $did_warn = \false;
                    if (!$arg2_type->canCastToUnionType(StringType::instance(\false)->asPHPDocUnionType())) {
                        $did_warn = \true;
                        Issue::maybeEmit($code_base, $context, Issue::ParamSpecial1, $context->getLineNumberStart(), 2, 'glue', (string) $arg2_type->asNonLiteralType(), $function->getRepresentationForIssue(), 'string', 1, 'array');
                    }
                    if (!self::canCastToStringArrayLike($code_base, $context, $arg1_type)) {
                        $did_warn = \true;
                        Issue::maybeEmit($code_base, $context, Issue::TypeMismatchArgumentInternal, $context->getLineNumberStart(), 1, 'pieces', ASTReverter::toShortString($args[0]), $arg1_type, $function->getRepresentationForIssue(), 'string[]');
                    }
                    if (!$did_warn) {
                        Issue::maybeEmit($code_base, $context, Issue::CompatibleImplodeOrder, $context->getLineNumberStart(), $function->getRepresentationForIssue(), (string) $arg1_type->asNonLiteralType(), (string) $arg2_type->asNonLiteralType());
                    }
                    throw $stop_exception;
                } elseif ($arg1_type->isNonNullStringType()) {
                    if (!$arg2_type->canCastToUnionType(ArrayType::instance(\false)->asPHPDocUnionType())) {
                        Issue::maybeEmit($code_base, $context, Issue::ParamSpecial1, $context->getLineNumberStart(), 2, 'pieces', (string) $arg2_type->asNonLiteralType(), $function->getRepresentationForIssue(), 'string[]', 1, 'string');
                    } elseif (!self::canCastToStringArrayLike($code_base, $context, $arg2_type)) {
                        Issue::maybeEmit($code_base, $context, Issue::TypeMismatchArgumentInternal, $context->getLineNumberStart(), 2, 'pieces', ASTReverter::toShortString($args[1]), $arg2_type, $function->getRepresentationForIssue(), 'string[]');
                    }
                    throw $stop_exception;
                }
            }
         ...
         * @param list<Node|int|float|string> $args
         */
        $array_uintersect_uassoc_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $function, array $args, ?Node $_) : void {
            $argcount = \count($args);
            if ($argcount < 4) {
                return;
            }
            ...
            self::analyzeNodeUnionTypeCast($args[$argcount - 2], $context, $code_base, CallableType::instance(\false)->asPHPDocUnionType(), static function (UnionType $unused_node_type) use($context, $function) : IssueInstance {
                // "The second last argument to {$function->getFQSEN()} must be a callable"
                return Issue::fromType(Issue::ParamSpecial4)($context->getFile(), $context->getLineNumberStart(), [$function->getRepresentationForIssue(), 'callable']);
            });
         ...
         */
        $get_variable = static function (CodeBase $code_base, Context $context, $node) : ?Variable {
            if (!$node instanceof Node) {
                return null;
            }
            try {
                return (new ContextNode($code_base, $context, $node))->getVariableStrict();
         ...
         * @param list<Node|int|float|string> $args
         */
        $array_add_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $function, array $args, ?Node $_) : void {
            ...
            $dim_node = new ast\Node(\ast\AST_DIM, $lineno, ['expr' => $modified_array_node, 'dim' => null], 0);
            $new_context = $context;
            for ($i = 1; $i < \count($args); $i++) {
                ...
                $new_context = (new AssignmentVisitor($code_base, $new_context, $dim_node, $right_type, 1))->__invoke($modified_array_node);
            }
            if ($function->getName() === 'array_unshift' && $modified_array_node->kind === \ast\AST_VAR) {
         ...
         * @param list<Node|int|float|string> $args
         */
        $array_remove_single_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $unused_function, array $args, ?Node $_) use($get_variable) : void {
            // TODO: support nested adds, like AssignmentVisitor
            ...
            if (!$old_type->containsFalsey()) {
                ...
                $arg_node->__phan_is_nonempty = \true;
            }
            $variable->setUnionType($old_type->withFlattenedTopLevelArrayShapeTypeInstances()->withPossiblyEmptyArrays());
        };
        /**
         * @param list<Node|int|float|string> $args
         */
        $array_splice_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $unused_function, array $args, ?Node $_) use($get_variable) : void {
            // TODO: support nested adds, like AssignmentVisitor
            // TODO: Could be more specific for arrays with known length and order
            if (count($args) < 4) {
                return;
         ...
         * @param list<Node|int|float|string> $args
         */
        $sort_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $unused_function, array $args, ?Node $_) use($get_variable) : void {
            // TODO: support nested adds, like AssignmentVisitor
            // TODO: Could be more specific for arrays with known length and order
            if (count($args) < 1) {
                return;
            ...
            // TODO: handle empty array
            $new_types = $variable->getUnionType()->withFlattenedTopLevelArrayShapeTypeInstances()->asMappedListUnionType(
                /** @return list<Type> */
         ...
         * @param list<Node|int|float|string> $args
         */
        $associative_sort_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $unused_function, array $args, ?Node $_) use($get_variable) : void {
            // TODO: support nested adds, like AssignmentVisitor
            // TODO: Could be more specific for arrays with known length and order
            if (count($args) < 1) {
                return;
         ...
         */
        $extract_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $unused_function, array $args, ?Node $_) : void {
            // TODO: support nested adds, like AssignmentVisitor
            // TODO: Could be more specific for arrays with known length and order
            if (count($args) < 1) {
                return;
            ...
            foreach ($shape->getFieldTypes() as $field_name => $field_type) {
                ...
                $add_variable = static function (string $name) use($context, $field_type, $scope) : void {
                    ...
                    $scope->addVariable(new Variable($context, $name, $field_type, 0));
                };
         ...
         */
        $define_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $unused_function, array $args, ?Node $_) : void {
            if (count($args) < 2) {
                return;
            }
            $name = $args[0];
            ...
            if (isset($args[2])) {
                ...
                if (!$case_sensitive_arg_type->isType(FalseType::instance(\false))) {
                    Issue::maybeEmit($code_base, $context, Issue::DeprecatedCaseInsensitiveDefine, $args[2]->lineno ?? $context->getLineNumberStart());
                }
            }
            ...
            ParseVisitor::addConstant($code_base, $context, $context->getLineNumberStart(), $name, $args[1], 0, '', \false, \true);
        };
        /**
         * @param list<Node|int|float|string> $args
         */
        $class_alias_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $unused_function, array $args, ?Node $_) : void {
            if (count($args) < 2) {
                return;
            }
                ...
                try {
                    ...
                    if ($code_base->hasClassWithFQSEN($first_param_fqsen)) {
                        ...
                        if ($class->isPHPInternal()) {
                            Issue::maybeEmit($code_base, $context, Issue::ParamMustBeUserDefinedClassname, $args[0]->lineno ?? $context->getLineNumberStart(), $class->getName());
                        }
                    }
                } catch (FQSENException $_) {
                    Issue::maybeEmit($code_base, $context, Issue::TypeComparisonToInvalidClass, $context->getLineNumberStart(), $class_alias_first_param);
                }
            }
        };
        /**
         * @param list<Node|int|float|string> $args
         */
        $in_array_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $unused_function, array $args, ?Node $node) : void {
            if (count($args) < 2) {
                return;
            }
         ...
         * @param list<Node|int|float|string> $args
         */
        $array_key_exists_callback = static function (CodeBase $code_base, Context $context, FunctionInterface $unused_function, array $args, ?Node $node) : void {
            if (count($args) < 2) {
                return;
            }
    ...
    private static function analyzeNodeUnionTypeCast($node, Context $context, CodeBase $code_base, UnionType $cast_type, Closure $issue_instance) : bool
    {
        ...
        $node_type = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $node, \true);
     ...
     * @param Closure(UnionType):IssueInstance $issue_instance
     */
    private static function analyzeNodeUnionTypeCastStringArrayLike($node, Context $context, CodeBase $code_base, Closure $issue_instance) : bool
    {
        // Get the type of the node
        $node_type = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $node, \true);
        // See if it can be cast to the given type
    ...
    {
        if ($union_type->canCastToUnionType(UnionType::fromFullyQualifiedPHPDocString('string[]|int[]'))) {
            return \true;
        }
