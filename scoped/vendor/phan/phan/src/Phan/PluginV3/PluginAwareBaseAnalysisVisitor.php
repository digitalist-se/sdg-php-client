...
abstract class PluginAwareBaseAnalysisVisitor extends AnalysisVisitor
{
     ...
     */
    public function emit(string $issue_type, string $issue_message_fmt, array $issue_message_args = [], int $severity = Issue::SEVERITY_NORMAL, int $remediation_difficulty = Issue::REMEDIATION_B, int $issue_type_id = Issue::TYPE_ID_UNKNOWN) : void
    {
        $this->emitPluginIssue($this->code_base, $this->context, $issue_type, $issue_message_fmt, $issue_message_args, $severity, $remediation_difficulty, $issue_type_id);
    }
