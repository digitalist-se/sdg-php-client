...
class Context extends FileRef
{
    ...
    public function withNamespace(string $namespace) : Context
    {
        ...
        $context->namespace_id += 1;
        // Assumes namespaces are walked in order
    ...
    public function getNamespaceMapFor(int $flags, string $name) : FullyQualifiedGlobalStructuralElement
    {
        ...
        $namespace_map_entry->is_used = \true;
        ...
        switch ($flags) {
            case \ast\flags\USE_NORMAL:
                ...
                return FullyQualifiedClassName::fromFullyQualifiedString($fqsen->__toString() . '\\' . $suffix);
            case \ast\flags\USE_FUNCTION:
                // @phan-suppress-next-line PhanThrowTypeAbsentForCall
                return FullyQualifiedFunctionName::fromFullyQualifiedString($fqsen->__toString() . '\\' . $suffix);
            case \ast\flags\USE_CONST:
                // @phan-suppress-next-line PhanThrowTypeAbsentForCall
                return FullyQualifiedGlobalConstantName::fromFullyQualifiedString($fqsen->__toString() . '\\' . $suffix);
        }
     ...
     */
    public function withNamespaceMap(int $flags, string $alias, FullyQualifiedGlobalStructuralElement $target, int $lineno, CodeBase $code_base = null) : Context
    {
    ...
    private function warnDuplicateUse(CodeBase $code_base, FullyQualifiedGlobalStructuralElement $target, int $lineno, int $flags, string $alias) : void
    {
        ...
        Issue::maybeEmit($code_base, $this, $issue, $lineno, $target, $alias);
    }
    /**
    ...
    {
        return $this->withScope($this->scope->withVariable($variable));
    }
    /**
     * @param Variable $variable
     * A variable to add to the scope for the new
     ...
     */
    public function addScopeVariable(Variable $variable) : void
    {
     ...
     */
    public function unsetScopeVariable(string $variable_name) : void
    {
    ...
    public function getPropertyInScope(CodeBase $code_base) : Property
    {
        ...
        if (!$code_base->hasPropertyWithFQSEN($property_fqsen)) {
            throw new CodeBaseException($property_fqsen, "Cannot find class with FQSEN {$property_fqsen} in context {$this}");
        }
        return $code_base->getPropertyByFQSEN($property_fqsen);
    }
    /**
     * @return bool
    ...
    public function warnAboutUnusedUseElements(CodeBase $code_base) : void
    {
        foreach ($this->namespace_map as $flags => $entries_for_flag) {
            foreach ($entries_for_flag as $namespace_map_entry) {
                ...
                Issue::maybeEmit($code_base, $this, $issue_type, $namespace_map_entry->lineno, $namespace_map_entry->original_name, (string) $namespace_map_entry->fqsen);
            }
        }
    }
    /**
     ...
     */
    protected final function copyPropertiesFrom(Context $other) : void
    {
     ...
     */
    public const VAR_NAME_THIS_PROPERTIES = "phan\x00\$this";
    /**
    ...
    public function withThisPropertySetToType(Property $property, UnionType $type) : Context
    {
        ...
        if ($this->scope->hasVariableWithName(self::VAR_NAME_THIS_PROPERTIES)) {
            ...
            $override_type = ArrayShapeType::fromFieldTypes([$property->getName() => $type], \false);
            $variable = new Variable($this, self::VAR_NAME_THIS_PROPERTIES, $override_type->asPHPDocUnionType(), 0);
        }
