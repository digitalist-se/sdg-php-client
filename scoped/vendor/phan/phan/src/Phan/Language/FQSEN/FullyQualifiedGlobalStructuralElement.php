...
namespace SdgScoped\Phan\Language\FQSEN;
...
{
    use \SdgScoped\Phan\Language\FQSEN\Alternatives;
    ...
    public static function make(string $namespace, string $name, int $alternate_id = 0)
    {
        ...
        if ($name === '') {
            throw new EmptyFQSENException("Empty name of fqsen", \rtrim($namespace, "\\") . "\\" . \implode("\\", \array_merge($name_parts, [$name])));
        }
        foreach ($name_parts as $i => $part) {
            if ($part === '') {
                if ($i > 0) {
                    throw new InvalidFQSENException("Invalid part '' of fqsen", \rtrim($namespace, "\\") . "\\" . \implode('\\', \array_merge(array_slice($name_parts, $i), [$name])));
                }
                continue;
            }
            if (!\preg_match(self::VALID_STRUCTURAL_ELEMENT_REGEX_PART, $part)) {
                throw new InvalidFQSENException("Invalid part '{$part}' of fqsen", \rtrim($namespace, "\\") . "\\{$part}\\" . \implode('\\', \array_merge(array_slice($name_parts, $i), [$name])));
            }
            if ($namespace === '\\') {
                $namespace = '\\' . $part;
            } else {
        ...
        $key = static::class . '|' . static::toString(\strtolower($namespace), static::canonicalLookupKey($name), $alternate_id);
    ...
    public static function makeIfLoaded(string $namespace, string $name)
    {
        ...
        $key = static::class . '|' . static::toString(\strtolower($namespace), static::canonicalLookupKey($name), 0);
        try {
            return self::memoizeStatic(
                $key,
                /**
    ...
    public static function fromStringInContext(string $fqsen_string, Context $context)
    {
        ...
        if ($namespace_map_type === \ast\AST_CONST && \in_array(\strtolower($fqsen_string), ['true', 'false', 'null'], \true)) {
            return static::fromFullyQualifiedString($fqsen_string);
        }
        ...
        if ($context->hasNamespaceMapFor($namespace_map_type, $fqsen_string)) {
            return $context->getNamespaceMapFor($namespace_map_type, $fqsen_string);
        }
     ...
     */
    protected static abstract function getNamespaceMapType() : int;
    /**
     * @return string
    ...
    public function withNamespace(string $namespace)
    {
        ...
        return static::make(self::cleanNamespace($namespace), $this->name, $this->getAlternateId());
    }
    /**
     * @return static
    ...
    public function withAlternateId(int $alternate_id)
    {
        ...
        // @phan-suppress-next-line PhanThrowTypeAbsentForCall the class name was already validated
        return static::make($this->namespace, $this->name, $alternate_id);
    }
    /**
     ...
     */
    public static function toString(string $namespace, string $name, int $alternate_id) : string
    {
    ...
    {
        return $this->as_string ?? ($this->as_string = static::toString($this->namespace, $this->name, $this->getAlternateId()));
    }
}
