...
abstract class FullyQualifiedClassElement extends AbstractFQSEN
{
    ...
    public static function make(FullyQualifiedClassName $fully_qualified_class_name, string $name, int $alternate_id = 0)
    {
        ...
        $key = $fully_qualified_class_name->__toString() . '::' . $name . ',' . $alternate_id . '|' . static::class;
        static $cache = [];
        return $cache[$key] ?? ($cache[$key] = new static($fully_qualified_class_name, $name, $alternate_id));
    }
    /**
     * @return static
    ...
    public function getCanonicalFQSEN()
    {
        ...
        return static::make(
            ...
            // @phan-suppress-current-line PhanPartialTypeMismatchArgument
            $this->name,
            0
        );
    }
    ...
    public static function fromFullyQualifiedString(string $fully_qualified_string)
    {
        ...
        [$fully_qualified_class_name_string, $name_string] = $parts;
    ...
    public static function fromStringInContext(string $fqsen_string, Context $context)
    {
        ...
        if (\count($parts) === 1) {
            ...
            [$class_name_string, $fqsen_string] = $parts;
    ...
    public function withAlternateId(int $alternate_id)
    {
        ...
        return static::make($this->fully_qualified_class_name, $this->name, $alternate_id);
    }
    /**
     * @return string
     * A string representation of the given values
     */
    public static function toString(FullyQualifiedClassName $fqsen, string $name, int $alternate_id) : string
    {
    ...
    public function __toString() : string
    {
        return $this->memoize(__METHOD__, function () : string {
            return self::toString($this->fully_qualified_class_name, $this->name, $this->alternate_id);
        });
    }
}
