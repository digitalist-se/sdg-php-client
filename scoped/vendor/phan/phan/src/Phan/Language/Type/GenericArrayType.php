...
class GenericArrayType extends ArrayType implements GenericArrayInterface
{
     ...
     */
    public const KEY_EMPTY = 0;
    // No way to create this type yet.
    ...
    public const KEY_MIXED = 3;
    // i.e. KEY_INT|KEY_STRING
    public const KEY_NAMES = [self::KEY_EMPTY => 'empty', self::KEY_INT => 'int', self::KEY_STRING => 'string', self::KEY_MIXED => 'mixed'];
    /**
     * @var Type
    ...
    public function withIsNullable(bool $is_nullable) : Type
    {
        ...
        return static::fromElementType($this->element_type, $is_nullable, $this->key_type);
    }
    /**
     * @return bool
     ...
     */
    public static function fromElementType(Type $type, bool $is_nullable, int $key_type) : GenericArrayType
    {
        ...
        if (!$map->contains($type)) {
            $map->attach($type, new GenericArrayType($type, $is_nullable, $key_type));
        }
            ...
            try {
                foreach ($union_type->getTypeSet() as $generic_array_type) {
                    if ($generic_array_type->__toString() !== $representation) {
                        $recursive_union_type_builder->addUnionType($generic_array_type->asExpandedTypes($code_base, $recursion_depth + 1));
                    } else {
    ...
    public function asExpandedTypesPreservingTemplate(CodeBase $code_base, int $recursion_depth = 0) : UnionType
    {
        ...
        return $this->memoize(__METHOD__, function () use($code_base, $recursion_depth) : UnionType {
            ...
            $union_type = $union_type->withUnionType($class_union_type->asGenericArrayTypes($this->key_type));
            // Recurse up the tree to include all types
            $recursive_union_type_builder = new UnionTypeBuilder();
            $representation = $this->__toString();
            try {
    ...
    private function addClassAliases(CodeBase $code_base, UnionTypeBuilder $union_type_builder, FullyQualifiedClassName $class_fqsen) : void
    {
        ...
        foreach ($fqsen_aliases as $alias_fqsen_record) {
            ...
            $union_type_builder->addType($alias_fqsen->asType()->asGenericArrayType($this->key_type));
        }
    }
    /**
     ...
     */
    public static function getKeyTypeOfArrayNode(CodeBase $code_base, Context $context, Node $node, bool $should_catch_issue_exception = \true) : int
    {
        ...
        foreach ($node->children as $child) {
            ...
            if ($key_node instanceof Node) {
                $key_type = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $key_node, $should_catch_issue_exception);
                if ($key_type->isVoidType()) {
                    Issue::maybeEmit($code_base, $context, Issue::TypeVoidExpression, $node->lineno, ASTReverter::toShortString($key_node));
                }
    ...
    public function withStaticResolvedInContext(Context $context) : Type
    {
        ...
        return static::fromElementType($resolved_element_type, $this->is_nullable, $this->key_type);
    }
    /**
    ...
    public function withErasedUnionTypes() : Type
    {
        return $this->memoize(__METHOD__, function () : GenericArrayType {
            ...
            return static::fromElementType($erased_element_type, $this->is_nullable, $this->key_type);
        });
    }
    ...
    {
        return NonEmptyGenericArrayType::fromElementType($this->element_type, \false, $this->key_type);
    }
    /**
    ...
    {
        return AssociativeArrayType::fromElementType($this->element_type, $this->is_nullable, $this->key_type);
    }
    /**
