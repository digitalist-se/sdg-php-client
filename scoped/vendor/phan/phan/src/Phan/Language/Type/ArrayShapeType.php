...
use SdgScoped\Phan\Language\AnnotatedUnionType;
...
final class ArrayShapeType extends ArrayType implements GenericArrayInterface
{
    ...
    public function withIsNullable(bool $is_nullable) : Type
    {
        ...
        return self::fromFieldTypes($this->field_types, $is_nullable);
    }
    /** @override */
    ...
    private function computeGenericArrayTypeInstances() : array
    {
        ...
        foreach ($this->field_types as $key => $field_union_type) {
            foreach ($field_union_type->getTypeSet() as $type) {
                $union_type_builder->addUnionType($type->asPHPDocUnionType()->withFlattenedArrayShapeOrLiteralTypeInstances()->asGenericArrayTypes(\is_string($key) ? GenericArrayType::KEY_STRING : GenericArrayType::KEY_INT)->withIsNullable($this->is_nullable));
            }
        }
    ...
    protected function canCastToNonNullableType(Type $type) : bool
    {
        if ($type instanceof ArrayType) {
            if ($type instanceof GenericArrayType) {
                return $this->canCastToGenericArrayKeys($type) && $this->canEachFieldTypeCastToExpectedUnionType($type->genericArrayElementUnionType());
     ...
     * TODO: deduplicate
     */
    public static function fromFieldTypes(array $field_types, bool $is_nullable) : ArrayShapeType
    {
     ...
     * @param bool $is_nullable
     */
    public static function empty(bool $is_nullable = \false) : ArrayShapeType
    {
        static $nullable_shape = null;
        static $nonnullable_shape = null;
        if ($is_nullable) {
            return $nullable_shape ?? ($nullable_shape = self::fromFieldTypes([], \true));
        }
        ...
        return $this->memoize(__METHOD__, function () use($code_base, $recursion_depth) : UnionType {
            ...
            foreach ($this->field_types as $key => $union_type) {
                ...
                if ($union_type->isPossiblyUndefined()) {
                    ...
                    $expanded_field_type = $union_type->withIsPossiblyUndefined(\true);
                }
    ...
    public function getTemplateTypeExtractorClosure(CodeBase $code_base, TemplateType $template_type) : ?Closure
    {
        ...
        foreach ($this->field_types as $key => $type) {
            ...
            $closure = TemplateType::combineParameterClosures($closure, static function (UnionType $union_type, Context $context) use($key, $field_closure) : UnionType {
    ...
    public function asFunctionInterfaceOrNull(CodeBase $code_base, Context $context) : ?FunctionInterface
    {
        if (\count($this->field_types) !== 2) {
            Issue::maybeEmit($code_base, $context, Issue::TypeInvalidCallableArraySize, $context->getLineNumberStart(), \count($this->field_types));
            return null;
        }
        $i = 0;
        foreach ($this->field_types as $key => $_) {
            if ($key !== $i) {
                ...
                Issue::maybeEmit($code_base, $context, Issue::TypeInvalidCallableArrayKey, $context->getLineNumberStart(), $i);
                return null;
            }
            $i++;
        }
