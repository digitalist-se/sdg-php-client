...
final class NonEmptyAssociativeArrayType extends AssociativeArrayType implements NonEmptyArrayInterface
{
    ...
    public static function fromElementType(Type $type, bool $is_nullable, int $key_type = GenericArrayType::KEY_INT) : GenericArrayType
    {
        ...
        if (!$map->contains($type)) {
            $map->attach($type, new NonEmptyAssociativeArrayType($type, $is_nullable, $key_type));
        }
    ...
    public function __toString() : string
    {
        ...
        return ($this->is_nullable ? '?' : '') . 'non-empty-associative-array<' . $key_details . $this->element_type->__toString() . '>';
    }
    /** @override */
