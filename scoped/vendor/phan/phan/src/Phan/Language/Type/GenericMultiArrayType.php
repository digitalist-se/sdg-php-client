...
final class GenericMultiArrayType extends ArrayType implements MultiType, GenericArrayInterface
{
     ...
     */
    protected function __construct(array $types, bool $is_nullable, int $key_type, bool $always_has_elements = \false, bool $is_list = \false, bool $is_associative = \false)
    {
        if (\count($types) < 2) {
    ...
    public function withIsNullable(bool $is_nullable) : Type
    {
        ...
        return GenericMultiArrayType::fromElementTypes($this->element_types, $is_nullable, $this->key_type, $this->always_has_elements, $this->is_list, $this->is_associative);
    }
    /**
     ...
     * @param bool $is_list
     */
    public static function fromElementTypes(array $element_types, bool $is_nullable, int $key_type, bool $always_has_elements = \false, bool $is_list = \false, bool $is_associative = \false) : GenericMultiArrayType
    {
    ...
    {
        return $this->element_types_union_type ?? ($this->element_types_union_type = UnionType::of(UnionType::normalizeMultiTypes($this->element_types), []));
    }
    ...
    public function asExpandedTypes(CodeBase $code_base, int $recursion_depth = 0) : UnionType
    {
        ...
        try {
            foreach ($this->element_types as $type) {
                $result->addUnionType(GenericArrayType::fromElementType($type, $this->is_nullable, $this->key_type)->asExpandedTypes($code_base, $recursion_depth + 1));
            }
    ...
    public function asExpandedTypesPreservingTemplate(CodeBase $code_base, int $recursion_depth = 0) : UnionType
    {
        ...
        try {
            foreach ($this->element_types as $type) {
                $result->addUnionType(GenericArrayType::fromElementType($type, $this->is_nullable, $this->key_type)->asExpandedTypesPreservingTemplate($code_base, $recursion_depth + 1));
            }
        } catch (RecursionDepthException $_) {
            return ArrayType::instance($this->is_nullable)->asPHPDocUnionType();
        }
