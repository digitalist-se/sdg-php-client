...
class AssociativeArrayType extends GenericArrayType
{
    ...
    public static function fromElementType(Type $type, bool $is_nullable, int $key_type = GenericArrayType::KEY_MIXED) : GenericArrayType
    {
        ...
        if (!$map->contains($type)) {
            $map->attach($type, new AssociativeArrayType($type, $is_nullable, $key_type));
        }
    ...
    {
        return $this->canCastToTypeCommon($type) && parent::canCastToNonNullableType($type);
    }
    protected function canCastToNonNullableTypeWithoutConfig(Type $type) : bool
    {
        return $this->canCastToTypeCommon($type) && parent::canCastToNonNullableTypeWithoutConfig($type);
    }
    ...
    {
        return NonEmptyAssociativeArrayType::fromElementType($this->element_type, \false, $this->key_type);
    }
    public function __toString() : string
    {
        return ($this->is_nullable ? '?' : '') . 'associative-array<' . self::KEY_NAMES[$this->key_type] . ',' . $this->element_type->__toString() . '>';
    }
    /**
