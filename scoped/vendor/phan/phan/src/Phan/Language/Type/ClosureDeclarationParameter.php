...
final class ClosureDeclarationParameter
{
    ...
    public function asNonVariadicRegularParameter(int $i) : Parameter
    {
        ...
        $result = Parameter::create((new Context())->withFile('phpdoc'), "p{$i}", $this->type, $flags);
        if ($this->is_optional && !$this->is_variadic) {
    ...
    public function asRegularParameter(int $i) : Parameter
    {
        ...
        $name = $this->name;
        $result = Parameter::create((new Context())->withFile('phpdoc'), StringUtil::isNonZeroLengthString($name) ? $name : "p{$i}", $this->type, $flags);
        if ($this->is_optional && !$this->is_variadic) {
