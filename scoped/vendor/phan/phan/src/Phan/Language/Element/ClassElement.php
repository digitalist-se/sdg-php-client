...
abstract class ClassElement extends AddressableElement
{
    ...
    private $class_fqsen;
    public function __construct(Context $context, string $name, UnionType $type, int $flags, FullyQualifiedClassElement $fqsen)
    {
    ...
    public function setFQSEN(FQSEN $fqsen) : void
    {
        if (!$fqsen instanceof FullyQualifiedClassElement) {
            throw new TypeError('SdgScoped\\Expected $fqsen to be a subclass of Phan\\Language\\Element\\FullyQualifiedClassElement');
        }
    ...
    public function getDefiningClassFQSEN() : FullyQualifiedClassName
    {
        if (\is_null($this->defining_fqsen)) {
            throw new CodeBaseException($this->fqsen, "No defining class for {$this->fqsen}");
        }
     ...
     */
    public function setDefiningFQSEN(FullyQualifiedClassElement $defining_fqsen) : void
    {
    ...
    public function getDefiningClass(CodeBase $code_base) : Clazz
    {
        ...
        if (!$code_base->hasClassWithFQSEN($class_fqsen)) {
            throw new CodeBaseException($class_fqsen, "Defining class {$class_fqsen} for {$this->fqsen} not found");
        }
     ...
     * class
     */
    public function getClass(CodeBase $code_base) : Clazz
    {
    ...
    {
        return parent::isNSInternal($code_base) || $this->getClass($code_base)->isNSInternal($code_base);
    }
    ...
    {
        $this->setPhanFlags(Flags::bitVectorWithState($this->getPhanFlags(), Flags::IS_PHPDOC_ABSTRACT, $is_abstract));
    }
    /**
     * @return bool
    ...
    {
        $this->setPhanFlags(Flags::bitVectorWithState($this->getPhanFlags(), Flags::IS_OVERRIDE_INTENDED, $is_override_intended));
    }
}
