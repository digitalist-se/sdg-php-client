...
use SdgScoped\ast;
use ast\Node;
use SdgScoped\Phan\Analysis\Analyzable;
...
use SdgScoped\Phan\Language\Scope\FunctionLikeScope;
...
class Method extends ClassElement implements FunctionInterface
{
     ...
     */
    public function __construct(Context $context, string $name, UnionType $type, int $flags, FullyQualifiedMethodName $fqsen, $parameter_list)
    {
        $internal_scope = new FunctionLikeScope($context->getScope(), $fqsen);
        $context = $context->withScope($internal_scope);
        if ($type->hasTemplateType()) {
            $this->recordHasTemplateType();
        }
        parent::__construct($context, FullyQualifiedMethodName::canonicalName($name), $type, $flags, $fqsen);
    ...
    {
        $this->setPhanFlags(Flags::bitVectorWithState($this->getPhanFlags(), Flags::IS_FROM_PHPDOC, $from_phpdoc));
    }
    /**
    ...
    {
        $this->setPhanFlags(Flags::bitVectorWithState($this->getPhanFlags(), Flags::IS_OVERRIDDEN_BY_ANOTHER, $is_overridden_by_another));
    }
    /**
     * @return bool
    ...
    public function isEffectivelyFinal() : bool
    {
        ...
        return Config::getValue('assume_no_external_class_overrides') && !$this->isOverriddenByAnother() && !$this->isAbstract();
    }
    /**
     * @return bool
     ...
     */
    public static function defaultConstructorForClass(Clazz $clazz, CodeBase $code_base) : Method
    {
        if ($clazz->getFQSEN()->getNamespace() === '\\' && $clazz->hasMethodWithName($code_base, $clazz->getName(), \true)) {
        ...
        $method = new Method($old_style_constructor ? $old_style_constructor->getContext() : $clazz->getContext(), '__construct', $clazz->getUnionType(), 0, $method_fqsen, $old_style_constructor ? $old_style_constructor->getParameterList() : null);
    ...
    {
        $method = clone $this;
        $method->setFlags($method->getFlags() & (\ast\flags\MODIFIER_PUBLIC | \ast\flags\MODIFIER_PROTECTED | \ast\flags\MODIFIER_PRIVATE | \ast\flags\MODIFIER_STATIC));
        // clear MODIFIER_ABSTRACT and other flags
        $method->setPhanFlags(($method->getPhanFlags() | Flags::IS_FROM_PHPDOC) & ~(Flags::IS_OVERRIDDEN_BY_ANOTHER | Flags::IS_OVERRIDE));
     ...
     */
    public function createUseAlias(Clazz $clazz, string $alias_method_name, int $new_visibility_flags) : Method
    {
        $method_fqsen = FullyQualifiedMethodName::make($clazz->getFQSEN(), $alias_method_name);
        $method = new Method($this->getContext(), $alias_method_name, $this->getUnionTypeWithUnmodifiedStatic(), $this->getFlags(), $method_fqsen, $this->getParameterList());
        ...
        switch ($new_visibility_flags) {
            ...
            case \ast\flags\MODIFIER_PRIVATE:
                ...
                $method->setFlags(Flags::bitVectorWithState(Flags::bitVectorWithState($method->getFlags(), \ast\flags\MODIFIER_PUBLIC | \ast\flags\MODIFIER_PROTECTED | \ast\flags\MODIFIER_PRIVATE, \false), $new_visibility_flags, \true));
                break;
            default:
                break;
        }
    ...
    private const NON_PURE_METHOD_NAME_SET = [
        ...
        '__destruct' => \true,
        '__set' => \true,
        // This could exist in a pure class to throw exceptions or do nothing
        '__unserialize' => \true,
        '__unset' => \true,
        // This could exist in a pure class to throw exceptions or do nothing
        '__wakeup' => \true,
     ...
     * given context
     */
    public static function fromNode(Context $context, CodeBase $code_base, Node $node, FullyQualifiedMethodName $fqsen, ?Clazz $class = null) : Method
    {
        // Create the skeleton method object from what
        // we know so far
        $method = new Method($context, (string) $node->children['name'], UnionType::empty(), $node->flags, $fqsen, null);
        $doc_comment = $node->children['docComment'] ?? '';
        $method->setDocComment($doc_comment);
        // Parse the comment above the method to get
        // extra meta information about the method.
        $comment = Comment::fromStringInContext($doc_comment, $code_base, $context, $node->lineno, Comment::ON_METHOD);
        ...
        // method
        $parameter_list = Parameter::listFromNode($element_context, $code_base, $node->children['params']);
        ...
        foreach ($parameter_list as $parameter) {
            $method->getInternalScope()->addVariable($parameter->cloneAsNonVariadic());
        }
        ...
        if ($class->isPure() && !$method->isStatic() && !\array_key_exists(\strtolower($method->getName()), self::NON_PURE_METHOD_NAME_SET)) {
            $method->setIsPure();
        }
        ...
        if ($return_type instanceof Node) {
            ...
            $return_union_type = (new UnionTypeVisitor($code_base, $context))->fromTypeInSignature($return_type);
    ...
    public function getUnionType() : UnionType
    {
        ...
        if ($union_type->hasStaticType()) {
            $union_type = $union_type->withType($this->getFQSEN()->getFullyQualifiedClassName()->asType());
        }
        ...
        if ($union_type->genericArrayElementTypes()->hasStaticType()) {
            ...
            $union_type = $union_type->withType($this->getFQSEN()->getFullyQualifiedClassName()->asType()->asGenericArrayType($key_type_enum));
        }
        return $union_type;
    }
     ...
     */
    public function getOverriddenMethods(CodeBase $code_base) : array
    {
        ...
        foreach ($ancestor_class_list as $ancestor_class) {
            ...
            if ($ancestor_class->hasMethodWithName($code_base, $this->name, \true)) {
                $method = $ancestor_class->getMethodByName($code_base, $this->name);
     ...
     */
    public function resolveTemplateType(CodeBase $code_base, UnionType $object_union_type) : Method
    {
