...
final class Builder
{
     ...
     */
    private $issues = [];
    public function __construct(string $comment, CodeBase $code_base, Context $context, int $lineno, int $comment_type, bool $did_add_template_types = \false)
    {
    ...
    /** @internal */
    public const PARAM_COMMENT_REGEX = '/@(?:phan-)?(param|var)\\b\\s*(' . UnionType::union_type_regex . ')?(?:\\s*(\\.\\.\\.)?\\s*&?(?:\\$' . self::WORD_REGEX . '))?/';
    /** @internal */
    public const UNUSED_PARAM_COMMENT_REGEX = '/@(?:phan-)?unused-param\\b\\s*(' . UnionType::union_type_regex . ')?(?:\\s*(\\.\\.\\.)?\\s*&?(?:\\$' . self::WORD_REGEX . '))/';
    /**
     * @param string $line
     ...
     */
    private function parameterFromCommentLine(string $line, bool $is_var, int $i) : Parameter
    {
        ...
        if ($matched && (isset($match[2]) || isset($match[17]))) {
            ...
            if ($is_var && $is_variadic) {
                ...
                if ($is_var && $variable_name === '' && $this->comment_type === Comment::ON_PROPERTY) {
                    ...
                    if (\ord($char_at_end_offset) > 32 && !\preg_match('@^\\*+/$@D', (string) \substr($line, $end_offset))) {
                        // Not a control character or space
                        $this->emitIssue(Issue::UnextractableAnnotationSuffix, $this->guessActualLineLocation($i), \trim($line), $original_type, $char_at_end_offset);
                    }
                }
            }
            ...
            if (0 !== \strpos($type, '$')) {
                $union_type = UnionType::fromStringInContext($type, $this->context, Type::FROM_PHPDOC, $this->code_base);
            } else {
        ...
        if (!\preg_match('/@(param|var)\\s+(\\.\\.\\.)?\\s*(\\$\\S+)/', $line)) {
            $this->emitIssue(Issue::UnextractableAnnotation, $this->guessActualLineLocation($i), \trim($line));
        }
     ...
     */
    private function returnTypeFromCommentLine(string $line, int $i) : UnionType
    {
        ...
        if (\preg_match(self::RETURN_COMMENT_REGEX, $line, $match)) {
            ...
            if (\ord($char_at_end_offset) > 32 && !\preg_match('@^\\*+/$@D', (string) \substr($line, $end_offset))) {
                // Not a control character or space
                $this->emitIssue(Issue::UnextractableAnnotationSuffix, $this->guessActualLineLocation($i), \trim($line), $return_union_type_string, $char_at_end_offset);
            }
        } else {
            $this->emitIssue(Issue::UnextractableAnnotation, $this->guessActualLineLocation($i), \trim($line));
        }
        ...
        $return_union_type = UnionType::fromStringInContext($return_union_type_string, $this->context, Type::FROM_PHPDOC, $this->code_base);
        return $return_union_type;
    }
    private static function rewritePHPDocType(string $original_type) : string
    {
     ...
     */
    private function checkParamWithoutVarName(string $line, string $raw_match, string $union_type_string, int $i) : void
    {
        $match_offset = \strpos($line, $raw_match);
        $end_offset = $match_offset + \strlen($raw_match);
        $char_at_end_offset = $line[$end_offset] ?? ' ';
        $issue_line = $this->guessActualLineLocation($i);
        if (\ord($char_at_end_offset) > 32) {
            // Not a control character or space
            $this->emitIssue(Issue::UnextractableAnnotationSuffix, $issue_line, \trim($line), $union_type_string, $char_at_end_offset);
        }
        $this->emitIssue(Issue::UnextractableAnnotationElementName, $issue_line, \trim($line), $union_type_string);
    }
    /**
    ...
    private function buildWithTemplateTypes() : Comment
    {
        ...
        return (new self($this->comment, $this->code_base, $new_context, $this->lineno, $this->comment_type, \true))->build();
    }
        ...
        if (\preg_match('/@((?i)param|deprecated|var|return|throws|throw|returns|inherits|extends|suppress|unused-param|phan-[a-z0-9_-]*(?-i)|method|property|property-read|property-write|abstract|template|PhanClosureScope|readonly|mixin|seal-(?:methods|properties))(?:[^a-zA-Z0-9_\\x7f-\\xff-]|$)/D', $line, $matches)) {
            ...
            switch ($type) {
                ...
                case 'return':
                    ...
                    break;
                case 'returns':
                    $this->emitIssue(Issue::MisspelledAnnotation, $this->guessActualLineLocation($i), '@returns', 'Did you mean @return?');
                    break;
                case 'throws':
                    $this->maybeParseThrows($i, $line);
                    break;
                case 'throw':
                    $this->emitIssue(Issue::MisspelledAnnotation, $this->guessActualLineLocation($i), '@throw', 'Did you mean @throws?');
                    break;
                case 'suppress':
    ...
    private function parseMixin(int $i, string $line, string $annotation_name) : void
    {
        ...
        if (\preg_match('/@(phan-)?mixin\\s+(\\\\?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*)\\b/', $line, $matches)) {
            ...
            $type = Type::fromStringInContext($type_string, $this->context, Type::FROM_PHPDOC, $this->code_base);
            // TODO: Warn about invalid mixins
            if ($type->isObjectWithKnownFQSEN()) {
                $this->phan_overrides['mixin'][] = $type;
            } else {
                Issue::maybeEmit($this->code_base, $this->context, Issue::InvalidMixin, $this->guessActualLineLocation($i), $type);
            }
        }
    }
    ...
    private function maybeParseVarLine(int $i, string $line) : void
    {
        ...
        $comment_var = self::parameterFromCommentLine($line, \true, $i);
        if (\in_array($this->comment_type, Comment::FUNCTION_LIKE, \true)) {
    ...
    private function assertFromCommentLine(string $line) : ?Assertion
    {
        ...
        if (\strlen($extra_text) > 0) {
            ...
            $type_string = $match[3];
            $union_type = UnionType::fromStringInContext($type_string, $this->context, Type::FROM_PHPDOC, $this->code_base);
        }
        $param_name = $match[17];
    ...
    private function maybeParseThrows(int $i, string $line) : void
    {
        ...
        $this->throw_union_type = $this->throw_union_type->withUnionType($this->returnTypeFromCommentLine($line, $i));
    }
        ...
        switch ($type) {
            ...
            case 'phan-param':
                if ($this->checkCompatible('@phan-param', Comment::FUNCTION_LIKE, $i)) {
                    $this->phan_overrides['param'][] = $this->parameterFromCommentLine($line, \false, $i);
                }
                return;
            ...
            case 'phan-var':
                ...
                $comment_var = $this->parameterFromCommentLine($line, \true, $i);
                if (\in_array($this->comment_type, Comment::FUNCTION_LIKE, \true)) {
                    if ($comment_var->getName() !== '') {
            ...
            case 'phan-write-only':
                $this->setPhanAccessFlag($i, \true, 'phan-write-only');
                return;
            case 'phan-transient':
            ...
            case 'phan-mixin':
                ...
                return;
            default:
                $this->emitIssueWithSuggestion(Issue::MisspelledAnnotation, $this->guessActualLineLocation($i), ['@' . $case_sensitive_type, "The annotations that this version of Phan supports can be seen by running 'phan --help-annotations' or by visiting https://github.com/phan/phan/wiki/Annotating-Your-Source-Code"], self::generateSuggestionForMisspelledAnnotation($case_sensitive_type));
                return;
        }
    }
    ...
    {
        if (\in_array($this->comment_type, $valid_types, \true) || $this->comment_type === Comment::ON_ANY) {
            return \true;
        }
        $this->emitInvalidCommentForDeclarationType($param_name, $this->guessActualLineLocation($i));
        return \false;
    }
    private function emitInvalidCommentForDeclarationType(string $annotation_type, int $issue_lineno) : void
    {
        $this->emitIssue(Issue::InvalidCommentForDeclarationType, $issue_lineno, $annotation_type, Comment::NAME_FOR_TYPE[$this->comment_type]);
    }
    /**
     * @param string $line
     * An individual line of a comment
     ...
     * wasn't found.
     */
    private static function templateTypeFromCommentLine(string $line) : ?TemplateType
    {
     ...
     */
    private function inheritsFromCommentLine(string $line) : Option
    {
        $match = [];
        if (\preg_match('/@(?:phan-)?(?:inherits|extends)\\s+(' . Type::type_regex . ')/', $line, $match)) {
            $type_string = $match[1];
            $type = new Some(Type::fromStringInContext($type_string, $this->context, Type::FROM_PHPDOC, $this->code_base));
            return $type;
        }
     ...
     */
    private static function suppressIssuesFromCommentLine(string $line) : array
    {
     ...
     */
    private function magicParamFromMagicMethodParamString(string $param_string, int $param_index, int $comment_line_offset) : ?Parameter
    {
        ...
        if (\preg_match(self::MAGIC_PARAM_REGEX, $param_string, $param_match)) {
            ...
            $union_type = UnionType::fromStringInContext($union_type_string, $this->context, Type::FROM_PHPDOC, $this->code_base);
     ...
     */
    private function magicMethodFromCommentLine(string $line, int $comment_line_offset) : ?Method
    {
        ...
        if (\preg_match('/@(?:phan-)?method(?:\\s+(static))?(?:(?:\\s+(' . UnionType::union_type_regex_or_this . '))?)\\s+' . self::WORD_REGEX . '\\s*\\(((?:[^()]|\\([()]*\\))*)\\)\\s*(.*)/', $line, $match)) {
            ...
            if ($arg_list !== '') {
                ...
                foreach ($params_strings as $i => $param_string) {
                    ...
                    if ($param === null) {
                        $this->emitIssue(Issue::UnextractableAnnotationPart, $this->guessActualLineLocation($comment_line_offset), \trim($line), $param_string);
                        $failed = \true;
                    }
            ...
            return new Method($method_name, $return_union_type, $comment_params, $is_static, $this->guessActualLineLocation($comment_line_offset));
        } else {
            $this->emitIssue(Issue::UnextractableAnnotation, $this->guessActualLineLocation($comment_line_offset), \trim($line));
        }
        return null;
    }
    /**
     ...
     */
    private function magicPropertyFromCommentLine(string $line, int $i) : ?Property
    {
        ...
        if (\preg_match('/@(?:phan-)?(property|property-read|property-write)(?:\\s+(' . UnionType::union_type_regex . '))?(?:\\s+(?:\\$' . self::WORD_REGEX . '))/', $line, $match)) {
            ...
            $union_type = UnionType::fromStringInContext($type, $this->context, Type::FROM_PHPDOC, $this->code_base);
            return new Property($property_name, $union_type, $this->guessActualLineLocation($i), $flags);
        } else {
            $this->emitIssue(Issue::UnextractableAnnotation, $this->guessActualLineLocation($i), \trim($line));
        }
        return null;
     ...
     */
    private function getPhanClosureScopeFromCommentLine(string $line, int $comment_line_offset) : Option
    {
        ...
        if ($closure_scope_union_type_string !== '') {
            return new Some(Type::fromStringInContext($closure_scope_union_type_string, $this->context, Type::FROM_PHPDOC, $this->code_base));
        }
        $this->emitIssue(Issue::UnextractableAnnotation, $this->guessActualLineLocation($comment_line_offset), \trim($line));
        return None::instance();
    }
     ...
     */
    protected function emitIssue(string $issue_type, int $issue_lineno, ...$parameters) : void
    {
        $this->issues[] = [$issue_type, $issue_lineno, $parameters, null];
    }
    /**
     * @param string $issue_type
     * The type of issue to emit such as Issue::ParentlessClass
     ...
     */
    protected function emitIssueWithSuggestion(string $issue_type, int $issue_lineno, array $parameters, Suggestion $suggestion = null) : void
    {
        $this->issues[] = [$issue_type, $issue_lineno, $parameters, $suggestion];
    }
    protected function emitDeferredIssues() : void
    {
        foreach ($this->issues as [$issue_type, $issue_lineno, $parameters, $suggestion]) {
            ...
            Issue::maybeEmitWithParameters($this->code_base, $this->context, $issue_type, $issue_lineno, $parameters, $suggestion);
        }
        $this->issues = [];
    }
}
