...
use SdgScoped\Phan\Analysis\FallbackMethodTypesVisitor;
use SdgScoped\Phan\Analysis\NegatedConditionVisitor;
use SdgScoped\Phan\Analysis\ParameterTypesAnalyzer;
...
use SdgScoped\Phan\Language\Element\Comment\Assertion;
...
trait FunctionTrait
{
    ...
    public abstract function getPhanFlags() : int;
    /** @return bool true if all of the bits in $bits are true in $this->getPhanFlags() */
    public abstract function getPhanFlagsHasState(int $bits) : bool;
    public abstract function setPhanFlags(int $phan_flags) : void;
    /**
     * @return string
     ...
     * structural element
     */
    public abstract function getFQSEN() : FQSEN;
    /**
     * @return string
    ...
    {
        return $this->number_of_required_real_parameters + $this->number_of_optional_real_parameters;
    }
    /**
     * @return int
    ...
    {
        return $this->number_of_required_parameters + $this->number_of_optional_parameters;
    }
    /**
     * @return int
    ...
    private function initParameterListInfo() : void
    {
        ...
        foreach ($parameter_list as $param) {
            if ($param->isPassByReference()) {
                $has_pass_by_reference_parameters = \true;
                break;
            }
        }
     ...
     */
    public static function addParamsToScopeOfFunctionOrMethod(Context $context, CodeBase $code_base, FunctionInterface $function, Comment $comment) : void
    {
        ...
        foreach ($function_parameter_list as $parameter) {
            ...
            self::addParamToScopeOfFunctionOrMethod($context, $code_base, $function, $comment, $parameter_offset, $parameter);
            ++$parameter_offset;
        }
        ...
        foreach ($comment->getParameterMap() as $comment_parameter_name => $comment_parameter) {
            if (!\array_key_exists($comment_parameter_name, $real_parameter_name_map)) {
                Issue::maybeEmit($code_base, $context, count($real_parameter_name_map) > 0 ? Issue::CommentParamWithoutRealParam : Issue::CommentParamOnEmptyParamList, $comment_parameter->getLineno(), $comment_parameter_name, (string) $function);
                continue;
            }
        ...
        foreach ($comment->getVariableList() as $comment_variable) {
            if (\array_key_exists($comment_variable->getName(), $real_parameter_name_map)) {
                Issue::maybeEmit($code_base, $context, Issue::CommentVarInsteadOfParam, $comment_variable->getLineno(), $comment_variable->getName(), (string) $function);
                continue;
            }
        }
     ...
     * Internally used.
     */
    public static function addParamToScopeOfFunctionOrMethod(Context $context, CodeBase $code_base, FunctionInterface $function, Comment $comment, int $parameter_offset, Parameter $parameter) : void
    {
        if ($function->isPHPInternal()) {
            return;
        }
        ...
        if ($comment->hasParameterWithNameOrOffset($parameter_name, $parameter_offset)) {
            $comment_param = $comment->getParameterWithNameOrOffset($parameter_name, $parameter_offset);
        ...
        if ($parameter->getNonVariadicUnionType()->isEmpty()) {
            ...
            if ($comment_param) {
                ...
                if ($parameter->isVariadic() !== $comment_param->isVariadic()) {
                    Issue::maybeEmit($code_base, $context, $parameter->isVariadic() ? Issue::TypeMismatchVariadicParam : Issue::TypeMismatchVariadicComment, $comment_param->getLineno(), $comment_param->__toString(), $parameter->__toString());
                }
        ...
        if ($parameter->hasDefaultValue()) {
            ...
            if (!$default_is_null) {
                if (!$default_type->canCastToUnionType($parameter->getUnionType())) {
                    Issue::maybeEmit($code_base, $context, Issue::TypeMismatchDefault, $function->getFileRef()->getLineNumberStart(), (string) $parameter->getUnionType(), $parameter_name, (string) $default_type);
                }
            }
            ...
            if ($default_is_null) {
                if ($was_empty) {
                    $parameter->addUnionType(MixedType::instance(\true)->asPHPDocUnionType());
                }
                ...
                if ($was_empty) {
                    ...
                    foreach ($default_type->getTypeSet() as $default_type_part) {
                        if (!$parameter->getNonvariadicUnionType()->hasType($default_type_part->withIsNullable(\true))) {
                            // if ($parameter->isCloneOfVariadic()) { throw new \Error("Impossible\n"); }
    ...
    public function analyzeWithNewParams(Context $context, CodeBase $code_base, array $parameter_list) : Context
    {
        ...
        if ($hash === $this->parameter_list_hash) {
            ...
            $has_pass_by_reference_variable = \true;
        }
    ...
    public function setFunctionCallAnalyzer(Closure $closure) : void
    {
        ...
        $this->function_call_analyzer_callback_set = [$closure_id => \true];
    ...
    public function ensureScopeInitialized(CodeBase $code_base) : void
    {
        ...
        $this->is_inner_scope_initialized = \true;
        $comment = $this->comment;
     ...
     */
    protected abstract function memoize(string $key, Closure $fn);
    protected abstract function memoizeFlushAll() : void;
    ...
    public abstract function setUnionType(UnionType $type) : void;
    /**
    ...
    {
        $this->setParameterList(\array_map(static function (Parameter $parameter) : Parameter {
    ...
    private function createFunctionLikeDeclarationType() : FunctionLikeDeclarationType
    {
        ...
        return new ClosureDeclarationType($this->getFileRef(), $params, $return_type, $this->returnsRef(), \false);
    }
    /**
    ...
    public function toFunctionSignatureArray() : array
    {
        ...
        '@phan-var array<mixed,string> $stub';
        // TODO: Should not warn about PhanTypeMismatchDimFetch in isset below
        foreach ($this->parameter_list as $parameter) {
    ...
    public function analyzeReturnTypes(CodeBase $code_base) : void
    {
        ...
        $this->did_analyze_return_types = \true;
        try {
     ...
     * Is this internal?
     */
    public abstract function isPHPInternal() : bool;
    /**
     * Returns this function's union type without resolving `static` in the function declaration's context.
     */
    public abstract function getUnionTypeWithUnmodifiedStatic() : UnionType;
    private function analyzeReturnTypesInner(CodeBase $code_base) : void
    {
        ...
        foreach ($return_type->withFlattenedArrayShapeOrLiteralTypeInstances()->getTypeSet() as $outer_type) {
            foreach ($outer_type->getReferencedClasses() as $type) {
                ...
                if ($type instanceof TemplateType) {
                    if ($this instanceof Method) {
                        if ($this->isStatic() && !$this->declaresTemplateTypeInComment($type)) {
                            Issue::maybeEmit($code_base, $context, Issue::TemplateTypeStaticMethod, $this->getFileRef()->getLineNumberStart(), (string) $this->getFQSEN());
                        }
                    }
                    continue;
                }
                ...
                if (!$code_base->hasClassWithFQSEN($type_fqsen)) {
                    Issue::maybeEmitWithParameters($code_base, $this->getContext(), Issue::UndeclaredTypeReturnType, $this->getFileRef()->getLineNumberStart(), [$this->getNameForIssue(), (string) $outer_type], IssueFixSuggester::suggestSimilarClass($code_base, $this->getContext(), $type_fqsen, null, 'Did you mean', IssueFixSuggester::CLASS_SUGGEST_CLASSES_AND_TYPES_AND_VOID));
                } elseif ($code_base->hasClassWithFQSEN($type_fqsen->withAlternateId(1))) {
                    UnionType::emitRedefinedClassReferenceWarning($code_base, $this->getContext(), $type_fqsen);
                }
            }
        }
        if (Config::getValue('check_docblock_signature_return_type_match') && !$real_return_type->isEmpty() && $phpdoc_return_type instanceof UnionType && !$phpdoc_return_type->isEmpty()) {
            ...
            foreach ($phpdoc_return_type->getTypeSet() as $phpdoc_type) {
                $is_exclusively_narrowed = $phpdoc_type->isExclusivelyNarrowedFormOrEquivalentTo($resolved_real_return_type, $context, $code_base);
    ...
    public function declaresTemplateTypeInComment(TemplateType $template_type) : bool
    {
        if ($this->comment) {
            ...
            return \in_array($template_type, $this->comment->getTemplateTypeList(), \true);
        }
        return \false;
    }
    ...
    private function addClosureForDependentTemplateType(CodeBase $code_base, Context $context, array $template_type_list) : void
    {
        ...
        $has_all_templates = \true;
        foreach ($template_type_list as $template_type) {
            if (!$this->isTemplateTypeUsed($template_type)) {
                Issue::maybeEmit($code_base, $context, Issue::TemplateTypeNotUsedInFunctionReturn, $context->getLineNumberStart(), $template_type, $this->getNameForIssue());
            ...
            if (!$parameter_extractor) {
                Issue::maybeEmit($code_base, $context, Issue::TemplateTypeNotDeclaredInFunctionParams, $context->getLineNumberStart(), $template_type, $this->getNameForIssue());
                $has_all_templates = \false;
                continue;
            }
    ...
    private function getPluginForParamAssertionMap(CodeBase $code_base, array $param_assertion_map, ?Closure $closure) : ?Closure
    {
        foreach ($param_assertion_map as $param_name => $assertion) {
            ...
            if ($i === null) {
                Issue::maybeEmit($code_base, $this->getContext(), Issue::CommentParamAssertionWithoutRealParam, $this->getContext()->getLineNumberStart(), $param_name, $this->getNameForIssue());
                continue;
            }
        ...
        if (is_int($last_mandatory_phpdoc_param_offset)) {
            ...
            if ($param) {
                ...
                $closure = static function (CodeBase $code_base, Context $context, FunctionInterface $function, array $array) use($param, $last_mandatory_phpdoc_param_offset) : void {
                    ...
                    Issue::maybeEmit($code_base, $context, Issue::ParamTooFewInPHPDoc, $context->getLineNumberStart(), count($array), $function->getRepresentationForIssue(\true), $last_mandatory_phpdoc_param_offset + 1, $param->getName(), $function->getContext()->getFile(), $function->getContext()->getLineNumberStart());
                };
            }
        }
