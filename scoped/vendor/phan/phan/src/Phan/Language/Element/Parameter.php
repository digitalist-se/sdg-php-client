...
class Parameter extends Variable
{
     ...
     * @return static
     */
    public static function create(Context $context, string $name, UnionType $type, int $flags)
    {
        ...
        if ($future_type !== null) {
            ...
            try {
                ...
                Issue::maybeEmitInstance(
                    ...
                    // @phan-suppress-current-line PhanAccessMethodInternal
     ...
     * @return list<Parameter>
     */
    public static function listFromReflectionParameterList(array $reflection_parameters) : array
    {
     ...
     */
    public static function fromReflectionParameter(\ReflectionParameter $reflection_parameter) : Parameter
    {
        $flags = 0;
        ...
        $parameter = self::create(new Context(), $reflection_parameter->getName() ?? "arg", $parameter_type, $flags);
        if ($reflection_parameter->isOptional()) {
            ...
            if ($reflection_parameter->isDefaultValueAvailable()) {
                try {
                    ...
                    $parameter->default_value_from_reflection = \true;
                } catch (Throwable $e) {
                    CLI::printErrorToStderr(\sprintf("Warning: encountered invalid ReflectionParameter information for param \$%s: %s %s\n", $reflection_parameter->getName(), \get_class($e), $e->getMessage()));
    ...
    public static function fromNode(Context $context, CodeBase $code_base, Node $node) : Parameter
    {
        ...
        $parameter = Parameter::create((clone $context)->withLineNumberStart($node->lineno), $parameter_name, $union_type, $node->flags);
        ...
        if (preg_match('/^(_$|unused)/iD', $parameter_name)) {
            if ($default_node !== null) {
                $parameter->should_warn_if_provided = \true;
            }
        ...
        if ($default_node !== null) {
            ...
            try {
                ...
                Issue::maybeEmit($code_base, $context, Issue::InvalidConstantExpression, $default_node->lineno ?? $node->lineno);
                $has_error = \true;
            }
            ...
            if ($default_value_union_type !== null) {
                ...
                $parameter->setDefaultValueType($default_value_union_type);
                if (!$has_error) {
                    $parameter->setDefaultValueFutureType(new FutureUnionType($code_base, (clone $context)->withLineNumberStart($default_node->lineno ?? 0), $default_node));
                }
            }
    ...
    private static function warnAboutParamNameIndicatingUnused(CodeBase $code_base, Context $context, Node $node, string $parameter_name) : void
    {
        ...
        Issue::maybeEmit($code_base, $context, $is_closure ? Issue::ParamNameIndicatingUnusedInClosure : Issue::ParamNameIndicatingUnused, $node->lineno, $parameter_name);
    }
    /**
     * @return bool
    ...
    public function __toString() : string
    {
        ...
        if ($flags & self::PARAM_MODIFIER_VISIBILITY_FLAGS) {
            $string .= $flags & \ast\flags\PARAM_MODIFIER_PUBLIC ? 'public ' : ($flags & \ast\flags\PARAM_MODIFIER_PROTECTED ? 'protected ' : 'private ');
        }
    ...
    private function generateDefaultNodeRepresentation(bool $is_internal = \true) : string
    {
        ...
        if ($default_value instanceof Node) {
            ...
            if (\in_array($kind, [\ast\AST_CONST, \ast\AST_CLASS_CONST, \ast\AST_MAGIC_CONST], \true)) {
    ...
    public function asClosureDeclarationParameter() : ClosureDeclarationParameter
    {
        ...
        return new ClosureDeclarationParameter($param_type, $this->isVariadic(), $this->isPassByReference(), $this->isOptional());
    }
    /**
    ...
    public function copyDefaultValueFrom(Parameter $other) : void
    {
        ...
        if ($other->default_value_from_reflection) {
            $this->default_value_from_reflection = \true;
        }
    }
    /**
