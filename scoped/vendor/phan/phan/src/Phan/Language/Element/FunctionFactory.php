...
class FunctionFactory
{
     ...
     */
    public static function functionListFromReflectionFunction(FullyQualifiedFunctionName $fqsen, \ReflectionFunction $reflection_function) : array
    {
        $context = new Context();
        $namespaced_name = $fqsen->getNamespacedName();
        $function = new Func($context, $namespaced_name, UnionType::empty(), 0, $fqsen, null);
     ...
     */
    public static function functionListFromSignature(FullyQualifiedFunctionName $fqsen, array $signature) : array
    {
        // TODO: Look into adding helper method in UnionType caching this to speed up loading.
        $context = new Context();
        $return_type = UnionType::fromStringInContext($signature[0], $context, Type::FROM_TYPE);
        unset($signature[0]);
        $func = new Func($context, $fqsen->getNamespacedName(), $return_type, 0, $fqsen, []);
     ...
     */
    public static function methodListFromReflectionClassAndMethod(Context $context, \ReflectionClass $class, \ReflectionMethod $reflection_method) : array
    {
        ...
        $method = new Method($context, $reflection_method->name, UnionType::empty(), $modifiers, $method_fqsen, null);
        ...
        if ($defining_class_name !== $class_name) {
            $method->setDefiningFQSEN(FullyQualifiedMethodName::make(
                // @phan-suppress-next-line PhanThrowTypeAbsentForCall
     ...
     */
    public static function functionListFromFunction(FunctionInterface $function) : array
    {
         ...
         */
        return \array_map(static function (array $map) use($function, &$alternate_id) : FunctionInterface {
            ...
            foreach ($map['parameter_name_type_map'] ?? [] as $parameter_name => $parameter_type) {
                ...
                $parameter = Parameter::create($function->getContext(), $parameter_name, $parameter_type, $flags);
            ...
            $alternate_function->setNumberOfRequiredParameters(\array_reduce($alternate_function->getParameterList(), static function (int $carry, Parameter $parameter) : int {
