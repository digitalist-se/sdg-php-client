...
class Func extends AddressableElement implements FunctionInterface
{
     ...
     */
    public function __construct(Context $context, string $name, UnionType $type, int $flags, FullyQualifiedFunctionName $fqsen, $parameter_list)
    {
        if ($fqsen->isClosure()) {
            $internal_scope = new ClosureScope($context->getScope(), $fqsen);
        } else {
     ...
     */
    private static function getClosureOverrideFQSEN(CodeBase $code_base, Context $context, Type $closure_scope_type, Node $node) : ?FullyQualifiedClassName
    {
        ...
        if ($closure_scope_type->isNativeType()) {
            ...
            // Emit an error
            Issue::maybeEmit($code_base, $context, Issue::TypeInvalidClosureScope, $node->lineno ?? 0, (string) $closure_scope_type);
            return null;
        } else {
            // TODO: handle 'parent'?
     ...
     * given context
     */
    public static function fromNode(Context $context, CodeBase $code_base, Node $node, FullyQualifiedFunctionName $fqsen) : Func
    {
        // Create the skeleton function object from what
        // we know so far
        $func = new Func($context, (string) $node->children['name'], UnionType::empty(), $node->flags, $fqsen, null);
        ...
        $comment = Comment::fromStringInContext($doc_comment, $code_base, $context, $node->lineno ?? 0, Comment::ON_FUNCTION);
        ...
        foreach ($parameter_list as $parameter) {
            $func->getInternalScope()->addVariable($parameter->cloneAsNonVariadic());
        }
        ...
        $func->setSuppressIssueSet($comment->getSuppressIssueSet());
        // Take a look at function return types
        if ($node->children['returnType'] !== null) {
            // Get the type of the parameter
            $union_type = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $node->children['returnType']);
        ...
        if ($comment->hasReturnUnionType()) {
            ...
            if ($union_type->hasSelfType()) {
                ...
                if ($context->isInClassScope()) {
                    ...
                    Issue::maybeEmit($code_base, $context, Issue::ContextNotObjectUsingSelf, $comment->getReturnLineno(), 'self', $fqsen);
                }
            }
