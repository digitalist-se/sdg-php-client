...
class Variable extends UnaddressableTypedElement implements TypedElementInterface
{
    ...
    public const _BUILTIN_SUPERGLOBAL_TYPES = [
        ...
        '_FILES' => 'array<string,array<string,int|string|array<string,int|string>>>',
        // Can have multiple files with the same name.
        '_SESSION' => 'array<string,mixed>',
        'GLOBALS' => 'array<string,mixed>',
        'http_response_header' => 'list<string>|null',
    ];
    /**
    ...
    public const _BUILTIN_GLOBAL_TYPES = [
        ...
        '_SERVER' => 'array<string,mixed>',
        '_ENV' => 'array<string,string>',
        '_FILES' => 'array<string,array<string,int>>|array<string,array<string,string>>|array<string,array<string,list<int>>>|array<string,array<string,list<string>>>',
        // Can have multiple files with the same name.
        '_SESSION' => 'array<string,mixed>',
        'GLOBALS' => 'array<string,mixed>',
        'http_response_header' => 'list<string>|null',
        // Revisit when we implement sub-block type refining
        'argv' => 'list<string>',
        'argc' => 'int',
    ];
    /**
    ...
    */
     ...
     */
    public static function fromNodeInContext(Node $node, Context $context, CodeBase $code_base, bool $should_check_type = \true) : Variable
    {
        ...
        $union_type = $should_check_type ? UnionTypeVisitor::unionTypeFromNode($code_base, $context, $node) : UnionType::empty();
        $variable = new Variable($context->withLineNumberStart($node->lineno), $variable_name, $union_type, 0);
        return $variable;
    }
    /**
     * @return bool
     ...
     */
    public static function isSuperglobalVariableWithName(string $name) : bool
    {
        if (\array_key_exists($name, self::_BUILTIN_SUPERGLOBAL_TYPES)) {
            return \true;
        }
        return \in_array($name, Config::getValue('runkit_superglobals'), \true);
    }
    /**
    ...
    {
        return self::isSuperglobalVariableWithName($name) || \array_key_exists($name, self::_BUILTIN_GLOBAL_TYPES) || \array_key_exists($name, Config::getValue('globals_type_map'));
    }
    /**
     * Returns true for all superglobals (if is_in_global_scope, also for variables in globals_type_map/built in globals)
     */
    public static function isHardcodedVariableInScopeWithName(string $name, bool $is_in_global_scope) : bool
    {
        return $is_in_global_scope ? self::isHardcodedGlobalVariableWithName($name) : self::isSuperglobalVariableWithName($name);
    }
    /**
     * @return ?UnionType
     ...
     * otherwise.
     */
    public static function getUnionTypeOfHardcodedGlobalVariableWithName(string $name) : ?UnionType
    {
        ...
        if (\array_key_exists($name, Config::getValue('globals_type_map')) || \in_array($name, Config::getValue('runkit_superglobals'), \true)) {
     ...
     * otherwise.
     */
    public static function getUnionTypeOfHardcodedVariableInScopeWithName(string $name, bool $is_in_global_scope) : ?UnionType
    {
        ...
        if ($is_in_global_scope && \array_key_exists($name, Config::getValue('globals_type_map')) || \in_array($name, Config::getValue('runkit_superglobals'), \true)) {
            $type_string = Config::getValue('globals_type_map')[$name] ?? '';
            // Want to allow 'resource' or 'mixed' as a type here,
            return UnionType::fromStringInContext($type_string, new Context(), Type::FROM_PHPDOC);
        }
