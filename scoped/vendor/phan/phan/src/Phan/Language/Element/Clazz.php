...
namespace SdgScoped\Phan\Language\Element;

use SdgScoped\ast;
use ast\Node;
use Closure;
use LogicException;
use SdgScoped\Phan\Analysis\AbstractMethodAnalyzer;
use SdgScoped\Phan\Analysis\ClassConstantTypesAnalyzer;
use SdgScoped\Phan\Analysis\ClassInheritanceAnalyzer;
use SdgScoped\Phan\Analysis\CompositionAnalyzer;
use SdgScoped\Phan\Analysis\DuplicateClassAnalyzer;
use SdgScoped\Phan\Analysis\ParentConstructorCalledAnalyzer;
use SdgScoped\Phan\Analysis\PropertyTypesAnalyzer;
...
use SdgScoped\Phan\Language\Element\Comment\Property as CommentProperty;
use SdgScoped\Phan\Language\ElementContext;
use SdgScoped\Phan\Language\FileRef;
...
use SdgScoped\Phan\Language\Scope\ClassScope;
...
use SdgScoped\Phan\Library\Option;
use SdgScoped\Phan\Library\Some;
use SdgScoped\Phan\Memoize;
use SdgScoped\Phan\Plugin\ConfigPluginSet;
use SdgScoped\Phan\Suggestion;
use ReflectionClass;
...
class Clazz extends AddressableElement
{
     ...
     */
    private $did_finish_parsing = \true;
    /**
    ...
    public function __construct(Context $context, string $name, UnionType $type, int $flags, FullyQualifiedClassName $fqsen, Type $parent_type = null, array $interface_fqsen_list = [], array $trait_fqsen_list = [])
    {
        ...
        $internal_scope = new ClassScope($context->getScope(), $fqsen, $flags);
    ...
    public static function fromReflectionClass(CodeBase $code_base, ReflectionClass $class) : Clazz
    {
        ...
        $clazz = new Clazz($context, $class_name, UnionType::fromFullyQualifiedRealString('\\' . $class_name), $flags, $class_fqsen);
        ...
        foreach (UnionType::internalPropertyMapForClassName($clazz->getName()) as $property_name => $property_type_string) {
            ...
            // dynamic properties
            if ($property_name === '*') {
                $clazz->setHasDynamicProperties(\true);
                continue;
            }
            $property_context = $context->withScope($class_scope);
            $property_type = UnionType::fromStringInContext($property_type_string, new Context(), Type::FROM_TYPE);
            ...
            $property = new Property($property_context, $property_name, $property_type->withRealTypeSet($real_type->getTypeSet()), $flags, $property_fqsen, $real_type);
        ...
        foreach ($class->getDefaultProperties() as $name => $default_value) {
            ...
            $property = new Property($property_context, $name, Type::fromObject($default_value)->asPHPDocUnionType()->withRealTypeSet($real_type->getTypeSet()), $flags, $property_fqsen, $real_type);
            $clazz->addProperty($code_base, $property, None::instance());
        }
        foreach ($class->getProperties() as $reflection_property) {
            ...
            $property_fqsen = FullyQualifiedPropertyName::make($clazz->getFQSEN(), $name);
            $real_type = self::getRealTypeForReflectionProperty($reflection_property);
            $property = new Property($property_context, $name, $real_type->asRealUnionType(), self::getASTFlagsForReflectionProperty($reflection_property), $property_fqsen, $real_type);
            $clazz->addProperty($code_base, $property, None::instance());
        }
        ...
        foreach ($class->getConstants() as $name => $value) {
            ...
            $constant = new ClassConstant(
                ...
                // TODO: These can vary based on OS/build flags
                0,
                $constant_fqsen
            );
        ...
        foreach ($class->getMethods() as $reflection_method) {
            ...
            $method_list = FunctionFactory::methodListFromReflectionClassAndMethod($method_context, $class, $reflection_method);
    ...
    public function setParentType(Type $parent_type, int $lineno = 0) : void
    {
        if ($this->getInternalScope()->hasAnyTemplateType()) {
            ...
            if ($contains_templated_type) {
                $parent_type = Type::fromType($parent_type, \array_map(static function (UnionType $union_type) use($template_type_map) : UnionType {
                    return UnionType::of(\array_map(static function (Type $type) use($template_type_map) : Type {
    ...
    private function getParentClassWithoutHydrating(CodeBase $code_base) : Clazz
    {
        ...
        return $code_base->getClassByFQSENWithoutHydrating($parent_fqsen);
    }
    /**
    ...
    public function getHierarchyRootFQSEN(CodeBase $code_base) : FullyQualifiedClassName
    {
        ...
        for ($current = $this; $current->hasParentType(); $current = $parent) {
            ...
            if (!$code_base->hasClassWithFQSEN($current->getParentClassFQSEN())) {
                // Let this emit an issue elsewhere for the
                // parent not existing
                return $fqsen;
            }
     ...
     */
    public function addProperty(CodeBase $code_base, Property $property, Option $type_option, bool $from_trait = \false) : void
    {
        ...
        if ($this->hasPropertyWithName($code_base, $property_name)) {
            ...
            self::checkPropertyCompatibility($code_base, $property, $this->getPropertyByName($code_base, $property_name));
            return;
        }
        ...
        if ($original_property_fqsen !== $property_fqsen) {
            ...
            try {
                ...
                // type through it
                if ($type_option->isDefined() && !$property->hasUnresolvedFutureUnionType() && $property->getUnionType()->hasTemplateType()) {
    ...
    private static function checkPropertyCompatibility(CodeBase $code_base, Property $inherited_property, Property $overriding_property) : void
    {
        $overriding_property->setIsOverride(\true);
        if ($inherited_property->isFromPHPDoc() || $inherited_property->isDynamicProperty() || $overriding_property->isFromPHPDoc() || $overriding_property->isDynamicProperty()) {
            return;
        }
        if ($inherited_property->isStrictlyMoreVisibleThan($overriding_property)) {
            if ($inherited_property->isPHPInternal()) {
                if (!$overriding_property->checkHasSuppressIssueAndIncrementCount(Issue::PropertyAccessSignatureMismatchInternal)) {
                    Issue::maybeEmit($code_base, new ElementContext($overriding_property), Issue::PropertyAccessSignatureMismatchInternal, $overriding_property->getFileRef()->getLineNumberStart(), $overriding_property->asVisibilityAndFQSENString(), $inherited_property->asVisibilityAndFQSENString());
                }
            } else {
                if (!$overriding_property->checkHasSuppressIssueAndIncrementCount(Issue::PropertyAccessSignatureMismatchInternal)) {
                    Issue::maybeEmit($code_base, new ElementContext($overriding_property), Issue::PropertyAccessSignatureMismatch, $overriding_property->getFileRef()->getLineNumberStart(), $overriding_property, $inherited_property, $inherited_property->getFileRef()->getFile(), $inherited_property->getFileRef()->getLineNumberStart());
                }
            }
        }
     ...
     */
    public function setMagicPropertyMap(array $magic_property_map, CodeBase $code_base) : bool
    {
        ...
        foreach ($magic_property_map as $comment_parameter) {
            ...
            $property = new Property((clone $context)->withLineNumberStart($comment_parameter->getLine()), $property_name, $union_type, 0, $property_fqsen, UnionType::empty());
     ...
     */
    public function setMagicMethodMap(array $magic_method_map, CodeBase $code_base) : bool
    {
        ...
        foreach ($magic_method_map as $comment_method) {
            ...
            if ($this->hasMethodWithName($code_base, $method_name, \true)) {
            ...
            $real_parameter_list = \array_map(static function (\SdgScoped\Phan\Language\Element\Comment\Parameter $parameter) use($method_context) : Parameter {
                return $parameter->asRealParameter($method_context);
            }, $comment_method->getParameterList());
            $method = new Method($method_context, $method_name, $comment_method->getUnionType(), $flags, $method_fqsen, $real_parameter_list);
     ...
     */
    public function getPropertyByName(CodeBase $code_base, string $name) : Property
    {
        return $code_base->getPropertyByFQSEN(FullyQualifiedPropertyName::make($this->getFQSEN(), $name));
    }
    /**
     * @param CodeBase $code_base
     ...
     * context
     */
    public function getPropertyByNameInContext(CodeBase $code_base, string $name, Context $context, bool $is_static, Node $node = null, bool $is_known_assignment = \false) : Property
    {
        ...
        if ($code_base->hasPropertyWithFQSEN($property_fqsen)) {
            ...
            if ($is_static !== $property->isStatic()) {
                if ($is_static) {
                    throw new IssueException(Issue::fromType(Issue::AccessPropertyNonStaticAsStatic)($context->getFile(), $context->getLineNumberStart(), [$property->asPropertyFQSENString()]));
                } else {
                    throw new IssueException(Issue::fromType(Issue::AccessPropertyStaticAsNonStatic)($context->getFile(), $context->getLineNumberStart(), [$property->asPropertyFQSENString()]));
                }
            }
            $is_property_accessible = $property->isAccessibleFromClass($code_base, $context->getClassFQSENOrNull());
        }
        ...
        if (!$is_static && $this->hasMethodWithName($code_base, '__get', \true)) {
            ...
            if ($method->isPrivate()) {
                throw new IssueException(Issue::fromType(Issue::AccessPropertyPrivate)($context->getFile(), $context->getLineNumberStart(), [$property ? $property->asPropertyFQSENString() : $property_fqsen, $method->getContext()->getFile(), $method->getContext()->getLineNumberStart()]));
            } elseif ($method->isProtected()) {
                if (!self::isAccessToElementOfThis($node)) {
                    throw new IssueException(Issue::fromType(Issue::AccessPropertyProtected)($context->getFile(), $context->getLineNumberStart(), [$property ? $property->asPropertyFQSENString() : $property_fqsen, $method->getContext()->getFile(), $method->getContext()->getLineNumberStart()]));
                }
            }
            $property = new Property($context, $name, $method->getUnionType(), 0, $property_fqsen, UnionType::empty());
            $property->setIsDynamicProperty(\true);
            $this->addProperty($code_base, $property, None::instance());
            return $property;
        } elseif ($property) {
            ...
            if ($property->isPrivate()) {
                throw new IssueException(Issue::fromType(Issue::AccessPropertyPrivate)($context->getFile(), $context->getLineNumberStart(), [$property->asPropertyFQSENString(), $property->getContext()->getFile(), $property->getContext()->getLineNumberStart()], $this->suggestGettersOrSetters($code_base, $context, $property, $is_known_assignment)));
            }
            if ($property->isProtected()) {
                if (self::isAccessToElementOfThis($node)) {
                    return $property;
                }
                throw new IssueException(Issue::fromType(Issue::AccessPropertyProtected)($context->getFile(), $context->getLineNumberStart(), [$property->asPropertyFQSENString(), $property->getContext()->getFile(), $property->getContext()->getLineNumberStart()], $this->suggestGettersOrSetters($code_base, $context, $property, $is_known_assignment)));
            }
        }
        // Check to see if missing properties are allowed
        // or we're working with a class with dynamic
        // properties such as stdClass.
        if (!$is_static && (Config::getValue('allow_missing_properties') || $this->hasDynamicProperties($code_base))) {
            $property = new Property($context, $name, UnionType::empty(), 0, $property_fqsen, UnionType::empty());
            $property->setIsDynamicProperty(\true);
            $this->addProperty($code_base, $property, None::instance());
            return $property;
        }
        throw new IssueException(Issue::fromType(Issue::UndeclaredProperty)($context->getFile(), $context->getLineNumberStart(), [$this->getFQSEN() . ($is_static ? '::$' : '->') . $name], IssueFixSuggester::suggestSimilarProperty($code_base, $context, $this, $name, $is_static)));
    }
    ...
    private static function getPropName($node) : ?string
    {
        ...
        if (!($obj instanceof Node && $obj->kind === \ast\AST_VAR && $obj->children['name'] === 'this')) {
            return null;
        }
     ...
     */
    public function inheritConstant(CodeBase $code_base, ClassConstant $constant) : void
    {
        ...
        if ($code_base->hasClassConstantWithFQSEN($constant_fqsen)) {
            ...
            $overriding_constant->setIsOverride(\true);
            self::checkConstantCompatibility($code_base, $constant, $code_base->getClassConstantByFQSEN($constant_fqsen));
            return;
        }
        ...
        if ($constant->isPHPDocAbstract() && !$constant->isPrivate() && !$this->isAbstract() && $this->isClass()) {
            Issue::maybeEmit($code_base, $this->getContext(), Issue::CommentAbstractOnInheritedConstant, $this->getContext()->getLineNumberStart(), $this->getFQSEN(), $constant->getRealDefiningFQSEN(), $constant->getContext()->getFile(), $constant->getContext()->getLineNumberStart(), '@abstract');
        }
    ...
    private static function checkConstantCompatibility(CodeBase $code_base, ClassConstant $inherited_constant, ClassConstant $overriding_constant) : void
    {
        ...
        if ($inherited_constant->isStrictlyMoreVisibleThan($overriding_constant)) {
            if ($inherited_constant->isPHPInternal()) {
                if (!$overriding_constant->checkHasSuppressIssueAndIncrementCount(Issue::ConstantAccessSignatureMismatchInternal)) {
                    Issue::maybeEmit($code_base, $overriding_constant->getContext(), Issue::ConstantAccessSignatureMismatchInternal, $overriding_constant->getFileRef()->getLineNumberStart(), $overriding_constant->asVisibilityAndFQSENString(), $inherited_constant->asVisibilityAndFQSENString());
                }
            } else {
                if (!$overriding_constant->checkHasSuppressIssueAndIncrementCount(Issue::ConstantAccessSignatureMismatchInternal)) {
                    Issue::maybeEmit($code_base, $overriding_constant->getContext(), Issue::ConstantAccessSignatureMismatch, $overriding_constant->getFileRef()->getLineNumberStart(), $overriding_constant->asVisibilityAndFQSENString(), $inherited_constant->asVisibilityAndFQSENString(), $inherited_constant->getFileRef()->getFile(), $inherited_constant->getFileRef()->getLineNumberStart());
                }
            }
        }
    }
     ...
     */
    public function addConstant(CodeBase $code_base, ClassConstant $constant) : void
    {
        $constant_fqsen = FullyQualifiedClassConstantName::make($this->getFQSEN(), $constant->getName());
        // Update the FQSEN if it's not associated with this
        // class yet
    ...
    {
        if ($code_base->hasClassConstantWithFQSEN(FullyQualifiedClassConstantName::make($this->getFQSEN(), $name))) {
            return \true;
        }
     ...
     * context
     */
    public function getConstantByNameInContext(CodeBase $code_base, string $name, Context $context) : ClassConstant
    {
        $constant_fqsen = FullyQualifiedClassConstantName::make($this->getFQSEN(), $name);
        if (!$code_base->hasClassConstantWithFQSEN($constant_fqsen)) {
            throw new IssueException(Issue::fromType(Issue::UndeclaredConstantOfClass)($context->getFile(), $context->getLineNumberStart(), [$this->getFQSEN() . '::' . $constant_fqsen], IssueFixSuggester::suggestSimilarClassConstant($code_base, $context, $constant_fqsen)));
        }
        ...
        if ($constant->isPrivate()) {
            ...
            throw new IssueException(Issue::fromType(Issue::AccessClassConstantPrivate)($context->getFile(), $context->getLineNumberStart(), [(string) $constant_fqsen, $constant->getContext()->getFile(), $constant->getContext()->getLineNumberStart()]));
        }
        ...
        if (!$is_remote_access) {
            ...
            throw new IssueException(Issue::fromType(Issue::AccessClassConstantProtected)($context->getFile(), $context->getLineNumberStart(), [(string) $constant_fqsen, $constant->getContext()->getFile(), $constant->getContext()->getLineNumberStart()]));
        }
     ...
     */
    public function addMethod(CodeBase $code_base, Method $method, Option $type_option) : void
    {
        ...
        if ($is_override) {
            ...
            if ($existing_method->getRealDefiningFQSEN() === $method_fqsen || $method->isAbstract() || !$existing_method->isAbstract() || $existing_method->isNewConstructor()) {
                ...
                $existing_method->setIsOverride(\true);
        ...
        if ($method->getFQSEN() !== $method_fqsen) {
            ...
            if ($type_option->isDefined()) {
                ...
                $method->setParameterList(\array_map(static function (Parameter $parameter) use($type_option, $code_base) : Parameter {
        ...
        if ($this->isInterface() && !$method->isNewConstructor()) {
            $method->setFlags(Flags::bitVectorWithState($method->getFlags(), \ast\flags\MODIFIER_ABSTRACT, \true));
        }
        if ($is_override) {
            $method->setIsOverride(\true);
        }
     ...
     */
    public function hasMethodWithName(CodeBase $code_base, string $name, bool $is_direct_invocation = \false) : bool
    {
     ...
     */
    public function getMethodByName(CodeBase $code_base, string $name) : Method
    {
        $method_fqsen = FullyQualifiedMethodName::make($this->getFQSEN(), $name);
        if (!$code_base->hasMethodWithFQSEN($method_fqsen)) {
            if ('__construct' === $name) {
                ...
                $default_constructor = Method::defaultConstructorForClass($this, $code_base);
    ...
    {
        return $this->hasMethodWithName($code_base, '__call', \true);
    }
    /**
     * @param CodeBase $code_base
     * The entire code base from which we'll find ancestor
    ...
    {
        $clone = new Method($method->getContext(), $method->getName(), $method->getUnionType(), $method->getFlags(), $method->getFQSEN(), [new VariadicParameter($method->getContext(), 'args', UnionType::empty(), 0)]);
    ...
    {
        return $this->hasMethodWithName($code_base, '__callStatic', \true);
    }
    /**
     * @param CodeBase $code_base
     * The entire code base from which we'll find ancestor
    ...
    {
        return $this->hasMethodWithName($code_base, '__get', \true);
    }
    /**
     * @param CodeBase $code_base
     * The entire code base from which we'll find ancestor
    ...
    {
        return $this->hasMethodWithName($code_base, '__set', \true);
    }
    /**
     * @param CodeBase $code_base
     * The entire code base from which we'll find ancestor
    ...
    public function setIsParentConstructorCalled(bool $is_parent_constructor_called) : void
    {
     ...
     * @suppress PhanUnreferencedPublicMethod
     */
    public function setForbidUndeclaredMagicProperties(bool $forbid_undeclared_dynamic_properties) : void
    {
     ...
     * @suppress PhanUnreferencedPublicMethod
     */
    public function setForbidUndeclaredMagicMethods(bool $forbid_undeclared_magic_methods) : void
    {
    ...
    private function hasFlagsRecursive(CodeBase $code_base, int $flags) : bool
    {
        ...
        while (\true) {
            ...
            $checked[$current->getFQSEN()->__toString()] = \true;
    ...
    public function setHasDynamicProperties(bool $has_dynamic_properties) : void
    {
    ...
    {
        return \array_merge($this->interface_fqsen_list, $this->trait_fqsen_list);
    }
    /**
     * @return list<FullyQualifiedClassName>
     */
     ...
     * @return list<Clazz>
     */
    private static function getClassListFromFQSENList(CodeBase $code_base, array $fqsen_list) : array
    {
        $class_list = [];
    ...
    {
        return self::getClassListFromFQSENList($code_base, $this->getAncestorFQSENList());
    }
    /**
    ...
    public function importConstantsFromAncestorClasses(CodeBase $code_base) : void
    {
        ...
        foreach ($this->interface_fqsen_list as $fqsen) {
            ...
            $this->importConstantsFromAncestorClass($code_base, $ancestor);
        }
    ...
    public function importAncestorClasses(CodeBase $code_base) : void
    {
        ...
        foreach ($this->interface_fqsen_list as $i => $fqsen) {
            ...
            $this->importAncestorClass($code_base, $ancestor, None::instance());
        }
    ...
    private function importMixin(CodeBase $code_base, Type $type) : void
    {
        ...
        if (!$code_base->hasClassWithFQSEN($fqsen) || $fqsen === $this->fqsen) {
            Issue::maybeEmit($code_base, $this->internal_context, Issue::InvalidMixin, $this->internal_context->getLineNumberStart(), $type);
            return;
        }
        ...
        foreach ($class->getMethodMap($code_base) as $name => $method) {
            ...
            if ($this->hasMethodWithName($code_base, $name, \true)) {
                continue;
            }
    ...
    private function emitWrongInheritanceCategoryWarning(CodeBase $code_base, Clazz $ancestor, string $expected_inheritance_category, int $lineno) : void
    {
        $context = $this->getContext();
        if ($ancestor->isPHPInternal()) {
            if (!$this->checkHasSuppressIssueAndIncrementCount(Issue::AccessWrongInheritanceCategoryInternal)) {
                Issue::maybeEmit($code_base, $context, Issue::AccessWrongInheritanceCategoryInternal, $lineno ?: $context->getLineNumberStart(), (string) $ancestor, $expected_inheritance_category);
            }
        } else {
            if (!$this->checkHasSuppressIssueAndIncrementCount(Issue::AccessWrongInheritanceCategory)) {
                Issue::maybeEmit($code_base, $context, Issue::AccessWrongInheritanceCategory, $lineno ?: $context->getLineNumberStart(), (string) $ancestor, $ancestor->getFileRef()->getFile(), $ancestor->getFileRef()->getLineNumberStart(), $expected_inheritance_category);
            }
        }
    }
    private function emitExtendsFinalClassWarning(CodeBase $code_base, Clazz $ancestor) : void
    {
        $context = $this->getContext();
        if ($ancestor->isPHPInternal()) {
            if (!$this->checkHasSuppressIssueAndIncrementCount(Issue::AccessExtendsFinalClassInternal)) {
                Issue::maybeEmit($code_base, $context, Issue::AccessExtendsFinalClassInternal, $this->parent_type_lineno ?: $context->getLineNumberStart(), (string) $ancestor->getFQSEN());
            }
        } else {
            if (!$this->checkHasSuppressIssueAndIncrementCount(Issue::AccessExtendsFinalClass)) {
                Issue::maybeEmit($code_base, $context, Issue::AccessExtendsFinalClass, $this->parent_type_lineno ?: $context->getLineNumberStart(), (string) $ancestor->getFQSEN(), $ancestor->getFileRef()->getFile(), $ancestor->getFileRef()->getLineNumberStart());
            }
        }
    }
    /**
     ...
     */
    public function importConstantsFromAncestorClass(CodeBase $code_base, Clazz $class) : void
    {
        $key = \strtolower((string) $class->getFQSEN());
        if (!$this->isFirstExecution(__METHOD__ . ':' . $key)) {
            return;
        }
     ...
     * methods
     */
    public function importAncestorClass(CodeBase $code_base, Clazz $class, Option $type_option) : void
    {
        ...
        foreach ($class->getPropertyMap($code_base) as $property) {
            if ($property->isPHPDocAbstract() && !$property->isPrivate() && $this->isClass() && !$this->isAbstract() && !$this->hasPropertyWithName($code_base, $property->getName())) {
                Issue::maybeEmit($code_base, $this->getContext(), Issue::CommentAbstractOnInheritedProperty, $this->getContext()->getLineNumberStart(), $this->getFQSEN(), $property->getRealDefiningFQSEN(), $property->getContext()->getFile(), $property->getContext()->getLineNumberStart(), '@abstract');
            }
            ...
            $this->addProperty($code_base, $property, $type_option, $is_trait);
        }
        // Copy methods
    ...
    private function adaptInheritedMethodFromTrait(Method $method) : Method
    {
        ...
        $method->setUnionType($method->getUnionTypeWithUnmodifiedStatic()->withSelfResolvedInContext($context));
        $method->setRealReturnType($method->getRealReturnType()->withSelfResolvedInContext($context));
     ...
     * methods
     */
    private function importTraitAdaptations(CodeBase $code_base, Clazz $class, TraitAdaptations $trait_adaptations, Option $type_option) : void
    {
        foreach ($trait_adaptations->alias_methods ?? [] as $alias_method_name => $original_trait_alias_source) {
            ...
            if ($class->hasMethodWithName($code_base, $source_method_name, \true)) {
            ...
            if (!$source_method || $source_method->isFromPHPDoc()) {
                Issue::maybeEmit(
                    ...
                    $original_trait_alias_source->getAliasLineno(),
                    // TODO: Track line number in TraitAdaptation
            ...
            $alias_method = $source_method->createUseAlias($this, $alias_method_name, $original_trait_alias_source->getAliasVisibilityFlags());
            $this->addMethod($code_base, $alias_method, $type_option);
        }
    }
    private function warnAboutAmbiguousInheritance(CodeBase $code_base, Clazz $inherited_class, FullyQualifiedClassName $alternate_class_fqsen) : void
    {
        ...
        Issue::maybeEmit($code_base, $this->getContext(), $issue_type, $this->getContext()->getLineNumberStart(), $this->getFQSEN(), $inherited_class->__toString(), $first_context->getFile(), $first_context->getLineNumberStart(), $second_context->getFile(), $second_context->getLineNumberStart());
    }
    /**
     * @return int
    ...
    public function getReferenceCount(CodeBase $code_base) : int
    {
        ...
        $list_count = function (array $list) : int {
            return \array_reduce($list, function (int $count, ClassElement $element) : int {
    ...
    protected function hydrateConstantsOnce(CodeBase $code_base) : void
    {
        foreach ($this->getAncestorFQSENList() as $fqsen) {
            if ($code_base->hasClassWithFQSEN($fqsen)) {
                $code_base->getClassByFQSENWithoutHydrating($fqsen)->hydrateConstants($code_base);
            }
        }
        // Create the 'class' constant
        $class_constant_value = \ltrim($this->getFQSEN()->__toString(), '\\');
        $class_constant = new ClassConstant($this->getContext(), 'class', LiteralStringType::instanceForValue($class_constant_value, \false)->asRealUnionType(), 0, FullyQualifiedClassConstantName::make($this->getFQSEN(), 'class'));
        ...
        $this->getInternalScope()->addVariable(new Variable($this->getContext(), 'this', StaticType::instance(\false)->asRealUnionType(), 0));
    ...
    protected function hydrateOnce(CodeBase $code_base) : void
    {
        ...
        foreach ($this->getAncestorFQSENList() as $fqsen) {
            if ($code_base->hasClassWithFQSEN($fqsen)) {
                $code_base->getClassByFQSENWithoutHydrating($fqsen)->hydrate($code_base);
            }
        }
        ...
        AbstractMethodAnalyzer::analyzeAbstractMethodsAreImplemented($code_base, $this);
    }
    /**
    ...
    private static function analyzeClassConstantOverrides(CodeBase $code_base, array $original_declared_class_constants) : void
    {
        foreach ($original_declared_class_constants as $constant) {
            if ($constant->isOverrideIntended() && !$constant->isOverride()) {
                ...
                Issue::maybeEmit($code_base, $context, Issue::CommentOverrideOnNonOverrideConstant, $context->getLineNumberStart(), (string) $constant->getFQSEN());
            }
        }
    }
    /**
    ...
    private static function analyzePropertyOverrides(CodeBase $code_base, array $original_declared_properties) : void
    {
        foreach ($original_declared_properties as $property) {
            if ($property->isOverrideIntended() && !$property->isOverride()) {
                ...
                Issue::maybeEmit($code_base, $context, Issue::CommentOverrideOnNonOverrideProperty, $context->getLineNumberStart(), (string) $property->getFQSEN());
            }
        }
    }
    /**
     ...
     */
    public final function analyze(CodeBase $code_base) : void
    {
        if ($this->isPHPInternal()) {
            return;
        }
        ...
        ClassConstantTypesAnalyzer::analyzeClassConstantTypes($code_base, $this);
        ...
        if ($this->isClass() && !$this->isAbstract()) {
            foreach ($this->getMethodMap($code_base) as $method) {
                ...
                if ($method->isPHPDocAbstract() && !$method->isPrivate()) {
                    Issue::maybeEmit($code_base, $this->getContext(), Issue::CommentAbstractOnInheritedMethod, $this->getContext()->getLineNumberStart(), $this->getFQSEN(), $method->getRealDefiningFQSEN(), $method->getContext()->getFile(), $method->getContext()->getLineNumberStart(), '@abstract');
                }
            }
        }
    }
    ...
    protected function hydrateConstants(CodeBase $code_base) : void
    {
        ...
        if ($this->are_constants_hydrated) {
            // Same as isFirstExecution(), inlined due to being called frequently.
            return;
        }
        if (!$code_base->shouldHydrateRequestedElements()) {
            return;
        }
        $this->are_constants_hydrated = \true;
    ...
    public function getPropertyMapExcludingDynamicAndMagicProperties(CodeBase $code_base) : array
    {
        return $this->memoize(
            ...
            function () use($code_base) : array {
                ...
                return \array_filter($this->getPropertyMap($code_base), static function (Property $property) : bool {
     ...
     */
    public function checkCanIterateFromContext(CodeBase $code_base, Context $context) : int
    {
        $accessing_class = $context->getClassFQSENOrNull();
        return $this->memoize('can_iterate:' . (string) $accessing_class, function () use($accessing_class, $code_base) : int {
    ...
    public function getGenericConstructorBuilder(CodeBase $code_base) : array
    {
        return $this->memoize(
            ...
            function () use($code_base) : array {
                ...
                $constructor_method = $this->getMethodByName($code_base, '__construct');
                $template_type_resolvers = [];
                foreach ($this->getTemplateTypeMap() as $template_type) {
                    $template_type_resolver = $constructor_method->getTemplateTypeExtractorClosure($code_base, $template_type);
                    if (!$template_type_resolver) {
                        ...
                        if (!$this->checkHasSuppressIssueAndIncrementCount(Issue::TemplateTypeNotDeclaredInFunctionParams)) {
                            Issue::maybeEmit($code_base, $constructor_method->getContext(), Issue::GenericConstructorTypes, $constructor_method->getContext()->getLineNumberStart(), $template_type, $this->getFQSEN());
                        }
    ...
    public static function getAncestorElement(CodeBase $code_base, FullyQualifiedClassName $ancestor_fqsen, ClassElement $element) : ?ClassElement
    {
        ...
        if ($element instanceof Method) {
            if (!$ancestor_class->hasMethodWithName($code_base, $name, \true)) {
                return null;
            }
            ...
            $constant_fqsen = FullyQualifiedClassConstantName::make($ancestor_fqsen, $name);
    ...
    private static function markMethodAsOverridden(CodeBase $code_base, FullyQualifiedMethodName $method_fqsen) : void
    {
        ...
        $method->setIsOverriddenByAnother(\true);
    }
    /**
