...
use SdgScoped\Phan\Language\Element\Comment\Method as CommentMethod;
use SdgScoped\Phan\Language\Element\Comment\NullComment;
...
use SdgScoped\Phan\Language\Element\Comment\ReturnComment;
...
class Comment
{
    ...
    public const FUNCTION_LIKE = [self::ON_METHOD, self::ON_FUNCTION];
    // Lists of types that can have (at)var annotations.
    public const HAS_VAR_ANNOTATION = [self::ON_METHOD, self::ON_FUNCTION, self::ON_VAR, self::ON_PROPERTY, self::ON_CONST];
    public const HAS_TEMPLATE_ANNOTATION = [self::ON_CLASS, self::ON_FUNCTION, self::ON_METHOD];
    public const NAME_FOR_TYPE = [self::ON_CLASS => 'class', self::ON_VAR => 'variable', self::ON_PROPERTY => 'property', self::ON_CONST => 'constant', self::ON_METHOD => 'method', self::ON_FUNCTION => 'function'];
    /**
        ...
        foreach ($this->parameter_list as $i => $parameter) {
            ...
            if (StringUtil::isNonZeroLengthString($name)) {
                if (isset($this->parameter_map[$name])) {
                    Issue::maybeEmit($code_base, $context, Issue::CommentDuplicateParam, $parameter->getLineno(), $name);
        ...
        foreach ($magic_property_list as $property) {
            ...
            if (StringUtil::isNonZeroLengthString($name)) {
                if (isset($this->magic_property_map[$name])) {
                    ...
                    Issue::maybeEmit($code_base, $context, Issue::CommentDuplicateMagicProperty, $property->getLine(), $name);
                }
                // Add it to the named map
        ...
        foreach ($magic_method_list as $method) {
            ...
            if (StringUtil::isNonZeroLengthString($name)) {
                if (isset($this->magic_method_map[$name])) {
                    // Emit warning for duplicates.
                    Issue::maybeEmit($code_base, $context, Issue::CommentDuplicateMagicMethod, $method->getLine(), $name);
                }
                // Add it to the named map
    ...
    private function mergeVariableList(array $override_comment_vars) : void
    {
        ...
        foreach ($override_comment_vars as $override_var) {
            $known_names[$override_var->getName()] = \true;
        }
     ...
     */
    public static function fromStringInContext(string $comment, CodeBase $code_base, Context $context, int $lineno, int $comment_type) : Comment
    {
        ...
        return (new Builder($comment, $code_base, $context, $lineno, $comment_type))->build();
    }
    ...
    private const FLAGS_FOR_PROPERTY = Flags::IS_NS_INTERNAL | Flags::IS_DEPRECATED | Flags::IS_READ_ONLY | Flags::IS_WRITE_ONLY | Flags::IS_PHPDOC_ABSTRACT | Flags::IS_OVERRIDE_INTENDED;
    /**
    ...
    private const FLAGS_FOR_CLASS = Flags::IS_NS_INTERNAL | Flags::IS_DEPRECATED | Flags::IS_SIDE_EFFECT_FREE | Flags::CLASS_FORBID_UNDECLARED_MAGIC_METHODS | Flags::CLASS_FORBID_UNDECLARED_MAGIC_PROPERTIES | Flags::IS_CONSTRUCTOR_USED_FOR_SIDE_EFFECTS;
    /**
    ...
    private const FLAGS_FOR_METHOD = Flags::IS_NS_INTERNAL | Flags::IS_DEPRECATED | Flags::HARDCODED_RETURN_TYPE | Flags::IS_SIDE_EFFECT_FREE | Flags::IS_PHPDOC_ABSTRACT | Flags::IS_OVERRIDE_INTENDED;
    /**
     ...
     */
    public function hasParameterWithNameOrOffset(string $name, int $offset) : bool
    {
        if (isset($this->parameter_map[$name])) {
            return \true;
        }
     ...
     */
    public function getParameterWithNameOrOffset(string $name, int $offset) : CommentParameter
    {
        if (isset($this->parameter_map[$name])) {
     ...
     */
    public function hasMagicPropertyWithName(string $name) : bool
    {
     ...
     */
    public function getMagicPropertyWithName(string $name) : CommentProperty
    {
