...
class Property extends ClassElement
{
     ...
     */
    public function __construct(Context $context, string $name, UnionType $type, int $flags, FullyQualifiedPropertyName $fqsen, UnionType $real_union_type)
    {
        $internal_scope = new PropertyScope($context->getScope(), $fqsen);
    ...
    {
        return $this->getClassFQSEN()->__toString() . ($this->isStatic() ? '::$' : '->') . $this->name;
    }
    /**
    ...
    {
        $this->setPhanFlags(Flags::bitVectorWithState($this->getPhanFlags(), Flags::HAS_STATIC_UNION_TYPE, $has_static));
    }
    /**
    ...
    {
        $this->setPhanFlags(Flags::bitVectorWithState($this->getPhanFlags(), Flags::IS_READ_ONLY, $is_read_only));
    }
    /**
     * Is this property declared in a way hinting that it should only be written to?
    ...
    {
        $this->setPhanFlags(Flags::bitVectorWithState($this->getPhanFlags(), Flags::IS_DYNAMIC_PROPERTY, $is_dynamic));
    }
    public function inheritStaticUnionType(FullyQualifiedClassName $old, FullyQualifiedClassName $new) : void
    {
        ...
        foreach ($union_type->getTypeSet() as $type) {
            ...
            if (FullyQualifiedClassName::fromType($type) === $old) {
                $union_type = $union_type->withoutType($type)->withType($new->asType()->withIsNullable($type->isNullable()));
            }
        }
    ...
    public function getFutureUnionType() : ?UnionType
    {
        ...
        try {
            ...
            if (!$this->real_union_type->isEmpty() && !$union_type->canStrictCastToUnionType($future_union_type->getCodeBase(), $this->real_union_type)) {
                Issue::maybeEmit($future_union_type->getCodeBase(), $future_union_type->getContext(), Issue::TypeMismatchPropertyDefaultReal, $future_union_type->getContext()->getLineNumberStart(), $this->real_union_type, $this->name, ASTReverter::toShortString($future_union_type->getNode()), $union_type);
            }
