...
use SdgScoped\Phan\Language\Type\GenericArrayInterface;
...
use SdgScoped\Phan\Language\Type\LiteralTypeInterface;
use SdgScoped\Phan\Language\Type\MixedType;
use SdgScoped\Phan\Language\Type\MultiType;
use SdgScoped\Phan\Language\Type\NonEmptyArrayInterface;
...
class UnionType implements Serializable
{
    ...
    public const union_type_regex = Type::type_regex . '(\\s*[|&]\\s*' . Type::type_regex . ')*';
    /**
     * @var string
     * A list of one or more types delimited by the '|'
     ...
     */
    // phpcs:ignore Generic.NamingConventions.UpperCaseConstantName
    public const union_type_regex_or_this = Type::type_regex_or_this . '(\\s*[|&]\\s*' . Type::type_regex_or_this . ')*';
    /**
    ...
    public static function of(array $type_list, array $real_type_set = []) : UnionType
    {
        ...
        if ($n === 1 && \count($real_type_set) <= 1) {
            ...
            return new self($type_list, \true, $real_type_set);
        } else {
    ...
    private static function ofUnique(array $type_list, array $real_type_set = []) : UnionType
    {
        ...
        if ($n === 0) {
            if ($real_type_set) {
                ...
                return new self($real_type_set, \true, $real_type_set);
            }
            return self::$empty_instance;
        }
        if ($n === 1 && \count($real_type_set) <= 1) {
     ...
     * @phan-pure
     */
    public static function fromFullyQualifiedPHPDocString(string $fully_qualified_string) : UnionType
    {
        ...
        if (\is_null($union_type)) {
            ...
            if (\count($unique_types) === 1) {
                ...
                $union_type = new UnionType($unique_types, \true);
            }
     ...
     * @phan-pure
     */
    public static function fromFullyQualifiedPHPDocAndRealString(string $fully_qualified_phpdoc_string, string $fully_qualified_real_string) : UnionType
    {
     ...
     * @phan-pure
     */
    public static function fromFullyQualifiedRealString(string $fully_qualified_string) : UnionType
    {
        if ($fully_qualified_string === '') {
            return self::$empty_instance;
        }
        ...
        if (\is_null($union_type)) {
            ...
            if (\count($unique_types) === 1) {
                ...
                // TODO: Support template types within <> and test?
                $union_type = new UnionType($unique_types, \true, $unique_types);
            }
            $memoize_map[$fully_qualified_string] = $union_type;
        }
        return $union_type;
    ...
    public static function getUniqueTypes(array $type_list) : array
    {
        ...
        foreach ($type_list as $type) {
            if (!\in_array($type, $new_type_list, \true)) {
                $new_type_list[] = $type;
            }
        }
    ...
    private static function mergeUniqueTypes(array $type_list, array $other_type_list) : array
    {
        if (\count($other_type_list) <= 4) {
            ...
            foreach ($other_type_list as $type) {
                if (!\in_array($type, $type_list, \true)) {
                    $type_list[] = $type;
                }
            }
            return $type_list;
     ...
     * @phan-pure
     */
    public static function fromStringInContext(string $type_string, Context $context, int $source, CodeBase $code_base = null) : UnionType
    {
        if ($type_string === '') {
        ...
        foreach (self::extractTypePartsForStringInContext($type_string) as $type_name) {
            $types[] = Type::fromStringInContext($type_name, $context, $source, $code_base);
        }
    ...
    public static function fromReflectionType(?\ReflectionType $reflection_type) : UnionType
    {
        if ($reflection_type !== null) {
            return self::fromStringInContext(Type::stringFromReflectionType($reflection_type), new Context(), Type::FROM_TYPE);
        }
        return self::$empty_instance;
    }
    /**
     ...
     * class name.
     */
    public static function internalPropertyMapForClassName(string $class_name) : array
    {
    ...
    {
        return \in_array($type, $this->type_set, \true);
    }
    /**
    ...
    public function eraseRealTypeSet() : UnionType
    {
        if ($this->real_type_set) {
            ...
            return new UnionType($this->type_set, \true, []);
        }
        return $this;
    }
    /**
    ...
    public function withUnionType(UnionType $union_type) : UnionType
    {
        ...
        return UnionType::ofUnique(self::mergeUniqueTypes($type_set, $other_type_set), $this->real_type_set && $union_type->real_type_set ? self::mergeUniqueTypes($this->real_type_set, $union_type->real_type_set) : []);
    }
    /**
     * @return bool
    ...
    {
        return UnionType::of(self::filterTypesInBoolFamily($this->type_set), self::filterTypesInBoolFamily($this->real_type_set))->asNormalizedTypes();
    }
    /**
     * @param Type[] $type_list
     * @return list<Type>
     ...
     */
    public function withStaticResolvedInFunctionLike(FunctionInterface $function) : UnionType
    {
     ...
     * in the given context.
     */
    public function withSelfResolvedInContext(Context $context) : UnionType
    {
     ...
     * in the given context.
     */
    public function withAddedClassForResolvedSelf(Context $context) : UnionType
    {
        // If the context isn't in a class scope, or if it doesn't have 'self',
        // there's nothing we can do
        if (!$context->isInClassScope() || !$this->hasSelfType()) {
        ...
        return UnionType::of(\array_merge($this->type_set, [$resolved_type]), $this->real_type_set);
    }
    /**
     * @return bool
    ...
    public function isEqualTo(UnionType $union_type) : bool
    {
        ...
        foreach ($type_set as $type) {
            if (!\in_array($type, $other_type_set, \true)) {
                return \false;
            }
        }
    ...
    public function isIdenticalTo(UnionType $union_type) : bool
    {
        ...
        foreach ($real_type_set as $type) {
            if (!\in_array($type, $other_real_type_set, \true)) {
                return \false;
            }
        }
        return !$union_type->isPossiblyUndefined();
    ...
    private function hasCommonTypeSetCheck(array $other_type_list) : bool
    {
        ...
        // Avoid worst-case quadratic runtime
        foreach ($this->type_set as $type) {
            $type_set[\spl_object_id($type)] = \true;
        }
        foreach ($other_type_list as $type) {
    ...
    {
        return UnionType::of(self::typesWithoutSubclassesOf($code_base, $this->type_set, $object_type), self::typesWithoutSubclassesOf($code_base, $this->real_type_set, $object_type));
    }
    /**
    ...
    public static function typesWithoutSubclassesOf(CodeBase $code_base, array $type_set, Type $object_type) : array
    {
        ...
        foreach ($new_type_set as $i => $type) {
            $new_type_set[$i] = $type->withIsNullable(\true);
        }
        return $new_type_set;
    }
    /**
    ...
    {
        return self::of(self::toNonNullableTypeList($this->type_set), self::toNonNullableTypeList($this->real_type_set));
    }
    /**
     * @param Type[] $type_list
    ...
    {
        return self::of(self::toNullableTypeList($this->type_set), self::toNullableTypeList($this->real_type_set));
    }
    /**
    ...
    private static function toNullableTypeList(array $type_list) : array
    {
        ...
        foreach ($type_list as $type) {
            if ($type->isNullableLabeled()) {
                ...
                $result[] = $type->withIsNullable(\true);
            }
        }
        return $result;
    }
    ...
    {
        return UnionType::of(self::toNonFalseyTypeSet($this->type_set), self::toNonFalseyTypeSet($this->real_type_set));
    }
    /**
     * @param list<Type> $type_set
    ...
    {
        return UnionType::of(self::toNonTruthyTypeSet($this->type_set), self::toNonTruthyTypeSet($this->real_type_set));
    }
    /**
     * @param list<Type> $type_set
     * @return list<Type>
    ...
    */
    ...
    {
        return UnionType::of(self::castToIterableTypesStrict($code_base, $this->type_set, \false), self::castToIterableTypesStrict($code_base, $this->real_type_set, \false));
    }
    /**
    ...
    {
        return UnionType::of(self::castToIterableTypesStrict($code_base, $this->type_set, \true), self::castToIterableTypesStrict($code_base, $this->real_type_set, \true));
    }
    /**
     ...
     */
    public function canCastToUnionTypeHandlingTemplates(UnionType $target, CodeBase $code_base) : bool
    {
        ...
        if (Config::get_null_casts_as_any_type()) {
            // null <-> null
            if ($this->isType($null_type) || $target->isType($null_type)) {
                return \true;
            }
        } else {
        ...
        // mixed <-> mixed
        if (\in_array($mixed_type, $type_set, \true) || \in_array($mixed_type, $target_type_set, \true)) {
            return \true;
        }
        // int -> float
        if (\in_array($int_type, $type_set, \true) && \in_array($float_type, $target_type_set, \true)) {
            return \true;
        }
        ...
        if (\in_array($null_type, $target_type_set, \true)) {
            foreach ($type_set as $source_type) {
    ...
    public function canCastToUnionType(UnionType $target) : bool
    {
        ...
        if (Config::get_null_casts_as_any_type()) {
            ...
            if ($this->hasType(NullType::instance(\false)) || $target->isType(NullType::instance(\false))) {
                return \true;
            }
     ...
     * i.e. int->float is allowed  while float->int is not.
     */
    public function canCastToUnionTypeWithoutConfig(UnionType $target) : bool
    {
        // Fast-track most common cases first
        $type_set = $this->type_set;
        // If either type is unknown, we can't call it
     ...
     * (e.g. int -> float)
     */
    public function canAnyTypeStrictCastToUnionType(CodeBase $code_base, UnionType $target, bool $allow_casting = \true) : bool
    {
        foreach ($this->type_set as $type) {
     ...
     */
    public function hasSubtypeOf(UnionType $target) : bool
    {
        // Fast-track most common cases first
        $type_set = $this->type_set;
        // If either type is unknown, we can't call it
        ...
        // any -> mixed
        if (\in_array($mixed_type, $target_type_set, \true)) {
            return \true;
        }
        // int -> float
        if (\in_array($int_type, $type_set, \true) && \in_array($float_type, $target_type_set, \true)) {
    ...
    public function canStrictCastToUnionType(CodeBase $code_base, UnionType $target) : bool
    {
        ...
        $matches = \true;
        foreach ($type_set as $type) {
            if (!\in_array($type, $target_type_set, \true)) {
                $matches = \false;
                break;
            }
        }
    ...
    {
        return $this->canAnyTypeStrictCastToUnionType($code_base, $other, \false) || $other->canAnyTypeStrictCastToUnionType($code_base, $this, \false);
    }
    /**
    ...
    {
        return UnionType::of(\array_filter($this->type_set, $cb), \array_filter($this->real_type_set, $cb));
    }
    /**
     ...
     */
    public function asClassFQSENList(Context $context) : Generator
    {
        ...
        foreach ($this->type_set as $class_type) {
            ...
            if ($class_type->isStaticType()) {
                if (!$context->isInClassScope()) {
                    throw new IssueException(Issue::fromType(Issue::ContextNotObject)($context->getFile(), $context->getLineNumberStart(), [$class_type->getName()]));
                }
                (yield $class_fqsen);
            } else {
                (yield $class_fqsen);
     ...
     * context
     */
    public function asClassList(CodeBase $code_base, Context $context) : Generator
    {
        // Iterate over each viable class type to see if any
        // have the constant we're looking for
        foreach ($this->type_set as $class_type) {
     ...
     */
    public static function emitRedefinedClassReferenceWarning(CodeBase $code_base, Context $context, FullyQualifiedClassName $class_fqsen) : void
    {
        ...
        Issue::maybeEmit($code_base, $context, Issue::RedefinedClassReference, $context->getLineNumberStart(), $class_fqsen->withAlternateId(0), $class->getContext()->getFile(), $class->getContext()->getLineNumberStart(), $other_class->getContext()->getFile(), $other_class->getContext()->getLineNumberStart());
    }
    /**
    ...
    {
        return UnionType::of(self::castToObjectTypesStrict($this->type_set) ?: [ObjectType::instance(\false)], self::castToObjectTypesStrict($this->real_type_set) ?: [ObjectType::instance(\false)]);
    }
    /**
     * Takes `MyClass|int|array|?object` and returns `MyClass|object`
     *
    ...
    public function scalarTypesStrict(bool $allow_empty = \false) : UnionType
    {
        ...
        if ($allow_empty) {
            return UnionType::of(self::typeSetToScalarTypesStrict($this->type_set), self::typeSetToScalarTypesStrict($this->real_type_set));
        } else {
            static $default;
            if ($default === null) {
                $default = [StringType::instance(\false), IntType::instance(\false), FloatType::instance(\false), BoolType::instance(\false)];
            }
            return UnionType::of(self::typeSetToScalarTypesStrict($this->type_set) ?: $default, self::typeSetToScalarTypesStrict($this->real_type_set) ?: $default);
        }
    }
    /**
     * @param Type[] $type_list
    ...
    {
        return UnionType::of(self::castTypeListToCallable($this->type_set), self::castTypeListToCallable($this->real_type_set));
    }
    /**
     * @param Type[] $type_list
    ...
    public function countableTypesStrictCast(CodeBase $code_base) : UnionType
    {
        ...
        return UnionType::of(self::castTypeListToCountable($code_base, $this->type_set, \false) ?: $default_types, self::castTypeListToCountable($code_base, $this->real_type_set, \false) ?: $default_types);
    }
    /**
     * @param Type[] $type_list
    ...
    {
        return UnionType::of(self::castTypeListToInt($this->type_set), self::castTypeListToInt($this->real_type_set));
    }
    /**
     * @param Type[] $type_set
     * @return list<Type>
    ...
    {
        return UnionType::of(self::castTypeListToFloat($this->type_set), self::castTypeListToFloat($this->real_type_set));
    }
    /**
     * @param Type[] $type_set
     * @return list<Type>
    ...
    {
        return UnionType::of(self::castTypeListToString($this->type_set), self::castTypeListToString($this->real_type_set));
    }
    /**
    ...
    {
        return UnionType::of(self::castTypeListToClassString($this->type_set), self::castTypeListToClassString($this->real_type_set));
    }
    /**
     * @param Type[] $type_set
     * @return list<Type>
    ...
    {
        return UnionType::of(self::castTypeListToClassStringOrObject($this->type_set), self::castTypeListToClassStringOrObject($this->real_type_set));
    }
    /**
     * @param Type[] $type_set
     * @return list<Type>
    ...
    public function numericTypes() : UnionType
    {
        ...
        return UnionType::of(self::toNumericTypes($this->type_set), self::toNumericTypes($this->real_type_set));
    }
    /**
     * @param list<Type> $type_set
    ...
    {
        return UnionType::of(self::castToArrayTypesStrict($this->type_set) ?: [ArrayType::instance(\false)], self::castToArrayTypesStrict($this->real_type_set) ?: [ArrayType::instance(\false)]);
    }
    /**
     * This is the union type Phan infers from `assert(is_array($x))`
     * Converts `iterable<key,value>` to `array<key, value>`
    ...
    {
        return UnionType::of(self::castToArrayTypesStrict($this->type_set), self::castToArrayTypesStrict($this->real_type_set));
    }
    /**
     * @param Type[] $type_list
     * @return list<Type>
    ...
    public function iterableValueUnionType(CodeBase $code_base) : UnionType
    {
        ...
        if ($array_type_nonnull === null) {
            ...
            $array_type_nullable = ArrayType::instance(\true);
            $mixed_type = MixedType::instance(\false);
            $null_type = NullType::instance(\false);
        }
        // If array is in there, then it can be any type
        if (\in_array($array_type_nonnull, $type_set, \true)) {
            $builder->addType($mixed_type);
        } elseif (\in_array($mixed_type, $type_set, \true) || \in_array($array_type_nullable, $type_set, \true)) {
            // Same for mixed
            $builder->addType($mixed_type);
        }
    ...
    public function genericArrayElementTypes(bool $add_real_types = \false) : UnionType
    {
        ...
        if (\in_array($array_type_nullable, $type_set, \true)) {
            // TODO: More consistency in what causes this check to infer null
            $result[] = $mixed_type;
            $result[] = $null_type;
        } elseif (\in_array($array_type_nonnull, $type_set, \true) || \in_array($mixed_type, $type_set, \true)) {
            $result[] = $mixed_type;
        }
    ...
    public static function computeRealElementTypesForDimAccess(array $real_type_set) : array
    {
        ...
        foreach ($real_type_set as $type) {
            ...
            foreach ($new_types as $element_type) {
                ...
                $result[] = $element_type->withIsNullable(\true);
            }
        }
        return $result;
    }
    ...
    {
        return $this->asMappedUnionType(static function (Type $type) use($key_type) : Type {
    ...
    public function asNonEmptyGenericArrayTypes(int $key_type) : UnionType
    {
        ...
        $result = $this->asMappedUnionType(static function (Type $type) use($key_type) : Type {
            return $type->asGenericArrayType($key_type);
        });
    ...
    public function asNonEmptyListTypes() : UnionType
    {
        ...
        $result = $this->asMappedUnionType(static function (Type $type) : Type {
            return ListType::fromElementType($type, \false);
        });
        if (!$result->hasRealTypeSet()) {
            return $result->withRealType($type);
    ...
    public function asExpandedTypes(CodeBase $code_base, int $recursion_depth = 0) : UnionType
    {
        ...
        if (\count($type_set) === 0) {
            ...
            // @phan-suppress-next-line PhanPossiblyNonClassMethodCall
            return \reset($type_set)->asExpandedTypes($code_base, $recursion_depth + 1);
        }
        // 2 or more union types to merge
        $builder = new UnionTypeBuilder();
        foreach ($type_set as $type) {
            $builder->addUnionType($type->asExpandedTypes($code_base, $recursion_depth + 1));
        }
    ...
    public function asExpandedTypesPreservingTemplate(CodeBase $code_base, int $recursion_depth = 0) : UnionType
    {
        ...
        if (\count($type_set) === 0) {
            ...
            // @phan-suppress-next-line PhanPossiblyNonClassMethodCall
            return \reset($type_set)->asExpandedTypesPreservingTemplate($code_base, $recursion_depth + 1);
        }
        // 2 or more union types to merge
        $builder = new UnionTypeBuilder();
        foreach ($type_set as $type) {
            $builder->addUnionType($type->asExpandedTypesPreservingTemplate($code_base, $recursion_depth + 1));
        }
        return UnionType::of($builder->getTypeSet(), $this->real_type_set);
    }
    /**
    ...
    public function unserialize($serialized) : void
    {
        ...
        if ($i !== \false) {
            $result = UnionType::fromFullyQualifiedPHPDocAndRealString(substr($serialized, 0, $i), (string) substr($serialized, $i + 1));
    ...
    public function __toString() : string
    {
        ...
        $types = $this->type_set;
        $type_name_list = \array_map(static function (Type $type) : string {
            return (string) $type;
        }, $types);
    ...
    public static function getLatestRealFunctionSignatureMap(int $target_php_version) : array
    {
        if ($target_php_version >= 80000) {
            static $map_80;
            return $map_80 ?? ($map_80 = self::computeLatestRealFunctionSignatureMap(\true));
        }
        static $map_73;
    ...
    {
        return self::applyDeltaToGetOlderSignatures($older_map, ['old' => $delta['new'], 'new' => $delta['old']]);
    }
    /**
        ...
        if ($nullable) {
            ...
            foreach ($type_set as $type) {
                ...
                if ($type->isNullable()) {
                    ...
                    $new_types[] = $type->withIsNullable(\true);
                }
            }
     ...
     */
    public static function merge(array $union_types, bool $normalize_array_shapes = \true) : UnionType
    {
        $n = \count($union_types);
        if ($n < 2) {
        ...
        if ($array_shape_types) {
            ...
            $new_real_type_set = self::normalizeArrayShapes($new_real_type_set, $array_shape_types, $union_types, \true);
        }
    ...
    private static function combineArrayShapeTypes(array $types, bool $add_mixed) : ArrayShapeType
    {
        ...
        foreach ($common_field_types as $key => $_) {
            ...
            if ($is_possibly_undefined) {
                $new_element_union_type = $new_element_union_type->withIsPossiblyUndefined(\true);
            }
    ...
    private static function convertToTypeSetWithNormalizedNullableBools(UnionTypeBuilder $builder) : void
    {
        ...
        static $bool_type = null;
        if ($bool_type === null) {
            $true_type = TrueType::instance(\true);
            $false_type = FalseType::instance(\true);
            $bool_type = BoolType::instance(\true);
        }
        if (!$builder->isEmpty()) {
            $builder->removeType($true_type);
            $builder->removeType($false_type);
    ...
    public function withFlattenedTopLevelArrayShapeTypeInstances() : UnionType
    {
        ...
        return UnionType::of(self::withFlattenedTopLevelArrayShapeTypeInstancesForSet($this->type_set), self::withFlattenedTopLevelArrayShapeTypeInstancesForSet($this->real_type_set));
    }
    /**
        ...
        if ($empty_array_shape_type) {
            ...
            if (!$has_other_array_type) {
                ...
                foreach ($result as $i => $type) {
                    ...
                    if ($is_nullable) {
                        $result[$i] = $type->withIsNullable(\true);
                    }
                }
            }
        }
    ...
    public function withFlattenedArrayShapeTypeInstances() : UnionType
    {
        ...
        return UnionType::of(self::withFlattenedArrayShapeTypeInstancesForSet($this->type_set), self::withFlattenedArrayShapeTypeInstancesForSet($this->real_type_set));
    }
    /**
     * @param list<Type> $type_set
     * @return list<Type>
    ...
    public function withFlattenedArrayShapeOrLiteralTypeInstances() : UnionType
    {
        ...
        return UnionType::of(self::withFlattenedArrayShapeOrLiteralTypeInstancesForSet($this->type_set), self::withFlattenedArrayShapeOrLiteralTypeInstancesForSet($this->real_type_set));
    }
    /**
     * @param list<Type> $type_set
     * @return list<Type>
    ...
    {
        return UnionType::of(self::withAssociativeArraysForSet($this->type_set, $can_reduce_size), self::withAssociativeArraysForSet($this->real_type_set, $can_reduce_size));
    }
    /**
     * @param list<Type> $type_set
    ...
    {
        return UnionType::of(self::withIntegerKeyArraysAsListsForSet($this->type_set), self::withIntegerKeyArraysAsListsForSet($this->real_type_set));
    }
    /**
     * @param list<Type> $type_set
    ...
    public function withIsPossiblyUndefined(bool $is_possibly_undefined) : UnionType
    {
        ...
        $result = new AnnotatedUnionType($this->type_set, \true, $this->real_type_set);
    ...
    {
        return UnionType::of(self::applyUnaryBitwiseNotOperatorToList($this->type_set, \false), self::applyUnaryBitwiseNotOperatorToList($this->real_type_set, \true));
    }
    /**
     * @param list<Type> $type_set
    ...
    {
        return UnionType::of(self::applyUnaryNotOperatorToList($this->type_set), self::applyUnaryNotOperatorToList($this->real_type_set));
    }
    /**
     * @param Type[] $type_set
     * @return list<Type>
    ...
    {
        return UnionType::of(self::applyBoolCastToList($this->type_set), self::applyBoolCastToList($this->real_type_set));
    }
    /**
     * @param Type[] $type_set
     * @return list<Type>
    ...
    {
        return UnionType::of(self::applyNumericOperationToList($this->type_set, $operation), self::applyNumericOperationToList($this->real_type_set, $operation));
    }
    /**
     * @param List<Type> $type_set
     * @param Closure(int|float): ScalarType $operation
    ...
    public function getTemplateTypeExtractorClosure(CodeBase $code_base, TemplateType $template_type) : ?Closure
    {
        $closure = null;
        foreach ($this->type_set as $type) {
            $closure = TemplateType::combineParameterClosures($closure, $type->getTemplateTypeExtractorClosure($code_base, $template_type));
        }
        return $closure;
    }
    /**
    ...
    {
        $new_union_type = $this->withTemplateParameterTypeMap([$template_type->getName() => UnionType::fromFullyQualifiedPHPDocString('mixed')]);
    ...
    public function getRealUnionType() : UnionType
    {
        ...
        return new UnionType($real_type_set, \true, $real_type_set);
    }
    /**
