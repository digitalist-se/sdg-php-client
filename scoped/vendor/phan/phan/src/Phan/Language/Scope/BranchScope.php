...
class BranchScope extends Scope
{
    ...
    {
        return \array_key_exists($name, $this->variable_map) || $this->parent_scope->hasVariableWithName($name);
    }
    public function getVariableByName(string $name) : Variable
    {
        return $this->variable_map[$name] ?? $this->parent_scope->getVariableByName($name);
    }
    public function getVariableByNameOrNull(string $name) : ?Variable
    {
        return $this->variable_map[$name] ?? $this->parent_scope->getVariableByNameOrNull($name);
    }
    /**
