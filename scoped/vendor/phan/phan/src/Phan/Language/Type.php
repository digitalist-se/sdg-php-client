...
namespace SdgScoped\Phan\Language;
...
use SdgScoped\Phan\Debug\Frame;
...
use SdgScoped\Phan\Language\Element\Comment\Builder;
...
use SdgScoped\Phan\Language\Type\CallableArrayType;
use SdgScoped\Phan\Language\Type\CallableDeclarationType;
use SdgScoped\Phan\Language\Type\CallableObjectType;
use SdgScoped\Phan\Language\Type\CallableStringType;
use SdgScoped\Phan\Language\Type\CallableType;
use SdgScoped\Phan\Language\Type\ClassStringType;
use SdgScoped\Phan\Language\Type\ClosureDeclarationParameter;
use SdgScoped\Phan\Language\Type\ClosureDeclarationType;
...
use SdgScoped\Phan\Language\Type\FunctionLikeDeclarationType;
use SdgScoped\Phan\Language\Type\GenericArrayTemplateKeyType;
use SdgScoped\Phan\Language\Type\GenericArrayType;
use SdgScoped\Phan\Language\Type\GenericIterableType;
use SdgScoped\Phan\Language\Type\GenericMultiArrayType;
...
use SdgScoped\Phan\Language\Type\NativeType;
use SdgScoped\Phan\Language\Type\NonEmptyAssociativeArrayType;
use SdgScoped\Phan\Language\Type\NonEmptyGenericArrayType;
use SdgScoped\Phan\Language\Type\NonEmptyListType;
use SdgScoped\Phan\Language\Type\NonEmptyMixedType;
use SdgScoped\Phan\Language\Type\NonEmptyStringType;
use SdgScoped\Phan\Language\Type\NonNullMixedType;
use SdgScoped\Phan\Language\Type\NonZeroIntType;
use SdgScoped\Phan\Language\Type\NullType;
use SdgScoped\Phan\Language\Type\ObjectType;
use SdgScoped\Phan\Language\Type\ResourceType;
use SdgScoped\Phan\Language\Type\ScalarRawType;
use SdgScoped\Phan\Language\Type\ScalarType;
...
class Type
{
     ...
     */
    public const simple_type_regex = '(\\??)(?:callable-(?:string|object|array)|associative-array|class-string|lowercase-string|non-(?:zero-int|null-mixed|empty-(?:associative-array|array|list|string|lowercase-string|mixed))|\\\\?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*)';
    public const simple_noncapturing_type_regex = '\\\\?(?:callable-(?:string|object|array)|associative-array|class-string|lowercase-string|non-(?:zero-int|null-mixed|empty-(?:associative-array|array|list|string|lowercase-string|mixed))|[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*)';
    /**
     * @var string
     * A legal type identifier (e.g. 'int' or 'DateTime')
     */
    public const simple_type_regex_or_this = '(\\??)(callable-(?:string|object|array)|associative-array|class-string|lowercase-string|non-(?:zero-int|null-mixed|empty-(?:associative-array|array|list|string|lowercase-string|mixed))|\\\\?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*|\\$this)';
    public const shape_key_regex = '(?:[-.\\/^;$%*+_a-zA-Z0-9\\x7f-\\xff]|\\\\(?:[nrt\\\\]|x[0-9a-fA-F]{2}))+\\??';
    /**
     ...
     * NOTE: The / is escaped
     */
    public const noncapturing_literal_regex = '\\??(?:-?(?:(?:0|[1-9][0-9]*)(?:\\.[0-9]+)?)|\'(?:[- ,.\\/?:;"!#$%^&*_+=a-zA-Z0-9_\\x80-\\xff]|\\\\(?:[\'\\\\]|x[0-9a-fA-F]{2}))*\')';
     ...
     */
    public const array_shape_entry_regex_noncapturing = '(?:' . self::shape_key_regex . '\\s*:)?\\s*(?:' . self::simple_noncapturing_type_regex . '=?)';
    /**
     * @var string
     ...
     */
    public const type_regex = '(' . '(?:\\??\\((?-1)(?:[|&](?-1))*\\)|' . '(?:' . '\\??(?:\\\\?Closure|callable)(\\((?:[^()]|(?-1))*\\))' . '(?:\\s*:\\s*' . '(?:' . self::simple_noncapturing_type_regex . '|' . '\\((?-2)(?:\\s*[|&]\\s*(?-2))*\\)' . ')' . ')?' . ')|' . self::noncapturing_literal_regex . '|' . '(' . self::simple_type_regex . ')' . '(?:' . '<' . '(' . '(?-5)(?:[|&](?-5))*' . '(?:\\s*,\\s*' . '(?-5)(?:[|&](?-5))*' . ')*' . ')' . '>' . '|' . '\\{(' . '(?:' . self::shape_key_regex . '\\s*:)?\\s*(?-6)(?:[|&](?-6))*=?' . '(?:,(?:\\s*' . self::shape_key_regex . '\\s*:)?\\s*(?-6)(?:[|&](?-6))*=?)*' . ')?\\})?' . ')' . '(?:\\[\\])*' . ')';
    /**
     * @var string
     * A legal type identifier matching a type optionally with a []
     ...
     * https://www.debuggex.com/ is useful for a visual description of these regexes
     */
    public const type_regex_or_this = '(' . '(' . '(?:' . '\\??\\((?-1)(?:[|&](?-1))*\\)|' . '(?:' . '\\??(?:\\\\?Closure|callable)(\\((?:[^()]|(?-1))*\\))' . '(?:\\s*:\\s*' . '(?:' . self::simple_noncapturing_type_regex . '|' . '\\((?-2)(?:\\s*[|&]\\s*(?-2))*\\)' . ')' . ')?' . ')|' . self::noncapturing_literal_regex . '|' . '(' . self::simple_type_regex_or_this . ')' . '(?:<' . '(' . '(?-6)(?:[|&](?-6))*' . '(?:\\s*,\\s*' . '(?-6)(?:[|&](?-6))*' . ')*' . ')' . '>' . '|' . '(\\{)(' . '(?:' . self::shape_key_regex . '\\s*:)?\\s*(?-8)(?:[|&](?-8))*=?' . '(?:,(?:\\s*' . self::shape_key_regex . '\\s*:)?\\s*(?-8)(?:[|&](?-8))*=?)*' . ')?\\})?' . ')' . '(?:\\[\\])*' . ')' . ')';
    /**
     * @var array<string,bool> - For checking if a string is an internal type. This is used for case-insensitive lookup.
     */
    public const _internal_type_set = ['associative-array' => \true, 'array' => \true, 'bool' => \true, 'callable' => \true, 'callable-array' => \true, 'callable-object' => \true, 'callable-string' => \true, 'class-string' => \true, 'false' => \true, 'float' => \true, 'int' => \true, 'iterable' => \true, 'list' => \true, 'lowercase-string' => \true, 'mixed' => \true, 'non-empty-array' => \true, 'non-empty-associative-array' => \true, 'non-empty-mixed' => \true, 'non-null-mixed' => \true, 'non-empty-list' => \true, 'non-empty-string' => \true, 'non-empty-lowercase-string' => \true, 'non-zero-int' => \true, 'null' => \true, 'object' => \true, 'resource' => \true, 'scalar' => \true, 'static' => \true, 'string' => \true, 'true' => \true, 'void' => \true];
    /**
     ...
     */
    public const _soft_internal_type_set = ['resource' => \true, 'scalar' => \true, 'true' => \true];
     ...
     * be null.
     */
    protected function __construct(string $namespace, string $name, $template_parameter_type_list, bool $is_nullable)
    {
     ...
     * Overridden in some subclasses but not others.
     */
    protected static function make(string $namespace, string $type_name, array $template_parameter_type_list, bool $is_nullable, int $source) : Type
    {
        ...
        if ($pos > 0) {
            return GenericArrayType::fromElementType(Type::make($namespace, \substr($type_name, 0, $pos), $template_parameter_type_list, \false, $source), $is_nullable, GenericArrayType::KEY_MIXED);
        }
        if ($namespace === '') {
        ...
        if (!$value) {
            if ($namespace === '\\') {
                switch (strtolower($type_name)) {
                    case 'closure':
                        $value = new ClosureType('\\', 'Closure', $template_parameter_type_list, $is_nullable);
                        break;
                    case 'callable':
                        $value = new CallableType('\\', 'callable', $template_parameter_type_list, $is_nullable);
                        break;
                    case 'callable-object':
                        $value = new CallableObjectType($is_nullable);
                        break;
                    case 'callable-string':
                        $value = new CallableStringType($is_nullable);
                        break;
                    case 'callable-array':
                        $value = new CallableArrayType('\\', 'callable-array', $template_parameter_type_list, $is_nullable);
                        break;
                    case 'lowercase-string':
                        $value = StringType::instance($is_nullable);
                        break;
                    case 'class-string':
                        $value = new ClassStringType('\\', 'class-string', $template_parameter_type_list, $is_nullable);
                        break;
                    case 'list':
                        $value = self::parseListTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable, \false);
                        break;
                    case 'associative-array':
                        $value = self::parseGenericArrayTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable, \false, \true);
                        break;
                    case 'non-empty-associative-array':
                        $value = self::parseGenericArrayTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable, \true, \true);
                        break;
                    case 'non-empty-array':
                        $value = self::parseGenericArrayTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable, \true, \false);
                        break;
                    case 'non-empty-list':
                        $value = self::parseListTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable, \true);
                        break;
            ...
            if (!$value) {
                $value = new static($namespace, $type_name, $template_parameter_type_list, $is_nullable);
            }
    ...
    public function memoize(string $key, Closure $fn)
    {
        if (($this->memoized_data['current_progress_state'] ?? null) !== self::$current_progress_state) {
            $this->memoized_data = ['current_progress_state' => self::$current_progress_state, $key => $fn()];
     ...
     */
    public static function fromType(Type $type, array $template_parameter_type_list) : Type
    {
        return self::make($type->getNamespace(), $type->getName(), $template_parameter_type_list, $type->is_nullable, Type::FROM_TYPE);
    }
    /**
     * @param mixed $object
     * @return Type
    ...
    public static function nonLiteralFromObject($object) : Type
    {
        ...
        if ($type_map === null) {
            $type_map = ['integer' => IntType::instance(\false), 'boolean' => BoolType::instance(\false), 'double' => FloatType::instance(\false), 'string' => StringType::instance(\false), 'object' => ObjectType::instance(\false), 'NULL' => NullType::instance(\false), 'array' => ArrayType::instance(\false), 'resource' => ResourceType::instance(\false)];
        }
    ...
    public static function fromObject($object) : Type
    {
        switch (\gettype($object)) {
            ...
            case 'resource':
                return ResourceType::instance(\false);
            // For inferring the type of constants STDIN, etc.
            default:
    ...
    {
        return ArrayShapeType::fromFieldTypes(\array_map(
            /**
             * @param mixed $value
             */
     ...
     * @phan-side-effect-free
     */
    public static function fromInternalTypeName(string $type_name, bool $is_nullable, int $source, array $template_parameter_type_list = []) : Type
    {
        ...
        if (\false !== ($pos = \strrpos($type_name, '[]'))) {
            return GenericArrayType::fromElementType(self::fromInternalTypeName(\substr($type_name, 0, $pos), \false, $source, $template_parameter_type_list), $is_nullable, GenericArrayType::KEY_MIXED);
        }
        $type_name = self::canonicalNameFromName($type_name);
        ...
        if (\substr($type_name, 0, 1) === '?') {
            ...
            return self::fromInternalTypeName(\substr($type_name, 1), \true, $source, $template_parameter_type_list);
        }
     ...
     * @suppress PhanThrowTypeAbsentForCall
     */
    public static function fromNamespaceAndName(string $namespace, string $type_name, bool $is_nullable) : Type
    {
     ...
     * @phan-side-effect-free
     */
    public static function stringFromReflectionType(?\ReflectionType $reflection_type) : string
    {
        if (!$reflection_type) {
            return '';
        }
     ...
     * @throws FQSENException
     */
    protected static function fromFullyQualifiedStringInner(string $fully_qualified_string) : Type
    {
        ...
        if (\substr($fully_qualified_string, -2) === '[]') {
            ...
            return GenericArrayType::fromElementType(Type::fromFullyQualifiedString($fully_qualified_substring), $is_nullable, GenericArrayType::KEY_MIXED);
        }
        ...
        if (\is_array($shape_components)) {
            if (\strcasecmp($type_name, 'array') === 0) {
                return ArrayShapeType::fromFieldTypes(self::shapeComponentStringsToTypes($shape_components, new Context(), Type::FROM_NODE), $is_nullable);
            }
        ...
        if (!$namespace) {
            if (count($template_parameter_type_name_list) > 0) {
                ...
                switch ($type_name) {
                    ...
                    case 'associative-array':
                    case 'non-empty-associative-array':
                        return self::parseGenericArrayTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable, $type_name === 'non-empty-array', \true);
                    case 'list':
                    case 'non-empty-list':
            ...
            return self::fromInternalTypeName($fully_qualified_string, $is_nullable, Type::FROM_NODE, $template_parameter_type_list);
        }
        ...
        return self::make($namespace, $type_name, $template_parameter_type_list, $is_nullable, Type::FROM_NODE);
    }
     ...
     * @suppress PhanPossiblyFalseTypeArgument, PhanPossiblyFalseTypeArgumentInternal
     */
    private static function fromFullyQualifiedFunctionLike(bool $is_closure_type, array $shape_components, bool $is_nullable) : FunctionLikeDeclarationType
    {
        if (!$shape_components) {
     ...
     * @param bool $is_nullable
     */
    private static function parseGenericArrayTypeFromTemplateParameterList(array $template_parameter_type_list, bool $is_nullable, bool $always_has_elements, bool $is_associative) : ArrayType
    {
        ...
        if (count($types) === 1) {
            ...
            return new GenericMultiArrayType($types, $is_nullable, $key_type, $always_has_elements, \false, $is_associative);
        }
        if ($always_has_elements || $is_associative) {
     ...
     * @param bool $is_nullable
     */
    private static function parseListTypeFromTemplateParameterList(array $template_parameter_type_list, bool $is_nullable, bool $always_has_elements) : ArrayType
    {
        $template_count = count($template_parameter_type_list);
        if ($template_count !== 1) {
            if ($always_has_elements) {
        ...
        if (count($types) === 1) {
            ...
            return new GenericMultiArrayType($types, $is_nullable, GenericArrayType::KEY_INT, $always_has_elements, \true);
        }
        if ($always_has_elements) {
            return NonEmptyListType::fromElementType(MixedType::instance(\false), $is_nullable);
        }
     ...
     * @param bool $is_nullable
     */
    private static function parseGenericIterableTypeFromTemplateParameterList(array $template_parameter_type_list, bool $is_nullable) : Type
    {
        $template_count = count($template_parameter_type_list);
        if ($template_count <= 2) {
            // iterable<T> or iterable<key, T>
            // TODO: Warn about unparseable type or throw if more arguments are seen?
            $key_union_type = $template_count === 2 ? $template_parameter_type_list[0] : UnionType::empty();
     ...
     * @param bool $is_nullable
     */
    private static function parseClassStringTypeFromTemplateParameterList(array $template_parameter_type_list, bool $is_nullable) : Type
    {
        $template_count = count($template_parameter_type_list);
        if ($template_count === 1) {
            return new ClassStringType('\\', 'class-string', $template_parameter_type_list, $is_nullable);
        }
        return ClassStringType::instance($is_nullable);
    }
    /**
     ...
     * @phan-side-effect-free
     */
    public static function fromStringInContext(string $string, Context $context, int $source, CodeBase $code_base = null) : Type
    {
        if ($string === '') {
        ...
        if (\substr($string, -2) === '[]') {
            ...
            $types = UnionType::fromStringInContext($substring, $context, $source, $code_base);
            $type_set = $types->getTypeSet();
            if (count($type_set) === 1) {
                return GenericArrayType::fromElementType(\reset($type_set), $is_nullable, GenericArrayType::KEY_MIXED);
            } else {
                return new GenericMultiArrayType($type_set, $is_nullable, GenericArrayType::KEY_MIXED);
            }
        }
        ...
        if (\is_array($shape_components)) {
            if (\strcasecmp($type_name, 'array') === 0) {
                return ArrayShapeType::fromFieldTypes(self::shapeComponentStringsToTypes($shape_components, $context, $source, $code_base), $is_nullable);
            }
            if ($type_name === 'Closure' || $type_name === 'callable') {
        ...
        // Map the names of the types to actual types in the
        // template parameter type list
        $template_parameter_type_list = \array_map(static function (string $type_name) use($code_base, $context, $source) : UnionType {
        ...
        if ($is_generic_array_type && \false !== \strrpos($non_generic_array_type_name, '[]')) {
            return GenericArrayType::fromElementType(Type::fromStringInContext($non_generic_partially_qualified_array_type_name, $context, $source), $is_nullable, GenericArrayType::KEY_MIXED);
        }
        if (\substr($non_generic_partially_qualified_array_type_name, 0, 1) !== '\\' && $context->hasNamespaceMapFor(\ast\flags\USE_NORMAL, $non_generic_partially_qualified_array_type_name)) {
            ...
            if ($is_generic_array_type) {
                return GenericArrayType::fromElementType(Type::make($fqsen->getNamespace(), $fqsen->getName(), $template_parameter_type_list, \false, $source), $is_nullable, GenericArrayType::KEY_MIXED);
            }
            return Type::make($fqsen->getNamespace(), $fqsen->getName(), $template_parameter_type_list, $is_nullable, $source);
        }
        // If this was a fully qualified type, we're all
        // set
        if ($namespace && $namespace[0] === '\\') {
            return self::make($namespace, $type_name, $template_parameter_type_list, $is_nullable, $source);
        }
        if (self::isInternalTypeString($type_name, $source)) {
            if (count($template_parameter_type_list) > 0) {
                switch (\strtolower($type_name)) {
                    ...
                    case 'associative-array':
                        return self::parseGenericArrayTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable, \false, \true);
                    case 'non-empty-array':
                        return self::parseGenericArrayTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable, \true, \false);
                    case 'non-empty-associative-array':
                        return self::parseGenericArrayTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable, \true, \true);
                    case 'iterable':
                        return self::parseGenericIterableTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable);
                    case 'class-string':
                    ...
                    case 'non-empty-list':
                        return self::parseListTypeFromTemplateParameterList($template_parameter_type_list, $is_nullable, \true);
                }
        ...
        if ($is_generic_array_type && self::isSelfTypeString($non_generic_array_type_name) && $context->isInClassScope()) {
            ...
            if (stripos($non_generic_array_type_name, 'parent') !== \false) {
                ...
                $element_type = self::fromFullyQualifiedString($context->getClassFQSEN()->__toString());
            }
            return GenericArrayType::fromElementType($element_type, $is_nullable, GenericArrayType::KEY_MIXED);
        }
        ...
        if (self::isSelfTypeString($type_name) && !$namespace) {
            ...
            return self::make($fqsen->getNamespace(), $fqsen->getName(), $template_parameter_type_list, $is_nullable, $source);
        }
    ...
    private static function checkClosureString(CodeBase $code_base, Context $context, string $string) : void
    {
        ...
        if ($context->hasNamespaceMapFor(\ast\flags\USE_NORMAL, 'Closure')) {
            $fqsen = $context->getNamespaceMapFor(\ast\flags\USE_NORMAL, 'Closure');
            $namespace = $fqsen->getNamespace();
        } else {
            $namespace = $context->getNamespace();
        }
        if (($namespace ?: '\\') !== '\\') {
            Issue::maybeEmit($code_base, $context, Issue::CommentAmbiguousClosure, $context->getLineNumberStart(), $string, $namespace . '\\Closure');
        }
    }
    /**
     ...
     */
    private static function fromFunctionLikeInContext(bool $is_closure_type, array $shape_components, Context $context, int $source, bool $is_nullable) : FunctionLikeDeclarationType
    {
        $return_type = \array_pop($shape_components);
    ...
    private static function shapeComponentStringsToTypes(array $shape_components, Context $context, int $source, CodeBase $code_base = null) : array
    {
        ...
        foreach ($shape_components as $key => $component_string) {
            ...
            if (\is_string($key) && \substr($key, -1) === '?') {
                ...
                $key = \substr($key, 0, -1);
                $result[$key] = UnionType::fromStringInContext($component_string, $context, $source, $code_base)->withIsPossiblyUndefined(\true);
    ...
    private static function closureParamComponentStringsToParams(array $param_components, Context $context, int $source) : array
    {
        ...
        foreach ($param_components as $param_string) {
            ...
            if (preg_match('/^(' . UnionType::union_type_regex . ')?\\s*(&\\s*)?(?:(\\.\\.\\.)\\s*)?(?:\\$' . Builder::WORD_REGEX . ')?((?:\\s*=.*)?)$/', $param_string, $param_match)) {
                ...
                $union_type = UnionType::fromStringInContext($union_type_string, $context, $source);
    ...
    public function asPHPDocUnionType() : UnionType
    {
        ...
        return $this->singleton_union_type ?? ($this->singleton_union_type = new UnionType([$this], \true, []));
    }
    /**
     * @return UnionType
    ...
    {
        $type_set = [$this];
        return new UnionType($type_set, \true, $type_set);
    }
    /**
     * @return FQSEN
    ...
    {
        return \false;
        // overridden in FalseType and NullType, as well as literal scalar types
    }
    /**
    ...
    {
        return \true;
        // overridden in various types. This base class (Type) is implicitly the type of an object, which is always truthy.
    }
    /**
    ...
    public function withIsNullable(bool $is_nullable) : Type
    {
        ...
        return static::make($this->namespace, $this->name, $this->template_parameter_type_list, $is_nullable, Type::FROM_TYPE);
    }
    /**
    ...
    private static function isInternalTypeString(string $original_type_name, int $source) : bool
    {
        ...
        if (!\array_key_exists($type_name, self::_internal_type_set)) {
            return $original_type_name === '$this';
            // This is the only case-sensitive check.
        }
     ...
     * @phan-side-effect-free
     */
    public static function isSelfTypeString(string $type_string) : bool
    {
     ...
     * @phan-side-effect-free
     */
    public static function isStaticTypeString(string $type_string) : bool
    {
        // Note: While 'self' and 'parent' are case-insensitive, '$this' is case-sensitive
        // Not sure if that should extend to phpdoc.
    ...
    {
        return \false;
        // Overridden in subclass CallableType, ClosureType, FunctionLikeDeclarationType
    }
    /**
     * @return bool
    ...
    public function canPossiblyCastToClass(CodeBase $code_base, Type $other) : bool
    {
        ...
        if (($this_class->isClass() || $this->isThrowableInterface()) && ($other_class->isClass() || $other->isThrowableInterface())) {
    ...
    {
        return \false;
        // Overridden in GenericArrayType and ArrayShapeType
    }
    /**
    ...
    {
        return \strcasecmp($this->getName(), 'ArrayAccess') === 0 && $this->getNamespace() === '\\';
    }
    /**
    ...
    {
        return \strcasecmp($this->getName(), 'Traversable') === 0 && $this->getNamespace() === '\\';
    }
    /**
    ...
    {
        return \strcasecmp($this->getName(), 'Generator') === 0 && $this->getNamespace() === '\\';
    }
    /**
     * @param string $type_name
    ...
    public function iterableKeyUnionType(CodeBase $code_base) : ?UnionType
    {
        ...
        if ($expanded_types->hasTypeWithFQSEN($iterator_aggregate_fqsen)) {
            ...
            if (!$class->hasMethodWithName($code_base, 'getIterator', \true)) {
        ...
        if ($expanded_types->hasTypeWithFQSEN($iterator_fqsen)) {
            $class = $code_base->getClassByFQSEN($fqsen);
            if (!$class->hasMethodWithName($code_base, 'key', \true)) {
                // Should be impossible
                return null;
            }
            ...
            if ($result->hasTemplateTypeRecursive()) {
                $result = $result->withTemplateParameterTypeMap($iterator_type->getTemplateParameterTypeMap($code_base))->withoutTemplateTypeRecursive();
            }
            return $result;
        }
    ...
    public function iterableValueUnionType(CodeBase $code_base) : ?UnionType
    {
        ...
        if ($expanded_types->hasTypeWithFQSEN($iterator_fqsen)) {
            $class = $code_base->getClassByFQSEN($fqsen);
            if (!$class->hasMethodWithName($code_base, 'current', \true)) {
                // Should be impossible
                return null;
            }
    ...
    private function computeExpandedTypes(CodeBase $code_base, int $recursion_depth) : UnionType
    {
        ...
        $union_type = $union_type->withUnionType($clazz->getUnionType()->withIsNullable($this->is_nullable));
        ...
        foreach ($union_type->getTypeSet() as $clazz_type) {
            if ($clazz_type->__toString() !== $representation) {
                $recursive_union_type_builder->addUnionType($clazz_type->asExpandedTypes($code_base, $recursion_depth + 1));
            } else {
                $recursive_union_type_builder->addType($clazz_type);
            }
        }
        if (count($this->template_parameter_type_list) > 0) {
            $recursive_union_type_builder->addUnionType($clazz->resolveParentTemplateType($this->getTemplateParameterTypeMap($code_base)));
        }
        // Add in aliases
        ...
        foreach ($fqsen_aliases as $alias_fqsen_record) {
            ...
            $recursive_union_type_builder->addType($alias_fqsen->asType()->withIsNullable($this->is_nullable));
        }
     ...
     * @suppress PhanPartialTypeMismatchReturn
     */
    public function asExpandedTypesPreservingTemplate(CodeBase $code_base, int $recursion_depth = 0) : UnionType
    {
        if (($this->memoized_data['current_progress_state'] ?? null) === self::$current_progress_state) {
    ...
    private function computeExpandedTypesPreservingTemplate(CodeBase $code_base, int $recursion_depth) : UnionType
    {
        ...
        // Recurse up the tree to include all types
        if (count($this->template_parameter_type_list) > 0) {
            $recursive_union_type_builder->addUnionType($template_union_type);
        }
        foreach ($union_type->getTypeSet() as $clazz_type) {
            if ($clazz_type->__toString() !== $representation) {
                $recursive_union_type_builder->addUnionType($clazz_type->asExpandedTypesPreservingTemplate($code_base, $recursion_depth + 1));
            } else {
                $recursive_union_type_builder->addType($clazz_type);
            }
        }
    ...
    public function canCastToType(Type $type) : bool
    {
        ...
        if ($this->is_nullable) {
            ...
            if (Config::get_null_casts_as_any_type()) {
                ...
                return \true;
            } elseif ($type->isScalar() && (Config::getValue('scalar_implicit_cast') || in_array($type->getName(), Config::getValue('scalar_implicit_partial')['null'] ?? [], \true))) {
    ...
    protected function canCastToNonNullableType(Type $type) : bool
    {
        ...
        if ($type::NAME === 'iterable') {
            if ($this->namespace === '\\' && in_array($this->name, ['Generator', 'Traversable', 'Iterator'], \true)) {
     ...
     */
    public function isExclusivelyNarrowedFormOrEquivalentTo(UnionType $union_type, Context $context, CodeBase $code_base) : bool
    {
    ...
    private function withStaticResolvedInContextTemplate(Context $context) : Type
    {
     ...
     */
    protected final function templateParameterTypeListAsString() : string
    {
        return '<' . \implode(',', \array_map(static function (UnionType $type) : string {
            return $type->__toString();
        }, $this->template_parameter_type_list)) . '>';
    }
    private const CANONICAL_NAME_MAP = ['boolean' => 'bool', 'callback' => 'callable', 'closure' => 'Closure', 'double' => 'float', 'integer' => 'int'];
    /**
     * @param string $name
     * Any type name
     *
     ...
     * @phan-side-effect-free
     */
    public static function canonicalNameFromName(string $name) : string
    {
     ...
     */
    private static function typeStringComponents(string $type_string) : Tuple5
    {
     ...
     * @suppress PhanPossiblyFalseTypeArgument
     */
    private static function typeStringComponentsInner(string $type_string) : Tuple5
    {
        ...
        if (\preg_match('/^' . self::type_regex_or_this . '$/', $type_string, $match)) {
            ...
            if (!isset($match[2])) {
                ...
                if (\substr($type_string, -1) === ')') {
                    ...
                    return new Tuple5('', $match[0], [], \false, null);
                }
            }
            $type_string = $match[4];
            ...
            if (($match[8] ?? '') !== '') {
                $shape_components = self::extractShapeComponents($match[9] ?? '');
                // will be empty array for 'array{}'
            } else {
                // Recursively parse this
                $template_parameter_type_name_list = ($match[7] ?? '') !== '' ? self::extractNameList($match[7]) : [];
            }
        }
        ...
        return new Tuple5($namespace, $class_name, $template_parameter_type_name_list, $is_nullable, $shape_components);
    }
    /**
     * @return Tuple5<string,string,list<string>,bool,?array<string|int,string>>
    ...
    private static function closureTypeStringComponents(string $type_string, string $inner) : Tuple5
    {
        ...
        return new Tuple5('\\', preg_match('/^\\??callable/i', $type_string) > 0 ? 'callable' : 'Closure', [], $type_string[0] === '?', $parts);
    }
    /**
     * @return list<string>
     */
    ...
    public function getTemplateTypeExtractorClosure(CodeBase $code_base, TemplateType $template_type) : ?Closure
    {
        ...
        foreach ($this->template_parameter_type_list as $i => $actual_template_union_type) {
            ...
            $closure = TemplateType::combineParameterClosures($closure, static function (UnionType $type, Context $context) use($inner_extractor_closure, $i) : UnionType {
    ...
    public function asFunctionInterfaceOrNull(CodeBase $code_base, Context $context) : ?FunctionInterface
    {
        ...
        $class = $code_base->getClassByFQSEN($fqsen);
        if (!$class->hasMethodWithName($code_base, '__invoke', \true)) {
            Issue::maybeEmit($code_base, $context, Issue::UndeclaredInvokeInCallable, $context->getLineNumberStart(), '__invoke', $fqsen);
            return null;
        }
