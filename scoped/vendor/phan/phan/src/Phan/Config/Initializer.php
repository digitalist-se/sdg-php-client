...
namespace SdgScoped\Phan\Config;

use ast\Node;
use Closure;
use CompileError;
use SdgScoped\Composer\Semver\Constraint\ConstraintInterface;
use SdgScoped\Composer\Semver\VersionParser;
use ParseError;
...
class Initializer
{
    ...
    public static function initPhanConfig(array $opts) : void
    {
        ...
        if (isset($opts['init-no-composer'])) {
            ...
            $composer_settings = \json_decode($contents, \true);
        ...
        if (!\file_exists($phan_dir)) {
            if (!\mkdir($phan_dir)) {
                throw new UsageException("Failed to create directory '{$phan_dir}'", EXIT_FAILURE, UsageException::PRINT_INIT_ONLY, \true);
            }
        }
    ...
    public static function generateEntrySnippetForSetting(string $setting_name, $setting_value, array $additional_comment_lines) : string
    {
        ...
        $source .= '    ';
        $source .= \var_export($setting_name, \true) . ' => ';
...
EOT;
    ...
    public const LEVEL_MAP = ['strict' => 1, 'strong' => 2, 'average' => 3, 'normal' => 3, 'weak' => 4, 'weakest' => 5];
    /**
    ...
    public static function createPhanSettingsForComposerSettings(array $composer_settings, ?string $vendor_path, array $opts) : InitializedSettings
    {
        ...
        if ($is_weakest_level) {
            ...
            $plugins = ['AlwaysReturnPlugin', 'PregRegexCheckerPlugin', 'UnreachableCodePlugin'];
        } else {
            $plugins = ['AlwaysReturnPlugin', 'DollarDollarPlugin', 'DuplicateArrayKeyPlugin', 'DuplicateExpressionPlugin', 'PregRegexCheckerPlugin', 'PrintfCheckerPlugin', 'SleepCheckerPlugin', 'UnreachableCodePlugin', 'UseReturnValuePlugin', 'EmptyStatementListPlugin'];
        }
        ...
        $phan_settings = [
            ...
            // this is only useful for migrating from php5
            ...
            'exclude_file_list' => [],
            'exclude_analysis_directory_list' => $vendor_path !== null ? ['vendor/'] : [],
        ...
        foreach (\array_merge($require_directories, $require_dev_directories) as $requirement => $_) {
            ...
            // @phan-suppress-next-line PhanPossiblyFalseTypeArgumentInternal
            $library_composer_settings = \json_decode(\file_get_contents($path_to_composer_json), \true);
    ...
    private static function extractAutoloadFilesAndDirectories(string $relative_dir, array $composer_settings) : array
    {
        ...
        $autoload_directories = \array_merge($autoload_setting['psr-4'] ?? [], $autoload_setting['psr-0'] ?? [], $autoload_setting['classmap'] ?? []);
    ...
    public static function filterDirectoryAndFileList(array $directory_list, array $file_list) : array
    {
        ...
        return [\array_values(\array_unique($directory_list)), \array_values(\array_unique($file_list))];
    }
    /**
    ...
    public static function isPHPBinary(string $relative_path) : bool
    {
        ...
        try {
            ...
            $ast = Parser::parseCode(new CodeBase([], [], [], [], []), new Context(), null, $relative_path, $contents, \true);
