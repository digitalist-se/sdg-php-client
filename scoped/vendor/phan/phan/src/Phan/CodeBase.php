...
use SdgScoped\Phan\CodeBase\ClassMap;
use SdgScoped\Phan\CodeBase\UndoTracker;
use SdgScoped\Phan\Exception\FQSENException;
use SdgScoped\Phan\Language\Context;
use SdgScoped\Phan\Language\Element\ClassAliasRecord;
...
use SdgScoped\Phan\Language\Element\FunctionFactory;
...
use SdgScoped\Phan\Language\FQSEN\FullyQualifiedClassElement;
...
use SdgScoped\Phan\Language\NamespaceMapEntry;
use SdgScoped\Phan\Language\Type;
use SdgScoped\Phan\Language\UnionType;
use SdgScoped\Phan\Library\Map;
use SdgScoped\Phan\Library\Set;
use SdgScoped\Phan\Library\StringSuggester;
...
class CodeBase
{
     ...
     */
    public function __construct(array $internal_class_name_list, array $internal_interface_name_list, array $internal_trait_name_list, array $internal_constant_name_list, array $internal_function_name_list)
    {
    ...
    public function enableUndoTracking() : void
    {
        ...
        $this->has_enabled_undo_tracker = \true;
     ...
     */
    public function setShouldHydrateRequestedElements(bool $should_hydrate_requested_elements) : void
    {
    ...
    {
        $this->expect_changes_to_file_contents = \true;
    }
    /**
    ...
    private function addGlobalConstantsByNames(array $const_name_list) : void
    {
        ...
        if (is_array($included_extension_subset)) {
            ...
            foreach (get_defined_constants(\true) as $ext_name => $constant_values) {
                if (isset($included_extension_subset[strtolower($ext_name)])) {
                    continue;
                }
                foreach ($constant_values as $constant_name => $_) {
                    $excluded_constant_set[$constant_name] = \true;
                }
            }
    ...
    private static function handleGlobalConstantException(string $const_name, Exception $e) : void
    {
        ...
        if (\strncmp($const_name, "\x00__COMPILER_HALT_OFFSET__\x00", 26) === 0) {
            return;
        }
    ...
    {
        $this->getInternalClassMap();
        // Force initialization of remaining internal php classes to reduce latency of future analysis requests.
        $this->forceLoadingInternalFunctions();
        // Force initialization of internal functions to reduce latency of future analysis requests.
    }
    /**
        ...
        try {
            foreach ($internal_function_fqsen_set as $function_fqsen) {
                ...
                if (!$this->hasFunctionWithFQSEN($function_fqsen)) {
                    ...
                    $function_alternate_generator = FunctionFactory::functionListFromReflectionFunction($function_fqsen, new \ReflectionFunction($function_fqsen->getNamespacedName()));
        ...
        if (is_array($included_extension_subset)) {
            ...
            foreach (get_loaded_extensions() as $ext_name) {
                ...
                foreach (get_extension_funcs($ext_name) ?: [] as $function_name) {
                    $forbidden_function_set[strtolower($function_name)] = \true;
                }
            }
    ...
    {
        $this->fqsen_class_map = $this->fqsen_class_map->deepCopyValues();
        $this->fqsen_class_map_user_defined = new Map();
        $this->fqsen_class_map_internal = new Map();
        ...
        $this->fqsen_class_map_reflection = clone $this->fqsen_class_map_reflection;
        $this->fqsen_alias_map = $this->fqsen_alias_map->deepCopyValues();
        $this->fqsen_global_constant_map = $this->fqsen_global_constant_map->deepCopyValues();
        $this->fqsen_func_map = $this->fqsen_func_map->deepCopyValues();
        ...
        $this->class_fqsen_class_map_map = $this->class_fqsen_class_map_map->deepCopyValues();
        $this->internal_function_fqsen_set = clone $this->internal_function_fqsen_set;
    ...
    public function createRestorePoint() : array
    {
        ...
        return ['clone' => $clone, 'callbacks' => $callbacks];
    }
    /**
     * @return CodeBase
    ...
    public function shallowClone() : CodeBase
    {
        ...
        $code_base->fqsen_class_map_user_defined = clone $this->fqsen_class_map_user_defined;
        $code_base->fqsen_class_map_internal = clone $this->fqsen_class_map_internal;
        $code_base->fqsen_class_map_reflection = clone $this->fqsen_class_map_reflection;
        ...
        $code_base->internal_function_fqsen_set = clone $this->internal_function_fqsen_set;
        $code_base->class_fqsen_class_map_map = clone $this->class_fqsen_class_map_map;
        $code_base->method_set = clone $this->method_set;
        return $code_base;
    }
    /**
     * @param Clazz $class
     ...
     */
    public function addClassAlias(FullyQualifiedClassName $original, FullyQualifiedClassName $alias, Context $context, int $lineno) : void
    {
    ...
    private function resolveClassAliasesForAliasSet(FullyQualifiedClassName $original_fqsen, Set $alias_set) : void
    {
        if (!$this->hasClassWithFQSEN($original_fqsen)) {
            ...
            foreach ($alias_set as $alias_record) {
                ...
                Issue::maybeEmitWithParameters($this, $alias_record->context, Issue::UndeclaredClassAliasOriginal, $alias_record->lineno, [$original_fqsen, $alias_record->alias_fqsen], $suggestion);
            }
            return;
        }
        ...
        foreach ($alias_set as $alias_record) {
            ...
            if ($this->hasClassWithFQSEN($alias_fqsen)) {
                ...
                Issue::maybeEmit($this, $alias_record->context, Issue::RedefineClassAlias, $alias_record->lineno, $alias_fqsen, $alias_record->context->getFile(), $alias_record->lineno, $clazz->getFQSEN(), $clazz->getFileRef()->getFile(), $clazz->getFileRef()->getLineNumberStart());
            } else {
     ...
     */
    private function lazyLoadPHPInternalClassWithFQSEN(FullyQualifiedClassName $fqsen) : bool
    {
    ...
    private function loadPHPInternalClassWithFQSEN(FullyQualifiedClassName $fqsen, ReflectionClass $reflection_class) : void
    {
     ...
     */
    public function getClassByFQSENWithoutHydrating(FullyQualifiedClassName $fqsen) : Clazz
    {
     ...
     */
    public function getClassAliasesByFQSEN(FullyQualifiedClassName $original) : array
    {
        if ($this->fqsen_alias_map->offsetExists($original)) {
     ...
     */
    public function hasMethodWithFQSEN(FullyQualifiedMethodName $fqsen) : bool
    {
        return $this->getClassMapByFullyQualifiedClassName($fqsen->getFullyQualifiedClassName())->hasMethodWithName($fqsen->getNameWithAlternateId());
    }
    /**
     ...
     */
    public function getMethodByFQSEN(FullyQualifiedMethodName $fqsen) : Method
    {
        return $this->getClassMapByFullyQualifiedClassName($fqsen->getFullyQualifiedClassName())->getMethodByName($fqsen->getNameWithAlternateId());
    }
    /**
     * @return array<string,Method>
     * The set of methods associated with the given class
     */
    public function getMethodMapByFullyQualifiedClassName(FullyQualifiedClassName $fqsen) : array
    {
        return $this->getClassMapByFullyQualifiedClassName($fqsen)->getMethodMap();
    }
    /**
     * @return Set<Method>
    ...
    {
        if (!Config::get_track_references()) {
            throw new AssertionError(__METHOD__ . ' can only be called when dead code ' . ' detection (or force_tracking_references) is enabled.');
        }
     ...
     */
    public function hasFunctionWithFQSEN(FullyQualifiedFunctionName $fqsen) : bool
    {
    ...
    {
        $this->getClassMapByFullyQualifiedClassName($class_constant->getClassFQSEN())->addClassConstant($class_constant);
    }
    /**
     * @return bool
    ...
    {
        return $this->getClassMapByFullyQualifiedClassName($fqsen->getFullyQualifiedClassName())->hasClassConstantWithName($fqsen->getNameWithAlternateId());
    }
    /**
    ...
    {
        return $this->getClassMapByFullyQualifiedClassName($fqsen->getFullyQualifiedClassName())->getClassConstantByName($fqsen->getNameWithAlternateId());
    }
    /**
     * @return ClassConstant[]
     ...
     */
    public function hasGlobalConstantWithFQSEN(FullyQualifiedGlobalConstantName $fqsen) : bool
    {
     ...
     */
    public function getGlobalConstantByFQSEN(FullyQualifiedGlobalConstantName $fqsen) : GlobalConstant
    {
    ...
    {
        $this->getClassMapByFullyQualifiedClassName($property->getClassFQSEN())->addProperty($property);
    }
    /**
     * @return bool
     * True if a property with the given FQSEN exists
     */
    public function hasPropertyWithFQSEN(FullyQualifiedPropertyName $fqsen) : bool
    {
        return $this->getClassMapByFullyQualifiedClassName($fqsen->getFullyQualifiedClassName())->hasPropertyWithName($fqsen->getNameWithAlternateId());
    }
    /**
     ...
     */
    public function getPropertyByFQSEN(FullyQualifiedPropertyName $fqsen) : Property
    {
     ...
     */
    private function getClassMapByFullyQualifiedClassName(FullyQualifiedClassName $fqsen) : ClassMap
    {
    ...
    private function hasInternalFunctionWithFQSEN(FullyQualifiedFunctionName $fqsen) : bool
    {
        ...
        $function_signature_map = UnionType::internalFunctionSignatureMap(Config::get_closest_target_php_version_id());
        ...
        if (isset($function_signature_map[$name])) {
            ...
            foreach (FunctionFactory::functionListFromSignature($canonical_fqsen, $signature) as $function) {
                if ($name === 'each' && Config::get_closest_target_php_version_id() >= 70200) {
                    $function->setIsDeprecated(\true);
                }
                if ($found) {
            ...
            foreach (FunctionFactory::functionListFromReflectionFunction($canonical_fqsen, new \ReflectionFunction($name)) as $function) {
                $this->addFunction($function);
                $this->updatePluginsOnLazyLoadInternalFunction($function);
            }
            return \true;
    ...
    public function getPlaceholdersForUndeclaredFunction(FullyQualifiedFunctionName $fqsen) : array
    {
        ...
        $signature = $function_signature_map[$name];
        return FunctionFactory::functionListFromSignature($canonical_fqsen, $signature);
    }
    ...
    {
        $sum = \count($this->fqsen_func_map) + \count($this->fqsen_global_constant_map) + \count($this->fqsen_class_map_user_defined) + \count($this->fqsen_class_map_internal) + \count($this->fqsen_class_map_reflection);
        foreach ($this->class_fqsen_class_map_map as $class_map) {
    ...
    public static function getPHPInternalConstantNameList() : array
    {
        ...
        if ($constant_name_list === null) {
            ...
            $constant_name_list = \array_keys(\array_merge(['true' => \true, 'false' => \false, 'null' => null], ...\array_values(\array_diff_key(\get_defined_constants(\true), ['user' => []]))));
        }
    ...
    {
        $namespace = strtolower($namespace);
        return $this->class_names_suggester_in_namespace[$namespace] ?? ($this->class_names_suggester_in_namespace[$namespace] = new StringSuggester($this->getClassNamesOfNamespace($namespace)));
    }
    /**
    ...
    {
        $namespace = strtolower($namespace);
        return $this->function_names_suggester_in_namespace[$namespace] ?? ($this->function_names_suggester_in_namespace[$namespace] = new StringSuggester($this->getFunctionNamesOfNamespace($namespace)));
    }
    /**
    ...
    {
        $namespace = strtolower($namespace);
        return $this->constant_names_suggester_in_namespace[$namespace] ?? ($this->constant_names_suggester_in_namespace[$namespace] = new StringSuggester($this->getConstantNamesOfNamespace($namespace)));
    }
    /**
     ...
     */
    public function suggestSimilarClassInOtherNamespace(FullyQualifiedClassName $missing_class, Context $context) : array
    {
     ...
     */
    public function suggestSimilarGlobalFunctionInOtherNamespace(string $namespace, string $function_name, Context $context, bool $include_same_namespace = \false) : array
    {
     ...
     */
    public function suggestSimilarGlobalFunctionInNewerVersion(string $namespace, string $function_name, Context $context, bool $suggest_in_global_namespace) : array
    {
        ...
        foreach ($targets as $i => $target) {
            ...
            if (isset($signature_map[$function_name_lower])) {
                ...
                if (!isset($old_signature_map[$function_name_lower])) {
                    ...
                    $suggestion = \sprintf('(to use the function %s() added in PHP %s in a project with %s without a polyfill parsed by Phan)', $function_name, self::phpVersionIdToString($next_target), $details);
                    return [$suggestion];
                }
            }
        }
     ...
     * @internal
     */
    protected const _NON_CLASS_TYPE_SUGGESTION_SET = ['array' => 'array', 'bool' => 'bool', 'callable' => 'callable', 'false' => 'false', 'float' => 'float', 'int' => 'int', 'iterable' => 'iterable', 'mixed' => 'mixed', 'null' => 'null', 'object' => 'object', 'resource' => 'resource', 'scalar' => 'scalar', 'self' => 'self', 'static' => 'static', 'string' => 'string', 'true' => 'true'];
    /**
     * @return list<FullyQualifiedFunctionName|string> 0 or more namespaced function names found in this code base in $namespace
     */
    public function suggestSimilarGlobalFunctionInSameNamespace(string $namespace, string $name, Context $context, bool $suggest_in_global_namespace) : array
    {
        ...
        if ($namespace !== "\\" && $suggest_in_global_namespace) {
            $suggestions = \array_merge($suggestions, $this->suggestSimilarGlobalFunctionForNamespaceAndName("\\", $name, $context));
        }
        return $suggestions;
    }
    /**
    ...
    private function getClassIfConstructorAccessible(string $namespace, string $name, Context $context) : ?FullyQualifiedClassName
    {
        ...
        if (!$class->hasMethodWithName($this, '__construct', \true)) {
     ...
     */
    public function suggestSimilarNewInAnyNamespace(string $namespace, string $name, Context $context, bool $suggest_in_global_namespace) : array
    {
        try {
            $suggestions = [];
     ...
     */
    public function suggestSimilarGlobalFunctionForNamespaceAndName(string $namespace, string $name, Context $context) : array
    {
     ...
     */
    public function suggestSimilarGlobalConstantForNamespaceAndName(string $namespace, string $name) : array
    {
     ...
     */
    public function suggestSimilarClassInSameNamespace(FullyQualifiedClassName $missing_class, Context $context, int $class_suggest_type = IssueFixSuggester::CLASS_SUGGEST_ONLY_CLASSES) : array
    {
        ...
        $suggested_class_names = \array_keys(IssueFixSuggester::getSuggestionsForStringSet($class_name_lower, $suggestion_set));
