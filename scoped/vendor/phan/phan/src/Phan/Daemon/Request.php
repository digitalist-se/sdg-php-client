...
namespace SdgScoped\Phan\Daemon;

use Closure;
use SdgScoped\Phan\Analysis;
...
class Request
{
    public const METHOD_ANALYZE_FILES = 'analyze_files';
    ...
    public const STATUS_INVALID_FILES = 'invalid_files';
    // expected a valid string for 'files'/'file'
    public const STATUS_INVALID_METHOD = 'invalid_method';
     ...
     */
    public static function makeLanguageServerAnalysisRequest(Responder $responder, array $file_names, CodeBase $code_base, Closure $file_path_lister, FileMapping $file_mapping, ?NodeInfoRequest $most_recent_node_info_request, bool $should_exit) : Request
    {
        FileCache::clear();
        ...
        $result = new self($responder, [self::PARAM_FORMAT => 'json', self::PARAM_METHOD => self::METHOD_ANALYZE_FILES, self::PARAM_FILES => $file_names, self::PARAM_TEMPORARY_FILE_MAPPING_CONTENTS => $file_mapping_contents], $most_recent_node_info_request, $should_exit);
        return $result;
    }
    /**
     ...
     */
    private static function adjustFileMappingContentsForCompletionRequest(array $file_mapping_contents, CompletionRequest $completion_request) : array
    {
    ...
    public function getPrinter() : IssuePrinterInterface
    {
        ...
        if (!in_array($format, $factory->getTypes(), \true)) {
    ...
    public function respondWithIssues(int $issue_count) : void
    {
        ...
        $response = ["status" => self::STATUS_OK, "issue_count" => $issue_count, "issues" => $issues];
    ...
    {
        if ($this->responder) {
            $this->responder->sendResponseAndClose(['status' => self::STATUS_ERROR_UNKNOWN, 'message' => 'failed to send a response - Possibly encountered an exception. See daemon output: ' . StringUtil::jsonEncode(\debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS))]);
            $this->responder = null;
        }
    }
    /**
    ...
    public static function accept(CodeBase $code_base, Closure $file_path_lister, Responder $responder, bool $fork) : ?Request
    {
        ...
        if (!\is_array($request)) {
            $responder->sendResponseAndClose(['status' => self::STATUS_INVALID_REQUEST, 'message' => 'malformed JSON']);
            return null;
        }
        $new_file_mapping_contents = [];
        ...
        switch ($method) {
            ...
            case 'analyze_files':
                ...
                if ($error_message !== null) {
                    Daemon::debugf($error_message);
                    $responder->sendResponseAndClose(['status' => self::STATUS_INVALID_FILES, 'message' => $error_message]);
                    return null;
                }
                break;
            ...
            default:
                ...
                $responder->sendResponseAndClose(['status' => self::STATUS_INVALID_METHOD, 'message' => $message]);
                return null;
        }
        ...
        if ($fork_result < 0) {
            ...
            $request_obj = new self($responder, $request, null, \true);
            $temporary_file_mapping = $request_obj->getTemporaryFileMapping();
            if (count($temporary_file_mapping) > 0) {
                self::applyTemporaryFileMappingForParsePhase($code_base, $temporary_file_mapping);
            }
    ...
    public static function reloadFilePathListForDaemon(CodeBase $code_base, Closure $file_path_lister, array $file_mapping_contents, array $file_names = null) : void
    {
        ...
        $file_list = $file_path_lister(\true);
