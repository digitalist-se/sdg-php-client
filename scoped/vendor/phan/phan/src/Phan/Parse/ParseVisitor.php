...
namespace SdgScoped\Phan\Parse;

use AssertionError;
use SdgScoped\ast;
use ast\Node;
use InvalidArgumentException;
use SdgScoped\Phan\Analysis\ScopeVisitor;
...
use SdgScoped\Phan\Language\Element\Comment;
...
use SdgScoped\Phan\Language\FutureUnionType;
...
class ParseVisitor extends ScopeVisitor
{
    ...
    */
    ...
    public function visitClass(Node $node) : Context
    {
        ...
        $class_context = $this->context->withLineNumberStart($node->lineno ?? 0)->withLineNumberEnd($node->endLineno ?? 0);
        $class = new Clazz($class_context, $class_name, $class_fqsen->asRealUnionType(), $node->flags, $class_fqsen);
        ...
        try {
            ...
            $comment = Comment::fromStringInContext($doc_comment, $this->code_base, $class_context, $node->lineno, Comment::ON_CLASS);
            ...
            $class->setSuppressIssueSet($comment->getSuppressIssueSet());
            ...
            $class->setMagicPropertyMap($comment->getMagicPropertyMap(), $this->code_base);
            ...
            // are limited to classes with either __get or __set declared (or interface/abstract)
            $class->setMagicMethodMap($comment->getMagicMethodMap(), $this->code_base);
            ...
            if ($extends_node instanceof Node) {
                ...
                $parent_fqsen = FullyQualifiedClassName::fromFullyQualifiedString($parent_class_name);
            ...
            foreach ($node->children['implements']->children ?? [] as $name_node) {
                ...
                $class->addInterfaceClassFQSEN(FullyQualifiedClassName::fromFullyQualifiedString($name), $name_node->lineno);
            }
        } finally {
            $class->setDidFinishParsing(\true);
        }
    ...
    public function visitUseTrait(Node $node) : Context
    {
        ...
        $trait_fqsen_list = (new ContextNode($this->code_base, $this->context, $node->children['traits']))->getTraitFQSENList();
        ...
        $trait_adaptations_map = (new ContextNode($this->code_base, $this->context, $node->children['adaptations']))->getTraitAdaptationsMap($trait_fqsen_list);
    ...
    public function visitMethod(Node $node) : Context
    {
        ...
        $method_fqsen = FullyQualifiedMethodName::make($class->getFQSEN(), $method_name);
        // Hunt for an available alternate ID if necessary
        $alternate_id = 0;
        while ($code_base->hasMethodWithFQSEN($method_fqsen)) {
            $method_fqsen = $method_fqsen->withAlternateId(++$alternate_id);
        }
        $method = Method::fromNode(clone $context, $code_base, $node, $method_fqsen, $class);
        ...
        } elseif ('__tostring' === $method_name_lower && !$this->context->isStrictTypes()) {
     ...
     */
    private function addPromotedConstructorPropertyFromParam(Clazz $class, Method $method, Parameter $parameter, Node $parameter_node) : void
    {
        ...
        if ($parameter_node->flags & \ast\flags\PARAM_VARIADIC) {
            $this->emitIssue(Issue::InvalidNode, $lineno, "Cannot declare variadic promoted property");
            return;
        }
        ...
        $property_comment = Comment::fromStringInContext($doc_comment, $this->code_base, $this->context, $lineno, Comment::ON_PROPERTY);
        $property = $this->addProperty($class, $parameter->getName(), $parameter_node->children['default'], $parameter->getUnionType()->getRealUnionType(), $variable_comment, $lineno, $parameter_node->flags & Parameter::PARAM_MODIFIER_VISIBILITY_FLAGS, $doc_comment, $property_comment);
        if (!$property) {
            return;
        }
        // Get a comment on the property declaration
        $property->setHasWriteReference();
        // Assigned from within constructor
    ...
    public function visitPropGroup(Node $node) : Context
    {
        ...
        if ($type_node) {
            ...
            if (Config::get_closest_minimum_target_php_version_id() < 70400) {
                $this->emitIssue(Issue::CompatibleTypedProperty, $type_node->lineno, (string) $real_union_type ?: '(unknown)');
            }
        } else {
            $real_union_type = UnionType::empty();
        }
        ...
        // Get a comment on the property declaration
        $comment = Comment::fromStringInContext($doc_comment, $this->code_base, $this->context, $props_node->lineno ?? 0, Comment::ON_PROPERTY);
        foreach ($props_node->children as $i => $child_node) {
            // Ignore children which are not property elements
            if (!$child_node instanceof Node || $child_node->kind !== \ast\AST_PROP_ELEM) {
                continue;
            }
            ...
            if (!\is_string($property_name)) {
                throw new AssertionError('Property name must be a string. ' . 'Got ' . \print_r($property_name, \true) . ' at ' . (clone $this->context)->withLineNumberStart($child_node->lineno));
            }
    ...
    private function addProperty(Clazz $class, string $property_name, $default_node, UnionType $real_union_type, ?Comment\Parameter $variable, int $lineno, int $flags, ?string $doc_comment, Comment $property_comment) : ?Property
    {
        ...
        if ($default_node === null) {
            ...
            if ($real_union_type->isEmpty()) {
                ...
                if (!$union_type->isStrictSubtypeOf($this->code_base, $real_union_type)) {
                    $this->emitIssue(Issue::TypeMismatchPropertyDefaultReal, $context_for_property->getLineNumberStart(), $real_union_type, $property_name, ASTReverter::toShortString($default_node), $union_type);
                    $union_type = $real_union_type;
                } else {
        ...
        if ($this->code_base->hasPropertyWithFQSEN($property_fqsen)) {
            ...
            if ($old_property->getDefiningFQSEN() === $property_fqsen) {
                ...
                $this->emitIssue(Issue::RedefineProperty, $lineno, $property_name, $this->context->getFile(), $lineno, $this->context->getFile(), $old_property->getContext()->getLineNumberStart());
                return null;
            }
        }
        $property = new Property($context_for_property, $property_name, $union_type, $flags, $property_fqsen, $real_union_type);
        if ($variable) {
        ...
        if ($variable) {
            ...
            if ($default_node !== null && !$original_union_type->isType(NullType::instance(\false)) && !$variable->getUnionType()->asExpandedTypes($this->code_base)->canCastToUnionType($original_union_type) && !$original_union_type->asExpandedTypes($this->code_base)->canCastToUnionType($variable->getUnionType()) && !$property->checkHasSuppressIssueAndIncrementCount(Issue::TypeMismatchPropertyDefault)) {
                $this->emitIssue(Issue::TypeMismatchPropertyDefault, $lineno, (string) $variable->getUnionType(), $property->getName(), ASTReverter::toShortString($default_node), (string) $original_union_type);
            }
            ...
            if ($variable_type !== $original_variable_type) {
                ...
                if ($property->isStatic()) {
                    $this->emitIssue(Issue::StaticPropIsStaticType, $variable->getLineno(), $property->getRepresentationForIssue(), $original_variable_type, $variable_type);
                } else {
                    $property->setHasStaticInUnionType(\true);
                }
            }
    ...
    private function resolveDefaultPropertyNode(Node $node) : ?UnionType
    {
        if ($node->kind === \ast\AST_CONST) {
            try {
                return (new ContextNode($this->code_base, $this->context, $node))->getConst()->getUnionType()->eraseRealTypeSetRecursively();
            } catch (IssueException $_) {
                // ignore
            }
        }
        ...
        foreach ($node->children as $child_node) {
            ...
            if ($this->code_base->hasClassConstantWithFQSEN($fqsen)) {
                ...
                if ($old_constant->getDefiningFQSEN() === $fqsen) {
                    $this->emitIssue(Issue::RedefineClassConstant, $child_node->lineno, $name, $this->context->getFile(), $child_node->lineno, $this->context->getFile(), $old_constant->getContext()->getLineNumberStart());
                    continue;
                }
            }
            // Get a comment on the declaration
            $doc_comment = $child_node->children['docComment'] ?? '';
            $comment = Comment::fromStringInContext($doc_comment, $this->code_base, $this->context, $child_node->lineno ?? 0, Comment::ON_CONST);
            $line_number_start = $child_node->lineno ?? 0;
            $constant = new ClassConstant($this->context->withLineNumberStart($line_number_start)->withLineNumberEnd($child_node->endLineno ?? $line_number_start), $name, UnionType::empty(), $node->flags ?? 0, $fqsen);
            ...
            if ($value_node instanceof Node) {
                try {
                    ...
                    $constant->setFutureUnionType(new FutureUnionType($this->code_base, new ElementContext($constant), $value_node));
                } catch (InvalidArgumentException $_) {
                    $constant->setUnionType(MixedType::instance(\false)->asPHPDocUnionType());
                    $this->emitIssue(Issue::InvalidConstantExpression, $value_node->lineno);
                }
            } else {
            ...
            foreach ($comment->getVariableList() as $var) {
                if ($var->getUnionType()->hasTemplateTypeRecursive()) {
                    $this->emitIssue(Issue::TemplateTypeConstant, $constant->getFileRef()->getLineNumberStart(), (string) $constant->getFQSEN());
                    break;
                }
            }
        }
    ...
    private function checkNodeIsConstExpr(Node $node) : void
    {
        try {
            ...
            $this->emitIssue(Issue::InvalidConstantExpression, $node->lineno);
        }
    }
    /**
    ...
    public function visitConstDecl(Node $node) : Context
    {
        foreach ($node->children as $child_node) {
            ...
            try {
                ...
                $this->emitIssue(Issue::InvalidConstantExpression, $value_node->lineno ?? $child_node->lineno);
            ...
            self::addConstant($this->code_base, $this->context, $child_node->lineno, $child_node->children['name'], $value_node, $child_node->flags ?? 0, $child_node->children['docComment'] ?? '', \true);
        }
        return $this->context;
    }
    /**
    ...
    public function visitFuncDecl(Node $node) : Context
    {
        ...
        do {
            ...
            $function_fqsen = FullyQualifiedFunctionName::fromFullyQualifiedString(\rtrim($context->getNamespace(), '\\') . '\\' . $function_name)->withAlternateId($alternate_id++);
        } while ($code_base->hasFunctionWithFQSEN($function_fqsen));
        $func = Func::fromNode($context->withLineNumberStart($node->lineno ?? 0)->withLineNumberEnd($node->endLineno ?? 0), $code_base, $node, $function_fqsen);
        if ($context->isPHPInternal()) {
            // only for stubs
    ...
    {
        $closure_fqsen = FullyQualifiedFunctionName::fromClosureInContext($this->context->withLineNumberStart($node->lineno), $node);
    ...
    public function visitCall(Node $node) : Context
    {
        ...
        if ($expression instanceof Node && $expression->kind === \ast\AST_NAME) {
            ...
            if (\in_array($function_name, ['func_get_args', 'func_get_arg', 'func_num_args'], \true)) {
                if ($this->context->isInFunctionLikeScope()) {
                    $this->context->getFunctionLikeInScope($this->code_base)->setNumberOfOptionalParameters(FunctionInterface::INFINITE_PARAMETERS);
                }
    ...
    private function analyzeDefine(Node $node) : void
    {
        ...
        self::addConstant($this->code_base, $this->context, $node->lineno, $name, $args->children[1], 0, '', \true, \true);
    }
    /**
            ...
            if ($func_name === 'parent') {
                ...
                if (!$node->children['method'] instanceof Node) {
                    ...
                    if ($meth === '__construct' && $this->context->isInClassScope()) {
                        $class = $this->getContextClass();
                        $class->setIsParentConstructorCalled(\true);
                    }
                }
            }
        }
    ...
    public function visitDim(Node $node) : Context
    {
        ...
        // check for $$var[]
        if ($expr->kind === \ast\AST_VAR && ($expr->children['name']->kind ?? null) === \ast\AST_VAR) {
            ...
            // $foo->$bar['baz'];
        } elseif ($expr->kind === \ast\AST_PROP && ($expr->children['expr']->kind ?? null) === \ast\AST_VAR && ($expr->children['prop']->kind ?? null) === \ast\AST_VAR) {
            $cache_entry = FileCache::getOrReadEntry($this->context->getFile());
     ...
     */
    public static function addConstant(CodeBase $code_base, Context $context, int $lineno, string $name, $value, int $flags, string $comment_string, bool $use_future_union_type, bool $is_fully_qualified = \false) : void
    {
        $i = \strrpos($name, '\\');
        if ($i !== \false) {
        ...
        if (\in_array(\strtolower($name_fragment), ['true', 'false', 'null'], \true)) {
            Issue::maybeEmit($code_base, $context, Issue::ReservedConstantName, $lineno, $name);
            return;
        }
        try {
            // Give it a fully-qualified name
            if ($is_fully_qualified) {
                $fqsen = FullyQualifiedGlobalConstantName::fromFullyQualifiedString($name);
            } else {
                $fqsen = FullyQualifiedGlobalConstantName::fromStringInContext($name, $context);
            }
        } catch (InvalidArgumentException|FQSENException $_) {
            Issue::maybeEmit($code_base, $context, Issue::InvalidConstantFQSEN, $lineno, $name);
            return;
        }
        // Create the constant
        $constant = new GlobalConstant($context->withLineNumberStart($lineno), $name, UnionType::fromFullyQualifiedRealString('array|bool|float|int|string|resource|null'), $flags, $fqsen);
        ...
        // Get a comment on the declaration
        $comment = Comment::fromStringInContext($comment_string, $code_base, $context, $lineno, Comment::ON_CONST);
        if ($use_future_union_type) {
            if ($value instanceof Node) {
                // TODO: Avoid using this when it only contains literals (nothing depending on the CodeBase),
                // e.g. `['key' => 'value']`
                $constant->setFutureUnionType(new FutureUnionType($code_base, $context, $value));
            } else {
        ...
        $constant->setIsDeprecated($comment->isDeprecated());
        $constant->setIsNSInternal($comment->isNSInternal());
        $code_base->addGlobalConstant($constant);
    }
    /**
     * @return Clazz
     ...
     * @internal
     */
    public const ALLOWED_CONST_EXPRESSION_KINDS = [\ast\AST_ARRAY_ELEM => \true, \ast\AST_ARRAY => \true, \ast\AST_BINARY_OP => \true, \ast\AST_CLASS_CONST => \true, \ast\AST_CLASS_NAME => \true, \ast\AST_CONDITIONAL => \true, \ast\AST_CONST => \true, \ast\AST_DIM => \true, \ast\AST_MAGIC_CONST => \true, \ast\AST_NAME => \true, \ast\AST_UNARY_OP => \true, \ast\AST_UNPACK => \true];
    /**
    ...
    protected const ALLOWED_NON_VARIABLE_EXPRESSION_KINDS = [
        ...
        \ast\AST_ARRAY => \true,
        \ast\AST_BINARY_OP => \true,
        \ast\AST_CLASS_CONST => \true,
        \ast\AST_CLASS_NAME => \true,
        \ast\AST_CONDITIONAL => \true,
        \ast\AST_CONST => \true,
        \ast\AST_DIM => \true,
        \ast\AST_MAGIC_CONST => \true,
        \ast\AST_NAME => \true,
        \ast\AST_UNARY_OP => \true,
        ...
        \ast\AST_NEW => \true,
        \ast\AST_PRINT => \true,
        \ast\AST_SHELL_EXEC => \true,
        \ast\AST_STATIC_CALL => \true,
        \ast\AST_STATIC_PROP => \true,
        \ast\AST_UNPACK => \true,
        // Stop here
