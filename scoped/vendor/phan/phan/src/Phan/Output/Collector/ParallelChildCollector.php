...
namespace SdgScoped\Phan\Output\Collector;
...
class ParallelChildCollector implements IssueCollectorInterface
{
    ...
    public static function getQueueForProcessGroup()
    {
        ...
        if (!$resource) {
            throw new AssertionError('Expected msg_get_queue to return a valid resource or SysvMessageQueue');
        }
        return $resource;
    }
    /**
     ...
     * @internal
     */
    public static final function assertSharedMemoryCommunicationEnabled() : void
    {
        if (!\extension_loaded('sysvsem')) {
            throw new AssertionError('PHP must be compiled with --enable-sysvsem in order to use -j(>=2).');
        }
        if (!\extension_loaded('sysvmsg')) {
            throw new AssertionError('PHP must be compiled with --enable-sysvmsg in order to use -j(>=2).');
        }
    }
    /**
     * Collect issue
    ...
    {
        return;
        // Never going to be called - daemon mode isn't combined with parallel execution.
    }
    /**
