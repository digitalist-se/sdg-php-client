...
final class CheckstylePrinter implements BufferedPrinterInterface
{
    ...
    public function print(IssueInstance $instance) : void
    {
        ...
        $this->files[$file][] = ['line' => $instance->getLine(), 'source' => $instance->getIssue()->getType(), 'message' => $instance->getMessageAndMaybeSuggestion(), 'severity' => $instance->getIssue()->getSeverityName()];
    }
    ...
    public function flush() : void
    {
        ...
        foreach ($this->files as $file_name => $error_list) {
            ...
            foreach ($error_list as $error_map) {
                ...
                // of the error
                $error->appendChild(new \DOMAttr('line', \htmlspecialchars((string) $error_map['line'], ENT_NOQUOTES, 'UTF-8')));
                ...
                $error->appendChild(new \DOMAttr('severity', \htmlspecialchars($level, ENT_NOQUOTES, 'UTF-8')));
                $error->appendChild(new \DOMAttr('message', \htmlspecialchars((string) $error_map['message'], ENT_NOQUOTES, 'UTF-8')));
                $error->appendChild(new \DOMAttr('source', \htmlspecialchars((string) $error_map['source'], ENT_NOQUOTES, 'UTF-8')));
            }
        }
