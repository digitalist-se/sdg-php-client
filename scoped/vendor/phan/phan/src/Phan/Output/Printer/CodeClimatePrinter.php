...
final class CodeClimatePrinter implements BufferedPrinterInterface
{
    ...
    {
        $this->messages[] = ['type' => 'issue', 'check_name' => $instance->getIssue()->getType(), 'description' => $instance->getMessageAndMaybeSuggestion(), 'categories' => ['Bug Risk'], 'severity' => self::mapSeverity($instance->getIssue()->getSeverity()), 'location' => [
            // XXX this puts the docker volume in /code/ when running on codeclimate.
            'path' => \preg_replace('/^\\/code\\//', '', $instance->getFile()),
            'lines' => ['begin' => $instance->getLine(), 'end' => $instance->getLine()],
        ]];
    }
    ...
    public function flush() : void
    {
        ...
        foreach ($this->messages as $message) {
            $encoded_message = StringUtil::jsonEncode($message) . "\x00";
