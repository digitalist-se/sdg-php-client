...
use SdgScoped\Phan\Config\Initializer;
use SdgScoped\Phan\Daemon\ExitException;
use SdgScoped\Phan\Debug\SignalHandler;
use SdgScoped\Phan\Exception\UsageException;
use SdgScoped\Phan\ForkPool\Writer;
use SdgScoped\Phan\Language\Element\AddressableElement;
use SdgScoped\Phan\Language\Element\Comment\Builder;
use SdgScoped\Phan\Language\FQSEN;
use SdgScoped\Phan\Language\Type;
use SdgScoped\Phan\Library\Restarter;
use SdgScoped\Phan\Library\StderrLogger;
use SdgScoped\Phan\Library\StringUtil;
use SdgScoped\Phan\Output\Collector\BufferingCollector;
use SdgScoped\Phan\Output\Colorizing;
use SdgScoped\Phan\Output\Filter\CategoryIssueFilter;
use SdgScoped\Phan\Output\Filter\ChainedIssueFilter;
use SdgScoped\Phan\Output\Filter\FileIssueFilter;
use SdgScoped\Phan\Output\Filter\MinimumSeverityFilter;
use SdgScoped\Phan\Output\PrinterFactory;
use SdgScoped\Phan\Plugin\ConfigPluginSet;
use SdgScoped\Phan\Plugin\Internal\MethodSearcherPlugin;
...
use SdgScoped\Symfony\Component\Console\Output\StreamOutput;
use SdgScoped\Symfony\Component\Console\Terminal;
...
class CLI
{
     ...
     */
    public const PHAN_VERSION = '3.2.10';
    /**
     ...
     * @internal
     */
    public const GETOPT_LONG_OPTIONS = ['absolute-path-issue-messages', 'allow-polyfill-parser', 'analyze-all-files', 'assume-real-types-for-internal-functions', 'automatic-fix', 'backward-compatibility-checks', 'baseline-summary-type:', 'color', 'color-scheme:', 'config-file:', 'constant-variable-detection', 'daemonize-socket:', 'daemonize-tcp-host:', 'daemonize-tcp-port:', 'dead-code-detection', 'dead-code-detection-prefer-false-positive', 'debug', 'debug-emitted-issues:', 'debug-signal-handler', 'directory:', 'disable-cache', 'disable-plugins', 'dump-analyzed-file-list', 'dump-ast', 'dump-ctags:', 'dump-parsed-file-list', 'dump-signatures-file:', 'find-signature:', 'exclude-directory-list:', 'exclude-file:', 'extended-help', 'file-list:', 'file-list-only:', 'force-polyfill-parser', 'force-polyfill-parser-with-original-tokens', 'help', 'help-annotations', 'ignore-undeclared', 'include-analysis-file-list:', 'init', 'init-level:', 'init-analyze-dir:', 'init-analyze-file:', 'init-no-composer', 'init-overwrite', 'language-server-allow-missing-pcntl', 'language-server-analyze-only-on-save', 'language-server-completion-vscode', 'language-server-disable-completion', 'language-server-disable-go-to-definition', 'language-server-disable-hover', 'language-server-disable-output-filter', 'language-server-enable', 'language-server-enable-completion', 'language-server-enable-go-to-definition', 'language-server-enable-hover', 'language-server-force-missing-pcntl', 'language-server-hide-category', 'language-server-min-diagnostics-delay-ms:', 'language-server-on-stdin', 'language-server-require-pcntl', 'language-server-tcp-connect:', 'language-server-tcp-server:', 'language-server-verbose', 'load-baseline:', 'analyze-twice', 'always-exit-successfully-after-analysis', 'long-progress-bar', 'markdown-issue-messages', 'memory-limit:', 'minimum-severity:', 'minimum-target-php-version:', 'native-syntax-check:', 'no-color', 'no-progress-bar', 'output:', 'output-mode:', 'parent-constructor-required:', 'polyfill-parse-all-element-doc-comments', 'plugin:', 'print-memory-usage-summary', 'processes:', 'progress-bar', 'project-root-directory:', 'quick', 'redundant-condition-detection', 'require-config-exists', 'save-baseline:', 'signature-compatibility', 'strict-method-checking', 'strict-object-checking', 'strict-param-checking', 'strict-property-checking', 'strict-return-checking', 'strict-type-checking', 'target-php-version:', 'unused-variable-detection', 'use-fallback-parser', 'version'];
    /**
     * @internal
     */
    ...
    private function __construct(array $opts, array $argv)
    {
        ...
        if (\array_key_exists('extended-help', $opts)) {
            throw new UsageException('', EXIT_SUCCESS, UsageException::PRINT_EXTENDED);
            // --extended-help prints help and calls exit(0)
        }
        if (\array_key_exists('h', $opts) || \array_key_exists('help', $opts)) {
            throw new UsageException('', EXIT_SUCCESS, UsageException::PRINT_NORMAL);
        ...
        if (\is_string($overridden_project_root_directory)) {
            if (!\is_dir($overridden_project_root_directory)) {
                throw new UsageException(StringUtil::jsonEncode($overridden_project_root_directory) . ' is not a directory', EXIT_FAILURE, null, \true);
            }
        ...
        if (isset($opts['language-server-force-missing-pcntl'])) {
            Config::setValue('language_server_use_pcntl_fallback', \true);
        ...
        foreach ($opts as $key => $value) {
            ...
            switch ($key) {
                ...
                case 'file-list':
                    ...
                    foreach ($file_list as $file_name) {
                        if (!is_string($file_name)) {
                            ...
                            throw new UsageException("invalid argument for --file-list", EXIT_FAILURE, null, \true);
                        }
                        ...
                        if (\is_file($file_path) && \is_readable($file_path)) {
                            ...
                            if (is_array($lines)) {
                                $this->file_list_in_config = \array_merge($this->file_list_in_config, $lines);
                                continue;
                            }
                        }
                        throw new UsageException("Unable to read --file-list of {$file_path}", EXIT_FAILURE, null, \true);
                    }
                    break;
                case 'l':
                case 'directory':
                    if (!$this->file_list_only) {
                        ...
                        foreach ($directory_list as $directory_name) {
                            if (!is_string($directory_name)) {
                                throw new UsageException('Invalid --directory setting (expected a single argument)', EXIT_FAILURE, null, \false);
                            }
                            $this->file_list_in_config = \array_merge($this->file_list_in_config, $this->directoryNameToFileList($directory_name));
                        }
                    }
                    break;
                case 'k':
                ...
                case 'm':
                case 'output-mode':
                    if (!is_string($value) || !in_array($value, $factory->getTypes(), \true)) {
                        throw new UsageException(\sprintf('Unknown output mode %s. Known values are [%s]', StringUtil::jsonEncode($value), \implode(',', $factory->getTypes())), EXIT_FAILURE, null, \true);
                    }
                    $printer_type = $value;
                    break;
                case 'c':
                ...
                case 'q':
                case 'quick':
                    Config::setValue('quick_mode', \true);
                    break;
                case 'b':
                case 'backward-compatibility-checks':
                    Config::setValue('backward_compatibility_checks', \true);
                    break;
                case 'p':
                case 'progress-bar':
                    $progress_bar = \true;
                ...
                case 'long-progress-bar':
                    Config::setValue('__long_progress_bar', \true);
                    $progress_bar = \true;
                    break;
                ...
                case 'D':
                case 'debug':
                    Config::setValue('debug_output', \true);
                    break;
                ...
                case 'dump-ctags':
                    ...
                    Config::setValue('plugins', \array_merge(Config::getValue('plugins'), [__DIR__ . '/Plugin/Internal/CtagsPlugin.php']));
                    break;
                case 'dump-parsed-file-list':
                    Config::setValue('dump_parsed_file_list', \true);
                    break;
                ...
                case 'find-signature':
                    ...
                    Config::setValue('plugins', \array_merge(Config::getValue('plugins'), [__DIR__ . '/Plugin/Internal/MethodSearcherPluginLoader.php']));
                    break;
                case 'automatic-fix':
                    Config::setValue('plugins', \array_merge(Config::getValue('plugins'), [__DIR__ . '/Plugin/Internal/IssueFixingPlugin.php']));
                    break;
                case 'o':
                case 'output':
                    if (!is_string($value)) {
                ...
                case 'exclude-directory-list':
                    ...
                    break;
                case 'exclude-file':
                    Config::setValue('exclude_file_list', \array_merge(Config::getValue('exclude_file_list'), \is_array($value) ? $value : [$value]));
                    break;
                case 'I':
                ...
                case 'signature-compatibility':
                    Config::setValue('analyze_signature_compatibility', \true);
                    break;
                case 'y':
                ...
                case 'language-server-hide-category':
                    Config::setValue('language_server_hide_category_of_issues', \true);
                    break;
                ...
                case 'use-fallback-parser':
                    Config::setValue('use_fallback_parser', \true);
                    break;
                case 'strict-method-checking':
                    Config::setValue('strict_method_checking', \true);
                    break;
                case 'strict-param-checking':
                    Config::setValue('strict_param_checking', \true);
                    break;
                case 'strict-property-checking':
                    Config::setValue('strict_property_checking', \true);
                    break;
                case 'strict-object-checking':
                    Config::setValue('strict_object_checking', \true);
                    break;
                case 'strict-return-checking':
                    Config::setValue('strict_return_checking', \true);
                    break;
                case 'S':
                ...
                case 'daemonize-socket':
                    ...
                    if (!is_string($socket_dirname) || !\file_exists($socket_dirname) || !\is_dir($socket_dirname)) {
                        $msg = \sprintf('Requested to create Unix socket server in %s, but folder %s does not exist', StringUtil::jsonEncode($value), StringUtil::jsonEncode($socket_dirname));
                        throw new UsageException($msg, 1, null, \true);
                    } else {
                        Config::setValue('daemonize_socket', $value);
                ...
                case 'daemonize-tcp-host':
                    ...
                    Config::setValue('daemonize_tcp', \true);
                ...
                case 'language-server-on-stdin':
                    Config::setValue('language_server_config', ['stdin' => \true]);
                    break;
                ...
                case 'language-server-analyze-only-on-save':
                    Config::setValue('language_server_analyze_only_on_save', \true);
                    break;
                ...
                case 'language-server-disable-output-filter':
                    Config::setValue('language_server_disable_output_filter', \true);
                    break;
                case 'x':
                case 'dead-code-detection':
                    Config::setValue('dead_code_detection', \true);
                    break;
                case 'X':
                ...
                case 'unused-variable-detection':
                    Config::setValue('unused_variable_detection', \true);
                    break;
                case 'constant-variable-detection':
                    Config::setValue('constant_variable_detection', \true);
                    Config::setValue('unused_variable_detection', \true);
                    break;
                case 't':
                case 'redundant-condition-detection':
                    Config::setValue('redundant_condition_detection', \true);
                    break;
                case 'assume-real-types-for-internal-functions':
                    Config::setValue('assume_real_types_for_internal_functions', \true);
                    break;
                case 'allow-polyfill-parser':
                    ...
                    if (!\extension_loaded('ast')) {
                        Config::setValue('use_polyfill_parser', \true);
                        break;
                    }
                ...
                case 'force-polyfill-parser-with-original-tokens':
                    Config::setValue('use_polyfill_parser', \true);
                    Config::setValue('__parser_keep_original_node', \true);
                    break;
                case 'memory-limit':
                ...
                case 'print-memory-usage-summary':
                    Config::setValue('print_memory_usage_summary', \true);
                    break;
                case 'markdown-issue-messages':
                    Config::setValue('markdown_issue_messages', \true);
                    break;
                case 'absolute-path-issue-messages':
                    Config::setValue('absolute_path_issue_messages', \true);
                    break;
                case 'color-scheme':
                case 'C':
                case 'color':
                ...
                case 'baseline-summary-type':
                    ...
                    break;
                case 'analyze-twice':
                    Config::setValue('__analyze_twice', \true);
                    break;
                case 'always-exit-successfully-after-analysis':
                    Config::setValue('__always_exit_successfully_after_analysis', \true);
                    break;
                default:
                    // All of phan's long options are currently at least 2 characters long.
                    $key_repr = strlen($key) >= 2 ? "--{$key}" : "-{$key}";
                    if ($value === \false && in_array($key . ':', self::GETOPT_LONG_OPTIONS, \true)) {
        ...
        if (Config::getValue('color_issue_messages') === null && in_array($printer_type, ['text', 'verbose'], \true)) {
            if (Config::getValue('color_issue_messages_if_supported') && self::supportsColor(\STDOUT)) {
                Config::setValue('color_issue_messages', \true);
            }
        }
        ...
        if (!$this->file_list_only) {
            ...
            $this->file_list_in_config = \array_merge($this->file_list_in_config, array_slice($argv, 1));
        }
        ...
        if (Config::getValue('processes') !== 1 && Config::getValue('dead_code_detection')) {
    ...
    {
        Config::setValue('plugins', \array_unique(\array_merge(Config::getValue('plugins'), $plugins)));
    }
    /**
    ...
    private static function warnSuspiciousShortOptions(array $argv) : void
    {
        ...
        foreach (self::GETOPT_LONG_OPTIONS as $opt) {
            $opt_set['-' . \rtrim($opt, ':')] = \true;
        }
        foreach (array_slice($argv, 1) as $arg) {
            $arg = \preg_replace('/=.*$/D', '', $arg);
            if (\array_key_exists($arg, $opt_set)) {
                self::printHelpSection("WARNING: Saw suspicious CLI arg '{$arg}' (did you mean '-{$arg}')\n", \false, \true);
            }
        }
    }
    /**
    ...
    private static function detectAndConfigureColorSupport(array $opts) : void
    {
        ...
        if (isset($opts['C']) || isset($opts['color'])) {
            ...
            Config::setValue('color_issue_messages_if_supported', \true);
        }
    }
        ...
        if ($include_analysis_file_list) {
            ...
            foreach ($include_analysis_file_list as $file) {
                ...
                if (\file_exists($absolute_path)) {
                    ...
                    \fprintf(STDERR, "%sCould not find file '%s' passed in %s" . PHP_EOL, self::colorizeHelpSectionIfSupported('WARNING: '), $absolute_path, self::colorizeHelpSectionIfSupported('--include-analysis-file-list'));
                }
            }
            if ($valid_files === 0) {
                // TODO convert this to an error in Phan 3.
                $error_message = \sprintf("None of the files to analyze in %s exist - This will be an error in future Phan releases." . PHP_EOL, Config::getProjectRootDirectory());
    ...
    private static function isTerminal($output) : bool
    {
        if (\defined('PHP_WINDOWS_VERSION_BUILD')) {
            ...
            return \function_exists('sapi_windows_vt100_support') && \sapi_windows_vt100_support($output);
        }
        if (\function_exists('stream_isatty')) {
            return \stream_isatty($output);
        } elseif (\function_exists('posix_isatty')) {
    ...
    private static function checkPluginsExist() : void
    {
        ...
        foreach (Config::getValue('plugins') as $plugin_path_or_name) {
            ...
            if (!\is_file($plugin_file_name)) {
                ...
                self::printErrorToStderr(\sprintf("Phan %s could not find plugin %s%s\n", CLI::PHAN_VERSION, StringUtil::jsonEncode($plugin_file_name), $details));
    ...
    public function recomputeFileList() : void
    {
        ...
        if (!$this->file_list_only) {
            ...
            /** @var list<string> */
            $this->file_list = \array_merge($this->file_list, Config::getValue('file_list'));
            // Merge in any directories given in the config
            foreach (Config::getValue('directory_list') as $directory_name) {
                $this->file_list = \array_merge($this->file_list, self::directoryNameToFileList($directory_name));
            }
        ...
        if (count(Config::getValue('exclude_file_list')) > 0) {
            ...
            foreach (Config::getValue('exclude_file_list') as $file) {
                ...
                $exclude_file_set[$normalized_file] = \true;
                $exclude_file_set["./{$normalized_file}"] = \true;
            }
            $this->file_list = \array_values(\array_filter($this->file_list, static function (string $file) use($exclude_file_set) : bool {
...
EOT;
     ...
     * @internal
     */
    public static function usage(string $msg = '', ?int $exit_code = EXIT_SUCCESS, int $usage_type = UsageException::PRINT_NORMAL, bool $forbid_color = \true) : void
    {
        global $argv;
        if ($msg !== '') {
...
, $forbid_color);
    ...
    {
        $colorize_flag_cb = static function (array $match) : string {
        ...
        $colorize_opt_cb = static function (array $match) : string {
            $cli_flag = $match[0];
     ...
     * @internal
     */
    public static function getFlagSuggestionString(string $key) : string
    {
        ...
        if (strlen($key) === 1) {
            if (in_array($key, $short_options, \true)) {
    ...
    public static function shouldParse(string $file_path) : bool
    {
        ...
        if (!is_string($extension) || !in_array($extension, $file_extensions, \true)) {
            return \false;
        }
     ...
     */
    private static function directoryNameToFileList(string $directory_name) : array
    {
        $file_list = [];
        try {
            $file_extensions = Config::getValue('analyzed_file_extensions');
            ...
            $exclude_file_regex = Config::getValue('exclude_file_regex');
            $filter_folder_or_file = static function (SplFileInfo $file_info, $unused_key, \RecursiveIterator $iterator) use($file_extensions, $exclude_file_regex) : bool {
                try {
                    if (\in_array($file_info->getBaseName(), ['.', '..'], \true)) {
                        // Exclude '.' and '..'
                        return \false;
                    }
                    if ($file_info->isDir()) {
                    ...
                    if (!in_array($file_info->getExtension(), $file_extensions, \true)) {
                        return \false;
                    }
                    if (!$file_info->isFile()) {
                        // Handle symlinks to invalid real paths
                        $file_path = $file_info->getRealPath() ?: $file_info->__toString();
                        CLI::printErrorToStderr("Unable to read file {$file_path}: SplFileInfo->isFile() is false for SplFileInfo->getType() == " . \var_export(self::getSplFileInfoType($file_info), \true) . "\n");
                        return \false;
                    }
            ...
            $iterator = new \RecursiveIteratorIterator(new \RecursiveCallbackFilterIterator(new \RecursiveDirectoryIterator($directory_name, \RecursiveDirectoryIterator::FOLLOW_SYMLINKS), $filter_folder_or_file));
        ...
        foreach ($file_list as $file_path) {
            ...
            $normalized_file_list[\preg_replace("@[/\\\\]+@", "\x00", $file_path)] = $file_path;
        }
    ...
    {
        return (Config::getValue('progress_bar') || Config::getValue('debug_output')) && !Config::getValue('dump_ast') && !self::isDaemonOrLanguageServer();
    }
    /**
    ...
    {
        return Config::getValue('daemonize_tcp') || Config::getValue('daemonize_socket') || Config::getValue('language_server_config');
    }
    /**
     ...
     */
    public static function isPathMatchedByRegex(string $exclude_file_regex, string $path_name) : bool
    {
     ...
     */
    public static function progress(string $msg, float $p, $details = null, ?int $offset = null, ?int $count = null) : void
    {
    ...
    public static function outputProgressLine(string $msg, float $p, float $memory, float $peak, ?int $offset = null, ?int $count = null) : void
    {
        ...
        $right_side = " " . $percent_progress . "%" . \sprintf(' %' . $width . 'dMB/%' . $width . 'dMB', (int) $memory, (int) $peak);
        ...
        $msg = "\r" . $left_side . $progress_bar . $right_side . "\r";
        fwrite(STDERR, $msg);
    }
    /**
    ...
    private static function renderInnerProgressBar(int $length, float $p) : string
    {
        ...
        if ($delta > 1.0 / 3) {
            ...
            $first = $delta > 2.0 / 3 ? "▓" : "▒";
            $progress_bar .= $first . str_repeat("░", $rest - 1);
        } else {
            $progress_bar .= str_repeat("░", $rest);
        }
    ...
    private static function renderLongProgress(string $msg, float $p, float $memory, ?int $offset, ?int $count) : string
    {
        ...
        if (in_array($msg, ['analyze', 'parse'], \true)) {
            while (self::$current_progress_offset_long_progress < $offset) {
                ...
                if ($mod === 0 || self::$current_progress_offset_long_progress === $count) {
                    ...
                    $buf .= " " . \sprintf("%" . strlen((string) (int) $count) . "d / %d (%3d%%) %.0fMB" . PHP_EOL, min(self::$current_progress_offset_long_progress, $count), (int) $count, 100 * $p, $memory);
                }
            }
        } else {
    ...
    private function maybeReadConfigFile(bool $require_config_exists) : void
    {
        ...
        $config_file_name = StringUtil::isNonZeroLengthString($config_file_name) ? \realpath($config_file_name) : \implode(DIRECTORY_SEPARATOR, [Config::getProjectRootDirectory(), '.phan', 'config.php']);
        ...
        if ($config_file_name === \false || !\file_exists($config_file_name)) {
            if ($require_config_exists) {
                ...
                if ($config_file_name !== \false) {
                    ...
                    $msg = \sprintf("Could not figure out the path for config file %s, but --require-config-exists was set", StringUtil::encodeValue($this->config_file));
    ...
    private static function ensureASTParserExists() : void
    {
        ...
        if (!\extension_loaded('ast')) {
            ...
            \SdgScoped\phan_output_ast_installation_instructions();
            exit(EXIT_FAILURE);
        }
        self::sanityCheckAstVersion();
        try {
            // Split up the opening PHP tag to fix highlighting in vim.
            \ast\parse_code('<' . '?php 42;', Config::AST_VERSION);
        } catch (\LogicException $_) {
            self::printHelpSection('ERROR: Unknown AST version (' . Config::AST_VERSION . ') in configuration. ' . "You may need to rebuild the latest version of the php-ast extension.\n" . "See https://github.com/phan/phan#getting-started for more details.\n" . "(You are using php-ast " . (new ReflectionExtension('ast'))->getVersion() . ", but " . Config::MINIMUM_AST_EXTENSION_VERSION . " or newer is required. Alternately, test with --force-polyfill-parser (which is noticeably slower))\n", \false, \true);
            exit(EXIT_FAILURE);
        }
        // Workaround for https://github.com/nikic/php-ast/issues/79
        try {
            \ast\parse_code('<' . '?php syntaxerror', Config::AST_VERSION);
            self::printHelpSection('ERROR: Expected ast\\parse_code to throw ParseError on invalid inputs. Configured AST version: ' . Config::AST_VERSION . '. ' . "You may need to rebuild the latest version of the php-ast extension.\n", \false, \true);
            exit(EXIT_FAILURE);
        } catch (\ParseError $_) {
    ...
    private static function sanityCheckAstVersion() : void
    {
        ...
        if (\version_compare($ast_version, '1.0.0') <= 0) {
            ...
            \fprintf(STDERR, "ERROR: Phan 3.x requires php-ast 1.0.1+ because it depends on AST version 70. php-ast '%s' is installed." . PHP_EOL, $ast_version);
...
EOT
        ...
        if (self::shouldRestartToExclude('grpc') && self::willUseMultipleProcesses()) {
            ...
            fwrite(STDERR, "[info] grpc can cause php to hang when Phan is run with options that require forking." . PHP_EOL . "[info] Restarting with grpc disabled." . PHP_EOL . "[info] Set the environment variable PHAN_ALLOW_GRPC to 1 to disable this message and to allow grpc." . PHP_EOL);
        }
