...
use ParseError;
use SdgScoped\Phan\Analysis\DuplicateFunctionAnalyzer;
use SdgScoped\Phan\Analysis\ParameterTypesAnalyzer;
use SdgScoped\Phan\Analysis\ReferenceCountsAnalyzer;
use SdgScoped\Phan\Analysis\ThrowsTypesAnalyzer;
use SdgScoped\Phan\AST\ASTSimplifier;
...
class Analysis
{
    ...
    public static function parseFile(CodeBase $code_base, string $file_path, bool $suppress_parse_errors = \false, string $override_contents = null, bool $is_php_internal_stub = \false, ?Request $request = null) : Context
    {
        ...
        if ($file_contents === '') {
            ...
            Issue::maybeEmit($code_base, $context, Issue::EmptyFile, 0, $original_file_path);
            return $context;
        }
        ...
        if (Config::getValue('simplify_ast')) {
            try {
                ...
                Issue::maybeEmit(
                    ...
                    $context,
                    Issue::SyntaxError,
                    // Not the right kind of error. I don't think it would throw, anyway.
                    $e->getLine(),
                    $e->getMessage()
                );
            }
        }
        $context = self::parseNodeInContext($code_base, $context, $node);
    ...
    public static function parseNodeInContext(CodeBase $code_base, Context $context, Node $node)
    {
        ...
        $inner_context = (new ParseVisitor($code_base, $context))->{Element::VISIT_LOOKUP_TABLE[$kind] ?? 'handleMissingNodeKind'}($node);
    ...
    public static function analyzeFunctions(CodeBase $code_base, array $file_filter = null) : void
    {
        ...
        $analyze_function_or_method = static function (FunctionInterface $function_or_method) use($code_base, $plugin_set, $has_function_or_method_plugins, $file_filter) : void {
            ...
            ThrowsTypesAnalyzer::analyzeThrowsTypes($code_base, $function_or_method);
    ...
    public static function loadMethodPlugins(CodeBase $code_base) : void
    {
        ...
        foreach ($return_type_overrides as $fqsen_string => $unused_closure) {
            if (\strpos($fqsen_string, '::') !== \false) {
                ...
                $return_type_override_fqsen_strings[$fqsen->__toString()] = \true;
            }
        }
                    ...
                    if ($code_base->hasClassWithFQSEN($class_fqsen)) {
                        ...
                        if ($code_base->hasMethodWithFQSEN($fqsen)) {
                            ...
                            foreach ($methods_by_defining_fqsen->offsetGet($fqsen) as $child_method) {
                                if ($child_method->getFQSEN() !== $fqsen && isset($return_type_override_fqsen_strings[$child_method->getFQSEN()->__toString()])) {
     ...
     * were $override_contents
     */
    public static function analyzeFile(CodeBase $code_base, string $file_path, ?Request $request, string $override_contents = null) : Context
    {
    ...
    private static function emitSyntaxError(CodeBase $code_base, Context $context, Throwable $e) : void
    {
        Issue::maybeEmit($code_base, $context, Issue::SyntaxError, $e->getLine(), $e->getMessage());
    }
}
