...
use SdgScoped\Phan\Daemon\Transport\CapturerResponder;
use SdgScoped\Phan\Daemon\Transport\StreamResponder;
use SdgScoped\Phan\Issue;
use SdgScoped\Phan\Language\Element\MarkupDescription;
use SdgScoped\Phan\Language\FileRef;
use SdgScoped\Phan\LanguageServer\Protocol\ClientCapabilities;
use SdgScoped\Phan\LanguageServer\Protocol\CompletionContext;
use SdgScoped\Phan\LanguageServer\Protocol\CompletionItem;
use SdgScoped\Phan\LanguageServer\Protocol\CompletionOptions;
use SdgScoped\Phan\LanguageServer\Protocol\Diagnostic;
use SdgScoped\Phan\LanguageServer\Protocol\DiagnosticSeverity;
use SdgScoped\Phan\LanguageServer\Protocol\Hover;
use SdgScoped\Phan\LanguageServer\Protocol\InitializeResult;
use SdgScoped\Phan\LanguageServer\Protocol\Location;
use SdgScoped\Phan\LanguageServer\Protocol\Message;
use SdgScoped\Phan\LanguageServer\Protocol\Position;
use SdgScoped\Phan\LanguageServer\Protocol\Range;
use SdgScoped\Phan\LanguageServer\Protocol\SaveOptions;
use SdgScoped\Phan\LanguageServer\Protocol\ServerCapabilities;
use SdgScoped\Phan\LanguageServer\Protocol\TextDocumentSyncKind;
use SdgScoped\Phan\LanguageServer\Protocol\TextDocumentSyncOptions;
use SdgScoped\Phan\LanguageServer\Server\TextDocument;
use SdgScoped\Phan\Library\StringUtil;
use SdgScoped\Phan\Phan;
use SdgScoped\Sabre\Event\Loop;
...
use function SdgScoped\Sabre\Event\coroutine;
use function strlen;
...
class LanguageServer extends AdvancedJsonRpc\Dispatcher
{
     ...
     * @var bool
     */
    private $is_accepting_new_requests = \true;
    /**
    ...
    public static function run(CodeBase $code_base, Closure $file_path_lister, array $options) : ?Request
    {
        ...
        $make_language_server = static function (ProtocolStreamReader $in, ProtocolStreamWriter $out) use($code_base, $file_path_lister) : LanguageServer {
            return new LanguageServer($in, $out, $code_base, $file_path_lister);
        };
     ...
     */
    public function awaitDefinition(string $uri, Position $position, bool $is_type_definition_request) : Promise
    {
     ...
     */
    public function awaitHover(string $uri, Position $position) : Promise
    {
     ...
     * @return Promise <Location|Location[]|null>
     */
    public function awaitCompletion(string $uri, Position $position, CompletionContext $completion_context = null) : Promise
    {
        // TODO: Add a way to "go to definition" without emitting analysis results as a side effect
        $path_to_analyze = Utils::uriToPath($uri);
    ...
    private function getFilteredURIsToAnalyze() : array
    {
        ...
        $file_path_list = ($this->file_path_lister)(\true);
        ...
        foreach ($uris_to_analyze as $path_to_analyze => $uri) {
            ...
            if (!\in_array($uri, $file_path_list, \true) && !\in_array($relative_path_to_analyze, $file_path_list, \true)) {
    ...
    private function finalizeAnalyzingURIs() : void
    {
        ...
        if ($pid > 0) {
            ...
            $json_contents = \json_decode($concatenated, \true);
        ...
        Loop\stop();
        // abort the loop (without closing streams?)
    }
    /**
    ...
    private function finishAnalyzingURIsWithoutPcntl(array $uris_to_analyze) : void
    {
        ...
        $analysis_request = Request::makeLanguageServerAnalysisRequest($responder, $paths_to_analyze, $code_base, $this->file_path_lister, $this->file_mapping, $this->most_recent_node_info_request, \false);
    ...
    private function handleJSONResponseFromWorker(array $uris_to_analyze, array $response_data) : void
    {
        ...
        if ($most_recent_node_info_request) {
            if ($most_recent_node_info_request instanceof GoToDefinitionRequest) {
                ...
                if ($most_recent_node_info_request->isHoverRequest()) {
                    ...
                    $this->cached_hover_response = new CachedHoverResponse($most_recent_node_info_request->getUrl(), $most_recent_node_info_request->getPosition(), $this->file_mapping, $normalized_hover);
                }
        ...
        foreach ($uris_to_analyze as $uri) {
            ...
            $diagnostics[$normalized_requested_uri] = [];
            // send an empty diagnostic list on failure.
        }
    ...
    private static function delayBeforePublishDiagnostics() : void
    {
        ...
        if ($delay > 0) {
            $elapsed_ms = 1000 * (\microtime(\true) - self::$last_publish_timestamp);
            $remaining_ms = $delay - $elapsed_ms;
            if ($remaining_ms > 0) {
                \usleep((int) ($remaining_ms * 1000));
            }
            self::$last_publish_timestamp = \microtime(\true);
        }
    }
    ...
    public function initialize(ClientCapabilities $capabilities, string $rootPath = null, int $processId = null) : Promise
    {
        return coroutine(function () : \Generator {
            ...
            if ($this->textDocument === null) {
                $this->textDocument = new TextDocument($this->client, $this, $this->file_mapping);
            }
