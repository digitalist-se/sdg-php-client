...
use SdgScoped\Phan\AST\TolerantASTConverter\TolerantASTConverter;
...
use SdgScoped\Phan\LanguageServer\Protocol\CompletionItem;
use SdgScoped\Phan\LanguageServer\Protocol\CompletionItemKind;
...
class CompletionResolver
{
        ...
        return static function (Context $context, Node $node, array $parent_node_list) use($request, $code_base) : void {
            ...
            switch ($kind) {
                ...
                case \ast\AST_PROP:
                    ...
                    self::locatePropertyCompletion($request, $code_base, $context, $node, $kind === \ast\AST_STATIC_PROP, $prop_name);
                    if ($kind === \ast\AST_PROP) {
                        self::locateMethodCompletion($request, $code_base, $context, $node, \false, $prop_name);
                    }
                    return;
                case \ast\AST_CLASS_CONST:
                    ...
                    if ($const_name === TolerantASTConverter::INCOMPLETE_CLASS_CONST) {
                        $const_name = '';
                        self::locatePropertyCompletion($request, $code_base, $context, $node, \true, '');
                    }
                    self::locateClassConstantCompletion($request, $code_base, $context, $node, $const_name);
                    self::locateMethodCompletion($request, $code_base, $context, $node, \true, $const_name);
                    return;
                case \ast\AST_CONST:
     ...
     */
    public static function locatePropertyCompletion(CompletionRequest $request, CodeBase $code_base, Context $context, Node $node, bool $is_static, $incomplete_prop_name) : void
    {
        ...
        $class_list_generator = (new ContextNode($code_base, $context, $node->children['class'] ?? $node->children['expr']))->getClassList(\true, $expected_type_categories, $expected_issue);
     ...
     */
    private static function locateClassConstantCompletion(CompletionRequest $request, CodeBase $code_base, Context $context, Node $node, $constant_name) : void
    {
        ...
        // Find all of the classes on the left-hand side
        // TODO: Filter by properties that match $node->children['prop']
        $class_list_generator = (new ContextNode($code_base, $context, $class_node))->getClassList(\true, ContextNode::CLASS_LIST_ACCEPT_OBJECT_OR_CLASS_NAME);
        // And find all of the instance/static properties that can be used as completions
        foreach ($class_list_generator as $class) {
            // @phan-suppress-next-line PhanAccessMethodInternal
            $visible_constant_map = IssueFixSuggester::filterSimilarConstants($code_base, $context, $class->getConstantMap($code_base));
     ...
     */
    private static function locateMethodCompletion(CompletionRequest $request, CodeBase $code_base, Context $context, Node $node, bool $is_static, $incomplete_method_name) : void
    {
        ...
        // Find all of the classes on the left-hand side
        // TODO: Filter by properties that match $node->children['prop']
        $class_list_generator = (new ContextNode($code_base, $context, $node->children['class'] ?? $node->children['expr']))->getClassList(\true, ContextNode::CLASS_LIST_ACCEPT_OBJECT_OR_CLASS_NAME);
        // And find all of the instance/static properties that can be used as completions
        foreach ($class_list_generator as $class) {
    ...
    private const COMPLETING_STATEMENT_TOKENS = [
        ...
        'trait',
        'use',
        // Only support use statements for now, not closure use
        'static',
        'unset',
        'list',
    ];
    ...
    private const COMPLETING_TOKENS = [
        ...
        'require_once',
        'namespace',
        // namespace-relative identifiers such as namespace\foo()
        // 'insteadof',
        'global',
        'isset',
        'empty',
     ...
     */
    private static function locateMiscellaneousTokenCompletion(CompletionRequest $request, CodeBase $code_base, Context $context, Node $node, string $incomplete_constant_name, array $parent_node_list) : void
    {
     ...
     * @suppress PhanUnusedPrivateMethodParameter
     */
    private static function locateGlobalConstantCompletion(CompletionRequest $request, CodeBase $code_base, Context $context, Node $node, string $incomplete_constant_name) : void
    {
     ...
     */
    private static function locateClassCompletion(CompletionRequest $request, CodeBase $code_base, Context $context, Node $node, string $incomplete_class_name) : void
    {
        // TODO: Use the alias map
        ...
        foreach ($class_names_in_namespace as $class_name) {
            ...
            $request->recordCompletionElement($code_base, $code_base->getClassByFQSEN($class_fqsen), $class_name);
        }
    }
    /**
     * @suppress PhanUnusedPrivateMethodParameter TODO: Use $node and check if fully qualified
     */
    private static function locateGlobalFunctionCompletion(CompletionRequest $request, CodeBase $code_base, Context $context, Node $node, string $incomplete_function_name) : void
    {
        ...
        foreach ($code_base->getFunctionMap() as $func) {
            ...
            $request->recordCompletionElement($code_base, $func, $function_name);
        }
    }
    private static function locateVariableCompletion(CompletionRequest $request, CodeBase $code_base, Context $context, string $incomplete_variable_name) : void
    {
        ...
        foreach ($variable_candidates as $suggested_variable_name => $variable) {
            ...
            $request->recordCompletionElement($code_base, $variable, $prefix . $incomplete_variable_name);
        }
        ...
        foreach ($superglobal_names as $superglobal_name) {
            ...
            $request->recordCompletionElement($code_base, new Variable(
                $context,
                $superglobal_name,
