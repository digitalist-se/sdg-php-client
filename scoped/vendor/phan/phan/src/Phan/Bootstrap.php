...
if (\PHP_VERSION_ID < 70200) {
    \fprintf(\STDERR, "ERROR: Phan 3.x requires PHP 7.2+ to run, but PHP %s is installed." . \PHP_EOL, \PHP_VERSION);
    ...
    if (!\getenv('PHAN_SUPPRESS_PHP_UPGRADE_NOTICE')) {
        \fprintf(\STDERR, "Phan %s does not support PHP 8.1+ (PHP %s is installed)" . \PHP_EOL, CLI::PHAN_VERSION, \PHP_VERSION);
        \fwrite(\STDERR, "Phan 4 or newer may support the latest features of PHP 8.1+." . \PHP_EOL);
        \fwrite(\STDERR, "Executing anyway (Phan may crash). This notice can be disabled with PHAN_SUPPRESS_PHP_UPGRADE_NOTICE=1." . \PHP_EOL);
    }
}
\define('LATEST_KNOWN_PHP_AST_VERSION', '1.0.10');
/**
...
function phan_output_ast_installation_instructions() : void
{
    ...
    if (\DIRECTORY_SEPARATOR === '\\') {
        if (\PHP_VERSION_ID < 70500 || !\preg_match('/[a-zA-Z]/', \PHP_VERSION)) {
            \fprintf(\STDERR, \PHP_EOL . "Windows users can download php-ast from https://windows.php.net/downloads/pecl/releases/ast/%s/php_ast-%s-%s-%s-%s-%s.zip" . \PHP_EOL, \LATEST_KNOWN_PHP_AST_VERSION, \LATEST_KNOWN_PHP_AST_VERSION, \PHP_MAJOR_VERSION . '.' . \PHP_MINOR_VERSION, \PHP_ZTS ? 'ts' : 'nts', 'vc15', \PHP_INT_SIZE == 4 ? 'x86' : 'x64');
...
if (\extension_loaded('ast')) {
    ...
    if (\PHP_VERSION_ID >= 80000 && \version_compare($ast_version, '1.0.10') < 0) {
        \fprintf(\STDERR, "ERROR: Phan 3.x requires php-ast 1.0.10+ to properly analyze ASTs for php 8.0+. php-ast %s and php %s is installed." . \PHP_EOL, $ast_version, \PHP_VERSION);
        ...
        $did_warn = \true;
        \fprintf(\STDERR, "WARNING: Phan 3.x requires php-ast 1.0.2+ to properly analyze ASTs for php 7.4+ (1.0.10+ is recommended). php-ast %s and php %s is installed." . \PHP_EOL, $ast_version, \PHP_VERSION);
        phan_output_ast_installation_instructions();
        ...
        \fprintf(\STDERR, "ERROR: Phan 3.x requires php-ast %s+ because it depends on AST version %d. php-ast '%s' is installed." . \PHP_EOL, Config::MINIMUM_AST_EXTENSION_VERSION, Config::AST_VERSION, $ast_version);
        phan_output_ast_installation_instructions();
        \fwrite(\STDERR, "Exiting without analyzing files." . \PHP_EOL);
        exit(1);
    }
...
foreach ([
    ...
    // autoloader is in this project (we're in src/Phan and want vendor/autoload.php)
    \dirname(__DIR__, 5) . '/vendor/autoload.php',
    // autoloader is in parent project (we're in vendor/phan/phan/src/Phan/Bootstrap.php and want autoload.php
    \dirname(__DIR__, 4) . '/autoload.php',
] as $file) {
    if (\file_exists($file)) {
        require_once $file;
        $found_autoloader = \true;
        break;
    }
}
unset($file);
...
\assert_options(\ASSERT_ACTIVE, \true);
\assert_options(\ASSERT_WARNING, \false);
\assert_options(\ASSERT_BAIL, \false);
// ASSERT_QUIET_EVAL has been removed starting with PHP 8
if (\defined('ASSERT_QUIET_EVAL')) {
    \assert_options(\ASSERT_QUIET_EVAL, \false);
    // @phan-suppress-current-line UnusedPluginSuppression, PhanTypeMismatchArgumentNullableInternal
}
\assert_options(\ASSERT_CALLBACK, '');
// Can't explicitly set ASSERT_CALLBACK to null?
...
\set_exception_handler(static function (\Throwable $throwable) : void {
    ...
    foreach ($throwable->getTrace() as $i => $frame) {
        $frame_details = \SdgScoped\Phan\Debug\Frame::frameToString($frame);
    ...
    if ($truncated) {
        \fwrite(\STDERR, "(Some long strings (usually JSON of AST Nodes) were truncated. To print more details for some stack frames of this uncaught exception," . "increase the Phan config setting debug_max_frame_length)" . \PHP_EOL);
    }
    exit(\EXIT_FAILURE);
});
/**
...
function phan_print_backtrace(bool $is_crash = \false, int $frames_to_skip = 2) : void
{
    ...
    */
    ...
    if (isset($frames[1])) {
        ...
        if ($truncated) {
            \fwrite(\STDERR, "(Some long strings (usually JSON of AST Nodes) were truncated. To print more details for some stack frames of this " . ($is_crash ? "crash" : "log") . ", " . "increase the Phan config setting debug_max_frame_length)" . \PHP_EOL);
        }
    }
}
/**
...
function phan_error_handler(int $errno, string $errstr, string $errfile, int $errline) : bool
{
    ...
    if ($errno === \E_DEPRECATED && \preg_match('/ast\\\\parse_.*Version.*is deprecated/i', $errstr)) {
        ...
        if (!$did_warn) {
            $did_warn = \true;
            if (!\getenv('PHAN_SUPPRESS_AST_DEPRECATION')) {
                CLI::printWarningToStderr(\sprintf("php-ast AST version %d used by Phan %s has been deprecated in php-ast %s. Check if a newer version of Phan is available." . \PHP_EOL, Config::AST_VERSION, CLI::PHAN_VERSION, (string) \phpversion('ast')));
                \fwrite(\STDERR, "(Set PHAN_SUPPRESS_AST_DEPRECATION=1 to suppress this message)" . \PHP_EOL);
            }
        }
        // Don't execute the PHP internal error handler
