...
use ast\Node;
use Closure;
use SdgScoped\Phan\Analysis\ConditionVisitor\HasTypeCondition;
use SdgScoped\Phan\Analysis\ConditionVisitor\NotHasTypeCondition;
use SdgScoped\Phan\AST\ASTReverter;
use SdgScoped\Phan\AST\ContextNode;
use SdgScoped\Phan\AST\UnionTypeVisitor;
use SdgScoped\Phan\AST\Visitor\KindVisitorImplementation;
...
class ConditionVisitor extends KindVisitorImplementation implements ConditionVisitorInterface
{
    ...
    protected function checkVariablesDefined(Node $node) : void
    {
        ...
        // is defined.
        UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node, \true);
    }
    /**
    ...
    private function checkArrayAccessDefined(Node $node) : void
    {
        ...
        /* $array_type = */
        UnionTypeVisitor::unionTypeFromNode($code_base, $context, $node->children['expr'], \true);
    }
    /**
     * @param Node $node
     * A node to parse
    ...
    public function visitUnaryOp(Node $node) : Context
    {
        ...
        if ($flags !== \ast\flags\UNARY_BOOL_NOT) {
            ...
            if ($expr_node instanceof Node) {
                if ($flags === \ast\flags\UNARY_SILENCE) {
        ...
        if ($expr_node instanceof Node) {
            ...
            $this->checkRedundantOrImpossibleTruthyCondition($expr_node, $this->context, null, \true);
        }
        return $this->context;
    }
    /**
    ...
    /** @internal */
    public const DEFAULTS_FOR_ACCESS_TYPE = [self::ACCESS_IS_OBJECT => 'object', self::ACCESS_ARRAY_KEY_EXISTS => 'non-empty-array|object', self::ACCESS_IS_SET => 'int|string|float|bool|non-empty-array|object|resource', self::ACCESS_DIM_SET => 'string|non-empty-array|object', self::ACCESS_STRING_DIM_SET => 'non-empty-array|object'];
    /**
    ...
    private function withSetVariable(string $var_name, Node $var_node, Node $ancestor_node) : Context
    {
        ...
        if (!$scope->hasVariableWithName($var_name)) {
            ...
            return $context->withScopeVariable(new Variable($context->withLineNumberStart($var_node->lineno ?? 0), $var_name, $new_type, 0));
        }
        if ($is_object) {
        ...
        if ($var_node !== $ancestor_node) {
            return $this->removeTypesNotSupportingAccessFromVariable($var_node, $context, $ancestor_node->kind === \ast\AST_PROP ? self::ACCESS_IS_OBJECT : self::ACCESS_DIM_SET);
        }
        return $this->removeNullFromVariable($var_node, $context, \true);
    }
    /**
    ...
    private function checkComplexIsset(Node $node) : Context
    {
        ...
        if (!$context->getScope()->hasVariableWithName($var_name)) {
            ...
            return $context->withScopeVariable(new Variable(
                ...
                $var_name,
                $new_type,
                // can be array or (unlikely) ArrayAccess
                0
            ));
        }
        if ($var_node === $node) {
            $context = $this->removeNullFromVariable($var_node, $context, \true);
        } else {
            ...
            $context = $this->removeTypesNotSupportingAccessFromVariable($var_node, $context, $access_kind);
        }
        ...
        if ($var_node_union_type->hasTopLevelArrayShapeTypeInstances()) {
            $new_union_type = $this->withSetArrayShapeTypes($var_node_union_type, $parent_node->children['dim'], $context, \true);
    ...
    private function withSetArrayShapeTypes(UnionType $union_type, $dim_node, Context $context, bool $non_nullable) : UnionType
    {
        ...
        if (!$dim_union_type) {
            ...
            return ArrayType::combineArrayShapeTypesWithField($union_type, $dim_value, $dim_union_type->nonNullableClone()->withIsPossiblyUndefined(\false));
        }
        return $union_type;
    }
    /**
    ...
    public function visitProp(Node $node) : Context
    {
        ...
        return $this->modifyPropertyOfThisSimple($node, function (UnionType $type) use($node) : UnionType {
    ...
    private function setInstanceofVariableType(Variable $variable, Node $class_node) : void
    {
        ...
        $type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $class_node);
        ...
        if (!$object_types->isEmpty()) {
            ...
            if ($class_node->kind !== \ast\AST_NAME && !$type->canCastToUnionType(StringType::instance(\false)->asPHPDocUnionType())) {
                Issue::maybeEmit($this->code_base, $this->context, Issue::TypeInvalidInstanceof, $this->context->getLineNumberStart(), ASTReverter::toShortString($class_node), (string) $type->asNonLiteralType());
            }
    ...
    private static function initTypeModifyingClosuresForVisitCall() : array
    {
        ...
        $is_a_callback = static function (CodeBase $code_base, Context $context, Variable $variable, array $args) use($object_callback) : void {
            ...
            try {
                ...
                throw new IssueException(Issue::fromType(Issue::TypeComparisonToInvalidClass)($context->getFile(), $context->getLineNumberStart(), [StringUtil::encodeValue($class_name)]));
            }
        ...
        $has_count_callback = static function (CodeBase $code_base, Context $context, Variable $variable, array $args) : void {
            ...
            // If we already have possible countable types, then keep those
            // (E.g. ?ArrayObject|false becomes ArrayObject)
            $variable->setUnionType($variable->getUnionType()->withStaticResolvedInContext($context)->countableTypesStrictCast($code_base)->nonFalseyClone());
        };
        ...
        return [
            ...
            'count' => $has_count_callback,
            // handle `if (count($x))` but not yet `if (count($x) > 0)`
            'interface_exists' => $class_exists_callback,
            // Currently, there's just class-string, not trait-string or interface-string.
            ...
            'is_iterable' => $iterable_callback,
            // TODO: Could keep basic array types and classes extending iterable
    ...
    public function visitCall(Node $node) : Context
    {
        ...
        if (\count($args) !== 1) {
            if (!(\count($args) === 2 && \in_array($function_name, ['is_a', 'class_exists', 'method_exists'], \true))) {
                if (Config::getValue('redundant_condition_detection')) {
                    $this->checkRedundantOrImpossibleTruthyCondition($node, $this->context, null, \false);
                }
                return $this->context;
    ...
    private function analyzeArrayKeyExists(array $args) : Context
    {
        ...
        return $this->updateVariableWithConditionalFilter($var_node, $this->context, static function (UnionType $_) : bool {
            return \true;
    ...
    private function analyzeDefined(array $args) : Context
    {
        ...
        $context->addScopeVariable(new Variable($context, self::CONSTANT_EXISTS_PREFIX . \ltrim($constant_name, '\\'), UnionType::empty(), 0));
        return $context;
    }
    /**
     * @param Node $node
    ...
    public function visitEmpty(Node $node) : Context
    {
        ...
        if (\in_array($var_node->kind, [\ast\AST_VAR, \ast\AST_PROP, \ast\AST_DIM], \true)) {
            // Don't emit notices for if (empty($x)) {}, etc. We already do that in RedundantConditionPlugin.
            return $this->removeTruthyFromVariable($var_node, $this->context, \true, \true);
        }
        $this->checkVariablesDefinedInIsset($var_node);
        return $this->context;
    }
    ...
    public static function updateToHaveType(CodeBase $code_base, Context $context, Node $expression, UnionType $type) : Context
    {
        ...
        return $cv->analyzeBinaryConditionPattern($expression, 0, new HasTypeCondition($type));
    }
    /**
    ...
    {
        $cv = new ConditionVisitor($code_base, $context);
        return $cv->analyzeBinaryConditionPattern($expression, 0, new NotHasTypeCondition($type));
    }
}
