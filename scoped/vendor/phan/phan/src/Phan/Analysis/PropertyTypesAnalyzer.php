                ...
                foreach ($outer_type->getReferencedClasses() as $type) {
                    ...
                    if ($type instanceof TemplateType) {
                        if ($property->isStatic()) {
                            Issue::maybeEmit($code_base, $property->getContext(), Issue::TemplateTypeStaticProperty, $property->getFileRef()->getLineNumberStart(), $property->asPropertyFQSENString());
                        }
                        continue;
                    }
                    ...
                    if ($code_base->hasClassWithFQSEN($type_fqsen)) {
                        if ($code_base->hasClassWithFQSEN($type_fqsen->withAlternateId(1))) {
                            UnionType::emitRedefinedClassReferenceWarning($code_base, $property->getContext(), $type_fqsen);
                        }
                    } else {
                        Issue::maybeEmitWithParameters($code_base, $property->getContext(), Issue::UndeclaredTypeProperty, $property->getFileRef()->getLineNumberStart(), [$property->asPropertyFQSENString(), (string) $outer_type], IssueFixSuggester::suggestSimilarClass($code_base, $property->getContext(), $type_fqsen, null, 'Did you mean', IssueFixSuggester::CLASS_SUGGEST_CLASSES_AND_TYPES));
                    }
                }
            }
        }
