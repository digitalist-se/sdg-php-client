...
use SdgScoped\Phan\Language\Element\ClassElement;
...
class ReferenceCountsAnalyzer
{
    ...
    public static function analyzeReferenceCounts(CodeBase $code_base) : void
    {
        ...
        $i = 0;
        // Functions
        self::analyzeGlobalElementListReferenceCounts($code_base, $code_base->getFunctionMap(), Issue::UnreferencedFunction, $total_count, $i);
        // Constants
        self::analyzeGlobalElementListReferenceCounts($code_base, $code_base->getGlobalConstantMap(), Issue::UnreferencedConstant, $total_count, $i);
        // Classes
        self::analyzeGlobalElementListReferenceCounts($code_base, $code_base->getUserDefinedClassMap(), Issue::UnreferencedClass, $total_count, $i);
        // Class Maps
        $elements_to_analyze = [];
        foreach ($code_base->getClassMapMap() as $class_map) {
            foreach (self::getElementsFromClassMapForDeferredAnalysis($code_base, $class_map, $total_count, $i) as $element) {
                $elements_to_analyze[] = $element;
            }
        }
        static $issue_types = [
            ClassConstant::class => Issue::UnreferencedPublicClassConstant,
            // This is overridden
            Method::class => Issue::UnreferencedPublicMethod,
            // This is overridden
            Property::class => Issue::UnreferencedPublicProperty,
        ];
     ...
     */
    private static function getElementsFromClassMapForDeferredAnalysis(CodeBase $code_base, ClassMap $class_map, int $total_count, int &$i) : \Generator
    {
        // Constants
        yield from self::getElementsFromElementListForDeferredAnalysis($code_base, $class_map->getClassConstantMap(), $total_count, $i);
        // Properties
        yield from self::getElementsFromElementListForDeferredAnalysis($code_base, $class_map->getPropertyMap(), $total_count, $i);
        // Methods
        yield from self::getElementsFromElementListForDeferredAnalysis($code_base, $class_map->getMethodMap(), $total_count, $i);
    }
    /**
     * @param CodeBase $code_base
     ...
     * @param int $i
     */
    private static function analyzeGlobalElementListReferenceCounts(CodeBase $code_base, iterable $element_list, string $issue_type, int $total_count, int &$i) : void
    {
     ...
     * @phan-return \Generator<ClassElement>
     */
    private static function getElementsFromElementListForDeferredAnalysis(CodeBase $code_base, iterable $element_list, int $total_count, int &$i) : \Generator
    {
        foreach ($element_list as $element) {
            CLI::progress('dead code', ++$i / $total_count, $element);
            // Don't worry about internal elements
     ...
     */
    private static function analyzeElementReferenceCounts(CodeBase $code_base, AddressableElement $element, string $issue_type) : void
    {
        /*
        print "digraph G {\n";
        ...
        */
        ...
        Issue::maybeEmit($code_base, $element->getContext(), $issue_type, $element->getFileRef()->getLineNumberStart(), $element->getRepresentationForIssue());
    }
    ...
    private static function maybeWarnWriteOnlyProperty(CodeBase $code_base, Property $property) : void
    {
        ...
        Issue::maybeEmit($code_base, $property->getContext(), $issue_type, $property->getFileRef()->getLineNumberStart(), $property->getRepresentationForIssue());
    }
     ...
     */
    public static function findAlternateReferencedElementDeclaration(CodeBase $code_base, AddressableElement $element) : ?AddressableElement
    {
        ...
        if ($old_fqsen instanceof FullyQualifiedGlobalStructuralElement) {
            ...
            if ($fqsen === $old_fqsen) {
                return null;
                // $old_fqsen was not an alternative
            }
