...
class AssignOperatorFlagVisitor extends FlagVisitorImplementation
{
    ...
    public function visit(Node $node) : UnionType
    {
        ...
        if ($left->isExclusivelyArray() || $right->isExclusivelyArray()) {
            Issue::maybeEmit($this->code_base, $this->context, Issue::TypeArrayOperator, $node->lineno ?? 0, PostOrderAnalysisVisitor::NAME_FOR_BINARY_OP[$node->flags], $left, $right);
    ...
    public function visitBinaryCoalesce(Node $node) : UnionType
    {
        ...
        return (new BinaryOperatorFlagVisitor($this->code_base, $this->context, \true))->visitBinaryCoalesce($new_node);
    }
    /**
    ...
    public function visitBinaryBitwiseAnd(Node $node) : UnionType
    {
        ...
        if ($left->hasNonNullIntType() && $right->hasNonNullIntType()) {
            return IntType::instance(\false)->asPHPDocUnionType();
        } elseif ($left->hasNonNullStringType() && $right->hasNonNullStringType()) {
            // $x = 'a'; $x &= 'c';
    ...
    public function visitBinaryAdd(Node $node) : UnionType
    {
        ...
        if (($left->isNonNullIntType() || $left->isType(FloatType::instance(\false))) && ($right->isNonNullIntType() || $right->isType(FloatType::instance(\false)))) {
        ...
        if ($left_is_array && !$right->canCastToUnionType(ArrayType::instance(\false)->asPHPDocUnionType())) {
            ...
            return $unknown_type;
        } elseif ($right_is_array && !$left->canCastToUnionType(ArrayType::instance(\false)->asPHPDocUnionType())) {
