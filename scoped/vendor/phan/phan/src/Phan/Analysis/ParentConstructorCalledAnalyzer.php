...
class ParentConstructorCalledAnalyzer
{
     ...
     */
    public static function analyzeParentConstructorCalled(CodeBase $code_base, Clazz $clazz) : void
    {
        // Only look at classes configured to require a call
        // to its parent constructor
        if (!\in_array($clazz->getName(), Config::getValue('parent_constructor_required'), \true)) {
            return;
        }
        ...
        if (!$parent_clazz->isAbstract() && !$clazz->isParentConstructorCalled()) {
