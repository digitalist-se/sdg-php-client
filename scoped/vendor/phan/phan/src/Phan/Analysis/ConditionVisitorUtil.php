...
use Closure;
use Exception;
use SdgScoped\Phan\Analysis\ConditionVisitor\BinaryCondition;
use SdgScoped\Phan\Analysis\ConditionVisitor\ComparisonCondition;
use SdgScoped\Phan\Analysis\ConditionVisitor\EqualsCondition;
use SdgScoped\Phan\Analysis\ConditionVisitor\IdenticalCondition;
use SdgScoped\Phan\Analysis\ConditionVisitor\NotEqualsCondition;
use SdgScoped\Phan\Analysis\ConditionVisitor\NotIdenticalCondition;
...
trait ConditionVisitorUtil
{
     ...
     */
    protected final function removeTruthyFromVariable(Node $var_node, Context $context, bool $suppress_issues, bool $check_empty) : Context
    {
            ...
            function (UnionType $type) use($var_node, $context, $suppress_issues) : bool {
                ...
                if (!$suppress_issues) {
                    if (Config::getValue('redundant_condition_detection') && $type->hasRealTypeSet()) {
                        ...
                        $var_node->did_check_redundant_condition = \true;
                        ...
                        if ($contains_truthy) {
                            if (!$type->getRealUnionType()->containsFalsey()) {
                                RedundantCondition::emitInstance($var_node, $this->code_base, $context, Issue::ImpossibleCondition, [ASTReverter::toShortString($var_node), $type->getRealUnionType(), 'falsey'], static function (UnionType $type) : bool {
                                    return !$type->containsFalsey();
                                });
                            }
                        } else {
                            if (!$type->getRealUnionType()->containsTruthy()) {
                                RedundantCondition::emitInstance($var_node, $this->code_base, $context, Issue::RedundantCondition, [ASTReverter::toShortString($var_node), $type->getRealUnionType(), 'falsey'], static function (UnionType $type) : bool {
    ...
    protected final function getFalseyTypesFallback(Node $var_node, Context $context) : UnionType
    {
    ...
    protected final function getTypesFallback(Node $var_node, Context $context) : ?UnionType
    {
    ...
    {
        return $this->updateVariableWithConditionalFilter($var_node, $context, function (UnionType $type) use($context, $var_node, $suppress_issues) : bool {
            if (!$suppress_issues) {
                if (Config::getValue('redundant_condition_detection') && $type->hasRealTypeSet()) {
    ...
    private function checkErrorProneTruthyCast(Node $node, Context $context, UnionType $union_type) : void
    {
        ...
        $node->did_check_error_prone_truthy = \true;
        ...
        if ($has_truthy && $has_falsey && $has_array_or_object) {
            Issue::maybeEmit($this->code_base, $context, Issue::SuspiciousTruthyCondition, $node->lineno, ASTReverter::toShortString($node), $union_type);
        }
        if ($has_string) {
            Issue::maybeEmit($this->code_base, $context, Issue::SuspiciousTruthyString, $node->lineno, ASTReverter::toShortString($node), $union_type);
        }
    }
    /**
    ...
    public function checkRedundantOrImpossibleTruthyCondition($node, Context $context, ?UnionType $type, bool $is_negated) : void
    {
        if ($node instanceof Node) {
            ...
            $node->did_check_redundant_condition = \true;
        } elseif ($is_negated) {
            return;
        }
        if (!$type) {
        ...
        // for the same expression
        if (!$type->containsTruthy()) {
            RedundantCondition::emitInstance($node, $this->code_base, $context, $is_negated ? Issue::RedundantCondition : Issue::ImpossibleCondition, [ASTReverter::toShortString($node), $type->getRealUnionType(), $is_negated ? 'falsey' : 'truthy'], static function (UnionType $type) : bool {
                return !$type->containsTruthy();
            });
        } elseif (!$type->containsFalsey()) {
            RedundantCondition::emitInstance($node, $this->code_base, $context, $this->chooseIssueForUnconditionallyTrue($is_negated, $node), [ASTReverter::toShortString($node), $type->getRealUnionType(), $is_negated ? 'falsey' : 'truthy'], static function (UnionType $type) : bool {
                return !$type->containsFalsey();
            });
        }
    }
    ...
    {
        return $this->updateVariableWithConditionalFilter($var_node, $context, static function (UnionType $type) : bool {
    ...
    {
        $type = $type->asMappedListUnionType(
            /** @return list<Type> */
            static function (Type $type) use($access_type) : array {
                ...
                if ($type instanceof ScalarType) {
                    if ($type instanceof StringType) {
                        if (\in_array($access_type, [ConditionVisitor::ACCESS_IS_OBJECT, ConditionVisitor::ACCESS_ARRAY_KEY_EXISTS, ConditionVisitor::ACCESS_STRING_DIM_SET], \true)) {
                            return [];
                        }
     ...
     */
    protected final function removeTypesNotSupportingAccessFromVariable(Node $var_node, Context $context, int $access_type) : Context
    {
        return $this->updateVariableWithConditionalFilter($var_node, $context, static function (UnionType $type) use($access_type) : bool {
     ...
     */
    protected final function removeLiteralScalarFromVariable(Node $var_node, Context $context, $value, bool $strict_equality) : Context
    {
        ...
        return $this->updateVariableWithConditionalFilter($var_node, $context, static function (UnionType $union_type) use($cb) : bool {
     ...
     */
    protected final function updateVariableWithConditionalFilter(Node $var_node, Context $context, Closure $should_filter_cb, Closure $filter_union_type_cb, bool $suppress_issues, bool $check_empty) : Context
    {
        try {
            ...
            $variable->setUnionType($filter_union_type_cb($union_type));
     ...
     */
    protected final function updateDimExpressionWithConditionalFilter(Node $node, Context $context, Closure $should_filter_cb, Closure $filter_union_type_cb, bool $suppress_issues, bool $check_empty) : Context
    {
        ...
        try {
            ...
            $old_field_type = (new UnionTypeVisitor($this->code_base, $context))->visitDim($node, \true);
    ...
    {
        return $node instanceof Node && $node->kind === \ast\AST_VAR && $node->children['name'] === 'this';
    }
    /**
     ...
     */
    protected final function updatePropertyExpressionWithConditionalFilter(Node $node, Context $context, Closure $should_filter_cb, Closure $filter_union_type_cb, bool $suppress_issues) : Context
    {
        ...
        return $this->modifyPropertyOfThisSimple($node, static function (UnionType $type) use($should_filter_cb, $filter_union_type_cb) : UnionType {
    ...
    protected final function updateVariableWithNewType(Node $var_node, Context $context, UnionType $new_union_type, bool $suppress_issues, bool $is_weak_type_assertion) : Context
    {
     ...
     */
    public final function updateVariableToBeIdentical(Node $var_node, $expr, Context $context = null) : Context
    {
        ...
        return $this->updateVariableWithNewType($var_node, $context, $expr_type, \true, \false);
    }
    /**
     * @param Node $var_node
     * @param Node|int|float|string $expr
     ...
     */
    public final function updateVariableToBeEqual(Node $var_node, $expr, Context $context = null) : Context
    {
        $context = $context ?? $this->context;
        try {
            $expr_type = UnionTypeVisitor::unionTypeFromLiteralOrConstant($this->code_base, $context, $expr);
        ...
        return $this->updateVariableWithNewType($var_node, $context, $expr_type, \true, \true);
    }
    /**
     * @param Node $var_node
     * @param Node|int|float|string $expr
     ...
     */
    public final function updateVariableToBeCompared(Node $var_node, $expr, int $flags) : Context
    {
     ...
     * @return Context - Context after inferring type from an expression such as `if ($x !== 'literal')`
     */
    public final function updateVariableToBeNotIdentical(Node $var_node, $expr, Context $context = null) : Context
    {
        $context = $context ?? $this->context;
        try {
            if ($expr instanceof Node) {
                ...
                if (\is_int($value) || \is_string($value)) {
                    return $this->removeLiteralScalarFromVariable($var_node, $context, $value, \true);
                }
                if ($value === \false) {
                ...
                return $this->removeLiteralScalarFromVariable($var_node, $context, $expr, \true);
            }
        } catch (\Exception $_) {
            // Swallow it (E.g. IssueException for undefined variable)
        }
     ...
     */
    public final function updateVariableToBeNotEqual(Node $var_node, $expr, Context $context = null) : Context
    {
        $context = $context ?? $this->context;
        $var_name = $var_node->children['name'] ?? null;
    ...
    {
        return $this->analyzeBinaryConditionPattern($left, $right, new IdenticalCondition());
    }
    /**
     * @param Node|int|float|string $left
     * @param Node|int|float|string $right
    ...
    {
        return $this->analyzeBinaryConditionPattern($left, $right, new EqualsCondition());
    }
    /**
     * @param Node|int|float|string $left
     * @param Node|int|float|string $right
    ...
    {
        return $this->analyzeBinaryConditionPattern($left, $right, new NotIdenticalCondition());
    }
    /**
     * @param Node|int|float|string $left
     * @param Node|int|float|string $right
    ...
    private function analyzeBinaryConditionSide(Node $var_node, $expr_node, BinaryCondition $condition) : ?Context
    {
        ...
        $tmp = $var_node;
        while (\in_array($kind, [\ast\AST_ASSIGN, \ast\AST_ASSIGN_OP, \ast\AST_ASSIGN_REF], \true)) {
        ...
        if (($tmp === $var_node || $tmp->kind !== \ast\AST_ARRAY) && ParseVisitor::isConstExpr($expr_node)) {
    ...
    public function analyzeClassAssertion($object_node, $expr_node) : ?Context
    {
        ...
        if (!is_string($expr_value)) {
            ...
            if (!$expr_type->canCastToUnionType(UnionType::fromFullyQualifiedPHPDocString('string|false'))) {
                Issue::maybeEmit($this->code_base, $this->context, Issue::TypeComparisonToInvalidClassType, $this->context->getLineNumberStart(), $expr_type, 'false|string');
            }
        ...
        try {
            ...
            Issue::maybeEmit($this->code_base, $this->context, Issue::TypeComparisonToInvalidClass, $this->context->getLineNumberStart(), StringUtil::encodeValue($expr_value));
            return null;
        }
    ...
    {
        return $this->analyzeBinaryConditionPattern($left, $right, new NotEqualsCondition());
    }
    /**
     * @param Node|int|float|string $left
     * @param Node|int|float|string $right
    ...
    {
        return $this->analyzeBinaryConditionPattern($left, $right, new ComparisonCondition($flags));
    }
    /**
     ...
     */
    public final function getVariableFromScope(Node $var_node, Context $context) : ?Variable
    {
        if ($var_node->kind !== \ast\AST_VAR) {
            return null;
        }
        ...
        if ($var_name_node instanceof Node) {
            ...
            $name_node_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $context, $var_name_node, \true);
        ...
        if (!$context->getScope()->hasVariableWithName($variable_name)) {
            ...
            if ($new_type) {
                $variable = new Variable($context->withLineNumberStart($var_node->lineno), $variable_name, $new_type, 0);
            ...
            if (!($var_node->flags & PhanAnnotationAdder::FLAG_IGNORE_UNDEF)) {
                if ($is_in_global_scope) {
                    if (!Config::getValue('ignore_undeclared_variables_in_global_scope')) {
                        Issue::maybeEmitWithParameters($this->code_base, $context, Variable::chooseIssueForUndeclaredVariable($context, $variable_name), $var_node->lineno, [$variable_name], IssueFixSuggester::suggestVariableTypoFix($this->code_base, $context, $variable_name));
                    }
                } else {
                    throw new IssueException(Issue::fromType(Variable::chooseIssueForUndeclaredVariable($context, $variable_name))($context->getFile(), $var_node->lineno, [$variable_name], IssueFixSuggester::suggestVariableTypoFix($this->code_base, $context, $variable_name)));
                }
            }
            $variable = new Variable($context, $variable_name, UnionType::empty(), 0);
            $context->addScopeVariable($variable);
            return $variable;
        }
        return $context->getScope()->getVariableByName($variable_name);
    }
    /**
     ...
     */
    public static final function getFunctionName(Node $node) : ?string
    {
