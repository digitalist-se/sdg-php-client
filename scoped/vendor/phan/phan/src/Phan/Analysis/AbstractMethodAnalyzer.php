...
class AbstractMethodAnalyzer
{
     ...
     */
    public static function analyzeAbstractMethodsAreImplemented(CodeBase $code_base, Clazz $class) : void
    {
        ...
        foreach ($class->getMethodMap($code_base) as $method) {
            if ($method->isAbstract()) {
                if ($method->isPHPInternal()) {
                    Issue::maybeEmit($code_base, $class->getContext(), Issue::ClassContainsAbstractMethodInternal, $class->getFileRef()->getLineNumberStart(), (string) $class->getFQSEN(), self::toRealSignature($method));
                } else {
                    Issue::maybeEmit($code_base, $class->getContext(), Issue::ClassContainsAbstractMethod, $class->getFileRef()->getLineNumberStart(), (string) $class->getFQSEN(), self::toRealSignature($method), $method->getFileRef()->getFile(), $method->getFileRef()->getLineNumberStart());
                }
            }
        }
    }
    private static function toRealSignature(Method $method) : string
    {
        ...
        $result = \sprintf("%s::%s%s(%s)", (string) $fqsen->getFullyQualifiedClassName(), $method->returnsRef() ? '&' : '', $fqsen->getName(), $method->getRealParameterStubText());
