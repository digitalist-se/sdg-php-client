...
use SdgScoped\Phan\AST\Visitor\FlagVisitorImplementation;
...
final class BinaryOperatorFlagVisitor extends FlagVisitorImplementation
{
     ...
     */
    public function __construct(CodeBase $code_base, Context $context, bool $should_catch_issue_exception = \false)
    {
    ...
    {
        $left = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['left'], $this->should_catch_issue_exception);
        $right = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['right'], $this->should_catch_issue_exception);
        static $int_or_float = null;
        if ($left->isExclusivelyArray() || $right->isExclusivelyArray()) {
            return UnionType::empty();
        } elseif ($left->hasType(FloatType::instance(\false)) || $right->hasType(FloatType::instance(\false))) {
            if ($left->hasTypeMatchingCallback(static function (Type $type) : bool {
                return !$type instanceof FloatType;
            }) && $right->hasTypeMatchingCallback(static function (Type $type) : bool {
                return !$type instanceof FloatType;
            })) {
        ...
        } elseif ($left->hasNonNullIntType() && $right->hasNonNullIntType()) {
    ...
    private function analyzeBinaryBitwiseCommon(Node $node) : UnionType
    {
        ...
        if ($left->hasNonNullIntType()) {
            ...
            if ($right->hasNonNullStringType()) {
                $this->emitIssue(Issue::TypeMismatchBitwiseBinaryOperands, $node->lineno ?? 0, PostOrderAnalysisVisitor::NAME_FOR_BINARY_OP[$node->flags], $left, $right);
            }
        ...
        if (!$left->hasValidBitwiseOperand() || !$right->hasValidBitwiseOperand()) {
            $this->emitIssue(Issue::TypeInvalidBitwiseBinaryOperator, $node->lineno ?? 0, PostOrderAnalysisVisitor::NAME_FOR_BINARY_OP[$node->flags], $left, $right);
        }
     ...
     * @internal
     */
    public static function computeIntOrFloatOperationResult(Node $node, UnionType $left, UnionType $right) : UnionType
    {
            ...
            if ($right_value !== null) {
                ...
                switch ($node->flags) {
                    case \ast\flags\BINARY_BITWISE_OR:
                        return $make_literal_union_type(LiteralIntType::instanceForValue($left_value | $right_value, \false), $real_int);
                    ...
                    case \ast\flags\BINARY_MUL:
                        ...
                        return $make_literal_union_type(is_int($value) ? LiteralIntType::instanceForValue($value, \false) : LiteralFloatType::instanceForValue($value, \false), $real_int_or_float);
                    ...
                    case \ast\flags\BINARY_MOD:
                        ...
                        $value = $left_value % $right_value;
                        return $make_literal_union_type(LiteralIntType::instanceForValue($value, \false), $real_int);
        ...
        $is_binary_op = \in_array($node->flags, [\ast\flags\BINARY_BITWISE_XOR, \ast\flags\BINARY_BITWISE_AND, \ast\flags\BINARY_BITWISE_OR], \true);
        if ($is_binary_op) {
            return UnionType::fromFullyQualifiedPHPDocAndRealString('int', 'int|string');
        }
        ...
        return UnionType::fromFullyQualifiedPHPDocAndRealString($main_type, 'int|float');
    }
    /**
     ...
     */
    protected function emitIssue(string $issue_type, int $lineno, ...$parameters) : void
    {
    ...
    public function visitBinaryConcat(Node $node) : UnionType
    {
        ...
        $left_value = $left_node instanceof Node ? UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $left_node, $this->should_catch_issue_exception)->asSingleScalarValueOrNullOrSelf() : $left_node;
        if (\is_object($left_value)) {
            return StringType::instance(\false)->asRealUnionType();
        }
        $right_node = $node->children['right'];
        $right_value = $right_node instanceof Node ? UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $right_node, $this->should_catch_issue_exception)->asSingleScalarValueOrNullOrSelf() : $right_node;
    ...
    private function visitBinaryOpCommon(Node $node) : UnionType
    {
        ...
        $left_can_cast_to_array = $left->canCastToUnionType(ArrayType::instance(\false)->asPHPDocUnionType());
        $right_can_cast_to_array = $right->canCastToUnionType(ArrayType::instance(\false)->asPHPDocUnionType());
        if ($left_is_array_like && !$right->hasArrayLike() && !$right_can_cast_to_array && !$right->isEmpty() && !$right->containsNullable() && !$left->hasAnyType($right->getTypeSet())) {
            $this->emitIssue(Issue::TypeComparisonFromArray, $node->lineno ?? 0, (string) $right->asNonLiteralType());
        } elseif ($right_is_array_like && !$left->hasArrayLike() && !$left_can_cast_to_array && !$left->isEmpty() && !$left->containsNullable() && !$right->hasAnyType($left->getTypeSet())) {
            $this->emitIssue(Issue::TypeComparisonToArray, $node->lineno ?? 0, (string) $left->asNonLiteralType());
        }
        return BoolType::instance(\false)->asRealUnionType();
    }
    /**
    ...
    public function visitBinaryAdd(Node $node) : UnionType
    {
        ...
        $left = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $node->children['left'], $this->should_catch_issue_exception);
        $right = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $node->children['right'], $this->should_catch_issue_exception);
        ...
        }, $left, $right, Issue::TypeInvalidLeftOperandOfAdd, Issue::TypeInvalidRightOperandOfAdd);
        ...
        $left_is_array = !$left->genericArrayElementTypes()->isEmpty() && $left->nonArrayTypes()->isEmpty() || $left->isType($array_type);
        $right_is_array = !$right->genericArrayElementTypes()->isEmpty() && $right->nonArrayTypes()->isEmpty() || $right->isType($array_type);
        if ($left_is_array || $right_is_array) {
            if ($left_is_array && $right_is_array) {
                return ArrayType::combineArrayTypesOverriding($left, $right, \false);
            }
            if ($left_is_array && !$right->canCastToUnionType(ArrayType::instance(\false)->asPHPDocUnionType())) {
                $this->emitIssue(Issue::TypeInvalidRightOperand, $node->lineno ?? 0);
                return $probably_unknown_type;
            } elseif ($right_is_array && !$left->canCastToUnionType($array_type->asPHPDocUnionType())) {
                $this->emitIssue(Issue::TypeInvalidLeftOperand, $node->lineno ?? 0);
                return $probably_unknown_type;
            }
     ...
     * `+` or `+=` operators.
     */
    public static function checkInvalidArrayShapeCombination(CodeBase $code_base, Context $context, Node $node, UnionType $left, UnionType $right) : void
    {
        ...
        if ($common_left_fields && !\array_diff_key($possible_right_fields, $common_left_fields)) {
            Issue::maybeEmit($code_base, $context, Issue::UselessBinaryAddRight, $node->lineno, $left, $right, ASTReverter::toShortString($node));
            return;
        }
        ...
        if ($common_left_fields === \array_values($common_left_fields) && $possible_right_fields === \array_values($possible_right_fields)) {
            ...
            Issue::maybeEmit($code_base, $context, Issue::SuspiciousBinaryAddLists, $node->lineno, $left, $right, ASTReverter::toShortString($node));
        }
    }
    /**
    ...
    {
        $left_node = $node->children['left'];
        $left_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $left_node, $this->should_catch_issue_exception);
        ...
        $right_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $right_node, $this->should_catch_issue_exception);
        if ($left_type->isEmpty()) {
            if ($right_type->isEmpty()) {
                ...
                return MixedType::instance(\true)->asPHPDocUnionType();
            }
        }
