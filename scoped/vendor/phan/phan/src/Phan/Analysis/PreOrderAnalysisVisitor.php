...
use SdgScoped\ast;
use ast\Node;
use SdgScoped\Phan\AST\ArrowFunc;
...
use SdgScoped\Phan\Language\Scope\ClosureScope;
...
class PreOrderAnalysisVisitor extends ScopeVisitor
{
    ...
    */
    ...
    public function visitClass(Node $node) : Context
    {
        ...
        } while ($this->context->getProjectRelativePath() != $clazz->getFileRef()->getProjectRelativePath() || $node->children['__declId'] != $clazz->getDeclId() || $this->context->getLineNumberStart() != $clazz->getFileRef()->getLineNumberStart());
        return $clazz->getContext()->withScope($clazz->getInternalScope())->withoutLoops();
    }
    /**
    ...
    public function visitMethod(Node $node) : Context
    {
        ...
        if (!$clazz->hasMethodWithName($code_base, $method_name, \true)) {
            throw new CodeBaseException(null, "Can't find method {$clazz->getFQSEN()}::{$method_name}() - aborting");
        }
        $method = $clazz->getMethodByName($code_base, $method_name);
        ...
        $context = $this->context->withScope(clone $method->getInternalScope());
    ...
    private function addDefaultPropertiesOfThisToContext(Clazz $class, Context $context) : void
    {
        ...
        $variable = new Variable($context, Context::VAR_NAME_THIS_PROPERTIES, $override_type->asPHPDocUnionType(), 0);
    ...
    public function visitFuncDecl(Node $node) : Context
    {
        ...
        // @phan-suppress-next-line PhanThrowTypeMismatchForCall
        $canonical_function = (new ContextNode($code_base, $original_context, $node))->getFunction($function_name, \true);
        ...
        foreach ($canonical_function->alternateGenerator($code_base) as $alternate_function) {
            if ($alternate_function->getFileRef()->getProjectRelativePath() === $original_context->getProjectRelativePath()) {
        ...
        // Fix #2504 - add flags to ensure that DimOffset warnings aren't emitted inside closures
        Analyzable::ensureDidAnnotate($node);
        $context = $original_context->withScope(clone $function->getInternalScope())->withoutLoops();
    ...
    private static function getOverrideClassFQSEN(CodeBase $code_base, Func $func) : ?FullyQualifiedClassName
    {
        ...
        if ($closure_scope instanceof ClosureScope) {
            ...
            if (!$code_base->hasClassWithFQSEN($class_fqsen)) {
                ...
                Issue::maybeEmit($code_base, $func_context, Issue::UndeclaredClosureScope, $func_context->getLineNumberStart(), (string) $class_fqsen);
     ...
     */
    private static function addThisVariableToInternalScope(CodeBase $code_base, Context $context, Func $func) : void
    {
        ...
        if ($override_this_fqsen !== null) {
            if ($context->getScope()->hasVariableWithName('this') || !$context->isInClassScope()) {
                ...
                $func->getInternalScope()->addVariable(new Variable($context, 'this', $override_this_fqsen->asRealUnionType(), 0));
            }
            return;
        }
    ...
    public function visitClosure(Node $node) : Context
    {
        ...
        $closure_fqsen = FullyQualifiedFunctionName::fromClosureInContext($context->withLineNumberStart($node->lineno), $node);
        ...
        if (($node->children['uses']->kind ?? null) === \ast\AST_CLOSURE_USES) {
            foreach ($node->children['uses']->children ?? [] as $use) {
                if (!$use instanceof Node || $use->kind !== \ast\AST_CLOSURE_VAR) {
                    $this->emitIssue(Issue::VariableUseClause, $node->lineno, ASTReverter::toShortString($use));
                    continue;
                }
                $variable_name = (new ContextNode($code_base, $context, $use->children['name']))->getVariableName();
                ...
                if (!$context->getScope()->hasVariableWithName($variable_name)) {
                    ...
                    if (!($use->flags & \ast\flags\CLOSURE_USE_REF)) {
                        Issue::maybeEmitWithParameters($this->code_base, $context, Issue::UndeclaredVariable, $use->lineno, [$variable_name], IssueFixSuggester::suggestVariableTypoFix($this->code_base, $context, $variable_name));
    ...
    private function setReturnTypeOfGenerator(FunctionInterface $func, Node $node) : void
    {
        ...
        $func->setHasReturn(\true);
        // Returns \Generator, technically
        $func->setHasYield(\true);
        if ($func->getUnionType()->isEmpty()) {
            $func->setIsReturnTypeUndefined(\true);
        ...
        if (!$func->isReturnTypeUndefined()) {
            ...
            try {
                $func_return_type_can_cast = $func_return_type->canCastToExpandedUnionType(Type::fromNamespaceAndName('\\', 'Generator', \false)->asPHPDocUnionType(), $this->code_base);
            ...
            if (!$func_return_type_can_cast) {
                ...
                $this->emitIssue(Issue::TypeMismatchReturn, $node->lineno, '(a Generator due to existence of yield statements)', '\\Generator', $func->getNameForIssue(), (string) $func_return_type);
            }
        }
    }
    /**
    ...
    public function visitCatch(Node $node) : Context
    {
        ...
        if (!isset($node->children['var'])) {
            $this->emitIssue(Issue::CompatibleNonCapturingCatch, $node->lineno, ASTReverter::toShortString($node->children['class']));
        }
        try {
            ...
            if (Config::get_closest_minimum_target_php_version_id() < 70100 && \count($class_list) > 1) {
                $this->emitIssue(Issue::CompatibleMultiExceptionCatchPHP70, $node->lineno);
            }
            foreach ($class_list as $class) {
                $class->addReference($this->context);
            }
        } catch (CodeBaseException $exception) {
            Issue::maybeEmitWithParameters($this->code_base, $this->context, Issue::UndeclaredClassCatch, $node->lineno, [(string) $exception->getFQSEN()], IssueFixSuggester::suggestSimilarClassForGenericFQSEN($this->code_base, $this->context, $exception->getFQSEN()));
        }
        ...
        $variable_name = (new ContextNode($this->code_base, $this->context, $var_node))->getVariableName();
        if ($variable_name !== '') {
            $variable = Variable::fromNodeInContext($var_node, $this->context, $this->code_base, \false);
    ...
    public function visitIfElem(Node $node) : Context
    {
        ...
        // list.
        return (new ConditionVisitor($this->code_base, $this->context))->__invoke($cond);
    }
