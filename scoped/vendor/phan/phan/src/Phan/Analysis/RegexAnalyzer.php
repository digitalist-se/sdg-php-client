...
use SdgScoped\Phan\Library\RegexKeyExtractor;
/**
...
class RegexAnalyzer
{
     ...
     */
    public static function getPregMatchUnionType(CodeBase $code_base, Context $context, array $argument_list) : UnionType
    {
     ...
     * @param list<Node|string|float|int> $argument_list
     */
    public static function getPregMatchAllUnionType(CodeBase $code_base, Context $context, array $argument_list) : UnionType
    {
        if (\count($argument_list) > 3) {
            $offset_flags_node = $argument_list[3];
            $bit = (new ContextNode($code_base, $context, $offset_flags_node))->getEquivalentPHPScalarValue();
     ...
     */
    private static function makeArrayShape(array $regex_group_keys, UnionType $type) : UnionType
    {
