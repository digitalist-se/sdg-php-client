...
use SdgScoped\Phan\Language\Element\Comment\Parameter as CommentParameter;
use SdgScoped\Phan\Language\Element\Flags;
...
class ParameterTypesAnalyzer
{
     ...
     */
    public static function analyzeParameterTypes(CodeBase $code_base, FunctionInterface $method) : void
    {
        try {
     ...
     */
    private static function analyzeParameterTypesInner(CodeBase $code_base, FunctionInterface $method) : void
    {
            ...
            if ($parameter->getFlags() & Parameter::PARAM_MODIFIER_VISIBILITY_FLAGS) {
                if ($method instanceof Method && strcasecmp($method->getName(), '__construct') === 0) {
                    if (Config::get_closest_minimum_target_php_version_id() < 80000) {
                        Issue::maybeEmit($code_base, $parameter->createContext($method), Issue::CompatibleConstructorPropertyPromotion, $parameter->getFileRef()->getLineNumberStart(), $parameter, $method->getRepresentationForIssue(\true));
                    }
                } else {
                    // emit an InvalidNode warning for non-constructors (closures, global functions, other methods)
                    Issue::maybeEmit($code_base, $parameter->createContext($method), Issue::InvalidNode, $parameter->getFileRef()->getLineNumberStart(), "Cannot use visibility modifier on parameter {$parameter} of non-constructor " . $method->getRepresentationForIssue(\true));
                }
            }
            $union_type = $parameter->getUnionType();
            if ($parameter->isOptional()) {
                $is_optional_seen = \true;
            } else {
                if ($is_optional_seen) {
                    Issue::maybeEmit($code_base, $method->getContext(), Issue::ParamReqAfterOpt, $parameter->getFileRef()->getLineNumberStart(), '(' . $parameter->toStubString() . ')', '(' . $method->getParameterList()[$i - 1]->toStubString() . ')');
                }
            }
                ...
                if ($type instanceof TemplateType) {
                    if ($method instanceof Method) {
                        if ($method->isStatic() && !$method->declaresTemplateTypeInComment($type)) {
                            Issue::maybeEmit($code_base, $method->getContext(), Issue::TemplateTypeStaticMethod, $parameter->getFileRef()->getLineNumberStart(), (string) $method->getFQSEN());
                        }
                    }
                } else {
        ...
        foreach ($method->getRealParameterList() as $parameter) {
            if ($parameter->hasDefaultValue()) {
                ...
                if ($default_node instanceof Node && !$parameter->getUnionType()->containsNullableOrIsEmpty() && $parameter->getDefaultValueType()->isNull()) {
                    // @phan-suppress-next-next-line PhanPartialTypeMismatchArgumentInternal
                    if (!($default_node->kind === \ast\AST_CONST && \strtolower($default_node->children['name']->children['name'] ?? '') === 'null')) {
                        Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleDefaultEqualsNull, $default_node->lineno, ASTReverter::toShortString($default_node), $parameter->getUnionType() . ' $' . $parameter->getName());
                    }
                }
            }
            $union_type = $parameter->getUnionType();
            foreach ($union_type->getTypeSet() as $type) {
                ...
                if ($class->isTrait()) {
                    Issue::maybeEmit($code_base, $method->getContext(), Issue::TypeInvalidTraitParam, $parameter->getFileRef()->getLineNumberStart(), $method->getNameForIssue(), $parameter->getName(), $type_fqsen->__toString());
                }
            }
        }
        if ($method instanceof Method) {
                ...
                if ($php70_checks) {
                    if ($type->isNullableLabeled()) {
                        if ($real_parameter->isUsingNullableSyntax()) {
                            Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleNullableTypePHP70, $real_parameter->getFileRef()->getLineNumberStart(), (string) $type);
                        }
                    }
                    if ($type_class === IterableType::class) {
                        Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleIterableTypePHP70, $real_parameter->getFileRef()->getLineNumberStart(), (string) $type);
                        continue;
                    }
                    if ($minimum_target_php_version < 70000 && $type instanceof ScalarType) {
                        Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleScalarTypePHP56, $real_parameter->getFileRef()->getLineNumberStart(), (string) $type);
                    }
                }
                if ($type_class === ObjectType::class) {
                    if ($minimum_target_php_version < 70200) {
                        Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleObjectTypePHP71, $real_parameter->getFileRef()->getLineNumberStart(), (string) $type);
                    }
                } elseif ($type_class === MixedType::class) {
                    if ($minimum_target_php_version < 80000) {
                        Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleMixedType, $real_parameter->getFileRef()->getLineNumberStart(), (string) $type);
                    }
                }
            }
        }
        foreach ($method->getRealReturnType()->getTypeSet() as $type) {
            ...
            if ($php70_checks) {
                if ($minimum_target_php_version < 70000) {
                    Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleAnyReturnTypePHP56, $method->getFileRef()->getLineNumberStart(), (string) $method->getRealReturnType());
                }
                ...
                if ($minimum_target_php_version < 70100) {
                    if ($type_class === VoidType::class) {
                        Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleVoidTypePHP70, $method->getFileRef()->getLineNumberStart(), (string) $type);
                    } else {
                        if ($type->isNullableLabeled()) {
                            // Don't emit CompatibleNullableTypePHP70 for `void`.
                            Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleNullableTypePHP70, $method->getFileRef()->getLineNumberStart(), (string) $type);
                        }
                        if ($type_class === IterableType::class) {
                            Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleIterableTypePHP70, $method->getFileRef()->getLineNumberStart(), (string) $type);
                            continue;
                        }
                        if ($minimum_target_php_version < 70000 && $type instanceof ScalarType) {
                            Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleScalarTypePHP56, $method->getFileRef()->getLineNumberStart(), (string) $type);
                        }
                    }
                }
            }
            if ($type_class === ObjectType::class) {
                if ($minimum_target_php_version < 70200) {
                    Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleObjectTypePHP71, $method->getFileRef()->getLineNumberStart(), (string) $type);
                }
            } elseif ($type_class === MixedType::class) {
                if ($minimum_target_php_version < 80000) {
                    Issue::maybeEmit($code_base, $method->getContext(), Issue::CompatibleMixedType, $method->getFileRef()->getLineNumberStart(), (string) $type);
                }
            }
        }
    }
    private static function checkCommentParametersAreInOrder(CodeBase $code_base, FunctionInterface $method) : void
    {
        ...
        foreach ($method->getParameterList() as $parameter) {
            ...
            if ($parameter_index_in_comment < $prev_index) {
                ...
                Issue::maybeEmit($code_base, $method->getContext(), Issue::CommentParamOutOfOrder, $line, $prev_name, $parameter_name);
                return;
            }
     ...
     */
    private static function analyzeOverrideSignature(CodeBase $code_base, Method $method) : void
    {
        ...
        try {
            ...
            Issue::maybeEmit($code_base, $method->getContext(), Issue::UnanalyzableInheritance, $method->getFileRef()->getLineNumberStart(), $method->getFQSEN());
            return;
        }
    ...
    private static function analyzeOverrideComment(CodeBase $code_base, Method $method) : void
    {
        ...
        Issue::maybeEmit($code_base, $method->getContext(), Issue::CommentOverrideOnNonOverrideMethod, $method->getFileRef()->getLineNumberStart(), $method->getFQSEN());
    }
    /**
     ...
     */
    private static function analyzeOverrideSignatureForOverriddenMethod(CodeBase $code_base, Method $method, Clazz $class, Method $o_method) : void
    {
        if ($o_method->isFinal()) {
        ...
        if ($method->getName() === '__construct') {
            ...
            if ($construct_access_signature_mismatch_thrown) {
                Issue::maybeEmit($code_base, $method->getContext(), Issue::ConstructAccessSignatureMismatch, $method->getFileRef()->getLineNumberStart(), $method, $o_method, $o_method->getFileRef()->getFile(), $o_method->getFileRef()->getLineNumberStart());
            }
        ...
        if ($type_option->isDefined()) {
            $o_parameter_list = \array_map(static function (Parameter $parameter) use($type_option, $code_base) : Parameter {
        ...
        if ($method->getNumberOfRequiredParameters() > $o_method->getNumberOfRequiredParameters()) {
        ...
        } elseif ($method->getNumberOfParameters() < $o_method->getNumberOfParameters()) {
            $signatures_match = \false;
            ...
            foreach ($method->getParameterList() as $i => $parameter) {
                ...
                if ($o_real_parameter !== null && $real_parameter !== null && !$real_parameter->getUnionType()->isEmptyOrMixed() && $o_real_parameter->getUnionType()->isEmptyOrMixed() && (!$method->isFromPHPDoc() || $parameter->getUnionType()->isEmptyOrMixed())) {
                    $signatures_match = \false;
        ...
        if (!$signatures_match) {
            if ($o_method->isPHPInternal()) {
                Issue::maybeEmit($code_base, $method->getContext(), Issue::ParamSignatureMismatchInternal, $method->getFileRef()->getLineNumberStart(), $method, $o_method, $mismatch_details !== '' ? " ({$mismatch_details})" : '');
            } else {
                Issue::maybeEmit($code_base, $method->getContext(), Issue::ParamSignatureMismatch, $method->getFileRef()->getLineNumberStart(), $method, $o_method, $o_method->getFileRef()->getFile(), $o_method->getFileRef()->getLineNumberStart(), $mismatch_details !== '' ? " ({$mismatch_details})" : '');
            }
        }
    ...
    private static function canWeakCast(CodeBase $code_base, UnionType $overridden_type, UnionType $type) : bool
    {
        ...
        return $expanded_overridden_type->canCastToUnionType($type) && $expanded_overridden_type->hasAnyTypeOverlap($code_base, $type);
    }
    /**
     ...
     */
    private static function analyzeOverrideRealSignature(CodeBase $code_base, Method $method, Clazz $class, Method $o_method, Clazz $o_class) : void
    {
        if ($o_class->isTrait() && $method->getDefiningFQSEN()->getFullyQualifiedClassName() === $class->getFQSEN()) {
            // Give up on analyzing if the class **directly** overrides any (abstract OR non-abstract) method defined by the trait
            // TODO: Fix edge cases caused by hack changing FQSEN of private methods
        ...
        // Make sure the count of parameters matches
        if ($method->getNumberOfRequiredRealParameters() > $o_method->getNumberOfRequiredRealParameters()) {
            self::emitSignatureRealMismatchIssue($code_base, $method, $o_method, Issue::ParamSignatureRealMismatchTooManyRequiredParameters, Issue::ParamSignatureRealMismatchTooManyRequiredParametersInternal, Issue::ParamSignaturePHPDocMismatchTooManyRequiredParameters, null, $method->getNumberOfRequiredRealParameters(), $o_method->getNumberOfRequiredRealParameters());
            return;
        } elseif ($method->getNumberOfRealParameters() < $o_method->getNumberOfRealParameters()) {
            self::emitSignatureRealMismatchIssue($code_base, $method, $o_method, Issue::ParamSignatureRealMismatchTooFewParameters, Issue::ParamSignatureRealMismatchTooFewParametersInternal, Issue::ParamSignaturePHPDocMismatchTooFewParameters, null, $method->getNumberOfRealParameters(), $o_method->getNumberOfRealParameters());
            return;
            // If parameter counts match, check their types
        }
        $is_possibly_compatible = \true;
        ...
        foreach ($method->isFromPHPDoc() ? $method->getParameterList() : $method->getRealParameterList() as $i => $parameter) {
            ...
            if ($parameter_union_type->isEmptyOrMixed() != $o_parameter_union_type->isEmptyOrMixed()) {
                if ($parameter_union_type->isEmptyOrMixed()) {
                    ...
                    if (Config::getValue('allow_method_param_type_widening') === \false) {
                        ...
                        self::emitSignatureRealMismatchIssue($code_base, $method, $o_method, Issue::ParamSignatureRealMismatchHasNoParamType, Issue::ParamSignatureRealMismatchHasNoParamTypeInternal, Issue::ParamSignaturePHPDocMismatchHasNoParamType, self::guessCommentParamLineNumber($method, $parameter), $offset, (string) $o_parameter_union_type);
                    }
                    continue;
                } else {
                    $is_possibly_compatible = \false;
                    self::emitSignatureRealMismatchIssue($code_base, $method, $o_method, Issue::ParamSignatureRealMismatchHasParamType, Issue::ParamSignatureRealMismatchHasParamTypeInternal, Issue::ParamSignaturePHPDocMismatchHasParamType, self::guessCommentParamLineNumber($method, $parameter), $offset, (string) $parameter_union_type);
                    continue;
                }
            }
            ...
            if (!$parameter_union_type->isEmptyOrMixed()) {
                if (!$o_parameter_union_type->isEqualTo($parameter_union_type) && !($parameter_union_type->containsNullable() && $o_parameter_union_type->isEqualTo($parameter_union_type->nonNullableClone()))) {
                    ...
                    $is_exception_to_rule = $parameter_union_type->hasIterable() && $o_parameter_union_type->hasIterable() && ($parameter_union_type->hasType(IterableType::instance(\true)) || $parameter_union_type->hasType(IterableType::instance(\false)) && !$o_parameter_union_type->containsNullable());
        ...
        if (!$o_return_union_type->isEmpty()) {
            if (!($o_return_union_type->isEqualTo($return_union_type) || $o_return_union_type->containsNullable() && !$o_return_union_type->isNull() && $o_return_union_type->nonNullableClone()->isEqualTo($return_union_type))) {
                // There is one exception to this in php 7.1 - the pseudo-type "iterable" can replace ArrayAccess/array in a subclass
                // TODO: Traversable and array work, but Iterator doesn't. Check for those specific cases?
                $is_exception_to_rule = $return_union_type->hasIterable() && $o_return_union_type->hasIterable() && ($o_return_union_type->hasType(IterableType::instance(\true)) || $o_return_union_type->hasType(IterableType::instance(\false)) && !$return_union_type->containsNullable());
                if (!$is_exception_to_rule) {
                    $is_possibly_compatible = \false;
                    self::emitSignatureRealMismatchIssue($code_base, $method, $o_method, Issue::ParamSignatureRealMismatchReturnType, Issue::ParamSignatureRealMismatchReturnTypeInternal, Issue::ParamSignaturePHPDocMismatchReturnType, null, (string) $return_union_type, (string) $o_return_union_type);
                }
            }
        }
     ...
     */
    private static function inheritPHPDoc(CodeBase $code_base, Method $method, Method $o_method) : void
    {
     ...
     */
    private static function emitSignatureRealMismatchIssue(CodeBase $code_base, Method $method, Method $o_method, string $issue_type, string $internal_issue_type, string $phpdoc_issue_type, ?int $lineno, ...$args) : void
    {
        if ($method->isFromPHPDoc() || $o_method->isFromPHPDoc()) {
            Issue::maybeEmit($code_base, $method->getContext(), $phpdoc_issue_type, $lineno ?? $method->getFileRef()->getLineNumberStart(), $method->toRealSignatureString(), $o_method->toRealSignatureString(), ...array_merge($args, [$o_method->getFileRef()->getFile(), $o_method->getFileRef()->getLineNumberStart()]));
        } elseif ($o_method->isPHPInternal()) {
            Issue::maybeEmit($code_base, $method->getContext(), $internal_issue_type, $lineno ?? $method->getFileRef()->getLineNumberStart(), $method->toRealSignatureString(), $o_method->toRealSignatureString(), ...$args);
        } else {
            Issue::maybeEmit($code_base, $method->getContext(), $issue_type, $lineno ?? $method->getFileRef()->getLineNumberStart(), $method->toRealSignatureString(), $o_method->toRealSignatureString(), ...array_merge($args, [$o_method->getFileRef()->getFile(), $o_method->getFileRef()->getLineNumberStart()]));
        }
    }
    private static function analyzeParameterTypesDocblockSignaturesMatch(CodeBase $code_base, FunctionInterface $method) : void
    {
    ...
    private static function tryToAssignPHPDocTypeToParameter(CodeBase $code_base, FunctionInterface $method, int $i, Parameter $parameter, UnionType $real_param_type, UnionType $phpdoc_param_union_type) : void
    {
        $context = $method->getContext();
        $resolved_real_param_type = $real_param_type->withStaticResolvedInContext($context);
        $is_exclusively_narrowed = \true;
        foreach ($phpdoc_param_union_type->getTypeSet() as $phpdoc_type) {
            ...
            // return type.
            if (!$phpdoc_type->isExclusivelyNarrowedFormOrEquivalentTo($resolved_real_param_type, $context, $code_base)) {
                $is_exclusively_narrowed = \false;
                Issue::maybeEmit($code_base, $context, Issue::TypeMismatchDeclaredParam, self::guessCommentParamLineNumber($method, $parameter) ?: $context->getLineNumberStart(), $parameter->getName(), $method->getName(), $phpdoc_type->__toString(), $real_param_type->__toString());
            }
        }
        ...
        if ($is_exclusively_narrowed && Config::getValue('prefer_narrowed_phpdoc_param_type')) {
            ...
            if ($normalized_phpdoc_param_union_type) {
                ...
                Issue::maybeEmit($code_base, $context, Issue::TypeMismatchDeclaredParamNullable, self::guessCommentParamLineNumber($method, $parameter) ?: $context->getLineNumberStart(), $param_name, $method->getName(), $phpdoc_param_union_type->__toString(), $real_param_type->__toString());
            }
        }
    }
    ...
    private static function warnOverridingFinalMethod(CodeBase $code_base, Method $method, Clazz $class, Method $o_method) : void
    {
        if ($method->isFromPHPDoc()) {
            ...
            Issue::maybeEmit($code_base, $method->getContext(), Issue::AccessOverridesFinalMethodPHPDoc, $method->getFileRef()->getLineNumberStart(), $method->getFQSEN(), $o_method->getFQSEN(), $o_method->getFileRef()->getFile(), $o_method->getFileRef()->getLineNumberStart());
        } elseif ($o_method->isPHPInternal()) {
            Issue::maybeEmit($code_base, $method->getContext(), Issue::AccessOverridesFinalMethodInternal, $method->getFileRef()->getLineNumberStart(), $method->getFQSEN(), $o_method->getFQSEN());
        } else {
            ...
            Issue::maybeEmit($code_base, $method->getContext(), $issue_type, $method->getFileRef()->getLineNumberStart(), $method->getFQSEN(), $o_method->getFQSEN(), $o_method->getFileRef()->getFile(), $o_method->getFileRef()->getLineNumberStart());
        }
    }
}
