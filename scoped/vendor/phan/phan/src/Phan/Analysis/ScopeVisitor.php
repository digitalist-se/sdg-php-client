    ...
    */
    ...
    public function visitGroupUse(Node $node) : Context
    {
        ...
        $alias_target_map = self::aliasTargetMapFromUseNode(
            $children['uses'],
            // @phan-suppress-current-line PhanTypeMismatchArgumentNullable the key is also used by AST_CLOSURE
            $prefix,
            $node->flags ?? 0
        );
        foreach ($alias_target_map as $alias => [$flags, $target, $lineno]) {
            $context = $context->withNamespaceMap($flags, $alias, $target, $lineno, $this->code_base);
        }
        return $context;
    }
    /**
    ...
    private function maybeWarnSameNamespaceUse(string $alias, FullyQualifiedGlobalStructuralElement $target, int $flags, int $lineno) : void
    {
        ...
        $this->emitIssue($issue_type, $lineno, $target);
    }
    private const USE_ERRORS = ['iterable' => Issue::CompatibleUseIterablePHP71, 'object' => Issue::CompatibleUseObjectPHP71, 'mixed' => Issue::CompatibleUseMixed];
    private function analyzeUseElemCompatibility(string $alias, FQSEN $target, int $minimum_target_php_version, int $lineno) : void
    {
        $alias_lower = \strtolower($alias);
        if ($minimum_target_php_version < 70100) {
            if ($alias_lower === 'void') {
                Issue::maybeEmit($this->code_base, $this->context, Issue::CompatibleUseVoidPHP70, $lineno, $target);
                return;
            }
        }
        $issue_name = self::USE_ERRORS[$alias_lower] ?? null;
        if ($issue_name) {
            Issue::maybeEmit($this->code_base, $this->context, $issue_name, $lineno, $target);
        }
    }
    /**
     * @param Node $node
     ...
     */
    public static function aliasTargetMapFromUseNode(Node $node, string $prefix = '', int $flags = 0) : array
    {
        ...
        foreach ($node->children as $child_node) {
            ...
            if ($use_flag === \ast\flags\USE_FUNCTION) {
                ...
                $target = FullyQualifiedFunctionName::make(rtrim($prefix, '\\') . '\\' . implode('\\', $parts), $function_name);
            } elseif ($use_flag === \ast\flags\USE_CONST) {
                $parts = \explode('\\', $target);
                $name = \array_pop($parts);
                $target = FullyQualifiedGlobalConstantName::make(rtrim($prefix, '\\') . '\\' . implode('\\', $parts), $name);
            } elseif ($use_flag === \ast\flags\USE_NORMAL) {
                $target = FullyQualifiedClassName::fromFullyQualifiedString(rtrim($prefix, '\\') . '\\' . $target);
            } else {
