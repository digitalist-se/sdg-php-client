...
class CompositionAnalyzer
{
     ...
     */
    public static function analyzeComposition(CodeBase $code_base, Clazz $class) : void
    {
            ...
            foreach ($inherited_class_list as $inherited_class) {
                ...
                if (!$property->isDynamicOrFromPHPDoc()) {
                    ...
                    if (!$real_property_type->isEqualTo($real_inherited_property_type)) {
                        Issue::maybeEmit($code_base, $property->getContext(), Issue::IncompatibleRealPropertyType, $property->getFileRef()->getLineNumberStart(), $property->getFQSEN(), $real_property_type, $inherited_property->getFQSEN(), $real_inherited_property_type, $inherited_property->getFileRef()->getFile(), $inherited_property->getFileRef()->getLineNumberStart());
                    }
                }
                ...
                $can_cast = $property_union_type->canCastToExpandedUnionType($inherited_property_union_type, $code_base);
                if ($can_cast) {
                    continue;
                }
