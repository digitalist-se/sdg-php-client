...
use SdgScoped\Phan\Language\Element\TypedElementInterface;
...
class AssignmentVisitor extends AnalysisVisitor
{
     ...
     */
    public function __construct(CodeBase $code_base, Context $context, Node $assignment_node, UnionType $right_type, int $dim_depth = 0, UnionType $dim_type = null)
    {
        ...
        // null for `$x[] =` or when dim_depth is 0.
    ...
    public function visit(Node $node) : Context
    {
        ...
        $this->emitIssue(Issue::Unanalyzable, $node->lineno);
        return $this->context;
    }
    ...
    public function visitCall(Node $node) : Context
    {
        ...
        if ($this->dim_depth < 2) {
            ...
            $this->checkAssignmentToFunctionResult($node, (new ContextNode($this->code_base, $this->context, $expression))->getFunctionFromNode(\true));
        }
        return $this->context;
    }
    /**
    ...
    private function checkAssignmentToFunctionResult(Node $node, iterable $function_list_generator) : void
    {
        try {
            ...
            if (isset($function)) {
                $this->emitIssue(Issue::TypeInvalidCallExpressionAssignment, $node->lineno, ASTReverter::toShortString($this->assignment_node->children['var'] ?? $node), $function->getUnionType());
            }
    ...
    private function checkValidArrayDestructuring(Node $node) : void
    {
        ...
        foreach ($node->children as $c) {
            ...
            if ($bitmask === 3) {
                $this->emitIssue(Issue::SyntaxMixedKeyNoKeyArrayDestructuring, $c->lineno ?? $node->lineno, ASTReverter::toShortString($node));
                return;
            }
        }
    }
    ...
    private function analyzeShapedArrayAssignment(Node $node) : void
    {
        ...
        $get_fallback_element_type = function () use(&$fallback_element_type) : UnionType {
            return $fallback_element_type ?? ($fallback_element_type = $this->right_type->genericArrayElementTypes()->withRealTypeSet(UnionType::computeRealElementTypesForDestructuringAccess($this->right_type->getRealTypeSet())));
        };
        ...
        foreach ($node->children ?? [] as $child_node) {
            ...
            if ($child_node->kind !== \ast\AST_ARRAY_ELEM) {
                $this->emitIssue(Issue::InvalidNode, $child_node->lineno, "Spread operator is not supported in assignments");
                continue;
            }
            ...
            if ($key_node === null) {
                ...
                // list($x, $y) = ... is equivalent to list(0 => $x, 1 => $y) = ...
            } else {
            ...
            if (\is_scalar($key_value)) {
                ...
                if ($element_type === null) {
                    ...
                    $this->emitIssue(Issue::TypeInvalidDimOffsetArrayDestructuring, $child_node->lineno, StringUtil::jsonEncode($key_value), ASTReverter::toShortString($child_node), (string) $this->right_type);
                    $element_type = $get_fallback_element_type();
                } else {
     ...
     */
    private function analyzeValueNodeOfShapedArray(UnionType $element_type, $value_node) : void
    {
        ...
        if ($kind === \ast\AST_VAR) {
            ...
            try {
                $property = (new ContextNode($this->code_base, $this->context, $value_node))->getProperty(\false, \true);
                // Set the element type on each element of
                // the list
            ...
            $this->context = (new AssignmentVisitor($this->code_base, $this->context, $value_node, $element_type, 0))->__invoke($value_node);
        }
    }
     ...
     */
    private function analyzeSetUnionType(TypedElementInterface $element, UnionType $element_type, $node) : void
    {
     ...
     */
    public static function analyzeSetUnionTypeInContext(CodeBase $code_base, Context $context, TypedElementInterface $element, UnionType $element_type, $node) : void
    {
        $element->setUnionType($element_type);
        if ($element instanceof PassByReferenceVariable) {
            self::analyzeSetUnionTypePassByRef($code_base, $context, $element, $element_type, new Node(\ast\AST_ASSIGN, 0, ['expr' => $node], $node->lineno ?? $context->getLineNumberStart()));
        }
    }
    /**
     ...
     */
    private static function analyzeSetUnionTypePassByRef(CodeBase $code_base, Context $context, PassByReferenceVariable $reference_element, UnionType $new_type, $node) : void
    {
        ...
        if ($element instanceof Property) {
            ...
            if (!$new_type->asExpandedTypes($code_base)->canCastToUnionType($element->getPHPDocUnionType())) {
                $reference_context = $reference_element->getContextOfCreatedReference();
                if ($reference_context) {
                    Issue::maybeEmit($code_base, $reference_context, Issue::TypeMismatchPropertyByRef, $reference_context->getLineNumberStart(), isset($node->children['expr']) ? ASTReverter::toShortString($node->children['expr']) : '(unknown)', $new_type, $element->getRepresentationForIssue(), $element->getPHPDocUnionType(), $context->getFile(), $node->lineno ?? $context->getLineNumberStart());
                }
            }
        }
    }
    ...
    private function analyzeGenericArrayAssignment(Node $node) : void
    {
        ...
        if ($right_type->isEmpty()) {
            ...
            if ($array_access_types->isEmpty()) {
                $this->emitIssue(Issue::TypeInvalidExpressionArrayDestructuring, $node->lineno, $this->getAssignedExpressionString(), $right_type, 'array|ArrayAccess');
            }
            $element_type = $array_access_types->genericArrayElementTypes()->withRealTypeSet(UnionType::computeRealElementTypesForDestructuringAccess($right_type->getRealTypeSet()));
    ...
    private function checkMismatchArrayDestructuringKey($expect_int_keys_lineno, $expect_string_keys_lineno) : void
    {
        if ($expect_int_keys_lineno !== \false || $expect_string_keys_lineno !== \false) {
            ...
            if ($expect_int_keys_lineno !== \false && ($right_hand_key_type & GenericArrayType::KEY_INT) === 0) {
                Issue::maybeEmit($this->code_base, $this->context, Issue::TypeMismatchArrayDestructuringKey, $expect_int_keys_lineno, 'int', 'string');
            } elseif ($expect_string_keys_lineno !== \false && ($right_hand_key_type & GenericArrayType::KEY_STRING) === 0) {
                Issue::maybeEmit($this->code_base, $this->context, Issue::TypeMismatchArrayDestructuringKey, $expect_string_keys_lineno, 'string', 'int');
            }
        }
    }
    /**
    ...
    public function visitDim(Node $node) : Context
    {
        ...
        if (!$expr_node instanceof Node) {
            $this->emitIssue(Issue::InvalidWriteToTemporaryExpression, $node->lineno, ASTReverter::toShortString($node), Type::fromObject($expr_node));
            return $this->context;
        }
        ...
        if ($dim_node instanceof Node) {
            ...
            $dim_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $dim_node);
        ...
        if ($dim_type !== null && !\is_object($dim_value)) {
            ...
            $right_type = ArrayShapeType::fromFieldTypes([$dim_value => $this->right_type], \false)->asRealUnionType();
        } else {
        ...
        $context = (new AssignmentVisitor($this->code_base, $this->context, $this->assignment_node, $right_type, $this->dim_depth + 1, $dim_type))->__invoke($expr_node);
        return $context;
    }
    /**
    ...
    private function analyzeSuperglobalDim(Node $node, string $variable_name) : Context
    {
        ...
        if ('GLOBALS' === $variable_name) {
            ...
            $variable = new Variable($this->context, $dim, $this->right_type, 0);
    ...
    public function visitProp(Node $node) : Context
    {
        ...
        try {
            $class_list = (new ContextNode($this->code_base, $this->context, $node->children['expr']))->getClassList(\false, ContextNode::CLASS_LIST_ACCEPT_OBJECT, Issue::TypeExpectedObjectPropAccess);
        } catch (\Exception $_) {
        ...
        foreach ($class_list as $clazz) {
            ...
            if (!$clazz->hasPropertyWithName($this->code_base, $property_name)) {
                if (!$clazz->hasMethodWithName($this->code_base, '__set', \true)) {
                    continue;
                }
            }
            try {
                $property = $clazz->getPropertyByNameInContext($this->code_base, $property_name, $this->context, \false, $node, \true);
            } catch (IssueException $exception) {
                Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
                return $this->context;
            }
        ...
        if ($is_class_with_arbitrary_types || Config::getValue('allow_missing_properties')) {
            ...
            $this->emitIssueWithSuggestion(Issue::UndeclaredProperty, $node->lineno ?? 0, ["{$first_class->getFQSEN()}->{$property_name}"], IssueFixSuggester::suggestSimilarProperty($this->code_base, $this->context, $first_class, $property_name, \false));
        } else {
    ...
    private function analyzePropAssignment(Clazz $clazz, Property $property, Node $node) : Context
    {
        ...
        if ($this->dim_depth > 0) {
            if ($resolved_right_type->canCastToExpandedUnionType($property_union_type, $this->code_base)) {
                $this->addTypesToProperty($property, $node);
                ...
                $new_types = $this->typeCheckDimAssignment($property_union_type, $node)->withFlattenedArrayShapeOrLiteralTypeInstances()->withStaticResolvedInContext($this->context);
                // TODO: More precise than canCastToExpandedUnionType
                if (!$new_types->canCastToExpandedUnionType($property_union_type, $this->code_base)) {
                    ...
                    $this->emitTypeMismatchPropertyIssue($node, $property, $resolved_right_type, $new_types, $property_union_type);
                } else {
                    if (Config::get_strict_property_checking() && $resolved_right_type->typeCount() > 1) {
                        $this->analyzePropertyAssignmentStrict($property, $resolved_right_type, $node);
                    }
            ...
            if (!$resolved_right_type->canCastToExpandedUnionType($property_union_type, $this->code_base) && !($resolved_right_type->hasTypeInBoolFamily() && $property_union_type->hasTypeInBoolFamily()) && !$clazz->hasDynamicProperties($this->code_base) && !$property->isDynamicProperty()) {
                if ($resolved_right_type->nonNullableClone()->canCastToExpandedUnionType($property_union_type, $this->code_base) && !$resolved_right_type->isType(NullType::instance(\false))) {
                    if ($this->shouldSuppressIssue(Issue::TypeMismatchProperty, $node->lineno)) {
                        return $this->context;
                    }
                    $this->emitIssue(Issue::PossiblyNullTypeMismatchProperty, $node->lineno, ASTReverter::toShortString($node), (string) $this->right_type->withUnionType($resolved_right_type), $property->getRepresentationForIssue(), (string) $property_union_type, 'null');
                } else {
     ...
     */
    private function emitTypeMismatchPropertyIssue(Node $node, Property $property, UnionType $resolved_right_type, UnionType $warn_type, UnionType $property_union_type) : void
    {
        ...
        if (self::isRealMismatch($this->code_base, $property->getRealUnionType(), $resolved_right_type)) {
            $this->emitIssue(Issue::TypeMismatchPropertyReal, $node->lineno, $this->getAssignedExpressionString(), $warn_type, PostOrderAnalysisVisitor::toDetailsForRealTypeMismatch($warn_type), $property->getRepresentationForIssue(), $property_union_type, PostOrderAnalysisVisitor::toDetailsForRealTypeMismatch($property_union_type));
            return;
        }
        if ($this->context->hasSuppressIssue($this->code_base, Issue::TypeMismatchPropertyProbablyReal)) {
            return;
        }
        if ($resolved_right_type->hasRealTypeSet() && !$resolved_right_type->getRealUnionType()->canCastToDeclaredType($this->code_base, $this->context, $property_union_type)) {
            $this->emitIssue(Issue::TypeMismatchPropertyProbablyReal, $node->lineno, $this->getAssignedExpressionString(), $warn_type, PostOrderAnalysisVisitor::toDetailsForRealTypeMismatch($warn_type), $property->getRepresentationForIssue(), $property_union_type, PostOrderAnalysisVisitor::toDetailsForRealTypeMismatch($property_union_type));
            return;
        }
        $this->emitIssue(Issue::TypeMismatchProperty, $node->lineno, $this->getAssignedExpressionString(), $warn_type, $property->getRepresentationForIssue(), $property_union_type);
    }
    ...
    private function handleThisPropertyAssignmentInLocalScopeByName(Node $node, string $prop_name) : void
    {
        if ($this->dim_depth === 0) {
            ...
            if ($this->dim_depth > 1) {
                ...
                $new_type = ArrayType::combineArrayTypesOverriding($right_type, $old_type, \true);
            }
        }
    ...
    private function analyzeAssignmentToReadOnlyProperty(Property $property, Node $node) : void
    {
        ...
        $this->emitIssue($is_from_phpdoc ? Issue::AccessReadOnlyMagicProperty : Issue::AccessReadOnlyProperty, $node->lineno ?? 0, $property->asPropertyFQSENString(), $context->getFile(), $context->getLineNumberStart());
    }
    private function analyzePropertyAssignmentStrict(Property $property, UnionType $assignment_type, Node $node) : void
    {
        ...
        foreach ($type_set as $type) {
            ...
            // parameter
            if (!$individual_type_expanded->canCastToUnionType($property_union_type)) {
        ...
        if ($this->shouldSuppressIssue(Issue::TypeMismatchPropertyReal, $node->lineno) || $this->shouldSuppressIssue(Issue::TypeMismatchPropertyProbablyReal, $node->lineno) || $this->shouldSuppressIssue(Issue::TypeMismatchProperty, $node->lineno)) {
            // TypeMismatchProperty also suppresses PhanPossiblyNullTypeMismatchProperty, etc.
            return;
        }
        $this->emitIssue(self::getStrictPropertyMismatchIssueType($mismatch_type_set), $node->lineno, ASTReverter::toShortString($node), (string) $this->right_type, $property->getRepresentationForIssue(), (string) $property_union_type, (string) $mismatch_expanded_types);
    }
     ...
     */
    public static function addTypesToPropertyStandalone(CodeBase $code_base, Context $context, Property $property, UnionType $new_types) : void
    {
        $original_property_types = $property->getUnionType();
        if ($property->getRealUnionType()->isEmpty() && $property->getPHPDocUnionType()->isEmpty()) {
            $property->setUnionType($new_types->eraseRealTypeSetRecursively()->withUnionType($property->getUnionType()->eraseRealTypeSetRecursively())->withStaticResolvedInContext($context)->withFlattenedArrayShapeOrLiteralTypeInstances());
            return;
        }
        if ($original_property_types->isEmpty()) {
            // TODO: Be more precise?
            $property->setUnionType($new_types->withStaticResolvedInContext($context)->withFlattenedArrayShapeOrLiteralTypeInstances()->withRealTypeSet($property->getRealUnionType()->getTypeSet()));
            return;
        }
    ...
    {
        if ($property->getRealUnionType()->isEmpty() && $property->getPHPDocUnionType()->isEmpty()) {
            $property->setUnionType($this->right_type->withUnionType($property->getUnionType())->withStaticResolvedInContext($this->context)->withFlattenedArrayShapeOrLiteralTypeInstances()->eraseRealTypeSetRecursively());
            return;
        }
        $original_property_types = $property->getUnionType();
        if ($original_property_types->isEmpty()) {
            // TODO: Be more precise?
            $property->setUnionType($this->right_type->withStaticResolvedInContext($this->context)->withFlattenedArrayShapeOrLiteralTypeInstances()->eraseRealTypeSetRecursively()->withRealTypeSet($property->getRealUnionType()->getTypeSet()));
            return;
        }
        if ($this->dim_depth > 0) {
    ...
    public function visitStaticProp(Node $node) : Context
    {
        ...
        try {
            $class_list = (new ContextNode($this->code_base, $this->context, $node->children['class']))->getClassList(\false, ContextNode::CLASS_LIST_ACCEPT_OBJECT_OR_CLASS_NAME, Issue::TypeExpectedObjectStaticPropAccess);
        } catch (\Exception $_) {
            // If we can't figure out what kind of a class
            // this is, don't worry about it
            //
        ...
        foreach ($class_list as $clazz) {
            ...
            try {
                ...
                $property = $clazz->getPropertyByNameInContext($this->code_base, $property_name, $this->context, \true, null, \true);
            } catch (IssueException $exception) {
                Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
                return $this->context;
            }
        ...
        if (\count($class_list) > 0) {
            $this->emitIssue(Issue::UndeclaredStaticProperty, $node->lineno ?? 0, $property_name, (string) $class_list[0]->getFQSEN());
        } else {
            // If we hit this part, we couldn't figure out
            // the class, so we ignore the issue
        }
    ...
    public function visitVar(Node $node) : Context
    {
        ...
        if ($this->context->getScope()->hasVariableWithName($variable_name)) {
            ...
            $variable_type = Variable::getUnionTypeOfHardcodedVariableInScopeWithName($variable_name, $this->context->isInGlobalScope());
            if ($variable_type) {
                $variable = new Variable($this->context, $variable_name, $variable_type, 0);
            } else {
                $variable = null;
            }
        }
        ...
        if ($variable) {
            ...
            if ($this->dim_depth > 0) {
                ...
                if ($this->dim_depth === 1 && $old_variable_union_type->getRealUnionType()->isExclusivelyArray()) {
                    ...
                    $node->phan_is_assignment_to_real_array = \true;
                }
                ...
                if ($this->dim_depth > 1) {
                    ...
                    $new_union_type = $old_variable_union_type->withUnionType($right_type);
                    // echo "Combining array shape types $right_type $old_variable_union_type $new_union_type\n";
                } else {
                    $new_union_type = ArrayType::combineArrayTypesOverriding($right_type, $old_variable_union_type, \true);
                }
        ...
        if ($this->dim_depth > 0) {
            $this->emitIssue(Issue::UndeclaredVariableDim, $node->lineno ?? 0, $variable_name);
        }
        $variable = new Variable($this->context, $variable_name, UnionType::empty(), 0);
        if ($this->dim_depth > 0) {
            ...
            if ($this->assignment_node->kind === \ast\AST_ASSIGN_REF) {
                ...
                if ($expr instanceof Node && \in_array($expr->kind, [\ast\AST_STATIC_PROP, \ast\AST_PROP], \true)) {
                    try {
                        $property = (new ContextNode($this->code_base, $this->context, $expr))->getProperty($expr->kind === \ast\AST_STATIC_PROP);
                        $variable = new PassByReferenceVariable($variable, $property, $this->code_base, $this->context);
    ...
    private function computeTypeOfMultiDimensionalAssignment(UnionType $old_union_type, UnionType $right_type) : UnionType
    {
        ...
        return UnionType::of(self::computeTypeSetOfMergedArrayShapeTypes($old_union_type->getTypeSet(), $right_type->getTypeSet(), $this->dim_depth, \false), self::computeTypeSetOfMergedArrayShapeTypes($old_union_type->getRealTypeSet(), $right_type->getRealTypeSet(), $this->dim_depth, \true));
    }
    /**
    ...
    private static function computeTypeSetOfMergedArrayShapeTypes(array $old_type_set, array $new_type_set, int $dim_depth, bool $is_real) : array
    {
        ...
        foreach ($new_array_shape_type->getFieldTypes() as $field => $field_type) {
            ...
            if ($old_field_type) {
                if ($dim_depth >= 3) {
                    $combined_fields[$field] = UnionType::of(self::computeTypeSetOfMergedArrayShapeTypes($old_field_type->getTypeSet(), $field_type->getTypeSet(), $dim_depth - 1, \true));
                } else {
                    $combined_fields[$field] = ArrayType::combineArrayTypesOverriding($field_type, $old_field_type, \true);
                }
            } else {
    ...
    public function typeCheckDimAssignment(UnionType $assign_type, Node $node) : UnionType
    {
        ...
        if ($assign_type->isEmpty() || $assign_type->hasGenericArray() && !$assign_type->asExpandedTypes($this->code_base)->hasArrayAccess()) {
            ...
            if ($dim_type && !$dim_type->isEmpty() && !$dim_type->canCastToUnionType($int_or_string_type)) {
                $this->emitIssue(Issue::TypeMismatchDimAssignment, $node->lineno, (string) $assign_type, (string) $dim_type, (string) $int_or_string_type);
            }
            return $right_type;
        }
        ...
        if (!$assign_type_expanded->hasArrayLike()) {
            if ($assign_type->hasNonNullStringType()) {
                ...
                if (\is_null($dim_type)) {
                    $this->emitIssue(Issue::TypeMismatchDimEmpty, $node->lineno ?? 0, (string) $assign_type, 'int');
                } elseif (!$dim_type->isEmpty() && !$dim_type->hasNonNullIntType()) {
                    $this->emitIssue(Issue::TypeMismatchDimAssignment, $node->lineno, (string) $assign_type, (string) $dim_type, 'int');
                } else {
                ...
                $this->emitIssue(Issue::TypeArraySuspicious, $node->lineno, ASTReverter::toShortString($node), (string) $assign_type);
            }
        }
        return $right_type;
    }
