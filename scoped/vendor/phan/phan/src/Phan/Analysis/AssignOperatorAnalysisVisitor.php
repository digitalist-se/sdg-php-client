...
class AssignOperatorAnalysisVisitor extends FlagVisitorImplementation
{
    ...
    private function updateTargetVariableWithType(Node $node, Closure $get_type) : Context
    {
        ...
        if ($this->context->getScope()->hasVariableWithName($variable_name)) {
            $variable = clone $this->context->getScope()->getVariableByName($variable_name);
        ...
        Issue::maybeEmitWithParameters($this->code_base, $this->context, Issue::UndeclaredVariableAssignOp, $node->lineno, [$variable_name], IssueFixSuggester::suggestVariableTypoFix($this->code_base, $this->context, $variable_name));
        // Then create the variable
        $variable = new Variable($this->context, $variable_name, $get_type(NullType::instance(\false)->asPHPDocUnionType()), 0);
        return $this->context->withScopeVariable($variable);
    }
    /**
    ...
    private function updateTargetDimWithType(Node $assign_op_node, Closure $get_type) : Context
    {
        ...
        if (!$node instanceof Node) {
            ...
            $this->emitIssue(Issue::InvalidWriteToTemporaryExpression, $assign_op_node->lineno, ASTReverter::toShortString($node), Type::fromObject($node));
            return $this->context;
        }
            ...
            if (Variable::isHardcodedVariableInScopeWithName($variable_name, $this->context->isInGlobalScope())) {
                if ($variable_name === 'GLOBALS') {
                    if (\is_string($dim_node)) {
                        $assign_op_node = new Node(\ast\AST_ASSIGN_OP, 0, ['var' => new Node(\ast\AST_VAR, 0, ['name' => $dim_node], $node->lineno), 'expr' => $assign_op_node->children['expr']], $assign_op_node->lineno);
                        ...
                        (new self($this->code_base, $this->context->withScope(new GlobalScope())))->updateTargetWithType($assign_op_node, $get_type);
        ...
        try {
            $old_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node, \false);
        } catch (\Exception $_) {
            return $this->context;
        }
        $new_type = $get_type($old_type);
        // Recurse into whatever we're []'ing
        return (new AssignmentVisitor($this->code_base, $this->context, $node, $new_type))->visitDim($node);
    }
    /**
    ...
    private function updateTargetPropWithType(Node $assign_op_node, Closure $get_type) : Context
    {
        ...
        $new_type = $get_type($old_type);
        // Recurse into whatever we're []'ing
        return (new AssignmentVisitor($this->code_base, $this->context, $node, $new_type))->visitProp($node);
    }
    /**
     * @param Node $node
     * @param Closure(UnionType):UnionType $get_type
    ...
    public function visitBinaryCoalesce(Node $node) : Context
    {
        ...
        if (!$var_node instanceof Node) {
            // nonsense like `2 ??= $x`
            $this->emitIssue(Issue::InvalidNode, $node->lineno, "Invalid left hand side for ??=");
            return $this->context;
        }
        $new_node = new ast\Node(\ast\AST_BINARY_OP, $node->lineno, ['left' => $var_node, 'right' => $node->children['expr']], \ast\flags\BINARY_COALESCE);
        $new_type = (new BinaryOperatorFlagVisitor($this->code_base, $this->context, \true))->visitBinaryCoalesce($new_node);
        return (new AssignmentVisitor($this->code_base, $this->context, $var_node, $new_type))->__invoke($var_node);
    }
    ...
    private function warnRightSideZero(Node $node, UnionType $right_type) : void
    {
        ...
        $this->emitIssue($issue_type, $node->children['expr']->lineno ?? $node->lineno, ASTReverter::toShortString($node->children['expr']), $right_type);
    }
    /**
    ...
    private function warnAboutInvalidUnionType(Node $node, Closure $is_valid_type, UnionType $left, UnionType $right, string $left_issue_type, string $right_issue_type) : void
    {
        if (!$left->isEmpty()) {
            if (!$left->hasTypeMatchingCallback($is_valid_type)) {
                $this->emitIssue($left_issue_type, $node->children['var']->lineno ?? $node->lineno, PostOrderAnalysisVisitor::NAME_FOR_BINARY_OP[$node->flags] . '=', $left);
            }
        }
        if (!$right->isEmpty()) {
            if (!$right->hasTypeMatchingCallback($is_valid_type)) {
                $this->emitIssue($right_issue_type, $node->children['expr']->lineno ?? $node->lineno, PostOrderAnalysisVisitor::NAME_FOR_BINARY_OP[$node->flags] . '=', $right);
            }
        }
    }
    private function analyzeBitwiseOperation(Node $node) : Context
    {
        return $this->updateTargetWithType($node, function (UnionType $left_type) use($node) : UnionType {
            ...
            }, $left_type, $right_type, Issue::TypeInvalidLeftOperandOfBitwiseOp, Issue::TypeInvalidRightOperandOfBitwiseOp);
    ...
    {
        $left = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['var']);
        $right = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['expr']);
        if (!$right->isEmpty() && !$right->containsTruthy()) {
            $this->warnRightSideZero($node, $right);
        }
    ...
    {
        return $this->analyzeNumericArithmeticOp($node, \true);
    }
