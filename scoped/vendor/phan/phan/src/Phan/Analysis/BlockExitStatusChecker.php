...
final class BlockExitStatusChecker extends KindVisitorImplementation
{
    ...
    public const STATUS_PROCEED = 1 << 20;
    // At least one branch continues to completion.
    public const STATUS_GOTO = 1 << 21;
    // At least one branch leads to a goto statement
    public const STATUS_CONTINUE = 1 << 22;
    // At least one branch leads to a continue statement
    public const STATUS_BREAK = 1 << 23;
    // At least one branch leads to a break statement
    public const STATUS_THROW = 1 << 24;
    // At least one branch leads to a throw statement
    public const STATUS_RETURN = 1 << 25;
    // At least one branch leads to a return/exit() statement (or an infinite loop)
    public const STATUS_THROW_OR_RETURN_BITMASK = self::STATUS_THROW | self::STATUS_RETURN;
    // Any status which doesn't lead to proceeding.
    public const STATUS_NOT_PROCEED_BITMASK = self::STATUS_GOTO | self::STATUS_CONTINUE | self::STATUS_BREAK | self::STATUS_THROW | self::STATUS_RETURN;
    public const STATUS_BITMASK = self::STATUS_PROCEED | self::STATUS_NOT_PROCEED_BITMASK;
    public const STATUS_MAYBE_PROCEED = self::STATUS_PROCEED | self::STATUS_GOTO;
    ...
    {
        $default_status = self::STATUS_THROW;
        // UnhandledMatchError if no default node exists
        $combined_status = 0;
    ...
    public function visitFor(Node $node) : int
    {
        ...
        // NOTE: $node->children['cond'] is null for the expression `for (;;)`
        // TODO: Check for unconditionally false conditions.
    ...
    private static function computeDerivedStatusOfInfiniteLoop(int $inner_status) : int
    {
        ...
        if ($status === 0) {
            ...
            // this is an infinite loop, it didn't contain break/throw/return statements?
        }
    ...
    private static function computeTriggerErrorStatusCodeForConstant($constant_ast) : int
    {
        ...
        if (\in_array($name, ['E_ERROR', 'E_PARSE', 'E_CORE_ERROR', 'E_COMPILE_ERROR', 'E_USER_ERROR'], \true)) {
            return self::STATUS_RETURN;
        }
        ...
        return self::STATUS_PROCEED;
        // Assume this is a warning or notice?
    }
    /**
    ...
    private function computeStatusOfIf(Node $node) : int
    {
        ...
        foreach ($node->children as $child_node) {
            ...
            if ($cond_node === null || self::isTruthyLiteral($cond_node)) {
                $has_if_elems_for_all_cases = \true;
                break;
            }
        }
