...
namespace SdgScoped\Phan\Analysis;
...
use Closure;
use Exception;
use SdgScoped\Phan\AST\AnalysisVisitor;
use SdgScoped\Phan\AST\ASTReverter;
use SdgScoped\Phan\AST\ContextNode;
use SdgScoped\Phan\AST\PhanAnnotationAdder;
use SdgScoped\Phan\AST\ScopeImpactCheckingVisitor;
use SdgScoped\Phan\AST\UnionTypeVisitor;
use SdgScoped\Phan\BlockAnalysisVisitor;
use SdgScoped\Phan\CodeBase;
use SdgScoped\Phan\Config;
use SdgScoped\Phan\Exception\CodeBaseException;
use SdgScoped\Phan\Exception\EmptyFQSENException;
use SdgScoped\Phan\Exception\FQSENException;
use SdgScoped\Phan\Exception\IssueException;
use SdgScoped\Phan\Exception\NodeException;
use SdgScoped\Phan\Exception\RecursionDepthException;
use SdgScoped\Phan\Issue;
use SdgScoped\Phan\IssueFixSuggester;
use SdgScoped\Phan\Language\Context;
use SdgScoped\Phan\Language\Element\Clazz;
use SdgScoped\Phan\Language\Element\Func;
use SdgScoped\Phan\Language\Element\FunctionInterface;
use SdgScoped\Phan\Language\Element\Method;
use SdgScoped\Phan\Language\Element\Parameter;
use SdgScoped\Phan\Language\Element\PassByReferenceVariable;
use SdgScoped\Phan\Language\Element\Property;
use SdgScoped\Phan\Language\Element\Variable;
use SdgScoped\Phan\Language\FQSEN\FullyQualifiedClassName;
use SdgScoped\Phan\Language\FQSEN\FullyQualifiedGlobalConstantName;
use SdgScoped\Phan\Language\Type;
use SdgScoped\Phan\Language\Type\ArrayType;
use SdgScoped\Phan\Language\Type\FalseType;
use SdgScoped\Phan\Language\Type\GenericArrayType;
use SdgScoped\Phan\Language\Type\IntType;
use SdgScoped\Phan\Language\Type\LiteralFloatType;
use SdgScoped\Phan\Language\Type\LiteralStringType;
use SdgScoped\Phan\Language\Type\MixedType;
use SdgScoped\Phan\Language\Type\NonEmptyMixedType;
use SdgScoped\Phan\Language\Type\NonNullMixedType;
use SdgScoped\Phan\Language\Type\NullType;
use SdgScoped\Phan\Language\Type\ObjectType;
use SdgScoped\Phan\Language\Type\StringType;
use SdgScoped\Phan\Language\Type\VoidType;
...
class PostOrderAnalysisVisitor extends AnalysisVisitor
{
     ...
     */
    public function __construct(CodeBase $code_base, Context $context, array $parent_node_list)
    {
    ...
    public function visitAssign(Node $node) : Context
    {
        ...
        // assignment
        $right_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['expr'], \true);
        $var_node = $node->children['var'];
        if (!$var_node instanceof Node) {
            // Give up, this should be impossible except with the fallback
            $this->emitIssue(Issue::InvalidNode, $node->lineno, "Expected left side of assignment to be a variable");
            return $this->context;
        }
        if ($right_type->isVoidType()) {
            $this->emitIssue(Issue::TypeVoidAssignment, $node->lineno);
        }
        ...
        $context = (new AssignmentVisitor($this->code_base, $this->context, $node, $right_type))->__invoke($var_node);
        $expr_node = $node->children['expr'];
        if ($expr_node instanceof Node && $expr_node->kind === \ast\AST_CLOSURE) {
            $method = (new ContextNode($this->code_base, $this->context->withLineNumberStart($expr_node->lineno), $expr_node))->getClosure();
    ...
    private function analyzeUnsetDim(Node $node) : void
    {
        ...
        if ($expr_node->kind === \ast\AST_VAR) {
            ...
            if (!$resolved_union_type->asExpandedTypes($this->code_base)->hasArrayLike() && !$resolved_union_type->hasMixedType()) {
                $this->emitIssue(Issue::TypeArrayUnsetSuspicious, $node->lineno, ASTReverter::toShortString($expr_node), (string) $resolved_union_type);
            }
            ...
            $union_type = $union_type->withAssociativeArrays(\true)->asMappedUnionType(static function (Type $type) : Type {
                if ($type instanceof NonEmptyMixedType) {
                    ...
                    return $type->isNullableLabeled() ? MixedType::instance(\true) : NonNullMixedType::instance(\false);
                }
                return $type;
            });
    ...
    private function analyzeUnsetProp(Node $node) : Context
    {
        ...
        foreach ($type_fqsens->getTypeSet() as $type) {
            ...
            if ($class->hasPropertyWithName($this->code_base, $prop_name)) {
                ...
                $this->emitIssue(Issue::TypeObjectUnsetDeclaredProperty, $node->lineno, (string) $type, $prop_name, $prop->getFileRef()->getFile(), $prop->getFileRef()->getLineNumberStart());
            }
        }
        return $context;
    }
    ...
    private function checkExpressionInDynamicString(Node $expr_node) : void
    {
        ...
        $context = $this->context;
        $type = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $expr_node, \true);
        if (!$type->hasPrintableScalar()) {
            if ($type->isType(ArrayType::instance(\false)) || $type->isType(ArrayType::instance(\true)) || $type->isGenericArray()) {
                $this->emitIssue(Issue::TypeConversionFromArray, $expr_node->lineno, 'string');
                return;
            }
            ...
            try {
                foreach ($type->withStaticResolvedInContext($context)->asExpandedTypes($code_base)->asClassList($code_base, $context) as $clazz) {
                    if ($clazz->hasMethodWithName($code_base, "__toString", \true)) {
                        return;
                    }
                }
            ...
            $this->emitIssue(Issue::TypeSuspiciousStringExpression, $expr_node->lineno, (string) $type, ASTReverter::toShortString($expr_node));
        }
    }
    /**
    ...
    private function checkForUndeclaredVariable(Node $node) : void
    {
        ...
        if (Config::getValue('ignore_undeclared_variables_in_global_scope') && $this->context->isInGlobalScope()) {
            return;
        }
        if (!$this->context->getScope()->hasVariableWithName($variable_name) && !Variable::isHardcodedVariableInScopeWithName($variable_name, $this->context->isInGlobalScope())) {
            $this->emitIssueWithSuggestion(Variable::chooseIssueForUndeclaredVariable($this->context, $variable_name), $node->lineno, [$variable_name], IssueFixSuggester::suggestVariableTypoFix($this->code_base, $this->context, $variable_name));
        }
    }
    /**
     * @param Node $node (@phan-unused-param)
    ...
    public function visitGlobal(Node $node) : Context
    {
        ...
        $variable = new Variable($this->context->withLineNumberStart($node->lineno), $variable_name, UnionType::empty(), 0);
    ...
    public function visitStatic(Node $node) : Context
    {
        ...
        if (isset($node->children['default'])) {
            $default_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['default']);
        } else {
        ...
        $variable->enablePhanFlagBits(\SdgScoped\Phan\Language\Element\Flags::IS_CONSTANT_DEFINITION);
        // Note that we're not creating a new scope, just
        // adding variables to the existing scope
        $this->context->addScopeVariable($variable);
        return $this->context;
    ...
    public function visitPrint(Node $node) : Context
    {
        ...
        if (!$type->hasPrintableScalar()) {
            if ($type->isType(ArrayType::instance(\false)) || $type->isType(ArrayType::instance(\true)) || $type->isGenericArray()) {
                $this->emitIssue(Issue::TypeConversionFromArray, $expr_node->lineno ?? $node->lineno, 'string');
                return $context;
            }
            ...
            $this->emitIssue(Issue::TypeSuspiciousEcho, $expr_node->lineno ?? $node->lineno, ASTReverter::toShortString($expr_node), (string) $type);
        }
        return $context;
    }
    /**
    ...
    private const SKIP_VAR_CHECK_TYPES = [
        \ast\AST_ARG_LIST => \true,
        // may be a reference
        \ast\AST_ARRAY_ELEM => \true,
        // [$x, $y] = expr() is an AST_ARRAY_ELEM. visitArray() checks the right-hand side.
        \ast\AST_ASSIGN_OP => \true,
        // checked in visitAssignOp
        \ast\AST_ASSIGN_REF => \true,
        // Creates by reference?
        \ast\AST_ASSIGN => \true,
        // checked in visitAssign
        \ast\AST_DIM => \true,
        // should be checked elsewhere, as part of check for array access to non-array/string
        \ast\AST_EMPTY => \true,
        ...
        \ast\AST_ISSET => \true,
        // TODO: Enable this in the future?
        \ast\AST_PARAM_LIST => \true,
        // this creates the variable
        \ast\AST_STATIC => \true,
        // static $var;
        \ast\AST_STMT_LIST => \true,
        // ;$var; (Implicitly creates the variable. Already checked to emit PhanNoopVariable)
        \ast\AST_USE_ELEM => \true,
    ];
    /**
     * @param Node $node
     * A node to parse
    ...
    /** @internal */
    public const NAME_FOR_BINARY_OP = [\ast\flags\BINARY_BOOL_AND => '&&', \ast\flags\BINARY_BOOL_OR => '||', \ast\flags\BINARY_BOOL_XOR => 'xor', \ast\flags\BINARY_BITWISE_OR => '|', \ast\flags\BINARY_BITWISE_AND => '&', \ast\flags\BINARY_BITWISE_XOR => '^', \ast\flags\BINARY_CONCAT => '.', \ast\flags\BINARY_ADD => '+', \ast\flags\BINARY_SUB => '-', \ast\flags\BINARY_MUL => '*', \ast\flags\BINARY_DIV => '/', \ast\flags\BINARY_MOD => '%', \ast\flags\BINARY_POW => '**', \ast\flags\BINARY_SHIFT_LEFT => '<<', \ast\flags\BINARY_SHIFT_RIGHT => '>>', \ast\flags\BINARY_IS_IDENTICAL => '===', \ast\flags\BINARY_IS_NOT_IDENTICAL => '!==', \ast\flags\BINARY_IS_EQUAL => '==', \ast\flags\BINARY_IS_NOT_EQUAL => '!=', \ast\flags\BINARY_IS_SMALLER => '<', \ast\flags\BINARY_IS_SMALLER_OR_EQUAL => '<=', \ast\flags\BINARY_IS_GREATER => '>', \ast\flags\BINARY_IS_GREATER_OR_EQUAL => '>=', \ast\flags\BINARY_SPACESHIP => '<=>', \ast\flags\BINARY_COALESCE => '??'];
    /**
     * @param Node $node
    ...
    public function visitBinaryOp(Node $node) : Context
    {
        ...
        if ($this->isInNoOpPosition($node)) {
            if (\in_array($flags, [\ast\flags\BINARY_BOOL_AND, \ast\flags\BINARY_BOOL_OR, \ast\flags\BINARY_COALESCE], \true)) {
                // @phan-suppress-next-line PhanAccessMethodInternal
                if (!ScopeImpactCheckingVisitor::hasPossibleImpact($this->code_base, $this->context, $node->children['right'])) {
                    $this->emitIssue(Issue::NoopBinaryOperator, $node->lineno, self::NAME_FOR_BINARY_OP[$flags] ?? '');
                }
            } else {
                $this->emitIssue(Issue::NoopBinaryOperator, $node->lineno, self::NAME_FOR_BINARY_OP[$flags] ?? '');
            }
    ...
    private function analyzeBinaryShift(Node $node) : void
    {
        ...
        $this->warnAboutInvalidUnionType($node, static function (Type $type) : bool {
            ...
            return \false;
        }, $left, $right, Issue::TypeInvalidLeftOperandOfIntegerOp, Issue::TypeInvalidRightOperandOfIntegerOp);
    }
    private function analyzeBinaryBitwiseOp(Node $node) : void
    {
        ...
        $this->warnAboutInvalidUnionType($node, static function (Type $type) : bool {
            ...
            return \false;
        }, $left, $right, Issue::TypeInvalidLeftOperandOfBitwiseOp, Issue::TypeInvalidRightOperandOfBitwiseOp);
    }
    /** @internal used by AssignOperatorAnalysisVisitor */
    public const ISSUE_TYPES_RIGHT_SIDE_ZERO = [\ast\flags\BINARY_POW => Issue::PowerOfZero, \ast\flags\BINARY_DIV => Issue::DivisionByZero, \ast\flags\BINARY_MOD => Issue::ModuloByZero];
    private function analyzeBinaryNumericOp(Node $node) : void
    {
        $left = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['left']);
        $right = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['right']);
        if (!$right->isEmpty() && !$right->containsTruthy()) {
            $this->emitIssue(self::ISSUE_TYPES_RIGHT_SIDE_ZERO[$node->flags], $node->children['right']->lineno ?? $node->lineno, ASTReverter::toShortString($node), $right);
        }
        $this->warnAboutInvalidUnionType($node, static function (Type $type) : bool {
            return $type->isValidNumericOperand();
        }, $left, $right, Issue::TypeInvalidLeftOperandOfNumericOp, Issue::TypeInvalidRightOperandOfNumericOp);
    }
    /**
     * @param Node $node with type AST_BINARY_OP
     * @param Closure(Type):bool $is_valid_type
     */
    private function warnAboutInvalidUnionType(Node $node, Closure $is_valid_type, UnionType $left, UnionType $right, string $left_issue_type, string $right_issue_type) : void
    {
        if (!$left->isEmpty()) {
            if (!$left->hasTypeMatchingCallback($is_valid_type)) {
                $this->emitIssue($left_issue_type, $node->children['left']->lineno ?? $node->lineno, PostOrderAnalysisVisitor::NAME_FOR_BINARY_OP[$node->flags], $left);
            }
        }
        if (!$right->isEmpty()) {
            if (!$right->hasTypeMatchingCallback($is_valid_type)) {
                $this->emitIssue($right_issue_type, $node->children['right']->lineno ?? $node->lineno, PostOrderAnalysisVisitor::NAME_FOR_BINARY_OP[$node->flags], $right);
            }
        }
    }
    ...
    public const NAME_FOR_UNARY_OP = [\ast\flags\UNARY_BOOL_NOT => '!', \ast\flags\UNARY_BITWISE_NOT => '~', \ast\flags\UNARY_SILENCE => '@', \ast\flags\UNARY_PLUS => '+', \ast\flags\UNARY_MINUS => '-'];
    /**
     * @param Node $node
    ...
    {
        if ($this->isInNoOpPosition($node)) {
            $this->emitIssue(Issue::NoopEmpty, $node->lineno, ASTReverter::toShortString($node->children['expr']));
        }
        return $this->context;
    }
    /**
     ...
     */
    public const AST_TYPE_FLAGS_LOOKUP = [\ast\flags\TYPE_NULL => 'null'] + self::AST_CAST_FLAGS_LOOKUP;
    /**
     * @param Node $node
    ...
    {
        if ($this->isInNoOpPosition($node)) {
            $this->emitIssue(Issue::NoopCast, $node->lineno, self::AST_CAST_FLAGS_LOOKUP[$node->flags] ?? 'unknown', ASTReverter::toShortString($node->children['expr']));
        }
        if ($node->flags === \ast\flags\TYPE_NULL) {
            $this->emitIssue(Issue::CompatibleUnsetCast, $node->lineno, ASTReverter::toShortString($node));
        }
        return $this->context;
    }
    /**
    ...
    {
        if ($this->isInNoOpPosition($node)) {
            $this->emitIssue(Issue::NoopIsset, $node->lineno, ASTReverter::toShortString($node->children['var']));
        }
        return $this->context;
    }
    /**
    ...
    public function visitUnaryOp(Node $node) : Context
    {
        if ($node->flags === \ast\flags\UNARY_SILENCE) {
            ...
            if ($expr instanceof Node) {
                if ($expr->kind === \ast\AST_UNARY_OP && $expr->flags === \ast\flags\UNARY_SILENCE) {
                    $this->emitIssue(Issue::NoopRepeatedSilenceOperator, $node->lineno, ASTReverter::toShortString($node));
                }
            } else {
                // TODO: Other node kinds
                $this->emitIssue(Issue::NoopUnaryOperator, $node->lineno, self::NAME_FOR_UNARY_OP[$node->flags] ?? '');
            }
        } else {
            if ($this->isInNoOpPosition($node)) {
                $this->emitIssue(Issue::NoopUnaryOperator, $node->lineno, self::NAME_FOR_UNARY_OP[$node->flags] ?? '');
    ...
    public const NAME_FOR_INC_OR_DEC_KIND = [\ast\AST_PRE_INC => '++(expr)', \ast\AST_PRE_DEC => '--(expr)', \ast\AST_POST_INC => '(expr)++', \ast\AST_POST_DEC => '(expr)--'];
    private function analyzeIncOrDec(Node $node) : Context
    {
        ...
        if (!$old_type->canCastToUnionType(UnionType::fromFullyQualifiedPHPDocString('int|string|float'))) {
            $this->emitIssue(Issue::TypeInvalidUnaryOperandIncOrDec, $node->lineno, self::NAME_FOR_INC_OR_DEC_KIND[$node->kind], $old_type);
        }
        ...
        $new_node = new Node(\ast\AST_ASSIGN_OP, \ast\flags\BINARY_SUB, ['var' => $var, 'expr' => $node->kind === \ast\AST_POST_DEC || $node->kind === \ast\AST_PRE_DEC ? 1 : -1], $node->lineno);
    ...
    public function visitConst(Node $node) : Context
    {
        ...
        try {
            ...
            $constant = (new ContextNode($this->code_base, $context, $node))->getConst();
    ...
    {
        try {
            foreach ((new ContextNode($this->code_base, $this->context, $node->children['class']))->getClassList(\false, ContextNode::CLASS_LIST_ACCEPT_OBJECT_OR_CLASS_NAME) as $class) {
                $class->addReference($this->context);
            }
        } catch (CodeBaseException $exception) {
            $exception_fqsen = $exception->getFQSEN();
            $this->emitIssueWithSuggestion(Issue::UndeclaredClassReference, $node->lineno, [(string) $exception_fqsen], IssueFixSuggester::suggestSimilarClassForGenericFQSEN($this->code_base, $this->context, $exception_fqsen));
        } catch (IssueException $exception) {
            Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
        }
        // Check to make sure we're doing something with the
    ...
    public function visitClosure(Node $node) : Context
    {
        ...
        if (!$return_type->isEmpty() && !$func->hasReturn() && !self::declOnlyThrows($node) && !$return_type->hasType(VoidType::instance(\false)) && !$return_type->hasType(NullType::instance(\false))) {
        ...
        if (isset($uses->polyfill_has_trailing_comma) && Config::get_closest_minimum_target_php_version_id() < 80000) {
            $this->emitIssue(Issue::CompatibleTrailingCommaParameterList, end($uses->children)->lineno ?? $uses->lineno, ASTReverter::toShortString($node));
        }
    ...
    public function visitArrowFunc(Node $node) : Context
    {
        if (Config::get_closest_minimum_target_php_version_id() < 70400) {
            $this->emitIssue(Issue::CompatibleArrowFunction, $node->lineno, ASTReverter::toShortString($node));
        }
    ...
    public function visitReturn(Node $node) : Context
    {
        ...
        if ($expr !== null) {
            if ($method_return_type->hasRealTypeSet() && $method_return_type->asRealUnionType()->isVoidType()) {
                $this->emitIssue(Issue::SyntaxReturnValueInVoid, $expr->lineno ?? $node->lineno, 'void', $method->getNameForIssue(), 'return;', 'return ' . ASTReverter::toShortString($expr) . ';');
                return $context;
            }
        } else {
            // `function test() : ?string { return; }` is a fatal error. (We already checked for generators)
            if ($method_return_type->hasRealTypeSet() && !$method_return_type->asRealUnionType()->isVoidType()) {
                $this->emitIssue(Issue::SyntaxReturnExpectedValue, $node->lineno, $method->getNameForIssue(), $method_return_type, 'return null', 'return');
                return $context;
            }
        }
        ...
        foreach ($this->getReturnTypes($context, $expr, $node->lineno) as $lineno => [$expression_type, $inner_node]) {
            ...
            if ($method_return_type->isEmpty() || $method->isReturnTypeUndefined()) {
                if (!$is_trait) {
                    $method->setIsReturnTypeUndefined(\true);
    ...
    {
        $expr = $node->children['expr'];
        if (!$expr instanceof Node || !\in_array($expr->kind, ArgumentType::REFERENCE_NODE_KINDS, \true)) {
            ...
            if (!$is_possible_reference) {
                Issue::maybeEmit($this->code_base, $this->context, Issue::TypeNonVarReturnByRef, $expr->lineno ?? $node->lineno, $method->getRepresentationForIssue());
            }
        }
    }
    /**
    ...
    private function emitTypeMismatchReturnIssue(UnionType $expression_type, FunctionInterface $method, UnionType $method_return_type, int $lineno, $inner_node) : void
    {
        ...
        if (!$expression_type->isNull() && $this->checkCanCastToReturnTypeIfWasNonNullInstead($expression_type, $method_return_type)) {
            ...
            if ($method_return_type->hasRealTypeSet()) {
                ...
                if ($real_method_return_type->isVoidType() || $expression_type->hasRealTypeSet() && !$real_expression_type->canCastToDeclaredType($this->code_base, $this->context, $real_method_return_type)) {
                    $this->emitIssue(Issue::TypeMismatchReturnReal, $lineno, self::returnExpressionToShortString($inner_node), (string) $expression_type, self::toDetailsForRealTypeMismatch($expression_type), $method->getNameForIssue(), (string) $method_return_type, self::toDetailsForRealTypeMismatch($method_return_type));
                    return;
                }
            }
        }
        ...
        if ($issue_type === Issue::TypeMismatchReturn) {
            if ($expression_type->hasRealTypeSet() && !$expression_type->getRealUnionType()->canCastToDeclaredType($this->code_base, $this->context, $method_return_type)) {
                ...
                $this->emitIssue(Issue::TypeMismatchReturnProbablyReal, $lineno, self::returnExpressionToShortString($inner_node), $expression_type, PostOrderAnalysisVisitor::toDetailsForRealTypeMismatch($expression_type), $method->getNameForIssue(), $method_return_type, PostOrderAnalysisVisitor::toDetailsForRealTypeMismatch($method_return_type));
                return;
            }
        }
        $this->emitIssue($issue_type, $lineno, self::returnExpressionToShortString($inner_node), (string) $expression_type, $method->getNameForIssue(), (string) $method_return_type);
    }
    /**
    ...
    private function analyzeReturnInGenerator(FunctionInterface $method, Node $node) : void
    {
    ...
    private function compareYieldAgainstDeclaredType(Node $node, FunctionInterface $method, Context $context, array $template_type_list) : Context
    {
        ...
        try {
            if (!$yield_value_type->withStaticResolvedInContext($context)->asExpandedTypes($code_base)->canCastToUnionType($expected_value_type->withStaticResolvedInContext($context))) {
                $this->emitIssue(Issue::TypeMismatchGeneratorYieldValue, $node->lineno, ASTReverter::toShortString($yield_value_node), (string) $yield_value_type, $method->getNameForIssue(), (string) $expected_value_type, '\\Generator<' . implode(',', $template_type_list) . '>');
            }
        ...
        if ($type_list_count > 1) {
            ...
            if (!$yield_key_type->withStaticResolvedInContext($context)->asExpandedTypes($code_base)->canCastToUnionType($expected_key_type->withStaticResolvedInContext($context))) {
                $this->emitIssue(Issue::TypeMismatchGeneratorYieldKey, $node->lineno, ASTReverter::toShortString($yield_key_node), (string) $yield_key_type, $method->getNameForIssue(), (string) $expected_key_type, '\\Generator<' . implode(',', $template_type_list) . '>');
            }
        }
        return $context;
    }
    ...
    public function visitYieldFrom(Node $node) : Context
    {
        ...
        if (!$yield_from_expanded_type->hasIterable() && !$yield_from_expanded_type->hasTraversable()) {
            $this->emitIssue(Issue::TypeInvalidYieldFrom, $node->lineno, ASTReverter::toShortString($node), (string) $yield_from_type);
            return $context;
        }
        if (BlockAnalysisVisitor::isEmptyIterable($yield_from_type)) {
            RedundantCondition::emitInstance($node->children['expr'], $this->code_base, (clone $this->context)->withLineNumberStart($node->children['expr']->lineno ?? $node->lineno), Issue::EmptyYieldFrom, [(string) $yield_from_type], Closure::fromCallable([BlockAnalysisVisitor::class, 'isEmptyIterable']));
        }
        // Figure out what we intend to return
        $method_generator_type = $method->getReturnTypeAsGeneratorTemplateType();
        $type_list = $method_generator_type->getTemplateParameterTypeList();
    ...
    private function compareYieldFromAgainstDeclaredType(Node $node, FunctionInterface $method, Context $context, array $template_type_list, UnionType $yield_from_type) : Context
    {
        ...
        if (!$yield_value_type->withStaticResolvedInContext($context)->asExpandedTypes($code_base)->canCastToUnionType($expected_value_type)) {
            $this->emitIssue(Issue::TypeMismatchGeneratorYieldValue, $node->lineno, sprintf('(values of %s)', ASTReverter::toShortString($node)), (string) $yield_value_type, $method->getNameForIssue(), (string) $expected_value_type, '\\Generator<' . implode(',', $template_type_list) . '>');
        }
        if ($type_list_count > 1 && $actual_type_list_count > 1) {
            ...
            if (!$yield_key_type->withStaticResolvedInContext($context)->asExpandedTypes($code_base)->canCastToUnionType($expected_key_type)) {
                $this->emitIssue(Issue::TypeMismatchGeneratorYieldKey, $node->lineno, sprintf('(keys of %s)', ASTReverter::toShortString($node)), (string) $yield_key_type, $method->getNameForIssue(), (string) $expected_key_type, '\\Generator<' . implode(',', $template_type_list) . '>');
            }
        }
        return $context;
    }
    private function checkCanCastToReturnType(UnionType $expression_type, UnionType $method_return_type) : bool
    {
        ...
        if ($method_return_type->hasTemplateParameterTypes()) {
            ...
            return $expression_type->asExpandedTypesPreservingTemplate($this->code_base)->canCastToUnionTypeHandlingTemplates($method_return_type, $this->code_base) || $expression_type->canCastToUnionTypeHandlingTemplates($method_return_type->asExpandedTypesPreservingTemplate($this->code_base), $this->code_base);
        }
        // We allow base classes to cast to subclasses, and subclasses to cast to base classes,
        // but don't allow subclasses to cast to subclasses on a separate branch of the inheritance tree
        try {
            return $expression_type->asExpandedTypes($this->code_base)->canCastToUnionType($method_return_type) || $expression_type->canCastToUnionType($method_return_type->asExpandedTypes($this->code_base));
        } catch (RecursionDepthException $_) {
            return \false;
        }
    }
     ...
     * @param Node|string|int|float|null $inner_node
     */
    private function analyzeReturnStrict(CodeBase $code_base, FunctionInterface $method, UnionType $expression_type, UnionType $method_return_type, int $lineno, $inner_node) : bool
    {
        ...
        foreach ($type_set as $type) {
            ...
            // parameter
            if (!$individual_type_expanded->canCastToUnionType($method_return_type)) {
        ...
        $this->emitIssue(self::getStrictIssueType($mismatch_type_set), $lineno, self::returnExpressionToShortString($inner_node), (string) $expression_type, $method->getNameForIssue(), (string) $method_return_type, $mismatch_expanded_types);
        return \true;
    }
    /**
    ...
    private function getReturnTypes(Context $context, $node, int $return_lineno) : \Generator
    {
        if (!$node instanceof Node) {
            ...
            (yield $return_lineno => [UnionTypeVisitor::unionTypeFromNode($this->code_base, $context, $node, \true), $node]);
            return;
        }
        $kind = $node->kind;
        if ($kind === \ast\AST_CONDITIONAL) {
            ...
            $expression_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $context, $node, \true);
            ...
            foreach (self::deduplicateUnionTypes($this->getReturnTypesOfArray($context, $node)) as $return_lineno => [$elem_type, $elem_node]) {
                (yield $return_lineno => [
                    $elem_type->asGenericArrayTypes($key_type_enum),
                    // TODO: Infer corresponding key types
                    $elem_node,
                ]);
            }
            return;
    ...
    private function getReturnTypesOfConditional(Context $context, Node $node) : \Generator
    {
        ...
        if ($cond_node instanceof Node) {
            ...
            $true_context = (new ConditionVisitor($this->code_base, $base_context))->__invoke($cond_node);
            $false_context = (new NegatedConditionVisitor($this->code_base, $base_context))->__invoke($cond_node);
        } else {
    ...
    private function getReturnTypesOfArray(Context $context, Node $node) : \Generator
    {
        ...
        foreach ($node->children as $elem) {
            ...
            if ($elem->kind === \ast\AST_UNPACK) {
                ...
                (yield $elem->lineno => [UnionTypeVisitor::unionTypeFromNode($this->code_base, $context, $elem, \true), $elem]);
                continue;
            }
            $value_node = $elem->children['value'];
            if ($value_node instanceof Node) {
                (yield $elem->lineno => [UnionTypeVisitor::unionTypeFromNode($this->code_base, $context, $value_node, \true), $value_node]);
            } else {
                (yield $elem->lineno => [Type::fromObject($value_node)->asRealUnionType(), $value_node]);
            }
        }
    }
    /**
    ...
    public function visitCall(Node $node) : Context
    {
        ...
        try {
            ...
            $function_list_generator = (new ContextNode($this->code_base, $this->context, $expression))->getFunctionFromNode(\true);
            foreach ($function_list_generator as $function) {
                // Check the call for parameter and argument types
                $this->analyzeCallToFunctionLike($function, $node);
    ...
    private function analyzeAssert(Context $context, Node $node) : Context
    {
        ...
        return (new ConditionVisitor($this->code_base, $context))->__invoke($args_first_child);
    }
    /**
     * @param Node $node
     * A node to parse
    ...
    public function visitNew(Node $node) : Context
    {
        ...
        try {
            ...
            $method = $context_node->getMethod('__construct', \false, \false, \true);
            ...
            foreach ($class_list as $class) {
                ...
                if ($class->isDeprecated()) {
                    $this->emitIssue(Issue::DeprecatedClass, $node->lineno, (string) $class->getFQSEN(), $class->getContext()->getFile(), (string) $class->getContext()->getLineNumberStart(), $class->getDeprecationReason());
                }
            }
            $this->analyzeMethodVisibility($method, $node);
            $this->analyzeCallToFunctionLike($method, $node);
            foreach ($class_list as $class) {
        ...
        foreach (UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $class_node)->getTypeSet() as $type) {
            if ($type instanceof LiteralStringType) {
                try {
                    ...
                    Issue::maybeEmit($this->code_base, $this->context, Issue::TypeExpectedObjectOrClassName, $node->lineno, ASTReverter::toShortString($node), $type->getValue());
                    continue;
                }
    ...
    private function warnIfInvalidClassForNew(Clazz $class, Node $node) : void
    {
        ...
        // class
        if ($class->isAbstract()) {
            $this->emitIssue(self::isStaticNameNode($node, \false) ? Issue::TypeInstantiateAbstractStatic : Issue::TypeInstantiateAbstract, $node->lineno, (string) $class->getFQSEN());
        } elseif ($class->isInterface()) {
            // Make sure we're not instantiating an interface
            $this->emitIssue(Issue::TypeInstantiateInterface, $node->lineno, (string) $class->getFQSEN());
        } elseif ($class->isTrait()) {
            // Make sure we're not instantiating a trait
            $this->emitIssue(self::isStaticNameNode($node, \true) ? Issue::TypeInstantiateTraitStaticOrSelf : Issue::TypeInstantiateTrait, $node->lineno, (string) $class->getFQSEN());
        }
    }
    /**
     * @param Node $node
    ...
    public function visitInstanceof(Node $node) : Context
    {
        try {
            ...
            (new ContextNode($this->code_base, $this->context, $node->children['class']))->getClassList(\false, ContextNode::CLASS_LIST_ACCEPT_OBJECT_OR_CLASS_NAME, Issue::TypeInvalidInstanceof);
        } catch (IssueException $exception) {
            Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
        } catch (CodeBaseException $exception) {
            $this->emitIssueWithSuggestion(Issue::UndeclaredClassInstanceof, $node->lineno, [(string) $exception->getFQSEN()], IssueFixSuggester::suggestSimilarClassForGenericFQSEN(
                $this->code_base,
                $this->context,
                $exception->getFQSEN(),
                // Only suggest classes/interfaces for alternatives to instanceof checks. Don't suggest traits.
                IssueFixSuggester::createFQSENFilterForClasslikeCategories($this->code_base, \true, \false, \true)
            ));
        }
        return $this->context;
    }
        ...
        if (!\is_string($method_name)) {
            ...
            if (!\is_string($method_name)) {
                ...
                if (!$method_name_type->canCastToUnionType(StringType::instance(\false)->asPHPDocUnionType())) {
                    Issue::maybeEmit($this->code_base, $this->context, Issue::TypeInvalidStaticMethodName, $node->lineno, $method_name_type);
                }
                return $this->context;
            }
        }
        ...
        try {
            ...
            if (!$method->isStatic() && !$this->canCallInstanceMethodFromContext($method, $static_class)) {
                ...
                if (\count($class_list) > 0) {
                    ...
                    $this->emitIssue(Issue::StaticCallToNonStatic, $node->lineno, "{$class->getFQSEN()}::{$method_name}()", $method->getFileRef()->getFile(), (string) $method->getFileRef()->getLineNumberStart());
                }
            }
            $this->analyzeMethodVisibility($method, $node);
    ...
    private function canCallInstanceMethodFromContext(Method $method, string $static_class) : bool
    {
        ...
        if (\in_array(\strtolower($static_class), ['parent', 'self', 'static'], \true)) {
            return \true;
        }
     ...
     */
    private function checkNonAncestorConstructCall(Node $node, string $static_class, string $method_name) : void
    {
        ...
        if ($this->context->isInMethodScope()) {
            try {
                $possible_ancestor_type = UnionTypeVisitor::unionTypeFromClassNode($this->code_base, $this->context, $node->children['class']);
            } catch (FQSENException $e) {
                $this->emitIssue($e instanceof EmptyFQSENException ? Issue::EmptyFQSENInCallable : Issue::InvalidFQSENInCallable, $node->lineno, $e->getFQSEN());
                return;
            }
            ...
            if (!$possible_ancestor_type->isEmpty()) {
                ...
                if ($possible_ancestor_type->hasStaticType()) {
                    $this->emitIssue(Issue::AccessOwnConstructor, $node->lineno, $static_class);
                    $found_ancestor_constructor = \true;
    ...
    {
        $this->emitIssue(Issue::UndeclaredStaticMethod, $node->lineno, "{$static_class}::{$method_name}()");
    }
    /**
    ...
    private function getStaticMethodOrEmitIssue(Node $node, string $method_name) : ?Method
    {
        try {
            ...
            $result = (new ContextNode($this->code_base, $this->context, $node))->getMethod($method_name, \true, \true);
            ...
            if ($e instanceof FQSENException) {
                Issue::maybeEmit($this->code_base, $this->context, $e instanceof EmptyFQSENException ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike, $node->lineno, $e->getFQSEN());
            }
    ...
    public function visitMethod(Node $node) : Context
    {
        ...
        if (!$method->isAbstract() && !$method->isFromPHPDoc() && !$has_interface_class && !$return_type->isEmpty() && !$method->hasReturn() && !self::declOnlyThrows($node) && !$return_type->hasType(VoidType::instance(\false)) && !$return_type->hasType(NullType::instance(\false))) {
        ...
        if ($method->hasReturn() && $method->isMagicAndVoid()) {
            $this->emitIssue(Issue::TypeMagicVoidWithReturn, $node->lineno, (string) $method->getFQSEN());
        }
        return $this->context;
    }
    private function warnTypeMissingReturn(FunctionInterface $method, Node $node) : void
    {
        $this->emitIssue($method->getRealReturnType()->isEmpty() ? Issue::TypeMissingReturn : Issue::TypeMissingReturnReal, $node->lineno, $method->getFQSEN(), $method->getUnionType());
    }
    /**
    ...
    public function visitFuncDecl(Node $node) : Context
    {
        ...
        if (\strcasecmp($method->getName(), '__autoload') === 0) {
            $this->emitIssue(Issue::CompatibleAutoload, $node->lineno);
        }
        $return_type = $method->getUnionType();
        if (!$return_type->isEmpty() && !$method->hasReturn() && !self::declOnlyThrows($node) && !$return_type->hasType(VoidType::instance(\false)) && !$return_type->hasType(NullType::instance(\false))) {
            $this->warnTypeMissingReturn($method, $node);
        }
        $this->checkForFunctionInterfaceIssues($node, $method);
        return $this->context;
    ...
    private function checkForFunctionInterfaceIssues(Node $node, FunctionInterface $function) : void
    {
        ...
        foreach ($function->getParameterList() as $i => $parameter) {
            if (isset($parameters_seen[$parameter->getName()])) {
                $this->emitIssue(Issue::ParamRedefined, $node->lineno, '$' . $parameter->getName());
            } else {
        ...
        if (isset($params_node->polyfill_has_trailing_comma)) {
            $this->emitIssue(Issue::CompatibleTrailingCommaParameterList, end($params_node->children)->lineno ?? $params_node->lineno, ASTReverter::toShortString($node));
        }
    ...
    private function checkUnionTypeCompatibility(?Node $type) : void
    {
        ...
        if ($type->kind === \ast\AST_TYPE_UNION) {
            ...
            $this->emitIssue(Issue::CompatibleUnionType, $type->lineno, ASTReverter::toShortString($type));
            return;
        }
        ...
        if ($inner_type->kind !== \ast\AST_TYPE) {
            // e.g. ast\TYPE_UNION
            $this->emitIssue(Issue::InvalidNode, $inner_type->lineno, "Unsupported union type syntax " . ASTReverter::toShortString($inner_type));
            return;
        }
        if ($inner_type->flags === \ast\flags\TYPE_STATIC) {
            $this->emitIssue(Issue::CompatibleStaticType, $inner_type->lineno);
        } elseif (\in_array($inner_type->flags, [\ast\flags\TYPE_FALSE, \ast\flags\TYPE_NULL], \true)) {
            $this->emitIssue(Issue::InvalidNode, $inner_type->lineno, "Invalid union type " . ASTReverter::toShortTypeString($type) . " in element signature");
        }
    }
    ...
    {
        if (Config::get_closest_minimum_target_php_version_id() < 80000) {
            $this->emitIssue(Issue::CompatibleNullsafeOperator, $node->lineno, ASTReverter::toShortString($node));
        }
    }
    /**
     * @param Node $node
    ...
    public function visitMethodCall(Node $node) : Context
    {
        ...
        if (!\is_string($method_name)) {
            ...
            if (!\is_string($method_name)) {
                $method_name_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['method']);
                if (!$method_name_type->canCastToUnionType(StringType::instance(\false)->asPHPDocUnionType())) {
                    Issue::maybeEmit($this->code_base, $this->context, Issue::TypeInvalidMethodName, $node->lineno, $method_name_type);
                }
                return $this->context;
            }
        }
        try {
            $method = (new ContextNode($this->code_base, $this->context, $node))->getMethod($method_name, \false, \true);
        } catch (IssueException $exception) {
            Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
            return $this->context;
    ...
    public function visitArgList(Node $node) : Context
    {
        ...
        foreach ($node->children as $i => $argument) {
            ...
            if ($argument instanceof Node && $argument->kind === \ast\AST_NAMED_ARG) {
                if (Config::get_closest_minimum_target_php_version_id() < 80000) {
                    $this->emitIssue(Issue::CompatibleNamedArgument, $argument->lineno, ASTReverter::toShortString($argument));
                }
                ...
                if (isset($argument_name_set[$argument_name])) {
                    $this->emitIssue(Issue::DefinitelyDuplicateNamedArgument, $argument->lineno, ASTReverter::toShortString($argument), ASTReverter::toShortString($argument_name_set[$argument_name]));
                } else {
            ...
            if ($argument_name_set) {
                if ($argument === $argument_expression) {
                    $this->emitIssue(Issue::PositionalArgumentAfterNamedArgument, $argument->lineno ?? $node->lineno, ASTReverter::toShortString($argument), ASTReverter::toShortString(\end($argument_name_set)));
                }
            }
        ...
        if ($has_unpack && $argument_name_set) {
            $this->emitIssue(Issue::ArgumentUnpackingUsedWithNamedArgument, $node->lineno, ASTReverter::toShortString($node));
        }
        // @phan-suppress-next-line PhanUndeclaredProperty
        if (isset($node->polyfill_has_trailing_comma) && Config::get_closest_minimum_target_php_version_id() < 70300) {
            $this->emitIssue(Issue::CompatibleTrailingCommaArgumentList, end($node->children)->lineno ?? $node->lineno, ASTReverter::toShortString($node));
        }
        return $this->context;
    }
    private function checkForPossibleNonObjectInMethod(Node $node, string $method_name) : void
    {
        ...
        if ($type->containsDefiniteNonObjectType()) {
            Issue::maybeEmit($this->code_base, $this->context, Issue::PossiblyNonClassMethodCall, $node->lineno, $method_name, $type);
        }
    }
    ...
    public function visitDim(Node $node) : Context
    {
        ...
        /* $dim_type = */
        UnionTypeVisitor::unionTypeFromNode($code_base, $context, $node->children['dim'], \true);
        $this->analyzeNoOp($node, Issue::NoopArrayAccess);
        $flags = $node->flags;
        if ($flags & \ast\flags\DIM_ALTERNATIVE_SYNTAX) {
            $this->emitIssue(Issue::CompatibleDimAlternativeSyntax, $node->children['dim']->lineno ?? $node->lineno, ASTReverter::toShortString($node));
        }
        ...
        try {
            /* $array_type = */
            UnionTypeVisitor::unionTypeFromNode($code_base, $context, $node, \false);
    ...
    {
        if ($this->isInNoOpPosition($node)) {
            if (!ScopeImpactCheckingVisitor::hasPossibleImpact($this->code_base, $this->context, $node->children['true']) && !ScopeImpactCheckingVisitor::hasPossibleImpact($this->code_base, $this->context, $node->children['false'])) {
                $this->emitIssue(Issue::NoopTernary, $node->lineno);
            }
        }
    ...
    {
        if (Config::get_closest_minimum_target_php_version_id() < 80000) {
            $this->emitIssue(Issue::CompatibleMatchExpression, $node->lineno, ASTReverter::toShortString($node));
        }
        if ($this->isInNoOpPosition($node)) {
            if (!ScopeImpactCheckingVisitor::hasPossibleImpact($this->code_base, $this->context, $node->children['stmts'])) {
                $this->emitIssue(Issue::NoopMatchExpression, $node->lineno, ASTReverter::toShortString($node));
            }
        }
        return $this->context;
    }
    ...
    {
        if ($cond->flags & \ast\flags\PARENTHESIZED_CONDITIONAL) {
        ...
        $this->emitIssue(Issue::CompatibleUnparenthesizedTernary, $node->lineno, $description, $first_suggestion, $second_suggestion);
    }
    /**
     * @param list<Node> $parent_node_list
    ...
    {
        return $this->analyzeProp($node, \true);
    }
    ...
    {
        $type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['expr'], \true);
        if ($type->isEmpty()) {
            return $this->context;
        }
        if (!$type->hasPossiblyObjectTypes()) {
            $this->emitIssue(Issue::TypeInvalidCloneNotObject, $node->children['expr']->lineno ?? $node->lineno, $type);
            ...
            if ($type->containsNullable() || !$type->canStrictCastToUnionType($this->code_base, ObjectType::instance(\false)->asPHPDocUnionType())) {
                $this->emitIssue(Issue::TypePossiblyInvalidCloneNotObject, $node->children['expr']->lineno ?? $node->lineno, $type);
            }
        }
        return $this->context;
    }
    ...
    public function analyzeProp(Node $node, bool $is_static) : Context
    {
        ...
        try {
            $property = (new ContextNode($this->code_base, $this->context, $node))->getProperty($is_static);
        ...
        if (isset($property)) {
            ...
            if ($expr_or_class_node === null) {
                throw new AssertionError("Property nodes must either have an expression or class");
            }
            $class_list = [];
            try {
                // Get the set of classes that are being referenced
                $class_list = (new ContextNode($this->code_base, $this->context, $expr_or_class_node))->getClassList(\true, $is_static ? ContextNode::CLASS_LIST_ACCEPT_OBJECT_OR_CLASS_NAME : ContextNode::CLASS_LIST_ACCEPT_OBJECT, null, \false);
            } catch (IssueException $exception) {
                Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
            }
            if (!$is_static) {
    ...
    private function isAssignmentOrNestedAssignment(Node $node) : ?bool
    {
        ...
        while ($parent_kind === \ast\AST_ARRAY_ELEM) {
            ...
            // pop AST_ARRAY_ELEM
            $node = \array_pop($parent_node_list);
            // AST_ARRAY
    ...
    // TODO: ASSIGN_OP?
    private const READ_AND_WRITE_KINDS = [\ast\AST_PRE_INC, \ast\AST_PRE_DEC, \ast\AST_POST_INC, \ast\AST_POST_DEC];
    /**
     * @return ?bool
     * - false if this is a read reference
     * - true if this is a write reference
    ...
    private function isAssignmentOrNestedAssignmentOrModification(Node $node) : ?bool
    {
        ...
        if ($parent_kind === \ast\AST_DIM) {
            ...
            return \in_array($parent_kind, self::READ_AND_WRITE_KINDS, \true);
        }
    }
    /**
     ...
     * @param Node $node
     */
    private function analyzeMethodVisibility(Method $method, Node $node) : void
    {
        if ($method->isPublic()) {
            return;
        }
        ...
        if ($method->isPrivate()) {
            $has_call_magic_method = !$method->isStatic() && $method->getDefiningClass($this->code_base)->hasMethodWithName($this->code_base, '__call', \true);
            $this->emitIssue($has_call_magic_method ? Issue::AccessMethodPrivateWithCallMagicMethod : Issue::AccessMethodPrivate, $node->lineno, (string) $method->getFQSEN(), $method->getFileRef()->getFile(), (string) $method->getFileRef()->getLineNumberStart());
        } else {
            if (Clazz::isAccessToElementOfThis($node)) {
                return;
            }
            $has_call_magic_method = !$method->isStatic() && $method->getDefiningClass($this->code_base)->hasMethodWithName($this->code_base, '__call', \true);
            $this->emitIssue($has_call_magic_method ? Issue::AccessMethodProtectedWithCallMagicMethod : Issue::AccessMethodProtected, $node->lineno, (string) $method->getFQSEN(), $method->getFileRef()->getFile(), (string) $method->getFileRef()->getLineNumberStart());
        }
    }
    /**
     ...
     * @param Node $node
     */
    private function analyzeCallToFunctionLike(FunctionInterface $method, Node $node) : void
    {
        $code_base = $this->code_base;
        $context = $this->context;
        ...
        ArgumentType::analyze($method, $node, $context, $code_base);
        ...
        foreach ($argument_list as $i => $argument) {
            ...
            if ($parameter->isPassByReference()) {
                self::analyzePassByReferenceArgument($code_base, $context, $argument, $argument_list, $method, $parameter, $method->getRealParameterForCaller($i), $i);
            }
        }
        ...
        // well.
        if ($context->isInFunctionLikeScope() && $method->getFQSEN() === $context->getFunctionLikeFQSEN()) {
        ...
        // being passed in.
        $this->analyzeMethodWithArgumentTypes($node->children['args'], $method);
    }
    /**
    ...
    private function createPassByReferenceArgumentInCall(FunctionInterface $method, Node $argument, Parameter $parameter, ?Parameter $real_parameter) : void
    {
        if ($argument->kind === \ast\AST_VAR) {
            ...
            // if it doesn't exist
            try {
                $variable = (new ContextNode($this->code_base, $this->context, $argument))->getOrCreateVariableForReferenceParameter($parameter, $real_parameter);
                ...
                if ($variable_union_type->hasRealTypeSet()) {
                    ...
                    if ($method->getContext()->isPHPInternal() && \in_array($parameter->getReferenceType(), [Parameter::REFERENCE_IGNORED, Parameter::REFERENCE_READ_WRITE], \true)) {
            ...
            if (\is_string($property_name)) {
                ...
                // if it doesn't exist
                try {
                    $property = (new ContextNode($this->code_base, $this->context, $argument))->getOrCreateProperty($property_name, $argument->kind === \ast\AST_STATIC_PROP);
                    $property->setHasWriteReference();
                } catch (IssueException $exception) {
                    Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
                } catch (Exception $_) {
     ...
     * @param ?Parameter $real_parameter the real parameter type from the type signature
     */
    private static function analyzePassByReferenceArgument(CodeBase $code_base, Context $context, Node $argument, array $argument_list, FunctionInterface $method, Parameter $parameter, ?Parameter $real_parameter, int $parameter_offset) : void
    {
        $variable = null;
        $kind = $argument->kind;
        if ($kind === \ast\AST_VAR) {
            try {
                $variable = (new ContextNode($code_base, $context, $argument))->getOrCreateVariableForReferenceParameter($parameter, $real_parameter);
            } catch (NodeException $_) {
                // E.g. `function_accepting_reference(${$varName})` - Phan can't analyze outer type of ${$varName}
                return;
            }
        } elseif ($kind === \ast\AST_STATIC_PROP || $kind === \ast\AST_PROP) {
            $property_name = $argument->children['prop'];
            if ($property_name instanceof Node) {
            ...
            if (\is_string($property_name)) {
                ...
                // if it doesn't exist
                try {
                    $variable = (new ContextNode($code_base, $context, $argument))->getOrCreateProperty($property_name, $argument->kind === \ast\AST_STATIC_PROP);
            ...
            switch ($parameter->getReferenceType()) {
                ...
                case Parameter::REFERENCE_READ_WRITE:
                    ...
                    if ($variable_type->isEmpty()) {
                        ...
                        $set_variable_type($variable->getUnionType()->withUnionType($reference_parameter_type));
                    }
     ...
     */
    private static function analyzeWriteOnlyReference(CodeBase $code_base, Context $context, FunctionInterface $method, Closure $set_variable_type, array $argument_list, Parameter $parameter) : void
    {
        switch ($method->getFQSEN()->__toString()) {
            case '\\preg_match':
                $set_variable_type(RegexAnalyzer::getPregMatchUnionType($code_base, $context, $argument_list));
                return;
            case '\\preg_match_all':
                $set_variable_type(RegexAnalyzer::getPregMatchAllUnionType($code_base, $context, $argument_list));
                return;
            default:
                $reference_parameter_type = $parameter->getNonVariadicUnionType();
    ...
    private function trackReferenceToClosure(Node $argument) : void
    {
        try {
            ...
            $method = (new ContextNode($this->code_base, $inner_context, $argument))->getClosure();
     ...
     */
    public function analyzeCallableWithArgumentTypes(array $argument_types, FunctionInterface $method, array $arguments = [], bool $erase_old_return_type = \false) : void
    {
        ...
        try {
            ...
            foreach ($parameter_list as $i => $parameter_clone) {
                ...
                $method->getInternalScope()->addVariable($parameter_clone->asNonVariadic());
                // If there's no parameter at that offset, we may be in
                // a ParamTooMany situation. That is caught elsewhere.
                if (!isset($argument_types[$i]) || !$parameter_clone->hasEmptyNonVariadicType()) {
                    continue;
                }
                $this->updateParameterTypeByArgument($method, $parameter_clone, $arguments[$i] ?? null, $argument_types, $parameter_list, $i);
            }
            foreach ($parameter_list as $parameter_clone) {
                if ($parameter_clone->isVariadic()) {
                    ...
                    $method->getInternalScope()->addVariable($parameter_clone->cloneAsNonVariadic());
                }
            }
     ...
     */
    private function analyzeMethodWithArgumentTypes(Node $argument_list_node, FunctionInterface $method) : void
    {
        $method = $this->findDefiningMethod($method);
        $original_method_scope = $method->getInternalScope();
        $method->setInternalScope(clone $original_method_scope);
        ...
        try {
            ...
            foreach ($argument_list_node->children as $i => $argument) {
                ...
                $argument_types[$i] = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $argument, \true)->withStaticResolvedInContext($this->context)->eraseRealTypeSetRecursively();
            }
            foreach ($parameter_list as $i => $parameter_clone) {
                $argument = $argument_list_node->children[$i] ?? null;
                if ($argument === null && $parameter_clone->hasDefaultValue()) {
                    $parameter_type = $parameter_clone->getDefaultValueType()->withRealTypeSet($parameter_clone->getNonVariadicUnionType()->getRealTypeSet());
                    if ($parameter_type->isType(NullType::instance(\false))) {
                        // Treat a parameter default of null the same way as passing null to that parameter
                        // (Add null to the list of possibilities)
                ...
                // Add the parameter to the scope
                // TODO: asNonVariadic()?
                $method->getInternalScope()->addVariable($parameter_clone);
                // If there's no parameter at that offset, we may be in
                // a ParamTooMany situation. That is caught elsewhere.
                if ($argument === null) {
                    continue;
                ...
                $this->updateParameterTypeByArgument($method, $parameter_clone, $argument, $argument_types, $parameter_list, $i);
            }
            foreach ($parameter_list as $parameter_clone) {
                if ($parameter_clone->isVariadic()) {
                    // We're using this parameter clone to analyze the **inside** of the method, it's never seen on the outside.
    ...
    private function checkForInfiniteRecursion(Node $node, FunctionInterface $method) : void
    {
        ...
        foreach ($this->parent_node_list as $c) {
            if (\in_array($c->kind, [\ast\AST_FUNC_DECL, \ast\AST_METHOD, \ast\AST_CLOSURE], \true)) {
        ...
        if (ReachabilityChecker::willUnconditionallyBeReached($nearest_function_like->children['stmts'], $argument_list_node)) {
            $this->emitIssue(Issue::InfiniteRecursion, $node->lineno, $method->getNameForIssue());
            return;
        }
    ...
    private function checkForInfiniteRecursionWithSameArgs(Node $node, FunctionInterface $method) : void
    {
        ...
        if (\count($argument_list_node->children) === 0) {
            $this->emitIssue(Issue::PossibleInfiniteRecursionSameParams, $node->lineno, $method->getNameForIssue());
            return;
        }
     ...
     * signature.
     */
    private function updateParameterTypeByArgument(FunctionInterface $method, Parameter $parameter, $argument, array $argument_types, array &$parameter_list, int $parameter_offset) : void
    {
        ...
        if (!$argument_type->isEmpty()) {
            ...
            if ($parameter->isCloneOfVariadic()) {
                ...
                if ($parameter->hasEmptyNonVariadicType()) {
                    $parameter->setUnionType($argument_type->withFlattenedArrayShapeOrLiteralTypeInstances()->asListTypes()->withRealTypeSet($parameter->getNonVariadicUnionType()->getRealTypeSet()));
                } else {
                    $parameter->addUnionType($argument_type->withFlattenedArrayShapeOrLiteralTypeInstances()->asListTypes()->withRealTypeSet($parameter->getNonVariadicUnionType()->getRealTypeSet()));
                }
            } else {
                $parameter->addUnionType(($method instanceof Func && $method->isClosure() ? $argument_type : $argument_type->withFlattenedArrayShapeOrLiteralTypeInstances())->withRealTypeSet($parameter->getNonVariadicUnionType()->getRealTypeSet()));
            }
        ...
        if ($argument->kind === \ast\AST_VAR) {
            try {
                $variable = (new ContextNode($this->code_base, $this->context, $argument))->getOrCreateVariableForReferenceParameter($parameter, $method->getRealParameterForCaller($parameter_offset));
            } catch (NodeException $_) {
                // Could not figure out the node name
                return;
            }
        } elseif (\in_array($argument->kind, [\ast\AST_STATIC_PROP, \ast\AST_PROP], \true)) {
            try {
                $variable = (new ContextNode($this->code_base, $this->context, $argument))->getProperty($argument->kind === \ast\AST_STATIC_PROP);
            } catch (IssueException|NodeException $_) {
        ...
        $pass_by_reference_variable = new PassByReferenceVariable($parameter, $variable, $this->code_base, $this->context);
    ...
    private static function checkPassingPropertyByReference(CodeBase $code_base, Context $context, FunctionInterface $method, Parameter $parameter, Node $argument, Property $property, int $parameter_offset) : void
    {
        ...
        if ($expr_node instanceof Node && $expr_node->kind === \ast\AST_VAR && $expr_node->children['name'] === 'this') {
        ...
        if ($property_type->hasRealTypeSet()) {
            ...
            if (!$parameter_type->canCastToDeclaredType($code_base, $context, $property_type)) {
                Issue::maybeEmit($code_base, $context, Issue::TypeMismatchArgumentPropertyReferenceReal, $argument->lineno, $parameter_offset, $property->getRepresentationForIssue(), $property_type, self::toDetailsForRealTypeMismatch($property_type), $method->getRepresentationForIssue(), $parameter_type, self::toDetailsForRealTypeMismatch($parameter_type));
                return;
            }
        }
        if ($parameter_type->canCastToDeclaredType($code_base, $context, $property_type)) {
        ...
        Issue::maybeEmit($code_base, $context, Issue::TypeMismatchArgumentPropertyReference, $argument->lineno, $parameter_offset, $property->getRepresentationForIssue(), $property_type, $method->getRepresentationForIssue(), $parameter_type);
    }
     ...
     */
    private function warnNoopNew(Node $node, array $class_list) : void
    {
        $has_constructor_or_destructor = \count($class_list) === 0;
        foreach ($class_list as $class) {
            if ($class->getPhanFlagsHasState(\SdgScoped\Phan\Language\Element\Flags::IS_CONSTRUCTOR_USED_FOR_SIDE_EFFECTS)) {
                return;
            }
        }
        foreach ($class_list as $class) {
            if ($class->hasMethodWithName($this->code_base, '__construct', \true)) {
                $constructor = $class->getMethodByName($this->code_base, '__construct');
                if (!$constructor->getPhanFlagsHasState(\SdgScoped\Phan\Language\Element\Flags::IS_FAKE_CONSTRUCTOR)) {
                    if (!self::hasEmptyImplementation($constructor)) {
                        $has_constructor_or_destructor = \true;
                        break;
                    }
                }
            }
            if ($class->hasMethodWithName($this->code_base, '__destruct', \true)) {
        ...
        $this->emitIssue($has_constructor_or_destructor ? Issue::NoopNew : Issue::NoopNewNoSideEffects, $node->lineno, ASTReverter::toShortString($node));
    }
    public const LOOP_SCOPE_KINDS = [\ast\AST_FOR => \true, \ast\AST_FOREACH => \true, \ast\AST_WHILE => \true, \ast\AST_DO_WHILE => \true, \ast\AST_SWITCH => \true];
    /**
        ...
        for ($iter_node = \end($nodes); $iter_node instanceof Node; $iter_node = \prev($nodes)) {
            switch ($iter_node->kind) {
                ...
                case \ast\AST_SWITCH:
                    $depth--;
                    if ($depth <= 0) {
                        $this->emitIssue(Issue::ContinueTargetingSwitch, $node->lineno);
                        return $this->context;
                    }
                    break;
            }
    ...
    public function visitLabel(Node $node) : Context
    {
        ...
        if (!isset($used_labels[$label])) {
            $this->emitIssue(Issue::UnusedGotoLabel, $node->lineno, $label);
        }
        return $this->context;
    }
    private function warnBreakOrContinueWithoutLoop(Node $node) : void
    {
        ...
        if ($depth !== 1) {
            $this->emitIssue(Issue::ContinueOrBreakTooManyLevels, $node->lineno, $name, $depth);
            return;
        }
        $this->emitIssue(Issue::ContinueOrBreakNotInLoop, $node->lineno, $name);
    }
    /**
     * @param Node $node
    ...
    {
        if ($class->isClass() && ($class->getElementNamespace() ?: "\\") === "\\" && \strcasecmp($class->getName(), $method->getName()) === 0 && $class->hasMethodWithName($this->code_base, "__construct", \false)) {
            try {
                $constructor = $class->getMethodByName($this->code_base, "__construct");
                ...
                if ($constructor->getPhanFlagsHasState(\SdgScoped\Phan\Language\Element\Flags::IS_FAKE_CONSTRUCTOR)) {
                    Issue::maybeEmit($this->code_base, $this->context, Issue::CompatiblePHP8PHP4Constructor, $this->context->getLineNumberStart(), $method->getRepresentationForIssue());
                }
            } catch (CodeBaseException $_) {
    ...
    public function visitThrow(Node $node) : Context
    {
        ...
        if ($parent_node->kind !== \ast\AST_STMT_LIST) {
            if (Config::get_closest_minimum_target_php_version_id() < 80000) {
                $this->emitIssue(Issue::CompatibleThrowExpression, $parent_node->lineno, ASTReverter::toShortString($parent_node));
            }
        }
        return $this->context;
    }
