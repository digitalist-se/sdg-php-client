...
class DuplicateClassAnalyzer
{
     ...
     */
    public static function analyzeDuplicateClass(CodeBase $code_base, Clazz $clazz) : void
    {
        ...
        if ($original_class->isPHPInternal()) {
            ...
            if ($clazz->getFQSEN()->getAlternateId() === 1) {
                if (!$original_class->checkHasSuppressIssueAndIncrementCount(Issue::RedefineClass)) {
                    Issue::maybeEmit($code_base, $original_class->getContext(), Issue::RedefineClass, $original_class->getFileRef()->getLineNumberStart(), (string) $original_class, $original_class->getFileRef()->getFile(), $original_class->getFileRef()->getLineNumberStart(), (string) $clazz, $clazz->getFileRef()->getFile(), $clazz->getFileRef()->getLineNumberStart());
                }
            }
        }
        return;
