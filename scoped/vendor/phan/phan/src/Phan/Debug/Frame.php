...
namespace SdgScoped\Phan\Debug;
...
class Frame
{
    ...
    public static function encodeValue($value, int $max_depth = 2) : string
    {
        if (is_object($value)) {
            ...
            if ($value instanceof AddressableElement || $value instanceof UnaddressableTypedElement || $value instanceof UnionType || $value instanceof Context || $value instanceof Type) {
                return get_class($value) . '(' . $value . ')';
            }
    ...
    {
        return \SdgScoped\with_disabled_phan_error_handler(static function () use($frame) : string {
