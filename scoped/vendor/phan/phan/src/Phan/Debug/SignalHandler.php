...
class SignalHandler
{
    ...
    public static function init() : void
    {
        ...
        $handler = static function (int $signo, $unused_signinfo) : void {
            ...
            if ($signo === \SIGINT) {
                \SdgScoped\phan_error_handler(\E_ERROR, "Phan was interrupted with SIGINT, exiting\n", $trace[0]['file'] ?? 'unknown file', $trace[0]['line'] ?? 0);
                return;
            }
            $print_variables = $signo === \SIGUSR2;
            // @phan-suppress-next-line PhanPluginRemoveDebugCall
            \fprintf(\STDERR, "Phan was interrupted with %s, printing debug information then continuing\n", $signo === \SIGUSR2 ? "SIGUSR2" : "SIGUSR1");
            \SdgScoped\phan_print_backtrace($print_variables);
        };
