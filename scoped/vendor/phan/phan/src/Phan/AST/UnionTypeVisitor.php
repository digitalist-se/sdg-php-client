...
use ast\Node;
use Closure;
use SdgScoped\Phan\Analysis\AssignOperatorFlagVisitor;
use SdgScoped\Phan\Analysis\BinaryOperatorFlagVisitor;
use SdgScoped\Phan\Analysis\BlockExitStatusChecker;
use SdgScoped\Phan\Analysis\ConditionVisitor;
use SdgScoped\Phan\Analysis\NegatedConditionVisitor;
use SdgScoped\Phan\AST\Visitor\Element;
use SdgScoped\Phan\CodeBase;
use SdgScoped\Phan\Config;
use SdgScoped\Phan\Debug;
...
use SdgScoped\Phan\Exception\InvalidFQSENException;
...
use SdgScoped\Phan\Exception\UnanalyzableException;
use SdgScoped\Phan\Exception\UnanalyzableMagicPropertyException;
...
use SdgScoped\Phan\Language\FQSEN\FullyQualifiedFunctionLikeName;
use SdgScoped\Phan\Language\FQSEN\FullyQualifiedFunctionName;
use SdgScoped\Phan\Language\FQSEN\FullyQualifiedGlobalStructuralElement;
use SdgScoped\Phan\Language\FQSEN\FullyQualifiedMethodName;
use SdgScoped\Phan\Language\Scope\BranchScope;
use SdgScoped\Phan\Language\Scope\GlobalScope;
use SdgScoped\Phan\Language\Type;
use SdgScoped\Phan\Language\Type\ArrayShapeType;
use SdgScoped\Phan\Language\Type\ArrayType;
use SdgScoped\Phan\Language\Type\AssociativeArrayType;
use SdgScoped\Phan\Language\Type\BoolType;
use SdgScoped\Phan\Language\Type\CallableType;
use SdgScoped\Phan\Language\Type\ClassStringType;
use SdgScoped\Phan\Language\Type\ClosureType;
use SdgScoped\Phan\Language\Type\FalseType;
use SdgScoped\Phan\Language\Type\FloatType;
use SdgScoped\Phan\Language\Type\GenericArrayType;
use SdgScoped\Phan\Language\Type\IntType;
use SdgScoped\Phan\Language\Type\IterableType;
use SdgScoped\Phan\Language\Type\ListType;
use SdgScoped\Phan\Language\Type\LiteralIntType;
...
use SdgScoped\Phan\Language\Type\SelfType;
use SdgScoped\Phan\Language\Type\StaticOrSelfType;
use SdgScoped\Phan\Language\Type\StaticType;
...
use SdgScoped\Phan\Language\UnionTypeBuilder;
...
class UnionTypeVisitor extends AnalysisVisitor
{
     ...
     * instead of emitted as issues to the log.
     */
    public function __construct(CodeBase $code_base, Context $context, bool $should_catch_issue_exception = \true)
    {
     ...
     */
    public static function unionTypeFromNode(CodeBase $code_base, Context $context, $node, bool $should_catch_issue_exception = \true) : UnionType
    {
        ...
        if ($should_catch_issue_exception) {
            try {
                $union_type = (new self($code_base, $context, \true))->{Element::VISIT_LOOKUP_TABLE[$node->kind] ?? 'visit'}($node);
                $context->setCachedUnionTypeOfNode($node_id, $union_type, \true);
                return $union_type;
        ...
        $union_type = (new self($code_base, $context, \false))->{Element::VISIT_LOOKUP_TABLE[$node->kind] ?? 'visit'}($node);
    ...
    public function visitPostInc(Node $node) : UnionType
    {
        ...
        return self::unionTypeFromNode($this->code_base, $this->context, $node->children['var'])->asNonLiteralType();
    }
    /**
    ...
    public function visitPreDec(Node $node) : UnionType
    {
        // Real types aren't certain, since this doesn't throw even for object or array types
        // TODO: Check if union type is sane (string/int)
        return self::unionTypeFromNode($this->code_base, $this->context, $node->children['var'])->asNonLiteralType()->getTypeAfterIncOrDec();
    }
    /**
    ...
    public function visitClone(Node $node) : UnionType
    {
        ...
        $type = self::unionTypeFromNode($this->code_base, $this->context, $node->children['expr'])->objectTypes();
        if ($type->isEmpty()) {
    ...
    public const MAGIC_CONST_NAME_MAP = [\ast\flags\MAGIC_LINE => '__LINE__', \ast\flags\MAGIC_FILE => '__FILE__', \ast\flags\MAGIC_DIR => '__DIR__', \ast\flags\MAGIC_NAMESPACE => '__NAME__', \ast\flags\MAGIC_FUNCTION => '__FUNCTION__', \ast\flags\MAGIC_METHOD => '__METHOD__', \ast\flags\MAGIC_CLASS => '__CLASS__', \ast\flags\MAGIC_TRAIT => '__TRAIT__'];
    private function warnAboutUndeclaredMagicConstant(Node $node, string $details) : void
    {
        $this->emitIssue(Issue::UndeclaredMagicConstant, $node->lineno, self::MAGIC_CONST_NAME_MAP[$node->flags], $details);
    }
    /**
        ...
        switch ($flags) {
            ...
            case \ast\flags\MAGIC_FUNCTION:
                if ($this->context->isInFunctionLikeScope()) {
                    ...
                    if ($fqsen instanceof FullyQualifiedMethodName) {
                        ...
                        if ($fqsen->isClosure()) {
                            $this->emitIssue(Issue::SuspiciousMagicConstant, $node->lineno, '__FUNCTION__', "used inside of a closure instead of a function/method - the value is always '{closure}'");
                            $value = '{closure}';
                        } else {
            ...
            case \ast\flags\MAGIC_METHOD:
                if ($this->context->isInFunctionLikeScope()) {
                    ...
                    $fqsen = $this->context->getFunctionLikeFQSEN();
                    if (!$fqsen instanceof FullyQualifiedMethodName) {
                        $this->emitIssue(Issue::SuspiciousMagicConstant, $node->lineno, '__METHOD__', 'used inside of a function/closure instead of a method');
                    }
    ...
    {
        return StringType::instance(\true)->asRealUnionType();
    }
    /**
    ...
    public static function findParentType(Context $context, CodeBase $code_base) : ?Type
    {
        if (!$context->isInClassScope()) {
            throw new IssueException(Issue::fromType(Issue::ContextNotObject)($context->getFile(), $context->getLineNumberStart(), ['parent']));
        }
        ...
        if (!$class->isTrait()) {
            Issue::maybeEmit($code_base, $context, Issue::ParentlessClass, $context->getLineNumberStart(), (string) $class->getFQSEN());
        }
        return null;
    }
    /**
    ...
    public function visitName(Node $node) : UnionType
    {
        ...
        try {
            if ($node->flags & \ast\flags\NAME_NOT_FQ) {
                ...
                return Type::fromStringInContext($name, $this->context, Type::FROM_NODE)->asRealUnionType();
            }
            if ($node->flags & \ast\flags\NAME_RELATIVE) {
                // $x = new namespace\Foo();
                $name = \rtrim($this->context->getNamespace(), '\\') . '\\' . $name;
                return Type::fromFullyQualifiedString($name)->asRealUnionType();
            }
            // Sometimes 0 for a fully qualified name?
            return Type::fromFullyQualifiedString('\\' . $name)->asRealUnionType();
        } catch (FQSENException $e) {
            $this->emitIssue($e instanceof EmptyFQSENException ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike, $node->lineno, $e->getFQSEN());
            return UnionType::empty();
        }
    }
    /**
    ...
    public function visitType(Node $node) : UnionType
    {
        switch ($node->flags) {
            ...
            default:
                \SdgScoped\Phan\Debug::printNode($node);
                throw new AssertionError("All flags must match. Found ({$node->flags}) " . Debug::astFlagDescription($node->flags ?? 0, $node->kind));
        }
    }
    /**
    ...
    public function visitNullableType(Node $node) : UnionType
    {
        ...
        return $union_type->asMappedUnionType(static function (Type $type) : Type {
            return $type->withIsNullable(\true);
        });
    }
    /**
    ...
    public static function unionTypeFromLiteralOrConstant(CodeBase $code_base, Context $context, $node) : ?UnionType
    {
        if ($node instanceof Node) {
            ...
            if (\in_array($node->kind, [\ast\AST_CONST, \ast\AST_CLASS_CONST, \ast\AST_CLASS_NAME], \true)) {
                try {
    ...
    public function visitConditional(Node $node) : UnionType
    {
        ...
        if ($cond_truthiness !== null) {
            ...
            if ($cond_truthiness) {
                ...
                return UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $true_node);
            } else {
                // The condition is unconditionally false
                // Add the type for the 'false' side
                return UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['false']);
            }
        }
        if ($true_node !== $cond_node) {
            // Visit the condition to check for undefined variables.
            UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $cond_node);
        }
        ...
        if ($cond_node instanceof Node) {
            ...
            $true_context = (new ConditionVisitor($this->code_base, isset($node->children['true']) ? $base_context : $this->context))->__invoke($cond_node);
            $false_context = (new NegatedConditionVisitor($this->code_base, $base_context))->__invoke($cond_node);
            if (!isset($node->children['true'])) {
                $true_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $true_context, $true_node);
                $false_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $false_context, $false_node);
                ...
                if ($true_type_is_empty xor $false_type->isEmpty()) {
                    $union_type = $union_type->withType(MixedType::instance(\false));
                }
                return $union_type;
            }
        } else {
        ...
        $true_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $true_context, $true_node);
        $false_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $false_context, $node->children['false']);
        ...
        if (\count($children) > 0) {
            ...
            foreach ($children as $child) {
                ...
                if ($child->kind === \ast\AST_UNPACK) {
                    ...
                    $new_union_type = $this->analyzeUnpack($child, \true);
                    $value_types_builder->addUnionType($new_union_type);
                    $record_real_union_type($new_union_type);
                    continue;
                }
                $is_definitely_non_empty = \true;
                $value = $child->children['value'];
                $has_key = $has_key || isset($child->children['key']);
                if ($value instanceof Node) {
                    $element_value_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $value, $this->should_catch_issue_exception);
                    if ($element_value_type->isEmpty()) {
                        $value_types_builder->addType(MixedType::instance(\false));
                        $real_value_types_builder = null;
                    } else {
                        if ($element_value_type->isVoidType()) {
                            $this->emitIssue(Issue::TypeVoidExpression, $node->lineno, ASTReverter::toShortString($value));
                        }
    ...
    private function arrayTypeFromRealTypeBuilder(?UnionTypeBuilder $builder, Node $node, bool $has_key) : array
    {
        ...
        foreach ($builder->getTypeSet() as $type) {
            if ($has_key) {
                ...
                $real_types[] = GenericArrayType::fromElementType($type, \false, $has_int_keys ? GenericArrayType::KEY_INT : GenericArrayType::KEY_MIXED);
            } else {
    ...
    {
        return (new BinaryOperatorFlagVisitor($this->code_base, $this->context, $this->should_catch_issue_exception))->__invoke($node);
    }
    /**
    ...
    {
        return (new AssignOperatorFlagVisitor($this->code_base, $this->context))->__invoke($node);
    }
    /**
    ...
    public function visitCast(Node $node) : UnionType
    {
        ...
        if ($expr_type->isVoidType()) {
            $this->emitIssue(Issue::TypeVoidExpression, $expr->lineno ?? $node->lineno, ASTReverter::toShortString($expr));
        }
        switch ($node->flags) {
            case \ast\flags\TYPE_NULL:
                return NullType::instance(\false)->asRealUnionType();
    ...
    public function visitNew(Node $node) : UnionType
    {
        ...
        if (!$class_node instanceof Node) {
            $this->emitIssue(Issue::InvalidNode, $node->lineno, "Invalid ClassName for new ClassName()");
        ...
        $type_set = \array_map(function (Type $type) use($node) : Type {
            ...
            $arg_type_list = \array_map(function ($arg_node) : UnionType {
                return UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $arg_node);
    ...
    public function visitInstanceOf(Node $node) : UnionType
    {
        ...
        if (!$type->isEmpty() && $type->objectTypesWithKnownFQSENs()->isEmpty()) {
            if ($class_node->kind === \ast\AST_NAME || !$type->hasStringType()) {
                Issue::maybeEmit($code_base, $context, Issue::TypeInvalidInstanceof, $context->getLineNumberStart(), ASTReverter::toShortString($class_node), (string) $type);
            }
        }
        return BoolType::instance(\false)->asRealUnionType();
    }
    ...
    {
        $union_type = self::unionTypeFromNode($this->code_base, $this->context, $node->children['expr'], $this->should_catch_issue_exception)->withStaticResolvedInContext($this->context);
        if ($union_type->isEmpty()) {
            return UnionType::empty();
        }
        ...
        static $simple_xml_element_type;
        // SimpleXMLElement doesn't `implement` ArrayAccess, but can be accessed that way. See #542
        static $null_type;
        static $string_type;
        ...
        if ($array_access_type === null) {
            ...
            $simple_xml_element_type = Type::fromNamespaceAndName('\\', 'SimpleXMLElement', \false);
        ...
        if (self::hasArrayShapeOrList($union_type)) {
            ...
            if ($element_type !== null) {
                if ($element_type->isPossiblyUndefined() && !($node->flags & PhanAnnotationAdder::FLAG_IGNORE_UNDEF)) {
                    $this->emitIssue(Issue::TypePossiblyInvalidDimOffset, $node->lineno, ASTReverter::toShortString($node->children['dim']), ASTReverter::toShortString($node->children['expr']), $union_type);
        ...
        $dim_type = self::unionTypeFromNode($this->code_base, $this->context, $node->children['dim'], \true);
        // Figure out what the types of accessed array
        // elements would be.
        $generic_types = $union_type->genericArrayElementTypes(\true);
        ...
        if (!$generic_types->isEmpty()) {
            ...
            if (!($node->flags & self::FLAG_IGNORE_NULLABLE) && $union_type->containsNonMixedNullable()) {
                $this->emitIssue(Issue::TypeArraySuspiciousNullable, $node->lineno, ASTReverter::toShortString($node->children['expr']), (string) $union_type);
            }
            if (!$dim_type->isEmpty()) {
                try {
                ...
                if ($should_check) {
                    if (Config::getValue('scalar_array_key_cast')) {
                        ...
                        $expected_key_type = GenericArrayType::unionTypeForKeyType(GenericArrayType::keyTypeFromUnionTypeKeys($union_type), GenericArrayType::CONVERT_KEY_MIXED_TO_INT_OR_STRING_UNION_TYPE);
                    }
                    if (!$dim_type->canCastToUnionType($expected_key_type)) {
                        ...
                        if ($this->should_catch_issue_exception) {
                            $this->emitIssue($issue_type, $node->lineno, (string) $union_type, (string) $dim_type, (string) $expected_key_type);
                            return $generic_types;
                        }
                        throw new IssueException(Issue::fromType($issue_type)($this->context->getFile(), $node->lineno, [(string) $union_type, (string) $dim_type, (string) $expected_key_type]));
                    }
                }
            }
            return $generic_types;
        ...
        if ($union_type->isType($null_type)) {
            if (!($node->flags & self::FLAG_IGNORE_NULLABLE)) {
                $this->emitIssue(Issue::TypeArraySuspiciousNull, $node->lineno, ASTReverter::toShortString($node->children['expr']));
            }
        ...
        if ($union_type->isNonNullStringType() || $union_type->canCastToUnionType($string_union_type) && !$union_type->hasMixedType()) {
            ...
            if (!$dim_type->isEmpty() && !$dim_type->canCastToUnionType($int_union_type)) {
                ...
                if (!$union_type->isEmpty() && !$union_type->asExpandedTypes($this->code_base)->hasArrayLike()) {
                    $this->emitIssue(Issue::TypeMismatchDimFetch, $node->lineno, $union_type, (string) $dim_type, $int_union_type);
                }
            }
        ...
        if ($element_types->isEmpty()) {
            ...
            try {
                foreach ($union_type->asClassList($this->code_base, $this->context) as $class) {
                    ...
                    if ($expanded_types->hasType($array_access_type) || $expanded_types->hasType($simple_xml_element_type)) {
            ...
            if (!$union_type->hasArrayLike() && !$union_type->hasMixedType()) {
                $this->emitIssue(Issue::TypeArraySuspicious, $node->lineno, ASTReverter::toShortString($node->children['expr']), (string) $union_type);
                return $element_types;
            }
    ...
    private function checkIsValidStringOffset(UnionType $union_type, Node $node, UnionType $dim_type) : void
    {
        ...
        $exception = new IssueException(Issue::fromType(Issue::TypeInvalidDimOffset)($this->context->getFile(), $node->children['dim']->lineno ?? $node->lineno, [$dim_type, ASTReverter::toShortString($node->children['expr']), (string) $union_type]));
        if ($this->should_catch_issue_exception) {
    ...
    private function resolveArrayShapeElementTypes(Node $node, UnionType $union_type) : ?UnionType
    {
        ...
        if ($check_invalid_dim && $union_type->hasRealTypeSet()) {
            ...
            if (!$has_non_empty_array) {
                $exception = new IssueException(Issue::fromType(Issue::TypeInvalidDimOffset)($this->context->getFile(), $dim_node->lineno ?? $node->lineno, [is_scalar($dim_value) ? StringUtil::jsonEncode($dim_value) : ASTReverter::toShortString($dim_value), ASTReverter::toShortString($node->children['expr']), (string) $union_type]));
                if ($this->should_catch_issue_exception) {
                    Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
                    return null;
                } else {
        ...
        if ($resulting_element_type === \false) {
            ...
            if ($check_invalid_dim) {
                $exception = new IssueException(Issue::fromType(Issue::TypeInvalidDimOffset)($this->context->getFile(), $dim_node->lineno ?? $node->lineno, [StringUtil::jsonEncode($dim_value), ASTReverter::toShortString($node->children['expr']), (string) $union_type]));
                if ($this->should_catch_issue_exception) {
                    Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
                } else {
                    throw $exception;
    ...
    public static function resolveArrayShapeElementTypesForOffset(UnionType $union_type, $dim_value, bool $is_computing_real_type_set = \false)
    {
        ...
        if ($has_string || $has_generic_array) {
            ...
            if ($resulting_element_type->hasRealTypeSet()) {
                $resulting_element_type = UnionType::of($resulting_element_type->getTypeSet(), \array_map(static function (Type $type) : Type {
                    return $type->withIsNullable(\true);
        ...
        if (!$is_computing_real_type_set) {
            $resulting_real_element_type = self::resolveArrayShapeElementTypesForOffset($union_type->getRealUnionType(), $dim_value, \true);
            return $resulting_element_type->withRealTypeSet(\is_object($resulting_real_element_type) ? $resulting_real_element_type->getRealTypeSet() : []);
        }
        return $resulting_element_type;
    }
    /**
        ...
        try {
            if ($generic_types->isEmpty()) {
                if (!$union_type->asExpandedTypes($this->code_base)->hasIterable() && !$union_type->hasTypeMatchingCallback(static function (Type $type) : bool {
                    ...
                    throw new IssueException(Issue::fromType(Issue::TypeMismatchUnpackValue)($this->context->getFile(), $node->lineno, [(string) $union_type]));
                }
                return $generic_types;
            }
    ...
    private function checkInvalidUnpackKeyType(Node $node, UnionType $union_type, bool $is_array_spread) : void
    {
            ...
            foreach ($union_type->getTypeSet() as $type) {
                if ($type->isIterable()) {
                    if ($type instanceof AssociativeArrayType) {
                        $is_invalid_because_associative = \true;
                    } else {
                        $is_invalid_because_associative = \false;
                        break;
                    }
        ...
        if ($is_invalid_because_associative || !$key_type->isEmpty() && !$key_type->hasTypeMatchingCallback(static function (Type $type) : bool {
            ...
            throw new IssueException(Issue::fromType($is_array_spread ? Issue::TypeMismatchUnpackKeyArraySpread : Issue::TypeMismatchUnpackKey)($this->context->getFile(), $node->lineno, [(string) $union_type, $key_type]));
        }
    }
    /**
    ...
    public function visitClosure(Node $node) : UnionType
    {
        ...
        // at its definition
        $closure_fqsen = FullyQualifiedFunctionName::fromClosureInContext($this->context, $node);
        if ($this->code_base->hasFunctionWithFQSEN($closure_fqsen)) {
            $func = $this->code_base->getFunctionByFQSEN($closure_fqsen);
        } else {
            $func = null;
        }
        return ClosureType::instanceWithClosureFQSEN($closure_fqsen, $func)->asRealUnionType();
    }
    /**
    ...
    public function visitVar(Node $node) : UnionType
    {
        ...
        $variable_name = (string) $name_node;
                ...
                if ($this->context->isInGlobalScope()) {
                    ...
                    if ($this->should_catch_issue_exception) {
                        if (!Config::getValue('ignore_undeclared_variables_in_global_scope')) {
                            $this->emitIssue(Issue::PossiblyUndeclaredGlobalVariable, $node->lineno, $variable_name);
                        }
                    }
                } else {
                    if ($this->should_catch_issue_exception) {
                        $this->emitIssue(Issue::PossiblyUndeclaredVariable, $node->lineno, $variable_name);
                    }
                }
            }
            return $union_type;
        ...
        if (!($this->context->isInGlobalScope() && Config::getValue('ignore_undeclared_variables_in_global_scope'))) {
            if (!$this->should_catch_issue_exception) {
                throw new IssueException(Issue::fromType(Variable::chooseIssueForUndeclaredVariable($this->context, $variable_name))($this->context->getFile(), $node->lineno, [$variable_name], IssueFixSuggester::suggestVariableTypoFix($this->code_base, $this->context, $variable_name)));
            }
            Issue::maybeEmitWithParameters($this->code_base, $this->context, Variable::chooseIssueForUndeclaredVariable($this->context, $variable_name), $node->lineno, [$variable_name], IssueFixSuggester::suggestVariableTypoFix($this->code_base, $this->context, $variable_name));
        }
        if ($variable_name === 'this') {
            return ObjectType::instance(\false)->asRealUnionType();
        }
    ...
    public function visitEncapsList(Node $node) : UnionType
    {
        ...
        foreach ($node->children as $part) {
            $part_string = $part instanceof Node ? UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $part)->asSingleScalarValueOrNullOrSelf() : $part;
    ...
    public function visitClass(Node $node) : UnionType
    {
        ...
        return FullyQualifiedClassName::fromStringInContext($class_name, $this->context)->asType()->asRealUnionType();
    }
    /**
    ...
    public function visitClassName(Node $node) : UnionType
    {
        $class_node = $node->children['class'];
        try {
            $class_list = (new ContextNode($this->code_base, $this->context, $class_node))->getClassList(\false, ContextNode::CLASS_LIST_ACCEPT_OBJECT_OR_CLASS_NAME);
        } catch (IssueException $exception) {
            if ($this->should_catch_issue_exception) {
                Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
            ...
            $new_exception = new IssueException(Issue::fromType(Issue::UndeclaredClassReference)($this->context->getFile(), $node->lineno, [(string) $exception_fqsen], IssueFixSuggester::suggestSimilarClassForGenericFQSEN($this->code_base, $this->context, $exception_fqsen)));
            if ($this->should_catch_issue_exception) {
                Issue::maybeEmitInstance($this->code_base, $this->context, $new_exception->getIssueInstance());
                return LiteralStringType::instanceForValue(\ltrim($exception_fqsen->__toString(), '\\'), \false)->asRealUnionType();
            }
            throw $new_exception;
        }
        if (!$class_list) {
        ...
        foreach ($class_list as $class) {
            $types[] = LiteralStringType::instanceForValue(\ltrim($class->getFQSEN()->__toString(), '\\'), \false);
        }
    ...
    private function analyzeProp(Node $node, bool $is_static) : UnionType
    {
        ...
        try {
            ...
            if ($property->isWriteOnly()) {
                $this->emitIssue($property->isFromPHPDoc() ? Issue::AccessWriteOnlyMagicProperty : Issue::AccessWriteOnlyProperty, $node->lineno, $property->asPropertyFQSENString(), $property->getContext()->getFile(), $property->getContext()->getLineNumberStart());
            }
            ...
            if ($union_type->hasTemplateTypeRecursive()) {
                ...
                $expression_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $expr_node);
            ...
            if ($union_type->isEmptyArrayShape() && $property->getPHPDocUnionType()->isEmpty()) {
                return UnionType::of([ArrayType::instance($union_type->containsNullable())], $property->getRealUnionType()->getTypeSet());
            }
            return $union_type;
        } catch (IssueException $exception) {
            Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
            ...
            if ($exception_fqsen instanceof FullyQualifiedClassName && $this->code_base->hasClassWithFQSEN($exception_fqsen)) {
                ...
                $suggestion = IssueFixSuggester::suggestSimilarProperty($this->code_base, $this->context, $suggestion_class, $property_name, \false);
            }
            $this->emitIssueWithSuggestion(Issue::UndeclaredProperty, $node->lineno, ["{$exception_fqsen}->{$property_name}"], $suggestion);
        ...
        if (\is_string($property_name) && $expr_node instanceof Node && $expr_node->kind === \ast\AST_VAR && $expr_node->children['name'] === 'this') {
    ...
    private function warnIfPossiblyUndefinedProperty(Node $node, string $prop_name, UnionType $union_type) : void
    {
        ...
        $this->emitIssue(Issue::PossiblyUnsetPropertyOfThis, $node->lineno, '$this->' . $prop_name);
    }
    /**
    ...
    public function visitCall(Node $node) : UnionType
    {
        ...
        foreach ($function_list_generator as $function) {
            $function->analyzeReturnTypes($this->code_base);
            // For daemon/server mode, call this to consistently ensure accurate return types.
    ...
    public function visitMethodCall(Node $node) : UnionType
    {
        ...
        try {
            ...
            if (!$class_node instanceof Node) {
                ...
                $this->emitIssue(Issue::NonClassMethodCall, $node->lineno, $method_name, UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $class_node));
                return UnionType::empty();
            }
            $combined_union_type = null;
            foreach ($this->classListFromNode($class_node) as $class) {
                if (!$class->hasMethodWithName($this->code_base, $method_name, \true)) {
                    continue;
                }
                try {
                    $method = $class->getMethodByName($this->code_base, $method_name);
                    $method->analyzeReturnTypes($this->code_base);
                    // For daemon/server mode, call this to consistently ensure accurate return types.
                    if ($method->hasTemplateType()) {
                        try {
                            $method = $method->resolveTemplateType($this->code_base, UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $class_node));
                        } catch (RecursionDepthException $_) {
                        }
                    }
                    ...
                    if (isset($node->children['expr']) && $union_type->hasTemplateTypeRecursive()) {
                        ...
                        $expression_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['expr']);
                        // Map template types to concrete types
                        $union_type = $union_type->withTemplateParameterTypeMap($expression_type->getTemplateParameterTypeMap($this->code_base));
                    }
                    // Resolve any references to `static` or `static[]`
                    if ($this->context->isInClassScope() && $static_class_node instanceof Node && $static_class_node->kind === \ast\AST_NAME && \strcasecmp($static_class_node->children['name'], 'parent') === 0) {
            ...
            $exception_fqsen = $exception->getFQSEN();
            $this->emitIssueWithSuggestion(Issue::UndeclaredClassMethod, $node->lineno, [$method_name, (string) $exception->getFQSEN()], $exception_fqsen instanceof FullyQualifiedClassName ? IssueFixSuggester::suggestSimilarClassForMethod($this->code_base, $this->context, $exception_fqsen, $method_name, $node->kind === \ast\AST_STATIC_CALL) : null);
        }
    ...
    {
        $result = self::unionTypeFromNode($this->code_base, $this->context, $node->children['expr']);
        ...
        if ($flags === \ast\flags\UNARY_MINUS) {
            $this->warnAboutInvalidUnaryOp($node, static function (Type $type) : bool {
                return $type->isValidNumericOperand();
            }, $result, '-', Issue::TypeInvalidUnaryOperandNumeric);
            ...
            }, $result, '~', Issue::TypeInvalidUnaryOperandBitwiseNot);
     ...
     */
    private function warnAboutInvalidUnaryOp(Node $node, Closure $is_valid_type, UnionType $type, string $operator, string $issue_type) : void
    {
        if ($type->isEmpty()) {
            return;
        }
        if (!$type->hasTypeMatchingCallback($is_valid_type)) {
            $this->emitIssue($issue_type, $node->children['left']->lineno ?? $node->lineno, $operator, $type);
        }
    }
    /**
    ...
    private function visitClassNameNode(Node $node) : UnionType
    {
        ...
        if ($kind === \ast\AST_CLASS && $node->flags & \ast\flags\CLASS_ANONYMOUS) {
        ...
        if (!$this->context->isInClassScope()) {
            $this->emitIssue(Issue::ContextNotObject, $node->lineno, $class_name);
            return UnionType::empty();
        }
        ...
        if (\strcasecmp($class_name, 'parent') === 0) {
            ...
            $parent_type_option = $class->getParentTypeOption();
            if (!$parent_type_option->isDefined()) {
                $this->emitIssue(Issue::ParentlessClass, $node->lineno, (string) $class->getFQSEN());
                return UnionType::empty();
            }
    ...
    {
        $node_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node);
        ...
        if ($result->isEmpty() && !$is_valid) {
            ...
            $this->emitIssue(Issue::TypeExpectedObjectOrClassName, $node->lineno, ASTReverter::toShortString($node), $node_type);
        }
        return $result;
    }
    /**
    ...
    public static function unionTypeFromClassNode(CodeBase $code_base, Context $context, $node) : UnionType
    {
        ...
        if ($node instanceof Node && $node->kind === \ast\AST_NAME_LIST) {
            ...
            foreach ($node->children as $child_node) {
                $union_type = $union_type->withUnionType(self::unionTypeFromClassNode($code_base, $context, $child_node));
            }
            return $union_type;
        }
        ...
        if (\strcasecmp('parent', $class_name) === 0) {
            if (!$context->isInClassScope()) {
                throw new IssueException(Issue::fromType(Issue::ContextNotObject)($context->getFile(), $node->lineno ?? $context->getLineNumberStart(), [$class_name]));
            }
            $class = $context->getClassInScope($code_base);
            if ($class->isTrait()) {
                throw new IssueException(Issue::fromType(Issue::TraitParentReference)($context->getFile(), $node->lineno ?? $context->getLineNumberStart(), [(string) $context->getClassFQSEN()]));
            }
            if (!$class->hasParentType()) {
                throw new IssueException(Issue::fromType(Issue::ParentlessClass)($context->getFile(), $node->lineno ?? $context->getLineNumberStart(), [(string) $context->getClassFQSEN()]));
            }
            $parent_class_fqsen = $class->getParentClassFQSEN();
            if (!$code_base->hasClassWithFQSEN($parent_class_fqsen)) {
                throw new IssueException(Issue::fromType(Issue::UndeclaredClass)($context->getFile(), $node->lineno ?? $context->getLineNumberStart(), [(string) $parent_class_fqsen], IssueFixSuggester::suggestSimilarClass($code_base, $context, $parent_class_fqsen)));
            } else {
        ...
        if ($node->flags & \ast\flags\NAME_NOT_FQ) {
            ...
            $type = Type::fromStringInContext($class_name, $context, Type::FROM_NODE);
        } elseif ($node->flags & \ast\flags\NAME_RELATIVE) {
            // Relative to current namespace
            if (0 !== \strpos($class_name, '\\')) {
                $class_name = '\\' . $class_name;
            }
            $type = Type::fromFullyQualifiedString($context->getNamespace() . $class_name);
        } else {
            // Fully qualified
            if (0 !== \strpos($class_name, '\\')) {
                $class_name = '\\' . $class_name;
    ...
    private static function checkValidClassFQSEN(Context $context, Node $node, string $class_name) : void
    {
        ...
        throw new IssueException(Issue::fromType($class_name === '\\' ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike)($context->getFile(), $node->lineno, [$class_name]));
    }
    /**
    ...
    {
        return (new UnionTypeVisitor($code_base, $context, \true))->classListFromNode($node);
    }
    /**
    ...
    private function classListFromNode(Node $node) : \Generator
    {
        ...
        $union_type = self::unionTypeFromNode($this->code_base, $this->context, $node)->withStaticResolvedInContext($this->context);
        ...
        foreach ($union_type->nonNativeTypes()->getTypeSet() as $class_type) {
            ...
            try {
                ...
                throw new IssueException(Issue::fromType($e instanceof EmptyFQSENException ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike)($this->context->getFile(), $node->lineno, [(string) $class_type]));
            }
            // See if the class exists
            if (!$this->code_base->hasClassWithFQSEN($class_fqsen)) {
                throw new IssueException(Issue::fromType(Issue::UndeclaredClassReference)($this->context->getFile(), $node->lineno, [(string) $class_fqsen]));
            }
    ...
    {
        try {
            $function_fqsens = (new UnionTypeVisitor($code_base, $context, \true))->functionLikeFQSENListFromNode($node);
        } catch (FQSENException $e) {
            Issue::maybeEmit($code_base, $context, $e instanceof EmptyFQSENException ? Issue::EmptyFQSENInCallable : Issue::InvalidFQSENInCallable, $context->getLineNumberStart(), $e->getFQSEN());
            return [];
        } catch (\InvalidArgumentException $_) {
            Issue::maybeEmit($code_base, $context, Issue::InvalidFQSENInCallable, $context->getLineNumberStart(), '(unknown)');
            return [];
        }
        $functions = [];
    ...
    public static function classListFromClassNameNode(CodeBase $code_base, Context $context, $node) : array
    {
        ...
        foreach ($strings as $string) {
            try {
                ...
                Issue::maybeEmit($code_base, $context, $e instanceof EmptyFQSENException ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike, $context->getLineNumberStart(), $e->getFQSEN());
                continue;
            } catch (\InvalidArgumentException $_) {
                Issue::maybeEmit($code_base, $context, Issue::InvalidFQSENInClasslike, $context->getLineNumberStart(), '(unknown)');
                continue;
            }
            if (!$code_base->hasClassWithFQSEN($fqsen)) {
                // TODO: Different issue type?
                Issue::maybeEmit($code_base, $context, Issue::UndeclaredClassReference, $context->getLineNumberStart(), (string) $fqsen);
                continue;
            }
    ...
    {
        return (new UnionTypeVisitor($code_base, $context, \true))->functionLikeFQSENListFromNode($node);
    }
    /**
    ...
    private function methodFQSENListFromObjectAndMethodName($class_or_expr, string $method_name) : array
    {
        ...
        if ($object_types->isEmpty()) {
            if (!$union_type->canCastToUnionType(StringType::instance(\false)->asPHPDocUnionType())) {
                $this->emitIssue(Issue::TypeInvalidCallableObjectOfMethod, $context->getLineNumberStart(), (string) $union_type, $method_name);
            }
            return [];
        }
        $result_types = [];
        ...
        foreach ($object_types->getTypeSet() as $object_type) {
            ...
            if ($object_type instanceof StaticOrSelfType) {
                if (!$context->isInClassScope()) {
                    $this->emitIssue(Issue::ContextNotObjectInCallable, $context->getLineNumberStart(), (string) $class_fqsen, "{$class_fqsen}::{$method_name}");
                    continue;
                }
                $class_fqsen = $context->getClassFQSEN();
            }
            if (!$code_base->hasClassWithFQSEN($class_fqsen)) {
                $this->emitIssue(Issue::UndeclaredClassInCallable, $context->getLineNumberStart(), (string) $class_fqsen, "{$class_fqsen}::{$method_name}");
                continue;
            }
            $class = $code_base->getClassByFQSEN($class_fqsen);
            if (!$class->hasMethodWithName($code_base, $method_name, \true)) {
                // emit error below
                continue;
            }
        ...
        if (\count($result_types) === 0 && $class instanceof Clazz) {
            ...
            $this->emitIssue(Issue::UndeclaredMethodInCallable, $context->getLineNumberStart(), $method_name, (string) $union_type);
        }
        return $result_types;
    }
    /**
    ...
    private function lookupClassOfCallableByName(string $class_name) : ?FullyQualifiedClassName
    {
        switch (\strtolower($class_name)) {
            ...
            case 'static':
                $context = $this->context;
                if (!$context->isInClassScope()) {
                    $this->emitIssue(Issue::ContextNotObject, $context->getLineNumberStart(), \strtolower($class_name));
                    return null;
                }
            ...
            case 'parent':
                ...
                if ($class->isTrait()) {
                    $this->emitIssue(Issue::TraitParentReference, $context->getLineNumberStart(), (string) $class->getFQSEN());
                    return null;
                }
                if (!$class->hasParentType()) {
                    $this->emitIssue(Issue::ParentlessClass, $context->getLineNumberStart(), (string) $class->getFQSEN());
                    return null;
                }
                return $class->getParentClassFQSEN();
    ...
    {
        $this->emitIssue(Issue::ContextNotObjectInCallable, $this->context->getLineNumberStart(), $class_name, "{$class_name}::{$method_name}");
    }
    /**
     * @param string|Node $class_or_expr
    ...
    private function methodFQSENListFromParts($class_or_expr, $method_name) : array
    {
        ...
        if (!is_string($method_name)) {
            ...
            if (!is_string($method_name)) {
                ...
                if (!$method_name_type->canCastToUnionType(StringType::instance(\false)->asPHPDocUnionType())) {
                    Issue::maybeEmit($this->code_base, $this->context, Issue::TypeInvalidCallableMethodName, $method_name->lineno ?? $this->context->getLineNumberStart(), $method_name_type);
                }
                return [];
            }
        }
        try {
            if (is_string($class_or_expr)) {
                if (\in_array(\strtolower($class_or_expr), ['static', 'self', 'parent'], \true)) {
                ...
                if (\in_array(\strtolower($class_fqsen->getName()), ['static', 'self', 'parent'], \true)) {
                    if (!$context->isInClassScope()) {
            ...
            $this->emitIssue($e instanceof EmptyFQSENException ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike, $context->getLineNumberStart(), $e->getFQSEN());
            return [];
        }
        if (!$code_base->hasClassWithFQSEN($class_fqsen)) {
            $this->emitIssue(Issue::UndeclaredClassInCallable, $context->getLineNumberStart(), (string) $class_fqsen, "{$class_fqsen}::{$method_name}");
        ...
        $class = $code_base->getClassByFQSEN($class_fqsen);
        if (!$class->hasMethodWithName($code_base, $method_name, \true)) {
            $this->emitIssue(Issue::UndeclaredStaticMethodInCallable, $context->getLineNumberStart(), "{$class_fqsen}::{$method_name}");
            return [];
        }
        $method = $class->getMethodByName($code_base, $method_name);
        if (!$method->isStatic()) {
            $this->emitIssue(Issue::StaticCallToNonStatic, $context->getLineNumberStart(), (string) $method->getFQSEN(), $method->getFileRef()->getFile(), (string) $method->getFileRef()->getLineNumberStart());
        }
    ...
    private function functionFQSENListFromFunctionName(string $function_name) : array
    {
        ...
        try {
            ...
            $this->emitIssue($e instanceof EmptyFQSENException ? Issue::EmptyFQSENInCallable : Issue::InvalidFQSENInCallable, $this->context->getLineNumberStart(), $function_name);
            return [];
        }
        if (!$this->code_base->hasFunctionWithFQSEN($function_fqsen)) {
            $this->emitIssue(Issue::UndeclaredFunctionInCallable, $this->context->getLineNumberStart(), $function_name);
            return [];
        }
    ...
    private function functionLikeFQSENListFromNode($node) : array
    {
        ...
        if (\is_array($node)) {
            if (\count($node) !== 2) {
                $this->emitIssue(Issue::TypeInvalidCallableArraySize, $orig_node->lineno ?? $this->context->getLineNumberStart(), \count($node));
                return [];
            }
            $i = 0;
            foreach ($node as $key => $_) {
                if ($key !== $i) {
                    $this->emitIssue(Issue::TypeInvalidCallableArrayKey, $orig_node->lineno ?? $this->context->getLineNumberStart(), $i);
                    return [];
                }
                $i++;
            }
        ...
        switch ($key_enum_type) {
            ...
            default:
                foreach ($union_type->getTypeSet() as $type) {
                    ...
                    if (\in_array(\get_class($type), [Type::class, IterableType::class, TemplateType::class, MixedType::class, NonEmptyMixedType::class], \true)) {
                        return null;
                    }
                }
     ...
     */
    public static function anyStringLiteralForNode(CodeBase $code_base, Context $context, $node) : ?string
    {
        if (!$node instanceof Node) {
            return is_string($node) ? $node : null;
        }
        $node_type = self::unionTypeFromNode($code_base, $context, $node);
    ...
    private function analyzeNegativeStringOffsetCompatibility(Node $node, UnionType $dim_type) : void
    {
        ...
        $this->emitIssue(Issue::CompatibleNegativeStringOffset, $node->children['dim']->lineno ?? $node->lineno);
    }
    /**
