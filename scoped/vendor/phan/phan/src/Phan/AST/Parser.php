...
use ParseError;
use SdgScoped\Phan\AST\TolerantASTConverter\CachingTolerantASTConverter;
use SdgScoped\Phan\AST\TolerantASTConverter\ParseException;
use SdgScoped\Phan\AST\TolerantASTConverter\ParseResult;
use SdgScoped\Phan\AST\TolerantASTConverter\ShimFunctions;
use SdgScoped\Phan\AST\TolerantASTConverter\TolerantASTConverter;
use SdgScoped\Phan\AST\TolerantASTConverter\TolerantASTConverterPreservingOriginal;
use SdgScoped\Phan\AST\TolerantASTConverter\TolerantASTConverterWithNodeMapping;
...
use SdgScoped\Phan\Library\DiskCache;
...
class Parser
{
     ...
     */
    public static function parseCode(CodeBase $code_base, Context $context, ?Request $request, string $file_path, string $file_contents, bool $suppress_parse_errors) : Node
    {
        try {
    ...
    private static function parseCodeHandlingDeprecation(CodeBase $code_base, Context $context, string $file_contents, string $file_path) : Node
    {
        ...
        $__no_echo_phan_errors = static function (int $errno, string $errstr, string $unused_errfile, int $errline) use($code_base, $context) : bool {
            ...
            Issue::maybeEmit($code_base, $context, Issue::CompatibleSyntaxNotice, $errline, $errstr);
            // Return true to prevent printing to stderr
            return \true;
        };
        try {
            error_clear_last();
            $root_node = \ast\parse_code($file_contents, Config::AST_VERSION, $file_path);
            $error = error_get_last();
            if ($error && $error['type'] === \E_COMPILE_WARNING) {
                Issue::maybeEmit($code_base, $context, Issue::SyntaxCompileWarning, $error['line'], $error['message']);
            }
            return $root_node;
        } finally {
     ...
     */
    public static function handleParseError(CodeBase $code_base, Context $context, string $file_path, string $file_contents, bool $suppress_parse_errors, Error $native_parse_error, ?Request $request = null) : Node
    {
     ...
     * @param ParseError|CompileError $native_parse_error (can be CompileError in 7.3+, will be ParseError in most cases)
     */
    public static function emitSyntaxErrorForNativeParseError(CodeBase $code_base, Context $context, string $file_path, FileCacheEntry $file_cache_entry, Error $native_parse_error, ?Request $request = null) : void
    {
        ...
        $diagnostic_error_column = self::guessErrorColumnUsingTokens($file_cache_entry, $native_parse_error) ?: self::guessErrorColumnUsingPolyfill($code_base, $context, $file_path, $file_cache_entry, $native_parse_error, $request);
        Issue::maybeEmitWithParameters($code_base, $context, Issue::SyntaxError, $line, [$message], null, $diagnostic_error_column);
    }
    /**
     ...
     */
    private static function guessErrorColumnUsingTokens(FileCacheEntry $file_cache_entry, Error $native_parse_error) : int
    {
     ...
     * Returns the 1-based error column, or 0 if unknown.
     */
    private static function guessErrorColumnUsingPolyfill(CodeBase $code_base, Context $context, string $file_path, FileCacheEntry $file_cache_entry, Error $native_parse_error, ?Request $request) : int
    {
        ...
        if ($last_file_contents !== $file_contents) {
            ...
            $errors =& $new_errors;
            try {
                self::parseCodePolyfill($code_base, $context, $file_path, $file_contents, \true, $request, $errors);
            } catch (Throwable $_) {
    ...
    public static function parseCodePolyfill(CodeBase $code_base, Context $context, string $file_path, string $file_contents, bool $suppress_parse_errors, ?Request $request, array &$errors = []) : Node
    {
        ...
        if (!\in_array(Config::AST_VERSION, TolerantASTConverter::SUPPORTED_AST_VERSIONS, \true)) {
            ...
            if ($diagnostic->kind === 0) {
                ...
                if (!$suppress_parse_errors) {
                    ...
                    if ($emitted_warning_count <= self::MAX_POLYFILL_WARNINGS) {
                        Issue::maybeEmitWithParameters($code_base, $context, Issue::SyntaxError, $diagnostic_error_start_line, [$diagnostic_error_message], null, $diagnostic_error_column);
                    }
                }
                if (!Config::getValue('use_fallback_parser')) {
                    // By default, don't try to re-parse files with syntax errors.
    ...
    {
        if (\in_array($error['type'], [\E_DEPRECATED, \E_COMPILE_WARNING], \true) && \basename($error['file']) === 'PhpTokenizer.php') {
            $line = $error['line'];
            if (\preg_match('/line ([0-9]+)$/D', $error['message'], $matches)) {
                $line = (int) $matches[1];
            }
            Issue::maybeEmit($code_base, $context, $error['type'] === \E_COMPILE_WARNING ? Issue::SyntaxCompileWarning : Issue::CompatibleSyntaxNotice, $line, $error['message']);
        }
    }
    /**
    ...
    private static function createConverter(string $file_path, string $file_contents, Request $request = null) : TolerantASTConverter
    {
        if ($request) {
            if ($request->shouldUseMappingPolyfill($file_path)) {
                ...
                $converter = new TolerantASTConverterWithNodeMapping($request->getTargetByteOffset($file_contents), static function (Node $node) : void {
                ...
                if ($request->shouldAddPlaceholdersForPath($file_path)) {
                    $converter->setShouldAddPlaceholders(\true);
                }
                return $converter;
            }
