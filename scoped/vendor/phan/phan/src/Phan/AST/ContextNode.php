...
use SdgScoped\Phan\Analysis\ConditionVisitorUtil;
...
use SdgScoped\Phan\Language\Element\ClassConstant;
...
use SdgScoped\Phan\Language\Element\GlobalConstant;
...
use SdgScoped\Phan\Language\Element\TraitAdaptations;
use SdgScoped\Phan\Language\Element\TraitAliasSource;
use SdgScoped\Phan\Language\Element\Variable;
use SdgScoped\Phan\Language\FQSEN;
use SdgScoped\Phan\Language\FQSEN\FullyQualifiedClassConstantName;
...
use SdgScoped\Phan\Language\FQSEN\FullyQualifiedPropertyName;
...
use SdgScoped\Phan\Library\FileCache;
use SdgScoped\Phan\Library\None;
...
class ContextNode
{
    ...
    public function getQualifiedNameList() : array
    {
        ...
        foreach ($this->node->children as $name_node) {
            $union_type = $union_type->withUnionType(UnionTypeVisitor::unionTypeFromClassNode($this->code_base, $this->context, $name_node));
        }
    ...
    {
        return UnionTypeVisitor::unionTypeFromClassNode($this->code_base, $this->context, $this->node)->__toString();
    }
    /**
    ...
    public function getTraitFQSENList() : array
    {
        ...
        $result = [];
        foreach ($this->node->children as $name_node) {
            $trait_fqsen = (new ContextNode($this->code_base, $this->context, $name_node))->getTraitFQSEN([]);
            if ($trait_fqsen) {
    ...
    public function getTraitFQSEN(array $adaptations_map) : ?FullyQualifiedClassName
    {
        ...
        return FullyQualifiedClassName::fromStringInContext($trait_fqsen_string, $this->context);
    }
    /**
    ...
    private function handleTraitAlias(array $adaptations_map, Node $adaptation_node) : void
    {
        ...
        if (!\is_string($trait_original_method_name)) {
            $this->emitIssue(Issue::InvalidTraitUse, $trait_original_class_name_node->lineno ?? 0, "Expected original method name of a trait use to be a string");
            return;
        }
        if (!\is_string($trait_new_method_name)) {
            $this->emitIssue(Issue::InvalidTraitUse, $trait_original_class_name_node->lineno ?? 0, "Expected new method name of a trait use to be a string");
            return;
        }
        try {
            $trait_fqsen = (new ContextNode($this->code_base, $this->context, $trait_original_class_name_node))->getTraitFQSEN($adaptations_map);
        } catch (FQSENException $e) {
            $this->emitIssue(Issue::InvalidTraitUse, $trait_original_class_name_node->lineno ?? 0, $e->getMessage());
            return;
        }
        if ($trait_fqsen === null) {
            ...
            $this->emitIssue(Issue::AmbiguousTraitAliasSource, $trait_method_node->lineno ?? 0, $trait_new_method_name, $trait_original_method_name, '[' . implode(', ', \array_map(static function (TraitAdaptations $t) : string {
        ...
        if ($adaptations_info === null) {
            ...
            $this->emitIssue(Issue::RequiredTraitNotAdded, $trait_original_class_name_node->lineno ?? 0, $trait_fqsen->__toString());
            return;
        }
        ...
        if (strcasecmp($trait_new_method_name, $trait_original_method_name) === 0) {
            $adaptations_info->hidden_methods[\strtolower($trait_original_method_name)] = \true;
        }
    }
    /**
     ...
     */
    private function emitIssue(string $issue_type, int $lineno, ...$parameters) : void
    {
    ...
    private function handleTraitPrecedence(array $adaptations_map, Node $adaptation_node) : void
    {
        ...
        if (!is_string($trait_chosen_method_name)) {
            $this->emitIssue(Issue::InvalidTraitUse, $trait_method_node->lineno ?? 0, "Expected the insteadof method's name to be a string");
            return;
        }
        try {
            $trait_chosen_fqsen = (new ContextNode($this->code_base, $this->context, $trait_chosen_class_name_node))->getTraitFQSEN($adaptations_map);
        } catch (FQSENException $e) {
            $this->emitIssue(Issue::InvalidTraitUse, $trait_method_node->lineno ?? 0, $e->getMessage());
            return;
        }
        ...
        if (($adaptations_map[\strtolower($trait_chosen_fqsen->__toString())] ?? null) === null) {
            // This will probably correspond to a PHP fatal error, but keep going anyway.
            $this->emitIssue(Issue::RequiredTraitNotAdded, $trait_chosen_class_name_node->lineno ?? 0, $trait_chosen_fqsen->__toString());
        }
        ...
        foreach ($adaptation_node->children['insteadof']->children as $trait_insteadof_class_name) {
            try {
                $trait_insteadof_fqsen = (new ContextNode($this->code_base, $this->context, $trait_insteadof_class_name))->getTraitFQSEN($adaptations_map);
            } catch (\Exception $_) {
            ...
            if ($adaptations_info === null) {
                ...
                $this->emitIssue(Issue::RequiredTraitNotAdded, $trait_insteadof_class_name->lineno ?? 0, $trait_insteadof_fqsen->__toString());
                continue;
            }
            $adaptations_info->hidden_methods[strtolower($trait_chosen_method_name)] = \true;
        }
    }
    /**
     * @return string
    ...
    public function getVariableName() : string
    {
        ...
        $node = $this->node;
        while ($node instanceof Node && $node->kind !== \ast\AST_VAR && $node->kind !== \ast\AST_STATIC && $node->kind !== \ast\AST_MAGIC_CONST) {
        ...
        if ($name_node instanceof Node) {
            ...
            try {
                $name_node_type = UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $name_node, \true);
    ...
    public function getClassListInner(bool $ignore_missing_classes) : array
    {
        ...
        try {
            ...
            $this->emitIssue($e instanceof EmptyFQSENException ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike, $this->node->lineno ?? $context->getLineNumberStart(), $e->getFQSEN());
            $union_type = UnionType::empty();
        }
        ...
        $class_list = [];
        try {
            foreach ($union_type->asClassList($code_base, $context) as $clazz) {
                $class_list[] = $clazz;
            }
     ...
     */
    public function getClassList(bool $ignore_missing_classes = \false, int $expected_type_categories = self::CLASS_LIST_ACCEPT_ANY, string $custom_issue_type = null, bool $warn_if_wrong_type = \true) : array
    {
        ...
        if (\count($class_list) === 0) {
            if (!$union_type->hasTypeMatchingCallback(function (Type $type) use($expected_type_categories) : bool {
                ...
                if ($warn_if_wrong_type) {
                    ...
                    $this->emitIssue($custom_issue_type ?? ($expected_type_categories !== self::CLASS_LIST_ACCEPT_OBJECT ? Issue::TypeExpectedObjectOrClassName : Issue::TypeExpectedObject), $this->node->lineno ?? $this->context->getLineNumberStart(), ASTReverter::toShortString($this->node), (string) $union_type->asNonLiteralType());
                }
                    ...
                    if ($type instanceof LiteralStringType) {
                        ...
                        try {
                            ...
                            if ($this->code_base->hasClassWithFQSEN($fqsen)) {
                                ...
                                $this->emitIssue(Issue::UndeclaredClass, $this->node->lineno ?? $this->context->getLineNumberStart(), (string) $fqsen);
                            }
                        } catch (FQSENException $e) {
                            if ($warn_if_wrong_type) {
                                $this->emitIssue($e instanceof EmptyFQSENException ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike, $this->node->lineno ?? $this->context->getLineNumberStart(), $e->getFQSEN());
                            }
                        }
                    }
                }
     ...
     * @throws IssueException
     */
    public function getMethod($method_name, bool $is_static, bool $is_direct = \false, bool $is_new_expression = \false) : Method
    {
        ...
        try {
            ...
            $class_list = (new ContextNode($this->code_base, $this->context, $node->children['expr'] ?? $node->children['class']))->getClassList(\false, $is_static || $is_new_expression ? self::CLASS_LIST_ACCEPT_OBJECT_OR_CLASS_NAME : self::CLASS_LIST_ACCEPT_OBJECT, null, $is_new_expression);
        } catch (CodeBaseException $exception) {
            $exception_fqsen = $exception->getFQSEN();
            throw new IssueException(Issue::fromType(Issue::UndeclaredClassMethod)($this->context->getFile(), $node->lineno, [$method_name, (string) $exception_fqsen], $exception_fqsen instanceof FullyQualifiedClassName ? IssueFixSuggester::suggestSimilarClassForMethod($this->code_base, $this->context, $exception_fqsen, $method_name, $is_static) : null));
        }
        ...
        if (\count($class_list) === 0) {
            try {
                $union_type = UnionTypeVisitor::unionTypeFromClassNode($this->code_base, $this->context, $node->children['expr'] ?? $node->children['class']);
            } catch (FQSENException $e) {
                throw new IssueException(Issue::fromType($e instanceof EmptyFQSENException ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike)($this->context->getFile(), $node->lineno, [$e->getFQSEN()]));
            }
            if ($union_type->isDefinitelyUndefined() || !$union_type->isEmpty() && $union_type->isNativeType() && !$union_type->hasTypeMatchingCallback(static function (Type $type) : bool {
                return !$type->isNullableLabeled() && ($type instanceof MixedType || $type instanceof ObjectType);
            }) && !(($is_static || $is_new_expression) && $union_type->hasNonNullStringType()) && !(Config::get_null_casts_as_any_type() && $union_type->hasType(NullType::instance(\false)))) {
                throw new IssueException(Issue::fromType(Issue::NonClassMethodCall)($this->context->getFile(), $node->lineno, [$method_name, (string) $union_type]));
            }
            throw new NodeException($node, "Can't figure out method call for {$method_name}");
        }
        ...
        foreach ($class_list as $class) {
            if ($class->hasMethodWithName($this->code_base, $method_name, $is_direct)) {
                ...
                if ($method->hasTemplateType()) {
                    try {
                        $method = $method->resolveTemplateType($this->code_base, UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['expr'] ?? $node->children['class']));
        ...
        if ($method) {
            if ($class_without_method && Config::get_strict_method_checking()) {
                $this->emitIssue(Issue::PossiblyUndeclaredMethod, $node->lineno, $method_name, implode('|', \array_map(static function (Clazz $class) : string {
                    return $class->getFQSEN()->__toString();
                }, $class_list)), $class_without_method);
            }
            return $method;
        }
        $first_class = $class_list[0];
        // Figure out an FQSEN for the method we couldn't find
        $method_fqsen = FullyQualifiedMethodName::make($first_class->getFQSEN(), $method_name);
        if ($is_static) {
            throw new IssueException(Issue::fromType(Issue::UndeclaredStaticMethod)($this->context->getFile(), $node->lineno, [(string) $method_fqsen], IssueFixSuggester::suggestSimilarMethod($this->code_base, $this->context, $first_class, $method_name, $is_static)));
        }
        throw new IssueException(Issue::fromType(Issue::UndeclaredMethod)($this->context->getFile(), $node->lineno, [(string) $method_fqsen], IssueFixSuggester::suggestSimilarMethod($this->code_base, $this->context, $first_class, $method_name, $is_static)));
    }
    /**
    ...
    public function getFunctionFromNode(bool $return_placeholder_for_undefined = \false) : iterable
    {
        ...
        if (!$expression instanceof Node) {
            if (!\is_string($expression)) {
                Issue::maybeEmit($this->code_base, $this->context, Issue::TypeInvalidCallable, $this->context->getLineNumberStart(), (string) $expression);
            }
        ...
        if ($expression->kind === \ast\AST_NAME) {
            ...
            try {
                return [$this->getFunction($name, \false, $return_placeholder_for_undefined)];
            } catch (IssueException $exception) {
                Issue::maybeEmitInstance($this->code_base, $this->context, $exception->getIssueInstance());
            } catch (FQSENException $exception) {
                Issue::maybeEmit($this->code_base, $this->context, $exception instanceof EmptyFQSENException ? Issue::EmptyFQSENInCallable : Issue::InvalidFQSENInCallable, $expression->lineno, $exception->getFQSEN());
            }
            return [];
        }
    ...
    private function getFunctionLikeFromDynamicExpression() : \Generator
    {
        ...
        if (!$has_type) {
            if (!$union_type->hasPossiblyCallableType()) {
                Issue::maybeEmit($code_base, $context, Issue::TypeInvalidCallable, $expression->lineno ?? $context->getLineNumberStart(), $union_type);
                return;
            }
        }
        if (Config::get_strict_method_checking() && $union_type->containsDefiniteNonCallableType()) {
            Issue::maybeEmit($code_base, $context, Issue::TypePossiblyInvalidCallable, $expression->lineno ?? $context->getLineNumberStart(), $union_type);
        }
    }
    /**
    ...
    private function returnStubOrThrowUndeclaredFunctionIssueException(FullyQualifiedFunctionName $function_fqsen, bool $suggest_in_global_namespace, FullyQualifiedFunctionName $namespaced_function_fqsen = null, bool $return_placeholder_for_undefined = \false) : Func
    {
        if ($return_placeholder_for_undefined) {
            ...
            Issue::maybeEmitWithParameters($this->code_base, $this->context, Issue::UndeclaredFunction, $this->node->lineno ?? $this->context->getLineNumberStart(), ["{$function_fqsen}()"], IssueFixSuggester::suggestSimilarGlobalFunction($this->code_base, $this->context, $namespaced_function_fqsen ?? $function_fqsen, $suggest_in_global_namespace));
            if ($functions) {
                return $functions[0];
            }
        }
        throw new IssueException(Issue::fromType(Issue::UndeclaredFunction)($this->context->getFile(), $this->node->lineno ?? $this->context->getLineNumberStart(), ["{$function_fqsen}()"], IssueFixSuggester::suggestSimilarGlobalFunction($this->code_base, $this->context, $namespaced_function_fqsen ?? $function_fqsen, $suggest_in_global_namespace)));
    }
    /**
     ...
     */
    public function getFunction(string $function_name, bool $is_function_declaration = \false, bool $return_placeholder_for_undefined = \false) : FunctionInterface
    {
        $node = $this->node;
        if (!$node instanceof Node) {
        ...
        if (!$code_base->hasFunctionWithFQSEN($function_fqsen)) {
            ...
            return $this->returnStubOrThrowUndeclaredFunctionIssueException($function_fqsen, $not_fully_qualified, $not_fully_qualified ? FullyQualifiedFunctionName::make($namespace, $function_name) : $function_fqsen, $return_placeholder_for_undefined);
        }
        return $code_base->getFunctionByFQSEN($function_fqsen);
    }
    /**
    ...
    public function getOrCreateVariable() : Variable
    {
        ...
        $variable = Variable::fromNodeInContext($node, $this->context, $this->code_base, \false);
    ...
    public function getOrCreateVariableForReferenceParameter(Parameter $parameter, ?Parameter $real_parameter) : Variable
    {
        ...
        // Create a new variable
        $variable = Variable::fromNodeInContext($node, $this->context, $this->code_base, \false);
        if ($parameter->getReferenceType() === Parameter::REFERENCE_READ_WRITE || $real_parameter && !$real_parameter->getNonVariadicUnionType()->containsNullableOrIsEmpty()) {
     ...
     */
    public function getProperty(bool $is_static, bool $is_known_assignment = \false) : Property
    {
        $node = $this->node;
        if (!$node instanceof Node) {
            throw new AssertionError('$this->node must be a node');
        ...
        try {
            ...
            $class_list = (new ContextNode($this->code_base, $this->context, $node->children['expr'] ?? $node->children['class']))->getClassList(\false, $expected_type_categories, $expected_issue);
        } catch (CodeBaseException $exception) {
            $exception_fqsen = $exception->getFQSEN();
            if ($exception_fqsen instanceof FullyQualifiedClassName) {
                throw new IssueException(Issue::fromType($is_static ? Issue::UndeclaredClassStaticProperty : Issue::UndeclaredClassProperty)($this->context->getFile(), $node->lineno, [$property_name, $exception_fqsen], IssueFixSuggester::suggestSimilarClassForGenericFQSEN($this->code_base, $this->context, $exception_fqsen)));
            }
            // TODO: Is this ever used? The undeclared property issues should instead be caused by the hasPropertyWithFQSEN checks below.
            if ($is_static) {
                throw new IssueException(Issue::fromType(Issue::UndeclaredStaticProperty)($this->context->getFile(), $node->lineno, [$property_name, (string) $exception->getFQSEN()]));
            } else {
                throw new IssueException(Issue::fromType(Issue::UndeclaredProperty)($this->context->getFile(), $node->lineno, ["{$exception->getFQSEN()}->{$property_name}"]));
            }
        }
        ...
        foreach ($class_list as $class) {
            ...
            if (!$class->hasPropertyWithName($this->code_base, $property_name)) {
                ...
                if (!$is_static && $class->hasGetMethod($this->code_base) && !$class->getForbidUndeclaredMagicProperties($this->code_base)) {
                    throw new UnanalyzableMagicPropertyException($node, $class, $property_name, "Can't determine if property {$property_name} exists in class {$class->getFQSEN()} with __get defined");
                }
            ...
            $property = $class->getPropertyByNameInContext($this->code_base, $property_name, $this->context, $is_static, $node, $is_known_assignment);
            if ($property->isDeprecated()) {
                $this->emitIssue(Issue::DeprecatedProperty, $node->lineno, $property->getRepresentationForIssue(), $property->getFileRef()->getFile(), $property->getFileRef()->getLineNumberStart(), $property->getDeprecationReason());
            }
            if ($property->isNSInternal($this->code_base) && !$property->isNSInternalAccessFromContext($this->code_base, $this->context)) {
                $this->emitIssue(Issue::AccessPropertyInternal, $node->lineno, $property->getRepresentationForIssue(), $property->getElementNamespace() ?: '\\', $property->getFileRef()->getFile(), $property->getFileRef()->getLineNumberStart(), $this->context->getNamespace() ?: '\\');
            }
        }
        if (!$is_static && Config::get_strict_object_checking() && !($node->flags & PhanAnnotationAdder::FLAG_IGNORE_UNDEF)) {
            ...
            if (!$invalid->isEmpty()) {
                ...
                if (!$invalid->isEmpty()) {
                    foreach ($invalid->getTypeset() as $type) {
                        ...
                        $this->emitIssue(Issue::PossiblyUndeclaredProperty, $node->lineno, $property_name, $union_type, $invalid);
                        if ($property) {
                            return $property;
                        }
                        break;
        ...
        if ($property) {
            if ($class_without_property && Config::get_strict_object_checking() && !($node->flags & PhanAnnotationAdder::FLAG_IGNORE_UNDEF)) {
                $this->emitIssue(Issue::PossiblyUndeclaredProperty, $node->lineno, $property_name, UnionTypeVisitor::unionTypeFromNode($this->code_base, $this->context, $node->children['expr'] ?? $node->children['class']), $class_without_property->getFQSEN());
            }
            return $property;
        }
        ...
        if (!$is_static) {
            foreach ($class_list as $class) {
                if (Config::getValue('allow_missing_properties') || $class->hasDynamicProperties($this->code_base)) {
                    return $class->getPropertyByNameInContext($this->code_base, $property_name, $this->context, $is_static, $node, $is_known_assignment);
                }
            }
        }
        /*
        ...
        */
        ...
        if ($class_fqsen) {
            ...
            if ($is_static) {
                throw new IssueException(Issue::fromType(Issue::UndeclaredStaticProperty)($this->context->getFile(), $node->lineno, [$property_name, (string) $class_fqsen], $suggestion));
            } else {
                throw new IssueException(Issue::fromType(Issue::UndeclaredProperty)($this->context->getFile(), $node->lineno, ["{$class_fqsen}->{$property_name}"], $suggestion));
            }
        }
        throw new NodeException($node, "Cannot figure out property from {$this->context}");
    }
    /**
    ...
    private function createExceptionForInvalidPropertyName(Node $node, bool $is_static) : Exception
    {
        ...
        if ($property_type->canCastToUnionType(StringType::instance(\false)->asPHPDocUnionType())) {
            ...
            return new NodeException($node, "Cannot figure out property name");
        }
        return new IssueException(Issue::fromType($is_static ? Issue::TypeInvalidStaticPropertyName : Issue::TypeInvalidPropertyName)($this->context->getFile(), $node->lineno, [$property_type]));
    }
    /**
     * @return Property
     ...
     */
    public function getOrCreateProperty(string $property_name, bool $is_static) : Property
    {
        try {
        ...
        try {
            $expected_type_categories = $is_static ? self::CLASS_LIST_ACCEPT_OBJECT_OR_CLASS_NAME : self::CLASS_LIST_ACCEPT_OBJECT;
            $expected_issue = $is_static ? Issue::TypeExpectedObjectStaticPropAccess : Issue::TypeExpectedObjectPropAccess;
            $class_list = (new ContextNode($this->code_base, $this->context, $node->children['expr'] ?? null))->getClassList(\false, $expected_type_categories, $expected_issue);
        } catch (CodeBaseException $exception) {
            throw new IssueException(Issue::fromType(Issue::UndeclaredClassReference)($this->context->getFile(), $node->lineno, [$exception->getFQSEN()]));
        }
        $class = \reset($class_list);
        if (!$class instanceof Clazz) {
            // empty list
            throw new UnanalyzableException($node, "Could not get class name from node");
        }
        $flags = 0;
        ...
        $property = new Property($this->context, $property_name, UnionType::empty(), $flags, $property_fqsen, UnionType::empty());
    ...
    public function getConst() : GlobalConstant
    {
        ...
        if ($node->kind !== \ast\AST_CONST) {
            throw new AssertionError("SdgScoped\\Node must be of type ast\\AST_CONST");
        }
        ...
        if ($constant_name_lower === 'true' || $constant_name_lower === 'false' || $constant_name_lower === 'null') {
            return $code_base->getGlobalConstantByFQSEN(
                ...
                FullyQualifiedGlobalConstantName::fromFullyQualifiedString($constant_name_lower)
            );
        }
        $context = $this->context;
        ...
        if ($constant->isNSInternal($code_base) && !$constant->isNSInternalAccessFromContext($code_base, $context)) {
            // TODO: Refactor and also check namespaced constants
            $this->emitIssue(Issue::AccessConstantInternal, $node->lineno, (string) $constant->getFQSEN(), $constant->getElementNamespace(), $constant->getFileRef()->getFile(), $constant->getFileRef()->getLineNumberStart(), $context->getNamespace());
        }
        return $constant;
    }
    private function createPlaceholderGlobalConstant(FullyQualifiedGlobalConstantName $fqsen) : GlobalConstant
    {
    ...
    {
        throw new IssueException(Issue::fromType(Issue::UndeclaredConstant)($this->context->getFile(), $this->node->lineno ?? $context->getLineNumberStart(), [$fqsen], IssueFixSuggester::suggestSimilarGlobalConstant($code_base, $context, $fqsen)));
    }
    /**
    ...
    public function getClassConst() : ClassConstant
    {
        ...
        $class_fqsen = null;
        try {
            $class_list = (new ContextNode($this->code_base, $this->context, $node->children['class']))->getClassList(\false, self::CLASS_LIST_ACCEPT_OBJECT_OR_CLASS_NAME);
        } catch (CodeBaseException $exception) {
            $exception_fqsen = $exception->getFQSEN();
            if ($constant_name === 'class') {
                throw new IssueException(Issue::fromType(Issue::UndeclaredClassReference)($this->context->getFile(), $node->lineno, [(string) $exception_fqsen], IssueFixSuggester::suggestSimilarClassForGenericFQSEN($this->code_base, $this->context, $exception_fqsen)));
            }
            throw new IssueException(Issue::fromType(Issue::UndeclaredClassConstant)($this->context->getFile(), $node->lineno, [$constant_name, (string) $exception_fqsen], IssueFixSuggester::suggestSimilarClassForGenericFQSEN($this->code_base, $this->context, $exception_fqsen)));
        }
        foreach ($class_list as $class) {
            ...
            if ($constant->isNSInternal($this->code_base) && !$constant->isNSInternalAccessFromContext($this->code_base, $this->context)) {
                $this->emitIssue(Issue::AccessClassConstantInternal, $node->lineno, (string) $constant->getFQSEN(), $constant->getFileRef()->getFile(), $constant->getFileRef()->getLineNumberStart());
            }
            if ($constant->isDeprecated()) {
                $this->emitIssue(Issue::DeprecatedClassConstant, $node->lineno, (string) $constant->getFQSEN(), $constant->getFileRef()->getFile(), $constant->getFileRef()->getLineNumberStart(), $constant->getDeprecationReason());
            }
            return $constant;
        }
        ...
        if ($class_fqsen) {
            ...
            throw new IssueException(Issue::fromType(Issue::UndeclaredConstantOfClass)($this->context->getFile(), $node->lineno, ["{$class_fqsen}::{$constant_name}"], IssueFixSuggester::suggestSimilarClassConstant($this->code_base, $this->context, $class_constant_fqsen)));
        }
        throw new NodeException($node, "Can't figure out constant {$constant_name} in node");
    }
    /**
     * @return string
    ...
    public function getUnqualifiedNameForAnonymousClass() : string
    {
        ...
        $class_name = 'anonymous_class_' . \substr(\md5($this->context->getFile() . $this->context->getLineNumberStart()), 0, 8);
        return $class_name;
    }
    /**
    ...
    {
        $closure_fqsen = FullyQualifiedFunctionName::fromClosureInContext($this->context, $this->node);
    ...
    public function analyzeBackwardCompatibility() : void
    {
        ...
        $kind = $this->node->kind;
        if (\in_array($kind, [\ast\AST_STATIC_CALL, \ast\AST_METHOD_CALL, \ast\AST_NULLSAFE_METHOD_CALL], \true)) {
            return;
        }
        $llnode = $this->node;
        if ($kind !== \ast\AST_DIM) {
        ...
        // Foo::$bar['baz'] is not
        if ($lnode->kind === \ast\AST_STATIC_PROP && $kind !== \ast\AST_CALL) {
            return;
        }
        ...
        if (($lnode->children['prop'] instanceof Node && $lnode->children['prop']->kind === \ast\AST_VAR || ($lnode->children['class'] ?? null) instanceof Node && ($lnode->children['class']->kind === \ast\AST_VAR || $lnode->children['class']->kind === \ast\AST_NAME) || ($lnode->children['expr'] ?? null) instanceof Node && ($lnode->children['expr']->kind === \ast\AST_VAR || $lnode->children['expr']->kind === \ast\AST_NAME)) && ($temp->kind === \ast\AST_VAR || $temp->kind === \ast\AST_NAME)) {
            ...
            if (strpos($line, '}[') === \false && strpos($line, ']}') === \false && strpos($line, '>{') === \false) {
                Issue::maybeEmit($this->code_base, $this->context, Issue::CompatiblePHP7, $this->node->lineno);
            }
        }
    }
    /**
    ...
    public function resolveClassNameInContext() : ?FullyQualifiedClassName
    {
        ...
        try {
            ...
            if ($arg instanceof Node && $arg->kind === \ast\AST_CLASS_NAME) {
                $class_type = UnionTypeVisitor::unionTypeFromClassNode($this->code_base, $this->context, $arg->children['class']);
            ...
            throw new IssueException(Issue::fromType($e instanceof EmptyFQSENException ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike)($e instanceof EmptyFQSENException ? Issue::EmptyFQSENInClasslike : Issue::InvalidFQSENInClasslike, $this->node->lineno ?? $this->context->getLineNumberStart(), [$e->getFQSEN()]));
        }
        return null;
    }
    ...
    public const RESOLVE_DEFAULT = self::RESOLVE_ARRAYS | self::RESOLVE_ARRAY_KEYS | self::RESOLVE_ARRAY_VALUES | self::RESOLVE_CONSTANTS | self::RESOLVE_KEYS_USE_FALLBACK_PLACEHOLDER;
    ...
    public static function warnAboutEmptyArrayElements(CodeBase $code_base, Context $context, Node $node) : void
    {
        ...
        $node->didWarnAboutEmptyArrayElements = \true;
        $lineno = $node->lineno;
        foreach ($node->children as $child_node) {
            if (!$child_node instanceof Node) {
                // Emit the line number of the nearest Node before this empty element
                Issue::maybeEmit($code_base, $context, Issue::SyntaxError, $lineno, "Cannot use empty array elements in arrays");
                continue;
            }
    ...
    public function isVarWithConstantDefinition(Node $node) : bool
    {
        ...
        if ($scope->hasVariableWithName($name)) {
            return ($scope->getVariableByName($name)->getPhanFlags() & \SdgScoped\Phan\Language\Element\Flags::IS_CONSTANT_DEFINITION) !== 0;
        }
        return \false;
    }
    /**
    ...
    {
        $this->emitIssue(Issue::TypeErrorInOperation, $node->lineno, ASTReverter::toShortString($node), $e->getMessage());
    }
    /**
     * @return Node|string[]|int[]|float[]|string|float|int|bool|null -
     *         If this could be resolved and we're certain of the value,
    ...
    private const TYPE_CHECK_SET = ['is_array' => \true, 'is_bool' => \true, 'is_callable' => \true, 'is_double' => \true, 'is_float' => \true, 'is_int' => \true, 'is_integer' => \true, 'is_iterable' => \true, 'is_long' => \true, 'is_null' => \true, 'is_numeric' => \true, 'is_object' => \true, 'is_real' => \true, 'is_resource' => \true, 'is_scalar' => \true, 'is_string' => \true];
    /**
    ...
    {
        return $this->getEquivalentPHPValueForNode($this->node, self::RESOLVE_ARRAYS | self::RESOLVE_ARRAY_KEYS | self::RESOLVE_ARRAY_VALUES | self::RESOLVE_OPS | self::RESOLVE_ONLY_CONSTANT_VARS | self::RESOLVE_TYPE_CHECKS);
    }
    /**
