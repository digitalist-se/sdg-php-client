...
class FallbackUnionTypeVisitor extends KindVisitorImplementation
{
     ...
     */
    public static function unionTypeFromNode(CodeBase $code_base, Context $context, $node) : UnionType
    {
    ...
    public function visitConditional(Node $node) : UnionType
    {
        ...
        if ($cond_truthiness !== null) {
            ...
            if ($cond_truthiness) {
                ...
                return self::unionTypeFromNode($this->code_base, $this->context, $true_node);
            } else {
                // The condition is unconditionally false
                // Add the type for the 'false' side
                return self::unionTypeFromNode($this->code_base, $this->context, $node->children['false']);
            }
        }
        // Postcondition: This is (cond_expr) ? (true_expr) : (false_expr)
        $true_type = self::unionTypeFromNode($this->code_base, $this->context, $true_node);
        $false_type = self::unionTypeFromNode($this->code_base, $this->context, $node->children['false']);
    ...
    private function visitClassNameNode(Node $node) : ?UnionType
    {
        ...
        if (!Type::isSelfTypeString($class_name)) {
            ...
            return UnionTypeVisitor::unionTypeFromClassNode($this->code_base, $this->context, $node);
        }
