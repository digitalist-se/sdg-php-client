...
class ASTReverter
{
    public const EXEC_NODE_FLAG_NAMES = [\ast\flags\EXEC_EVAL => 'eval', \ast\flags\EXEC_INCLUDE => 'include', \ast\flags\EXEC_INCLUDE_ONCE => 'include_once', \ast\flags\EXEC_REQUIRE => 'require', \ast\flags\EXEC_REQUIRE_ONCE => 'require_once'];
    ...
    public static function init() : void
    {
        ...
        self::$closure_map = [
            ...
            \ast\AST_BINARY_OP => static function (Node $node) : string {
                return sprintf("(%s %s %s)", self::toShortString($node->children['left']), PostOrderAnalysisVisitor::NAME_FOR_BINARY_OP[$node->flags] ?? 'unknown', self::toShortString($node->children['right']));
            },
            \ast\AST_ASSIGN => static function (Node $node) : string {
                return sprintf("(%s = %s)", self::toShortString($node->children['var']), self::toShortString($node->children['expr']));
            },
            ...
            \ast\AST_ASSIGN_OP => static function (Node $node) : string {
                return sprintf("(%s %s= %s)", self::toShortString($node->children['var']), PostOrderAnalysisVisitor::NAME_FOR_BINARY_OP[$node->flags] ?? 'unknown', self::toShortString($node->children['expr']));
            },
            ...
            \ast\AST_PROP => static function (Node $node) : string {
                ...
                return sprintf('%s->%s', self::toShortString($node->children['expr']), $prop_node instanceof Node ? '{' . self::toShortString($prop_node) . '}' : (string) $prop_node);
            },
            ...
            \ast\AST_STATIC_CALL => static function (Node $node) : string {
                ...
                return sprintf('%s::%s%s', self::toShortString($node->children['class']), is_string($method_node) ? $method_node : self::toShortString($method_node), self::toShortString($node->children['args']));
            },
            \ast\AST_METHOD_CALL => static function (Node $node) : string {
                $method_node = $node->children['method'];
                return sprintf('%s->%s%s', self::toShortString($node->children['expr']), is_string($method_node) ? $method_node : self::toShortString($method_node), self::toShortString($node->children['args']));
            },
            ...
            \ast\AST_STATIC_PROP => static function (Node $node) : string {
                $prop_node = $node->children['prop'];
                return sprintf('%s::$%s', self::toShortString($node->children['class']), $prop_node instanceof Node ? '{' . self::toShortString($prop_node) . '}' : (string) $prop_node);
            },
            \ast\AST_INSTANCEOF => static function (Node $node) : string {
                return sprintf('(%s instanceof %s)', self::toShortString($node->children['expr']), self::toShortString($node->children['class']));
            },
            ...
            \ast\AST_CALL => static function (Node $node) : string {
                return sprintf('%s%s', self::toShortString($node->children['expr']), self::toShortString($node->children['args']));
            },
            \ast\AST_NEW => static function (Node $node) : string {
                ...
                return sprintf('new %s%s', self::toShortString($node->children['class']), self::toShortString($node->children['args']));
            },
            \ast\AST_CLONE => static function (Node $node) : string {
                ...
                return sprintf('(clone(%s))', self::toShortString($node->children['expr']));
            },
            ...
            \ast\AST_ISSET => static function (Node $node) : string {
                return sprintf('isset(%s)', self::toShortString($node->children['var']));
            },
            \ast\AST_EMPTY => static function (Node $node) : string {
                return sprintf('empty(%s)', self::toShortString($node->children['expr']));
            },
            \ast\AST_PRINT => static function (Node $node) : string {
                return sprintf('print(%s)', self::toShortString($node->children['expr']));
            },
            ...
            \ast\AST_UNPACK => static function (Node $node) : string {
                return sprintf('...(%s)', self::toShortString($node->children['expr']));
            },
            \ast\AST_INCLUDE_OR_EVAL => static function (Node $node) : string {
                return sprintf('%s(%s)', self::EXEC_NODE_FLAG_NAMES[$node->flags], self::toShortString($node->children['expr']));
            },
            ...
            \ast\AST_RETURN => static function (Node $node) : string {
                return sprintf('return %s;', self::toShortString($node->children['expr']));
            },
            \ast\AST_THROW => static function (Node $node) : string {
                return sprintf('(throw %s)', self::toShortString($node->children['expr']));
            },
