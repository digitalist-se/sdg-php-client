...
class ASTSimplifier
{
    ...
    {
        return new Node(\ast\AST_STMT_LIST, 0, $child_nodes, $lineno);
    }
    /**
    ...
    public const NON_SHORT_CIRCUITING_BINARY_OPERATOR_FLAGS = [\ast\flags\BINARY_BOOL_XOR, \ast\flags\BINARY_IS_IDENTICAL, \ast\flags\BINARY_IS_NOT_IDENTICAL, \ast\flags\BINARY_IS_EQUAL, \ast\flags\BINARY_IS_NOT_EQUAL, \ast\flags\BINARY_IS_SMALLER, \ast\flags\BINARY_IS_SMALLER_OR_EQUAL, \ast\flags\BINARY_IS_GREATER, \ast\flags\BINARY_IS_GREATER_OR_EQUAL, \ast\flags\BINARY_SPACESHIP];
    /**
    ...
    public static function isExpressionWithoutSideEffects($node) : bool
    {
        ...
        switch ($node->kind) {
            ...
            case \ast\AST_BINARY_OP:
                return self::isExpressionWithoutSideEffects($node->children['left']) && self::isExpressionWithoutSideEffects($node->children['right']);
    ...
    private static function normalizeIfStatement(Node $original_node) : array
    {
        ...
        do {
            ...
            if (!$if_cond instanceof Node) {
                break;
                // No transformation rules apply here.
            }
            if ($if_cond->kind === \ast\AST_UNARY_OP && $if_cond->flags === \ast\flags\UNARY_BOOL_NOT) {
                $cond_node = $if_cond->children['expr'];
                if ($cond_node instanceof Node && $cond_node->kind === \ast\AST_UNARY_OP && $cond_node->flags === \ast\flags\UNARY_BOOL_NOT) {
            ...
            if ($if_cond->kind === \ast\AST_BINARY_OP && in_array($if_cond->flags, self::NON_SHORT_CIRCUITING_BINARY_OPERATOR_FLAGS, \true)) {
                // if (($var = A) === B) {X} -> $var = A; if ($var === B) { X}
                $if_cond_children = $if_cond->children;
                if (in_array($if_cond_children['left']->kind ?? 0, [\ast\AST_ASSIGN, \ast\AST_ASSIGN_REF], \true) && ($if_cond_children['left']->children['var']->kind ?? 0) === \ast\AST_VAR && self::isExpressionWithoutSideEffects($if_cond_children['right'])) {
                    self::replaceLastNodeWithNodeList($nodes, ...self::applyAssignInLeftSideOfBinaryOpReduction($node));
                    continue;
                }
                if (in_array($if_cond_children['right']->kind ?? 0, [\ast\AST_ASSIGN, \ast\AST_ASSIGN_REF], \true) && ($if_cond_children['right']->children['var']->kind ?? 0) === \ast\AST_VAR && self::isExpressionWithoutSideEffects($if_cond_children['left'])) {
            ...
            if (count($node->children) === 1) {
                if ($if_cond->kind === \ast\AST_BINARY_OP && $if_cond->flags === \ast\flags\BINARY_BOOL_AND) {
                ...
                if ($if_cond->kind === \ast\AST_UNARY_OP && $if_cond->flags === \ast\flags\UNARY_BOOL_NOT && $node->children[1]->children['cond'] === null) {
            ...
            if ($if_cond->kind === \ast\AST_ASSIGN && ($if_cond->children['var']->kind ?? null) === \ast\AST_VAR) {
                // if ($var = A) {X} -> $var = A; if ($var) {X}
    ...
    private static function normalizeWhileStatement(Node $original_node) : array
    {
        ...
        while (\true) {
            ...
            if ($while_cond->kind === \ast\AST_UNARY_OP && $while_cond->flags === \ast\flags\UNARY_BOOL_NOT) {
            ...
            if ($while_cond->kind === \ast\AST_BINARY_OP && $while_cond->flags === \ast\flags\BINARY_BOOL_AND) {
    ...
    private static function normalizeForStatement(Node $node) : array
    {
        ...
        while (\true) {
            ...
            if ($for_cond->kind === \ast\AST_UNARY_OP && $for_cond->flags === \ast\flags\UNARY_BOOL_NOT) {
    ...
    private static function applyAssignInLeftSideOfBinaryOpReduction(Node $node) : array
    {
        ...
        if ($inner_assign_var->kind !== \ast\AST_VAR) {
            throw new AssertionError('SdgScoped\\Expected $inner_assign_var->kind === ast\\AST_VAR');
        }
    ...
    {
        return new Node(\ast\AST_IF, 0, [$l, $r], $l->lineno);
    }
    /**
    ...
    private static function applyIfChainReduction(Node $node) : Node
    {
        ...
        // Copy of array of Nodes of type IF_ELEM
        if (count($children) <= 2) {
            return $node;
        }
        while (count($children) > 2) {
            ...
            $new_r = new Node(\ast\AST_IF_ELEM, 0, ['cond' => null, 'stmts' => self::buildStatementList($inner_if_node->lineno, ...self::normalizeIfStatement($inner_if_node))], 0);
            $children[] = $new_r;
        }
    ...
    private static function applyIfAndReduction(Node $node) : Node
    {
        ...
        $inner_node_elem = clone $node->children[0];
        // AST_IF_ELEM
        ...
        $inner_node_stmts = self::normalizeIfStatement(new Node(\ast\AST_IF, 0, [$inner_node_elem], $inner_node_lineno));
        $inner_node_stmt_list = new Node(\ast\AST_STMT_LIST, 0, $inner_node_stmts, $inner_node_lineno);
        $outer_node_elem = clone $node->children[0];
        // AST_IF_ELEM
        ...
        return new Node(\ast\AST_IF, 0, [$outer_node_elem], $node->lineno);
    }
    /**
    ...
    private static function applyWhileAndReduction(Node $node) : Node
    {
        ...
        return new Node(\ast\AST_WHILE, 0, ['cond' => $cond_node->children['left'], 'stmts' => new Node(\ast\AST_STMT_LIST, 0, array_merge([$conditional_break_elem], $node->children['stmts']->children), $lineno)], $node->lineno);
    }
    /**
    ...
    {
        $break_if_elem = new Node(\ast\AST_IF_ELEM, 0, ['cond' => new Node(\ast\AST_UNARY_OP, \ast\flags\UNARY_BOOL_NOT, ['expr' => $cond_node], $lineno), 'stmts' => new Node(\ast\AST_STMT_LIST, 0, [new Node(\ast\AST_BREAK, 0, ['depth' => null], $lineno)], $lineno)], $lineno);
        return new Node(\ast\AST_IF, 0, [$break_if_elem], $lineno);
    }
    /**
    ...
    {
        if (!(count($node->children) === 2 && $node->children[0]->children['cond']->flags === \ast\flags\UNARY_BOOL_NOT && $node->children[1]->children['cond'] === null)) {
    ...
    {
        if (!($node->children[0]->children['cond']->flags === \ast\flags\UNARY_BOOL_NOT && $node->children[0]->children['cond']->children['expr']->flags === \ast\flags\UNARY_BOOL_NOT)) {
            throw new AssertionError('Failed precondition of ' . __METHOD__);
        }
    ...
    {
        if (!($node->children['cond']->flags === \ast\flags\UNARY_BOOL_NOT && $node->children['cond']->children['expr']->flags === \ast\flags\UNARY_BOOL_NOT)) {
            throw new AssertionError('Failed precondition of ' . __METHOD__);
        }
        return new Node(\ast\AST_WHILE, 0, ['cond' => $node->children['cond']->children['expr']->children['expr'], 'stmts' => $node->children['stmts']], $node->lineno);
    }
    /**
    ...
    private static function applyForDoubleNegateReduction(Node $node) : Node
    {
        ...
        if (!($cond_node->flags === \ast\flags\UNARY_BOOL_NOT && $cond_node->children['expr']->flags === \ast\flags\UNARY_BOOL_NOT)) {
            throw new AssertionError('Failed precondition of ' . __METHOD__);
        }
        $cond_node_list[] = $cond_node->children['expr']->children['expr'];
        $children['cond'] = new ast\Node(\ast\AST_EXPR_LIST, 0, $cond_node_list, $children['cond']->lineno);
        return new Node(\ast\AST_FOR, 0, $children, $node->lineno);
    }
    private static function applyIfNegatedToIfElseReduction(Node $node) : Node
    {
        ...
        $new_else_elem = new Node(\ast\AST_IF_ELEM, 0, ['cond' => null, 'stmts' => $if_elem->children['stmts']], $lineno);
        $new_if_elem = new Node(\ast\AST_IF_ELEM, 0, ['cond' => $if_elem->children['cond']->children['expr'], 'stmts' => new Node(\ast\AST_STMT_LIST, 0, [], $if_elem->lineno)], $lineno);
        return new Node(\ast\AST_IF, 0, [$new_if_elem, $new_else_elem], $node->lineno);
    }
    /**
