...
class Daemon
{
        ...
        try {
            ...
            while (\true) {
                ...
                try {
                    ...
                    if ($got_signal) {
                        continue;
                        // Ignore notices from stream_socket_accept if it's due to being interrupted by a child process terminating.
                    }
                } finally {
                ...
                $request = Request::accept($code_base, $file_path_lister, new StreamResponder($conn, \true), \true);
                if ($request instanceof Request) {
                    return $request;
                    // We forked off a worker process successfully, and this is the worker process
                }
            }
    ...
    private static function runWithoutPcntl(CodeBase $code_base, Closure $file_path_lister) : void
    {
        ...
        try {
            while (\true) {
                ...
                $request = Request::accept($code_base, $file_path_lister, new StreamResponder($conn, \true), \false);
                if ($request instanceof Request) {
    ...
    private static function createDaemonStreamSocketServer()
    {
        ...
        \printf("Listening for Phan analysis requests at %s\nAwaiting analysis requests for directory %s\n", $listen_url, \var_export(Config::getProjectRootDirectory(), \true));
