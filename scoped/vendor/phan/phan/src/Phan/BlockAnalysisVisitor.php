...
use ast\Node;
use Closure;
use SdgScoped\Phan\Analysis\AssignmentVisitor;
use SdgScoped\Phan\Analysis\BlockExitStatusChecker;
use SdgScoped\Phan\Analysis\ConditionVisitor;
use SdgScoped\Phan\Analysis\ContextMergeVisitor;
use SdgScoped\Phan\Analysis\LoopConditionVisitor;
use SdgScoped\Phan\Analysis\NegatedConditionVisitor;
use SdgScoped\Phan\Analysis\PostOrderAnalysisVisitor;
use SdgScoped\Phan\Analysis\PreOrderAnalysisVisitor;
use SdgScoped\Phan\Analysis\RedundantCondition;
use SdgScoped\Phan\AST\AnalysisVisitor;
use SdgScoped\Phan\AST\ASTReverter;
use SdgScoped\Phan\AST\ContextNode;
use SdgScoped\Phan\AST\InferPureSnippetVisitor;
...
use SdgScoped\Phan\Language\Scope\PropertyScope;
...
use SdgScoped\Phan\Parse\ParseVisitor;
use SdgScoped\Phan\Plugin\ConfigPluginSet;
use SdgScoped\Phan\Plugin\Internal\RedundantConditionVisitor;
use SdgScoped\Phan\Plugin\Internal\VariableTracker\VariableTrackerVisitor;
...
class BlockAnalysisVisitor extends AnalysisVisitor
{
    ...
    {
        $context = $this->context->withLineNumberStart($node->lineno);
    ...
    public function visitNamespace(Node $node) : Context
    {
        ...
        // given node
        $context = (new PreOrderAnalysisVisitor($this->code_base, $context))->visitNamespace($node);
    ...
    public function visitName(Node $node) : Context
    {
        ...
        ConfigPluginSet::instance()->postAnalyzeNode($this->code_base, $context, $node, $this->parent_node_list);
        return $context;
    }
    /**
    ...
    public function visitStmtList(Node $node) : Context
    {
        ...
        $plugin_set->preAnalyzeNode($this->code_base, $context, $node);
        ...
        $plugin_set->postAnalyzeNode($this->code_base, $context, $node, $this->parent_node_list);
        return $context;
    }
    /**
        ...
        try {
            foreach ($node->children as $child_node) {
                ...
                if ($child_node->kind === \ast\AST_ARRAY_ELEM) {
                    ...
                    $child_node->is_in_array = \true;
                    $context = $this->visitUnpack($child_node);
                } else {
                    throw new AssertionError("Unexpected node in ast\\AST_ARRAY: " . \SdgScoped\Phan\Debug::nodeToString($child_node));
                }
            }
        } finally {
    ...
    public function visitArrayElem(Node $node) : Context
    {
        ...
        $plugin_set->postAnalyzeNode($this->code_base, $context, $node);
        return $context;
    }
    /**
     * @param Node $node
    ...
    private function handleScalarStmt(Node $node, Context $context, $child_node) : void
    {
        if (\is_string($child_node)) {
            ...
            if (!$consumed) {
                Issue::maybeEmit($this->code_base, $context, Issue::NoopStringLiteral, ($context->getLineNumberStart() ?: $this->getLineNumberOfParent()) ?: $node->lineno, StringUtil::jsonEncode($child_node));
            }
        } elseif (\is_scalar($child_node)) {
            Issue::maybeEmit($this->code_base, $context, Issue::NoopNumericLiteral, ($context->getLineNumberStart() ?: $this->getLineNumberOfParent()) ?: $node->lineno, \var_export($child_node, \true));
        }
    }
    ...
    private const PHAN_FILE_SUPPRESS_REGEX = '/@phan-file-suppress\\s+' . Builder::SUPPRESS_ISSUE_LIST . '/';
    // @phan-suppress-current-line PhanAccessClassConstantInternal
    private const PHAN_VAR_REGEX = '/@(phan-var(?:-force)?)\\b\\s*(' . UnionType::union_type_regex . ')\\s*&?\\$' . Builder::WORD_REGEX . '/';
    // @phan-suppress-previous-line PhanAccessClassConstantInternal
    private const PHAN_DEBUG_VAR_REGEX = '/@phan-debug-var\\s+\\$(' . Builder::WORD_REGEX . '(,\\s*\\$' . Builder::WORD_REGEX . ')*)/';
    // @phan-suppress-previous-line PhanAccessClassConstantInternal
    /**
        ...
        if (\preg_match_all(self::PHAN_DEBUG_VAR_REGEX, $text, $matches, \PREG_SET_ORDER) > 0) {
            ...
            foreach ($matches as $group) {
                foreach (explode(',', $group[1]) as $var_name) {
                    ...
                    Issue::maybeEmit($this->code_base, $context, Issue::DebugAnnotation, $context->getLineNumberStart(), $var_name, $union_type_string);
                }
            }
        }
        if (!$has_known_annotations && preg_match('/@phan-.*/', $text, $match) > 0) {
            Issue::maybeEmit($code_base, $context, Issue::UnextractableAnnotation, $context->getLineNumberStart(), rtrim($match[0]));
        }
        return;
    }
    /**
    ...
    private static function createVarForInlineComment(CodeBase $code_base, Context $context, string $variable_name, UnionType $type, bool $create_variable) : void
    {
        if (!$context->getScope()->hasVariableWithName($variable_name)) {
            ...
            if (!$create_variable && !($context->isInGlobalScope() && Config::getValue('ignore_undeclared_variables_in_global_scope'))) {
                Issue::maybeEmitWithParameters($code_base, $context, Variable::chooseIssueForUndeclaredVariable($context, $variable_name), $context->getLineNumberStart(), [$variable_name], IssueFixSuggester::suggestVariableTypoFix($code_base, $context, $variable_name));
                return;
            }
            $variable = new Variable($context, $variable_name, $type, 0);
            $context->addScopeVariable($variable);
            return;
        }
        $variable = clone $context->getScope()->getVariableByName($variable_name);
    ...
    public function visit(Node $node) : Context
    {
        ...
        // indicating the state of the world within the
        // given node
        $context = (new PreOrderAnalysisVisitor($this->code_base, $context))->{Element::VISIT_LOOKUP_TABLE[$node->kind] ?? 'handleMissingNodeKind'}($node);
        // Let any configured plugins do a pre-order
        // analysis of the node.
        ConfigPluginSet::instance()->preAnalyzeNode($this->code_base, $context, $node);
        // With a context that is inside of the node passed
    ...
    public function visitFor(Node $node) : Context
    {
        ...
        if ($init_node instanceof Node) {
            $context = $this->analyzeAndGetUpdatedContext($context->withLineNumberStart($init_node->lineno), $node, $init_node);
        }
        ...
        if ($condition_node instanceof Node) {
            ...
            try {
                ...
                foreach ($condition_node->children as $condition_subnode) {
                    if ($condition_subnode instanceof Node) {
                        $context = $this->analyzeAndGetUpdatedContext($context->withLineNumberStart($condition_subnode->lineno), $condition_node, $condition_subnode);
                    }
                }
            } finally {
                \array_pop($this->parent_node_list);
            ...
            if ($condition_subnode instanceof Node) {
                ...
                // e.g. `while ($x = foo())`
                $context = $this->analyzeAndGetUpdatedContext($context->withLineNumberStart($condition_subnode->lineno), $node, $condition_subnode);
            ...
            $always_iterates_at_least_once = \true;
        }
        $original_context = $context;
        $stmts_node = $node->children['stmts'];
        if (!$stmts_node instanceof Node) {
            throw new AssertionError('SdgScoped\\Expected php-ast to always return a statement list for ast\\AST_FOR');
        }
        ...
        if ($condition_node instanceof Node) {
            $context = (new LoopConditionVisitor($this->code_base, $context, $condition_node, \false, BlockExitStatusChecker::willUnconditionallyProceed($stmts_node)))->__invoke($condition_node);
        } elseif (Config::getValue('redundant_condition_detection')) {
            $condition_node = $condition_node ?? new Node(\ast\AST_CONST, 0, ['name' => new Node(\ast\AST_NAME, \ast\flags\NAME_NOT_FQ, ['name' => 'true'], $node->lineno)], $node->lineno);
            (new LoopConditionVisitor($this->code_base, $context, $condition_node, \false, BlockExitStatusChecker::willUnconditionallyProceed($stmts_node)))->checkRedundantOrImpossibleTruthyCondition($condition_node, $context, null, \false);
        }
        // Give plugins a chance to analyze the loop condition now
        ConfigPluginSet::instance()->analyzeLoopBeforeBody($this->code_base, $context, $node);
        $context = $this->analyzeAndGetUpdatedContext($context->withScope(new BranchScope($context->getScope()))->withLineNumberStart($stmts_node->lineno), $node, $stmts_node);
        ...
        if ($loop_node instanceof Node) {
            $context = $this->analyzeAndGetUpdatedContext($context->withLineNumberStart($loop_node->lineno), $node, $loop_node);
        }
        ...
        if (Config::getValue('unused_variable_detection') && InferPureSnippetVisitor::isSideEffectFreeSnippet($this->code_base, $context, $loop_node) && InferPureSnippetVisitor::isSideEffectFreeSnippet($this->code_base, $context, $condition_node) && InferPureSnippetVisitor::isSideEffectFreeSnippet($this->code_base, $context, $stmts_node)) {
    ...
    {
        $context = $this->context->withLineNumberStart($node->lineno)->withEnterLoop($node);
        // Let any configured plugins do a pre-order
        // analysis of the node.
        ConfigPluginSet::instance()->preAnalyzeNode($this->code_base, $context, $node);
        $condition_node = $node->children['cond'];
        if ($condition_node instanceof Node) {
            ...
            // not the effect of the condition.
            // e.g. `while ($x = foo())`
            $context = $this->analyzeAndGetUpdatedContext($context->withLineNumberStart($condition_node->lineno), $node, $condition_node);
            if (!$this->context->isInGlobalScope() && !$this->context->isInLoop()) {
        ...
        // Check if this is side effect free with the variable types inferred by analyzing the loop body (heuristic).
        if (Config::getValue('unused_variable_detection') && InferPureSnippetVisitor::isSideEffectFreeSnippet($this->code_base, $context, $node)) {
            VariableTrackerVisitor::recordHasLoopBodyWithoutSideEffects($node);
        }
        // Now that we know all about our context (like what
        // 'self' means), we can analyze statements like
    ...
    public function visitForeach(Node $node) : Context
    {
        ...
        $expression_union_type = UnionTypeVisitor::unionTypeFromNode($code_base, $context, $expr_node)->withStaticResolvedInContext($context);
        if ($expr_node instanceof Node) {
            if ($expr_node->kind === \ast\AST_ARRAY) {
                ...
                $has_at_least_one_iteration = !$union_type->containsFalsey() && !$union_type->isEmpty() && !$union_type->hasTypeMatchingCallback(static function (Type $type) : bool {
        ...
        // Let any configured plugins do a pre-order
        // analysis of the node.
        ConfigPluginSet::instance()->preAnalyzeNode($code_base, $context, $node);
        ...
        // Give plugins a chance to analyze the loop condition now
        ConfigPluginSet::instance()->analyzeLoopBeforeBody($code_base, $inner_context, $node);
        $stmts_node = $node->children['stmts'];
        if ($stmts_node instanceof Node) {
    ...
    private function checkCanIterate(UnionType $union_type, Node $node) : void
    {
        ...
        if (!$union_type->hasPossiblyObjectTypes() && !$union_type->hasIterable()) {
            $this->emitIssue(Issue::TypeMismatchForeach, $node->children['expr']->lineno ?? $node->lineno, (string) $union_type);
            return;
        }
        $has_object = \false;
        ...
        if (self::isEmptyIterable($union_type)) {
            RedundantCondition::emitInstance($node->children['expr'], $this->code_base, (clone $this->context)->withLineNumberStart($node->children['expr']->lineno ?? $node->lineno), Issue::EmptyForeach, [(string) $union_type], Closure::fromCallable([self::class, 'isEmptyIterable']));
        }
        if (self::isDefinitelyNotObject($union_type)) {
            if (!($node->children['stmts']->children ?? null)) {
                RedundantCondition::emitInstance($node->children['expr'], $this->code_base, (clone $this->context)->withLineNumberStart($node->children['expr']->lineno ?? $node->lineno), Issue::EmptyForeachBody, [(string) $union_type], Closure::fromCallable([self::class, 'isDefinitelyNotObject']));
            }
        }
    }
    ...
    private function warnAboutNonTraversableType(Node $node, Type $type) : void
    {
        ...
        $status = $class->checkCanIterateFromContext($this->code_base, $this->context);
        switch ($status) {
        ...
        $this->emitIssue($issue, $node->children['expr']->lineno ?? $node->lineno, $type);
    }
    private function analyzeForeachIteration(Context $context, UnionType $expression_union_type, Node $node) : Context
    {
        ...
        if ($value_node instanceof Node) {
            ...
            $context = (new AssignmentVisitor($code_base, $context, $value_node, $expression_union_type->iterableValueUnionType($code_base)))->__invoke($value_node);
        }
        // If there's a key, make a variable out of that too
        $key_node = $node->children['key'];
        if ($key_node instanceof Node) {
            if ($key_node->kind === \ast\AST_ARRAY) {
                $this->emitIssue(Issue::InvalidNode, $key_node->lineno, "Can't use list() as a key element - aborting");
            } else {
                ...
                $context = (new AssignmentVisitor($code_base, $context, $key_node, $expression_union_type->iterableKeyUnionType($code_base)))->__invoke($key_node);
            }
        }
    ...
    public static function analyzeArrayAssignBackwardsCompatibility(CodeBase $code_base, Context $context, Node $node) : void
    {
        ...
        foreach ($node->children as $array_elem) {
            if (isset($array_elem->children['key'])) {
                Issue::maybeEmit($code_base, $context, Issue::CompatibleKeyedArrayAssignPHP70, $array_elem->lineno);
                break;
            }
        }
    }
    ...
    public function visitDoWhile(Node $node) : Context
    {
        ...
        // indicating the state of the world within the
        // given node
        $context = (new PreOrderAnalysisVisitor($this->code_base, $context))->visitDoWhile($node);
        // Let any configured plugins do a pre-order
        // analysis of the node.
        ConfigPluginSet::instance()->preAnalyzeNode($this->code_base, $context, $node);
        // With a context that is inside of the node passed
        ...
        if (Config::getValue('redundant_condition_detection')) {
            ...
            (new LoopConditionVisitor($this->code_base, $context, $cond_node, \true, !$stmts_node || BlockExitStatusChecker::willUnconditionallyProceed($stmts_node)))->checkRedundantOrImpossibleTruthyCondition($cond_node, $context, null, \false);
        }
        if (isset($node->phan_loop_contexts)) {
    ...
    public function visitClosedContext(Node $node) : Context
    {
        ...
        $context = (new ContextMergeVisitor($context, $child_context_list))->__invoke($node);
    ...
    public function visitSwitchList(Node $node) : Context
    {
        ...
        [$switch_variable_node, $switch_variable_condition, $switch_variable_negated_condition] = $this->createSwitchConditionAnalyzer(end($this->parent_node_list)->children['cond']);
        ...
        if (\count($children) <= 1 && !isset($children[0]->children['cond'])) {
            $this->emitIssue(Issue::NoopSwitchCases, end($this->parent_node_list)->lineno ?? $node->lineno);
        }
        ...
        foreach ($node->children as $i => $child_node) {
            ...
            try {
                ...
                ConfigPluginSet::instance()->preAnalyzeNode($this->code_base, $fallthrough_context, $child_node);
                ...
                if ($previous_child_context instanceof Context) {
                    ...
                    $child_context = (new ContextMergeVisitor($previous_child_context, [$previous_child_context, $fallthrough_context]))->combineScopeList([$previous_child_context->getScope(), $fallthrough_context->getScope()]);
                } else {
                ...
                ConfigPluginSet::instance()->postAnalyzeNode($this->code_base, $fallthrough_context, $child_node);
            } finally {
                \array_pop($this->parent_node_list);
            }
            if ($case_cond_node === null) {
        ...
        if (count($child_context_list) > 0) {
            ...
            if (count($child_context_list) >= 2) {
                ...
                // of all child context into a single scope based
                // on any possible branching structure
                $context = (new ContextMergeVisitor($context, $child_context_list))->combineChildContextList();
            } else {
                    ...
                    if ($name === 'get_class') {
                        ...
                        return [
                            ...
                            function (Context $child_context, $cond_node) use($switch_variable_node) : Context {
                                $visitor = new ConditionVisitor($this->code_base, $child_context);
                                return $visitor->analyzeClassAssertion($switch_variable_node, $cond_node) ?? $child_context;
                            },
                            null,
                        ];
                    }
            ...
            if (!ScopeImpactCheckingVisitor::hasPossibleImpact($this->code_base, $this->context, $switch_case_node)) {
                ...
                return [
                    ...
                    function (Context $child_context, $cond_node) use($switch_case_node) : Context {
                        ...
                        while ($cond_node instanceof Node && \in_array($cond_node->kind, [\ast\AST_ASSIGN, \ast\AST_ASSIGN_REF, \ast\AST_ASSIGN_OP], \true)) {
    ...
    public function visitMatchArmList(Node $node) : Context
    {
        ...
        if (\count($children) <= 1 && !isset($children[0]->children['cond'])) {
            ...
            $this->emitIssue(Issue::NoopMatchArms, $match_expression_node->lineno, ASTReverter::toShortString($match_expression_node));
        }
        ...
        [$unused_match_variable_node, $match_variable_condition, $match_variable_negated_condition] = $this->createMatchConditionAnalyzer($cond_node);
        if (Config::getValue('redundant_condition_detection')) {
        ...
        */
        ...
        foreach ($node->children as $arm_node) {
            ...
            if ($arm_node->children['cond'] === null) {
                if ($default_arm_node !== null) {
                    $this->emitIssue(Issue::InvalidNode, $arm_node->lineno, 'Saw an invalid match arm with multiple default nodes');
                    // fall through
                } else {
            ...
            try {
                $this->parent_node_list[] = $node;
                [$child_context, $fallthrough_context] = $this->analyzeMatchArm($child_context, $fallthrough_context, $match_variable_condition, $match_variable_negated_condition, $arm_node, $cond_node, $cond_type);
            } finally {
                \array_pop($this->parent_node_list);
            }
        ...
        if ($default_arm_node instanceof Node) {
            ...
            try {
                $this->parent_node_list[] = $node;
                [$child_context, $unused_fallthrough_context] = $this->analyzeMatchArm($child_context, $fallthrough_context, $match_variable_condition, $match_variable_negated_condition, $default_arm_node, $cond_node, $cond_type);
            } finally {
                \array_pop($this->parent_node_list);
            }
            // We can improve analysis of arms by using
     ...
     */
    private function analyzeMatchArm(Context $child_context, Context $fallthrough_context, ?Closure $match_variable_condition, ?Closure $match_variable_negated_condition, Node $arm_node, $match_cond_node, ?UnionType $cond_type) : array
    {
        ConfigPluginSet::instance()->preAnalyzeNode($this->code_base, $fallthrough_context, $arm_node);
        ...
        ConfigPluginSet::instance()->postAnalyzeNode($this->code_base, $fallthrough_context, $arm_node);
    ...
    public function visitIf(Node $node) : Context
    {
        ...
        foreach ($child_nodes as $child_node) {
            ...
            try {
                ...
                $old_context = $this->context;
                $this->context = $child_context->withScope(new BranchScope($child_context->getScope()))->withLineNumberStart($stmts_node->lineno);
                try {
                ...
                if ($condition_node instanceof Node) {
                    ...
                    $inferred_cond_value = $condition_node ?? \true;
                }
        ...
        if ($excluded_elem_count === count($child_nodes)) {
            ...
            $context = (new ContextMergeVisitor(
                $fallthrough_context,
                // e.g. "if (!is_string($x)) { $x = ''; }" should result in inferring $x is a string.
        ...
        if ($condition_node instanceof Node) {
            if ($condition_node->kind === \ast\AST_BINARY_OP) {
                ...
                if ($condition_node->flags === \ast\flags\BINARY_BOOL_AND) {
                    $child_context = $this->analyzeAndGetUpdatedContextAndAssertTruthy($fallthrough_context, $if_elem_node, $condition_node);
                    $fallthrough_context = $this->analyzeAndGetUpdatedContext($fallthrough_context->withLineNumberStart($condition_node->lineno), $if_elem_node, $condition_node);
                    return [$child_context, $fallthrough_context];
                }
            }
            $fallthrough_context = $this->analyzeAndGetUpdatedContext($fallthrough_context->withLineNumberStart($condition_node->lineno), $if_elem_node, $condition_node);
     ...
     */
    private function analyzeAndGetUpdatedContextAndAssertTruthy(Context $context, Node $parent_node, $condition_node) : Context
    {
        if (!$condition_node instanceof Node) {
            return $context;
        }
        if ($condition_node->kind === \ast\AST_BINARY_OP) {
            if ($condition_node->flags === \ast\flags\BINARY_BOOL_AND) {
                return $this->analyzeAndGetUpdatedContextAndAssertTruthy($this->analyzeAndGetUpdatedContextAndAssertTruthy($context, $condition_node, $condition_node->children['left']), $condition_node, $condition_node->children['right']);
            }
        }
        // Let any configured plugins do a pre-order
        // analysis of the node.
        // This may run multiple times on the same node due to need to analyze conditions and their negation.
        ConfigPluginSet::instance()->preAnalyzeNode($this->code_base, $context, $condition_node);
        // Infer the side effects of a generic node kind
        $context = $this->analyzeAndGetUpdatedContext($context->withLineNumberStart($condition_node->lineno), $parent_node, $condition_node);
        // Assert the generic node kind is truthy
        return (new ConditionVisitor($this->code_base, $context))->__invoke($condition_node);
    }
    /**
                    ...
                    if ($name === 'get_class') {
                        ...
                        return [
                            ...
                            function (Context $child_context, $cond_node) use($match_variable_node) : Context {
                                $visitor = new ConditionVisitor($this->code_base, $child_context);
                                return $visitor->analyzeClassAssertion($match_variable_node, $cond_node) ?? $child_context;
                            },
                            null,
                        ];
                    }
    ...
    public function visitTry(Node $node) : Context
    {
        ...
        // The conditions need to communicate to the outer
        // scope for things like assigning variables.
        $try_context = $context->withScope(new BranchScope($context->getScope()));
        $try_context->withLineNumberStart($try_node->lineno);
        ...
        $context = (new ContextMergeVisitor($context, [$try_context]))->mergeTryContext($node);
        // We collect all child context so that the
        // PostOrderAnalysisVisitor can optionally operate on
        // them
        ...
        foreach ($catch_nodes as $catch_node) {
            ...
            // The conditions need to communicate to the outer
            // scope for things like assigning variables.
            $catch_context = $context->withScope(new BranchScope($context->getScope()));
            $catch_context->withLineNumberStart($catch_node->lineno);
        ...
        if (count($catch_context_list) >= 2) {
            ...
            // of all child context into a single scope based
            // on any possible branching structure
            $context = (new ContextMergeVisitor($context, $catch_context_list))->mergeCatchContext($node);
        }
        ...
        if ($finally_node) {
            ...
            // The conditions need to communicate to the outer
            // scope for things like assigning variables.
            $context = $context->withScope(new BranchScope($context->getScope()));
            $context->withLineNumberStart($finally_node->lineno);
    ...
    private function checkUnreachableCatch(array $catch_nodes, Context $context) : void
    {
        ...
        foreach ($catch_nodes as $catch_node) {
            ...
            $union_type = UnionTypeVisitor::unionTypeFromClassNode($code_base, $context, $catch_node->children['class'])->objectTypesWithKnownFQSENs();
            ...
            foreach ($union_type->getTypeSet() as $type) {
                foreach ($type->asExpandedTypes($code_base)->getTypeSet() as $ancestor_type) {
                    ...
                    if ($line !== null) {
                        Issue::maybeEmit($code_base, $context, Issue::UnreachableCatch, $catch_line, (string) $type, $line, (string) $ancestor_type);
                        break;
                    }
                }
            }
    ...
    public function analyzeBinaryBoolAnd(Node $node) : Context
    {
        ...
        if ($left_node instanceof Node) {
            ...
            $context_with_left_condition = (new ConditionVisitor($this->code_base, $base_context))->__invoke($left_node);
            $context_with_false_left_condition = (new NegatedConditionVisitor($this->code_base, $base_context))->__invoke($left_node);
        } else {
        ...
        if ($right_node instanceof Node) {
            ...
            if (ScopeImpactCheckingVisitor::hasPossibleImpact($this->code_base, $context, $right_node)) {
                ...
                $context = (new ContextMergeVisitor($context, [$context, $context_with_false_left_condition, $right_context]))->combineChildContextList();
            }
        }
        return $this->postOrderAnalyze($context, $node);
    }
    ...
    public function analyzeBinaryBoolOr(Node $node) : Context
    {
        ...
        if ($left_node instanceof Node) {
            ...
            $context_with_true_left_condition = (new ConditionVisitor($this->code_base, $base_context))($left_node);
            $context_with_false_left_condition = (new NegatedConditionVisitor($this->code_base, $base_context))($left_node);
        } else {
            $context_with_false_left_condition = $context;
    ...
    private function analyzeBinaryCoalesceForRedundantCondition(Context $context, Node $node) : void
    {
        ...
        if ($right_node instanceof Node && $right_node->kind === \ast\AST_CONST && \strcasecmp($right_node->children['name']->children['name'] ?? '', 'null') === 0) {
            if ($left_node instanceof Node && self::isAlwaysDefined($context, $left_node)) {
                $this->emitIssue(Issue::CoalescingNeverUndefined, $node->lineno, ASTReverter::toShortString($left_node));
            }
        }
        ...
        if (!$left->containsNullableOrUndefined()) {
            ...
            RedundantCondition::emitInstance($left_node, $this->code_base, (clone $context)->withLineNumberStart($node->lineno), Issue::CoalescingNeverNull, [ASTReverter::toShortString($left_node), $left], static function (UnionType $type) : bool {
                return !$type->containsNullableOrUndefined();
            }, self::canNodeKindBeNull($left_node));
            return;
        } elseif ($left->isNull()) {
            RedundantCondition::emitInstance($left_node, $this->code_base, (clone $context)->withLineNumberStart($node->lineno), Issue::CoalescingAlwaysNull, [ASTReverter::toShortString($left_node), $left], static function (UnionType $type) : bool {
                return $type->isNull();
            });
            return;
        }
    ...
    private static function canNodeKindBeNull($node) : bool
    {
        ...
        switch ($node->kind) {
            ...
            case \ast\AST_UNARY_OP:
                return $node->flags === \ast\flags\UNARY_SILENCE && self::canNodeKindBeNull($node->children['expr']);
            case \ast\AST_BINARY_OP:
                return $node->flags === \ast\flags\BINARY_COALESCE && self::canNodeKindBeNull($node->children['left']) && self::canNodeKindBeNull($node->children['right']);
            case \ast\AST_CONST:
            case \ast\AST_VAR:
            case \ast\AST_SHELL_EXEC:
            // SHELL_EXEC will return null instead of an empty string for no output.
            ...
            // XXX could figure out what kinds of assign ops are guaranteed to be non-null
    ...
    public function visitConditional(Node $node) : Context
    {
        ...
        if ($cond_node instanceof Node) {
            ...
            $true_context = (new ConditionVisitor($this->code_base, isset($true_node) ? $base_context : $context))->__invoke($cond_node);
    ...
    private function postOrderAnalyze(Context $context, Node $node) : Context
    {
        ...
        // assignments and method calls.
        // Equivalent to (new PostOrderAnalysisVisitor(...)($node)) but faster than using __invoke()
        $context = (new PostOrderAnalysisVisitor($this->code_base, $context->withLineNumberStart($node->lineno), $this->parent_node_list))->{Element::VISIT_LOOKUP_TABLE[$node->kind] ?? 'handleMissingNodeKind'}($node);
    ...
    public function visitPropElem(Node $node) : Context
    {
        ...
        $context = $this->context->withScope(new PropertyScope($context->getScope(), FullyQualifiedPropertyName::make($class->getFQSEN(), $prop_name)))->withLineNumberStart($node->lineno);
