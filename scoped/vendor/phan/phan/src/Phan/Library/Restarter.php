...
use AssertionError;
use SdgScoped\Composer\XdebugHandler\XdebugHandler;
...
class Restarter extends XdebugHandler
{
    ...
    {
        $excluded_extensions = array_filter($this->disabledExtensions, static function (string $extension) : bool {
            return extension_loaded($extension);
        });
        $this->required = (bool) $excluded_extensions;
        if ($this->required) {
            // @phan-suppress-next-line PhanPluginRemoveDebugCall
            \fprintf(\STDERR, "[debug] Because %s %s installed, Phan will restart." . \PHP_EOL, implode(' and ', $excluded_extensions), \count($excluded_extensions) > 1 ? 'were' : 'was');
        }
