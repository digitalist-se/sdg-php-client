...
namespace SdgScoped\Jane\OpenApiCommon\SchemaParser;

use SdgScoped\Jane\OpenApiCommon\Exception\CouldNotParseException;
use SdgScoped\Jane\OpenApiCommon\Exception\OpenApiVersionSupportException;
use SdgScoped\Symfony\Component\Serializer\SerializerInterface;
use SdgScoped\Symfony\Component\Yaml\Exception\ExceptionInterface as YamlException;
...
abstract class SchemaParser
{
    ...
    public function parseSchema(string $openApiSpecPath)
    {
        if (!\array_key_exists($openApiSpecPath, static::$parsed)) {
            ...
            try {
                $content = Yaml::parse($openApiSpecContents, Yaml::PARSE_OBJECT | Yaml::PARSE_DATETIME | Yaml::PARSE_EXCEPTION_ON_INVALID_TYPE);
    ...
    {
        $openApiData = \json_decode($openApiSpecContents, \true);
        return $this->denormalize($openApiData, $openApiSpecPath);
    }
    protected abstract function validSchema($openApiSpecData) : bool;
    protected function denormalize($openApiSpecData, $openApiSpecPath)
    {
        ...
        return $this->serializer->denormalize($openApiSpecData, static::OPEN_API_MODEL, 'json', ['document-origin' => $openApiSpecPath]);
    }
}
