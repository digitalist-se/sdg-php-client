...
use SdgScoped\Jane\OpenApi3\Generator\RequestBodyContentGeneratorInterface;
...
abstract class AbstractBodyContentGenerator implements RequestBodyContentGeneratorInterface
{
    ...
    public function getTypeCondition(MediaType $content, string $reference, Context $context) : Node
    {
        ...
        if (null === $classGuess) {
            if ($array) {
                return new Expr\BinaryOp\LogicalAnd(new Expr\BinaryOp\LogicalAnd(new Expr\FuncCall(new Name('is_array'), [new Arg(new Expr\PropertyFetch(new Expr\Variable('this'), 'body'))]), new Expr\FuncCall(new Name('isset'), [new Arg(new Expr\ArrayDimFetch(new Expr\PropertyFetch(new Expr\Variable('this'), 'body'), new Expr\ConstFetch(new Name('0'))))])), $this->typeToCondition($schema->getType(), $schema->getFormat(), new Expr\ArrayDimFetch(new Expr\PropertyFetch(new Expr\Variable('this'), 'body'), new Expr\ConstFetch(new Name('0')))));
            }
            return $this->typeToCondition($schema->getType(), $schema->getFormat(), new Expr\PropertyFetch(new Expr\Variable('this'), 'body'));
        }
        $class = $context->getRegistry()->getSchema($classGuess->getReference())->getNamespace() . '\\Model\\' . $classGuess->getName();
        if ($array) {
            return new Expr\BinaryOp\LogicalAnd(new Expr\BinaryOp\LogicalAnd(new Expr\FuncCall(new Name('is_array'), [new Arg(new Expr\PropertyFetch(new Expr\Variable('this'), 'body'))]), new Expr\FuncCall(new Name('isset'), [new Arg(new Expr\ArrayDimFetch(new Expr\PropertyFetch(new Expr\Variable('this'), 'body'), new Expr\ConstFetch(new Name('0'))))])), new Expr\Instanceof_(new Expr\ArrayDimFetch(new Expr\PropertyFetch(new Expr\Variable('this'), 'body'), new Expr\ConstFetch(new Name('0'))), new Name('\\' . $class)));
        }
        return new Expr\Instanceof_(new Expr\PropertyFetch(new Expr\Variable('this'), 'body'), new Name('\\' . $class));
    }
    private function schemaTypeToPHP(?string $type, ?string $format = null) : array
    {
        ...
        $convertArray = ['string' => ['default' => ['string'], 'binary' => ['string', 'resource', '\\' . StreamInterface::class]], 'number' => ['default' => ['float']], 'boolean' => ['default' => ['bool']], 'integer' => ['default' => ['int']], 'array' => ['default' => ['array']], 'object' => ['default' => ['\\stdClass']], 'file' => ['default' => ['string', 'resource', '\\' . StreamInterface::class]]];
    ...
    private function typeToCondition(?string $type, ?string $format, Expr $fetch) : Expr
    {
        ...
        $convertArray = ['string' => ['default' => new Expr\FuncCall(new Name('is_string'), [$inputArg]), 'binary' => new Expr\BinaryOp\LogicalOr(new Expr\BinaryOp\LogicalOr(new Expr\FuncCall(new Name('is_string'), [$inputArg]), new Expr\FuncCall(new Name('is_resource'), [$inputArg])), new Expr\Instanceof_($fetch, new Name('\\' . StreamInterface::class)))], 'number' => ['default' => new Expr\FuncCall(new Name('is_float'), [$inputArg])], 'boolean' => ['default' => new Expr\FuncCall(new Name('is_bool'), [$inputArg])], 'integer' => ['default' => new Expr\FuncCall(new Name('is_int'), [$inputArg])], 'array' => ['default' => new Expr\FuncCall(new Name('is_array'), [$inputArg])], 'object' => ['default' => new Expr\Instanceof_($fetch, new Name('\\stdClass'))], 'file' => ['default' => new Expr\BinaryOp\LogicalOr(new Expr\BinaryOp\LogicalOr(new Expr\FuncCall(new Name('is_string'), [$inputArg]), new Expr\FuncCall(new Name('is_resource'), [$inputArg])), new Expr\Instanceof_($fetch, new Name('\\' . StreamInterface::class)))]];
        if (!isset($convertArray[$type]) || !isset($convertArray[$type][$format])) {
