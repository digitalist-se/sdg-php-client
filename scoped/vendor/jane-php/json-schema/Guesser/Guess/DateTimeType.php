...
class DateTimeType extends ObjectType
{
    ...
    protected function createNormalizationValueStatement(Context $context, Expr $input, bool $normalizerFromObject = \true) : Expr
    {
        ...
        return new Expr\MethodCall($input, 'format', [new Arg(new Scalar\String_($this->outputFormat))]);
    }
    /**
     * (@inheritDoc}.
     */
    ...
    {
        return new Expr\BinaryOp\LogicalAnd(new Expr\FuncCall(new Name('is_string'), [new Arg($input)]), new Expr\BinaryOp\NotIdentical(new Expr\ConstFetch(new Name('false')), $this->generateParseExpression($input)));
    }
    ...
    protected function generateParseExpression(Expr $input) : Expr
    {
        if (empty($this->inputFormat)) {
            ...
            $timezoneName = new Expr\MethodCall(new Expr\MethodCall($new, new Name('getTimezone')), new Name('getName'));
            ...
            return new Expr\Ternary(new Expr\BinaryOp\Equal($timezoneName, new Scalar\String_('Z')), new Expr\MethodCall($new, new Name('setTimezone'), [new Arg($gmtTimezone)]), $new);
        }
        // \DateTime::createFromFormat($format, $data)
        return new Expr\StaticCall(new Name('\\DateTime'), 'createFromFormat', [new Arg(new Scalar\String_($this->inputFormat)), new Arg($input)]);
    }
}
