...
class DateType extends ObjectType
{
    ...
    protected function createDenormalizationValueStatement(Context $context, Expr $input, bool $normalizerFromObject = \true) : Expr
    {
        ...
        return new Expr\MethodCall(new Expr\StaticCall(new Name('\\DateTime'), 'createFromFormat', [new Arg(new Scalar\String_($this->format)), new Arg($input)]), 'setTime', [new Arg(new Scalar\LNumber(0)), new Arg(new Scalar\LNumber(0)), new Arg(new Scalar\LNumber(0))]);
    }
    /**
     * (@inheritDoc}.
     */
    protected function createNormalizationValueStatement(Context $context, Expr $input, bool $normalizerFromObject = \true) : Expr
    {
        ...
        return new Expr\MethodCall($input, 'format', [new Arg(new Scalar\String_($this->format))]);
    }
    /**
     * (@inheritDoc}.
     */
    ...
    {
        return new Expr\BinaryOp\LogicalAnd(new Expr\FuncCall(new Name('is_string'), [new Arg($input)]), new Expr\BinaryOp\NotIdentical(new Expr\ConstFetch(new Name('false')), new Expr\MethodCall(new Expr\StaticCall(new Name('\\DateTime'), 'createFromFormat', [new Arg(new Scalar\String_($this->format)), new Arg($input)]), 'setTime', [new Arg(new Scalar\LNumber(0)), new Arg(new Scalar\LNumber(0)), new Arg(new Scalar\LNumber(0))])));
    }
