...
class ObjectType extends Type
{
    ...
    protected function createDenormalizationValueStatement(Context $context, Expr $input, bool $normalizerFromObject = \true) : Expr
    {
        ...
        return new Expr\MethodCall($denormalizerVar, 'denormalize', [new Arg($input), new Arg(new Scalar\String_($this->getFqdn(\false))), new Arg(new Scalar\String_('json')), new Arg(new Expr\Variable('context'))]);
    }
    /**
     * (@inheritDoc}.
     */
    protected function createNormalizationValueStatement(Context $context, Expr $input, bool $normalizerFromObject = \true) : Expr
    {
        ...
        return new Expr\MethodCall($normalizerVar, 'normalize', [new Arg($input), new Arg(new Scalar\String_('json')), new Arg(new Expr\Variable('context'))]);
    }
    /**
     * (@inheritDoc}.
     */
    public function createConditionStatement(Expr $input) : Expr
    {
        ...
        foreach ($this->discriminants as $key => $values) {
            $issetCondition = new Expr\FuncCall(new Name('isset'), [new Arg(new Expr\ArrayDimFetch($input, new Scalar\String_($key)))]);
            $logicalOr = null;
            if (null !== $values) {
                foreach ($values as $value) {
                    if (null === $logicalOr) {
                        $logicalOr = new Expr\BinaryOp\Equal(new Expr\ArrayDimFetch($input, new Scalar\String_($key)), new Scalar\String_($value));
                    } else {
                        $logicalOr = new Expr\BinaryOp\LogicalOr($logicalOr, new Expr\BinaryOp\Equal(new Expr\ArrayDimFetch($input, new Scalar\String_($key)), new Scalar\String_($value)));
                    }
                }
            }
            if (null !== $logicalOr) {
    ...
    private function getFqdn(bool $withRoot = \true) : string
    {
        if ($withRoot) {
