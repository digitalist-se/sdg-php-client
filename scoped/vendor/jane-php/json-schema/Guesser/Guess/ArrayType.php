...
class ArrayType extends Type
{
    ...
    public function createDenormalizationStatement(Context $context, Expr $input, bool $normalizerFromObject = \true) : array
    {
        ...
        $loopStatements = \array_merge($subStatements, [new Stmt\Expression(new Expr\Assign($this->createLoopOutputAssignement($valuesVar, $loopKeyVar), $outputExpr))]);
    ...
    {
        return new Expr\BinaryOp\BooleanAnd(parent::createConditionStatement($input), new Expr\MethodCall(new Expr\Variable('this'), 'isOnlyNumericKeys', [new Arg($input)]));
    }
    /**
     * (@inheritDoc}.
     */
    public function createNormalizationStatement(Context $context, Expr $input, bool $normalizerFromObject = \true) : array
    {
        ...
        $loopStatements = \array_merge($subStatements, [new Stmt\Expression(new Expr\Assign($this->createNormalizationLoopOutputAssignement($valuesVar, $loopKeyVar), $outputExpr))]);
        $statements[] = new Stmt\Foreach_($input, $loopValueVar, ['keyVar' => $loopKeyVar, 'stmts' => $loopStatements]);
        return [$statements, $valuesVar];
    }
