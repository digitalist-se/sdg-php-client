...
trait NormalizerGenerator
{
    ...
    {
        $traits = [new Stmt\TraitUse([new Name('DenormalizerAwareTrait')]), new Stmt\TraitUse([new Name('NormalizerAwareTrait')]), new Stmt\TraitUse([new Name('CheckArray')])];
    ...
    {
        return new Stmt\ClassMethod('supportsNormalization', ['type' => Stmt\Class_::MODIFIER_PUBLIC, 'params' => [new Param(new Expr\Variable('data')), new Param(new Expr\Variable('format'), new Expr\ConstFetch(new Name('null')))], 'stmts' => [new Stmt\Return_(new Expr\Instanceof_(new Expr\Variable('data'), new Name('\\' . $modelFqdn)))]]);
    }
    /**
     ...
     * @return Stmt\ClassMethod
     */
    protected function createNormalizeMethod(string $modelFqdn, Context $context, ClassGuess $classGuess, bool $skipNullValues = \true)
    {
        ...
        foreach ($classGuess->getProperties() as $property) {
            if (!$property->isReadOnly()) {
                ...
                if (!$context->isStrict() || $property->isNullable() || $property->getType() instanceof MultipleType && \count(\array_intersect([Type::TYPE_NULL], $property->getType()->getTypes())) === 1 || $property->getType()->getName() === Type::TYPE_NULL) {
                    if (!$context->isStrict() || $property->getType()->getName() !== Type::TYPE_NULL && ($property->getType() instanceof DateTimeType || $property->getType() instanceof MapType || $property->getType() instanceof ObjectType || $property->getType() instanceof PatternMultipleType || $property->getType() instanceof ArrayType)) {
        ...
        return new Stmt\ClassMethod('normalize', ['type' => Stmt\Class_::MODIFIER_PUBLIC, 'params' => [new Param($objectVariable), new Param(new Expr\Variable('format'), new Expr\ConstFetch(new Name('null'))), new Param(new Expr\Variable('context'), new Expr\Array_(), 'array')], 'stmts' => $statements]);
    }
    /**
