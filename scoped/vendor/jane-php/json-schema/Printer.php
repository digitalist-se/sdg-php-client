...
use SdgScoped\PhpCsFixer\Console\Command\FixCommand;
use SdgScoped\PhpCsFixer\ToolInfo;
use SdgScoped\PhpParser\PrettyPrinterAbstract;
use SdgScoped\Symfony\Component\Console\Input\ArrayInput;
use SdgScoped\Symfony\Component\Console\Output\NullOutput;
...
class Printer
{
        ...
        foreach ($registry->getSchemas() as $schema) {
            foreach ($schema->getFiles() as $file) {
                if (!\file_exists(\dirname($file->getFilename()))) {
                    \mkdir(\dirname($file->getFilename()), 0755, \true);
                }
    ...
    {
        $rules = ['@Symfony' => \true, 'self_accessor' => \true, 'array_syntax' => ['syntax' => 'short'], 'concat_space' => ['spacing' => 'one'], 'declare_strict_types' => \true, 'header_comment' => ['header' => <<<EOH
This file has been auto generated by Jane,

Do no edit it directly.
EOH
]];
        if (\version_compare(\SdgScoped\PhpCsFixer\Console\Application::VERSION, '2.6.0', '>=')) {
    ...
    protected function fix(string $path) : void
    {
        ...
        $config = ['path' => [$path], '--allow-risky' => \true];
