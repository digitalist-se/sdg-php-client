...
class Schema implements SchemaInterface
{
    ...
    {
        return \in_array($reference, $this->references, \true);
    }
    /**
     * @return mixed
     */
    ...
    public function addClassRelations(ClassGuess $classGuess) : void
    {
        ...
        foreach ($classGuess->getProperties() as $property) {
            ...
            if (($objectType = $property->getType()) instanceof ObjectType && '\\' !== \substr($objectType->getClassName(), 0, 1)) {
                $this->addRelation($baseModel, $objectType->getClassName());
            }
            if (($arrayType = $property->getType()) instanceof ArrayType && ($itemType = $arrayType->getItemType()) instanceof ObjectType && '\\' !== \substr($itemType->getClassName(), 0, 1)) {
