...
use SdgScoped\Symfony\Component\PropertyAccess\Exception\InvalidArgumentException;
use SdgScoped\Symfony\Component\PropertyAccess\Exception\NoSuchPropertyException;
use SdgScoped\Symfony\Component\PropertyAccess\Exception\UninitializedPropertyException;
use SdgScoped\Symfony\Component\PropertyInfo\PropertyTypeExtractorInterface;
use SdgScoped\Symfony\Component\PropertyInfo\Type;
use SdgScoped\Symfony\Component\Serializer\Encoder\CsvEncoder;
use SdgScoped\Symfony\Component\Serializer\Encoder\JsonEncoder;
use SdgScoped\Symfony\Component\Serializer\Encoder\XmlEncoder;
use SdgScoped\Symfony\Component\Serializer\Exception\ExtraAttributesException;
use SdgScoped\Symfony\Component\Serializer\Exception\LogicException;
use SdgScoped\Symfony\Component\Serializer\Exception\NotNormalizableValueException;
use SdgScoped\Symfony\Component\Serializer\Mapping\AttributeMetadataInterface;
use SdgScoped\Symfony\Component\Serializer\Mapping\ClassDiscriminatorFromClassMetadata;
use SdgScoped\Symfony\Component\Serializer\Mapping\ClassDiscriminatorResolverInterface;
use SdgScoped\Symfony\Component\Serializer\Mapping\Factory\ClassMetadataFactoryInterface;
use SdgScoped\Symfony\Component\Serializer\NameConverter\NameConverterInterface;
/**
...
abstract class AbstractObjectNormalizer extends AbstractNormalizer
{
    ...
    public function normalize($object, string $format = null, array $context = [])
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            try {
                ...
                if ($context[self::SKIP_UNINITIALIZED_VALUES] ?? $this->defaultContext[self::SKIP_UNINITIALIZED_VALUES] ?? \true) {
                    continue;
                }
                throw $e;
            } catch (\Error $e) {
    ...
    public function denormalize($data, string $type, string $format = null, array $context = [])
    {
        ...
        foreach ($normalizedData as $attribute => $value) {
            ...
            try {
                ...
                $exception = NotNormalizableValueException::createForUnexpectedDataType(\sprintf('Failed to denormalize attribute "%s" value for class "%s": ' . $e->getMessage(), $attribute, $type), $data, ['unknown'], $context['deserialization_path'] ?? null, \false, $e->getCode(), $e);
                if (isset($context['not_normalizable_value_exceptions'])) {
                    $context['not_normalizable_value_exceptions'][] = $exception;
                    continue;
                }
     ...
     */
    protected abstract function setAttributeValue(object $object, string $attribute, $value, string $format = null, array $context = []);
    /**
            ...
            try {
                ...
                if (\is_string($data) && (XmlEncoder::FORMAT === $format || CsvEncoder::FORMAT === $format)) {
                    if ('' === $data) {
                        ...
                        if ($type->isNullable() && \in_array($builtinType, [Type::BUILTIN_TYPE_BOOL, Type::BUILTIN_TYPE_INT, Type::BUILTIN_TYPE_FLOAT], \true)) {
                            return null;
                        }
                    }
    ...
    private function isMaxDepthReached(array $attributesMetadata, string $class, string $attribute, array &$context) : bool
    {
        ...
        if (!$enableMaxDepth || !isset($attributesMetadata[$attribute]) || null === ($maxDepth = $attributesMetadata[$attribute]->getMaxDepth())) {
            return \false;
        }
    ...
    private function getCacheKey(?string $format, array $context)
    {
        ...
        try {
            return \md5($format . \serialize(['context' => $context, 'ignored' => $context[self::IGNORED_ATTRIBUTES] ?? $this->defaultContext[self::IGNORED_ATTRIBUTES]]));
        } catch (\Exception $e) {
    ...
    {
        return \PHP_VERSION_ID >= 70400 && \str_starts_with($e->getMessage(), 'Typed property') && \str_ends_with($e->getMessage(), 'must not be accessed before initialization');
    }
}
