...
use SdgScoped\Symfony\Component\Uid\Ulid;
...
class SymfonyCaster
{
    ...
    public static function castUuid(Uuid $uuid, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (\method_exists($uuid, 'getDateTime')) {
            $a[Caster::PREFIX_VIRTUAL . 'time'] = $uuid->getDateTime()->format('SdgScoped\\Y-m-d H:i:s.u \\U\\T\\C');
        }
        return $a;
    }
    public static function castUlid(Ulid $ulid, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (\method_exists($ulid, 'getDateTime')) {
            $a[Caster::PREFIX_VIRTUAL . 'time'] = $ulid->getDateTime()->format('SdgScoped\\Y-m-d H:i:s.v \\U\\T\\C');
        }
        return $a;
    }
}
