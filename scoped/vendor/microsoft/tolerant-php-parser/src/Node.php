...
abstract class Node implements \JsonSerializable
{
    ...
    public function getDocCommentText()
    {
        ...
        $leadingTriviaTokens = PhpTokenizer::getTokensArrayFromContent($leadingTriviaText, ParseContext::SourceElements, $this->getFullStart(), \false);
    ...
    public function getImportTablesForCurrentScope()
    {
        ...
        if ($namespaceDefinition instanceof NamespaceDefinition) {
            $topLevelNamespaceStatements = $namespaceDefinition->compoundStatementOrSemicolon instanceof Token ? $namespaceDefinition->parent->statementList : $namespaceDefinition->compoundStatementOrSemicolon->statements;
            ...
            foreach (isset($useDeclaration->useClauses) ? $useDeclaration->useClauses->getValues() : [] as $useClause) {
                ...
                if ($useClause->groupClauses !== null && $useClause instanceof NamespaceUseClause) {
                    ...
                    foreach ($useClause->groupClauses->children as $groupClause) {
                        ...
                        $alias = $groupClause->namespaceAliasingClause === null ? $groupClause->namespaceName->getLastNamePart()->getText($contents) : $groupClause->namespaceAliasingClause->name->getText($contents);
                        $this->addToImportTable($alias, $functionOrConst, $namespaceNameParts, $contents, $namespaceImportTable, $functionImportTable, $constImportTable);
                    }
                } else {
                    ...
                    $alias = $useClause->namespaceAliasingClause === null ? $useClause->namespaceName->getLastNamePart()->getText($contents) : $useClause->namespaceAliasingClause->name->getText($contents);
    ...
    {
        $namespaceDefinition = $this instanceof NamespaceDefinition ? $this : $this->getFirstAncestor(NamespaceDefinition::class, SourceFileNode::class);
