...
class Token implements \JsonSerializable
{
    ...
    private static function getTokenKindNameFromValueMap()
    {
        ...
        if ($mapToKindName === null) {
            $constants = (new \ReflectionClass("SdgScoped\\Microsoft\\PhpParser\\TokenKind"))->getConstants();
    ...
    public function jsonSerialize()
    {
        ...
        if ($GLOBALS["SHORT_TOKEN_SERIALIZE"]) {
            return ["kind" => $kindName, "textLength" => $this->length - ($this->start - $this->fullStart)];
        } else {
            return ["kind" => $kindName, "fullStart" => $this->fullStart, "start" => $this->start, "length" => $this->length];
        }
    }
}
